...
static char * G_BUFF = NULL; /* wired buffer*/
static msrp_options_t opts = MSRP_OPTIONS_DEFAULT;

short init_frame_rw_buffer() 
{
        if (G_BUFF)
        {
           LM_WARN("Cannot initialize msrp frame buffer more than once!!");
           return -1;
        }
        if (set_msrp_options_on_buffer(&G_BUFF)!=0)
        {
...
int msrp_relay(msrp_frame_t *mf)
{
	...
	int port;
        ...
        if (unlikely(!G_BUFF))
        {
             if (unlikely(init_frame_rw_buffer() != 0)) 
             {
                LM_CRIT("Failed initializing msrp_frame!!");
                return -1;
             }
        }

	if(mf->buf.len>=GET_MSRP_MAX_FRAME_SIZE(&opts)-3)
        {
                LM_ERR("Incoming buffer length[%u] exceeds internal capacity[%u]!!", mf->buf.len,
                       GET_MSRP_MAX_FRAME_SIZE(&opts)-3); 
		return -1;
        }
	...
	dst = &env->dstinfo;
        ...
        LM_DBG("Relay: Sending frame on wire : size [%d]...", (int)(p - G_BUFF));
	if (dst->send_flags.f & SND_F_FORCE_CON_REUSE)
	{
		...
		else if (tcp_send(dst, 0, G_BUFF, p - G_BUFF) < 0) {
        ...
        LM_DBG("Relay: Sent frame OK : [%d]!!!", (int)(p - G_BUFF));
	return 0;
}
...
int msrp_reply(msrp_frame_t *mf, str *code, str *text, str *xhdrs)
{
	...
	*(p-3) = '$';
	...
	if (tcp_send(&env->srcinfo, 0, G_BUFF, p - G_BUFF) < 0) {
