...
int add_to_tree(pdt_tree_t *pt, str *sp, str *sd)
{
	...
	if(pt == NULL || sp == NULL || sp->s == NULL || sd == NULL
			|| sd->s == NULL) {
	...
	strncpy(itn0[strpos(pdt_char_list.s, sp->s[l]) % PDT_NODE_SIZE].domain.s,
			sd->s, sd->len);
	itn0[strpos(pdt_char_list.s, sp->s[l]) % PDT_NODE_SIZE].domain.len =
			sd->len;
...
int pdt_add_to_tree(pdt_tree_t **dpt, str *sdomain, str *code, str *domain)
{
	...
	if(sdomain == NULL || sdomain->s == NULL || code == NULL || code->s == NULL
...
str *get_domain(pdt_tree_t *pt, str *sp, int *plen)
{
	...
	while(itn != NULL && l < sp->len && l < PDT_MAX_DEPTH) {
		...
		if(itn[strpos(pdt_char_list.s, sp->s[l]) % PDT_NODE_SIZE].domain.s
				!= NULL) {
			domain = &itn[strpos(pdt_char_list.s, sp->s[l]) % PDT_NODE_SIZE]
							  .domain;
			len = l + 1;
		}
...
int pdt_check_pd_node(pdt_node_t *pn, str *sp, str *sd, char *code, int len)
{
	...
	for(i = 0; i < PDT_NODE_SIZE; i++) {
		...
		if(pn[i].domain.s != NULL) {
			...
			LM_DBG("[%.*s] [%.*s]\n", len + 1, code, pn[i].domain.len,
					pn[i].domain.s);
			if(sp->len == len + 1 && strncmp(sp->s, code, sp->len) == 0) {
