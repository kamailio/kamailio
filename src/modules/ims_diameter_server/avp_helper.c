	...
	switch(avp_t->type) {
		...
		case AAA_AVP_INTEGER32_TYPE:
			cJSON_AddNumberToObject(
					avp, "int32", htonl(*((unsigned int *)avp_t->data.s)));
			break;
		case AAA_AVP_INTEGER64_TYPE:
			cJSON_AddNumberToObject(
					avp, "int64", htonl(*((unsigned int *)avp_t->data.s)));
			break;
		...
		case AAA_AVP_DATA_TYPE:
			...
			if(avp_t->data.len > 4) {
				memset(dest, 0, STRSIZE);
				l = snprintf(
						dest, STRSIZE, "%.*s", avp_t->data.len, avp_t->data.s);
				...
				if(strlen(dest) > 0) {
					...
					while(avp_it) {
						LM_DBG("  AVP(%p < %p >%p);code=%u,"
							   "flags=%x;\nDataType=%u;VendorID=%u;DataLen=%u;"
							   "\n",
								avp_it->prev, avp_it, avp_it->next,
								avp_it->code, avp_it->flags, avp_it->type,
								avp_it->vendorId, avp_it->data.len);
 ...
 */
int diameterserver_add_avp(AAAMessage *m, char *d, int len, int avp_code,
 ...
 * @returns 1 on success or 0 on failure
 */
int diameterserver_add_avp_list(AAA_AVP_LIST *list, char *d, int len,
...
void parselist(AAAMessage *response, AAA_AVP_LIST *list, cJSON *item, int level)
{
	...
	LM_DBG("------------------------------------------------------------------"
		   "\n");
	LM_DBG("%i) Item %s (%i / %s)\n", level, item->string, item->valueint,
			item->valuestring);
	// LM_ERR("Got JSON:\n%s\n",  cJSON_Print(item));
	...
	if(cJSON_GetObjectItem(item, "Flags")) {
		LM_DBG("%i) Flags: %i\n", level,
				cJSON_GetObjectItem(item, "Flags")->valueint);
	...
	if(cJSON_GetObjectItem(item, "list")) {
		...
		for(i = 0; i < cJSON_GetArraySize(cJSON_GetObjectItem(item, "list"));
				i++) {
			cJSON *subitem =
					cJSON_GetArrayItem(cJSON_GetObjectItem(item, "list"), i);
		...
		if(list) {
			diameterserver_add_avp_list(list, x, 4,
					cJSON_GetObjectItem(item, "avpCode")->valueint, flags,
					cJSON_GetObjectItem(item, "vendorId")->valueint,
					AVP_DUPLICATE_DATA, __FUNCTION__);
		} else {
			diameterserver_add_avp(response, x, 4,
					cJSON_GetObjectItem(item, "avpCode")->valueint, flags,
					cJSON_GetObjectItem(item, "vendorId")->valueint,
					AVP_DUPLICATE_DATA, __FUNCTION__);
		}
		...
		if(list) {
			diameterserver_add_avp_list(list,
					cJSON_GetObjectItem(item, "string")->valuestring,
			...
			diameterserver_add_avp(response,
					cJSON_GetObjectItem(item, "string")->valuestring,
					strlen(cJSON_GetObjectItem(item, "string")->valuestring),
					cJSON_GetObjectItem(item, "avpCode")->valueint, flags,
					cJSON_GetObjectItem(item, "vendorId")->valueint,
		...
		if(list) {
			diameterserver_add_avp_list(list, binary_form,
					strlen(cJSON_GetObjectItem(item, HEXDUMP)->valuestring) / 2,
					cJSON_GetObjectItem(item, "avpCode")->valueint, flags,
					cJSON_GetObjectItem(item, "vendorId")->valueint,
					AVP_DUPLICATE_DATA, __FUNCTION__);
		} else {
