...
str xbuff_attributes[] = {STR_STATIC_INIT("type"), STR_STATIC_INIT("format"),
		STR_STATIC_INIT("length"), STR_NULL};

str xbuff_types[] = {STR_STATIC_INIT("atom"), STR_STATIC_INIT("integer"),
		STR_STATIC_INIT("string"), STR_STATIC_INIT("tuple"),
		STR_STATIC_INIT("list"), STR_STATIC_INIT("pid"), STR_STATIC_INIT("ref"),
		STR_NULL};
...
sr_xavp_t *xbuff_new(str *name)
{
	...
	if(!xbuffs_root) {
		xbuffs_root = xavp_add_xavp_value(
				&xbuff_list, name, &xbuff_val, xavp_get_crt_list());
		if(!xbuffs_root) {
...
int pv_xbuff_parse_name(pv_spec_t *sp, str *in)
{
	...
	if(is_in_str(p, in) && *p == '=') {
		...
		if(!is_in_str(p, in) || *p != '>') {
			l = p - in->s;
			LM_ERR("invalid operator (expected =>) for accessing attribute in "
		...
		while(is_in_str(p, in)) {
			if(!is_pv_xbuff_valid_char(*p)) {
				l = p - in->s;
				LM_ERR("invalid character in attribute name in token %.*s at "
					   "%d\n",
						STR_FMT(in), l);
				goto error;
			}
...
int pv_xbuff_new_xavp(
		sr_xavp_t **new, pv_value_t *pval, int *counter, char prefix)
{
	char s[101];
	str name;
	sr_xavp_t *xavp = NULL;
		...
		if(xbuff_match_type_re(&pval->rs, &type, &xavp)) {
			...
			switch(type) {
				...
				case XBUFF_TYPE_PID:
					...
					nval.v.data = (sr_data_t *)shm_malloc(
							sizeof(sr_data_t) + sizeof(erlang_pid));
					if(!nval.v.data) {
						LM_ERR("not enough shared memory\n");
						return -1;
					}
					memcpy((void *)nval.v.data, (void *)xavp,
							sizeof(sr_data_t) + sizeof(erlang_pid));
					break;
				case XBUFF_TYPE_REF:
					s[0] = 'r';
					nval.type = SR_XTYPE_DATA;
					nval.v.data = (sr_data_t *)shm_malloc(
							sizeof(sr_data_t) + sizeof(erlang_ref));
					if(!nval.v.data) {
						LM_ERR("not enough shared memory\n");
						return -1;
					}
	...
	switch(avp->name.s[0]) {
		...
		case 'l':
			return pv_get_strval(
					msg, param, res, &xbuff_types[XBUFF_TYPE_LIST]);
			break;
		case 'a':
			return pv_get_strval(
					msg, param, res, &xbuff_types[XBUFF_TYPE_ATOM]);
			break;
		case 's':
		...
		case 't':
			return pv_get_strval(
					msg, param, res, &xbuff_types[XBUFF_TYPE_TUPLE]);
			break;
		case 'p':
		...
		struct sip_msg *msg, pv_param_t *param, pv_value_t *res, sr_xavp_t *avp)
{
	...
	switch(avp->val.type) {
		...
		case SR_XTYPE_STR:
			switch(avp->name.s[0]) {
				case 'a':
					if(snprintf(_pv_xavp_buf, 128, "<<atom:%p>>",
							   avp->val.v.xavp)
							< 0)
						return pv_get_null(msg, param, res);
					break;
			...
			break;
		case SR_XTYPE_TIME:
			if(snprintf(_pv_xavp_buf, 128, "%lu", (long unsigned)avp->val.v.t)
					< 0)
				return pv_get_null(msg, param, res);
			break;
		case SR_XTYPE_LONG:
			if(snprintf(_pv_xavp_buf, 128, "%ld", (long unsigned)avp->val.v.l)
					< 0)
				return pv_get_null(msg, param, res);
			break;
		case SR_XTYPE_LLONG:
		...
		case SR_XTYPE_DATA:
			switch(avp->name.s[0]) {
				case 'p':
					if(snprintf(
							   _pv_xavp_buf, 128, "<<pid:%p>>", avp->val.v.data)
							< 0)
						return pv_get_null(msg, param, res);
					break;
				case 'r':
					if(snprintf(
							   _pv_xavp_buf, 128, "<<ref:%p>>", avp->val.v.data)
							< 0)
						return pv_get_null(msg, param, res);
					break;
				default:
					if(snprintf(_pv_xavp_buf, 128, "<<binary:%p>>",
							   avp->val.v.data)
							< 0)
						return pv_get_null(msg, param, res);
			}
		...
		case SR_XTYPE_XAVP:
			switch(avp->name.s[0]) {
				case 't':
					if(snprintf(_pv_xavp_buf, 128, "<<tuple:%p>>",
							   avp->val.v.xavp)
							< 0)
						return pv_get_null(msg, param, res);
					break;
				case 'l':
				default:
					if(snprintf(_pv_xavp_buf, 128, "<<list:%p>>",
							   avp->val.v.xavp)
							< 0)
						return pv_get_null(msg, param, res);
					//			break;
		...
		switch(xavp->name.s[0]) {
			...
			case 's':
				ei_x_encode_string_len(
						xbuff, xavp->val.v.s.s, xavp->val.v.s.len);
				break;
			case 'i':
			...
			case 't':
				...
				if(xavp_encode(xbuff, xavp->val.v.xavp, level + 1))
					return -1;
				break;
			case 'l':
				n = xavp_get_count(xavp->val.v.xavp);
...
int xavp_decode(ei_x_buff *xbuff, int *index, sr_xavp_t **xavp, int level)
{
	...
	switch(type) {
		...
		case ERL_LARGE_TUPLE_EXT:
			...
			name.len = snprintf(_s, sizeof(_s), "%c%d",
					type == ERL_LIST_EXT ? 'l' : 't', counter++);
