		...
		struct sip_msg *msg, pv_param_t *param, int op, pv_value_t *val)
{
	...
	if(!tuples_root) {
		...
		tuple = xavp_add_xavp_value(
				&tuple_list, &name, &tuple_val, xavp_get_crt_list());
		...
		struct sip_msg *msg, pv_param_t *param, pv_value_t *res, sr_xavp_t *avp)
{
	...
	switch(avp->val.type) {
		...
		case SR_XTYPE_DATA:
			if(avp->name.s[0] == 'p') {
				if(snprintf(_pv_xavp_buf, 128, "<<pid:%p>>", avp->val.v.data)
						< 0)
					return pv_get_null(msg, param, res);
			} else if(snprintf(_pv_xavp_buf, 128, "<<binary:%p>>",
							  avp->val.v.data)
					  < 0)
				return pv_get_null(msg, param, res);
			break;
...
int pv_tuple_get(struct sip_msg *msg, pv_param_t *param, pv_value_t *res)
{
	...
	switch(xbuff_is_attr_set(attr)) {
		...
		case XBUFF_ATTR_LENGTH:
			xavp = xbuff_is_no_index(attr)
						   ? th
						   : xavp_get_nth(&th->val.v.xavp, idx, NULL);
			if(xavp) {
