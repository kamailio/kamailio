...
static inline int get_pass(str *_username, str *_secret, str *_password)
{
	...
	switch(autheph_sha_alg) {
		case AUTHEPH_SHA1:
			...
			if(HMAC(EVP_sha1(), _secret->s, _secret->len,
					   (unsigned char *)_username->s, _username->len, hmac_sha1,
					   &hmac_len)
					== NULL) {
	...
	_password->len = base64_enc(hmac_sha1, hmac_len,
			(unsigned char *)_password->s, base64_enc_len(hmac_len));
...
static inline int get_ha1(
		struct username *_username, str *_domain, str *_secret, char *_ha1)
{
	...
	eph_auth_api.calc_HA1(
			HA_MD5, &_username->whole, _domain, &spassword, 0, 0, _ha1);
...
static inline int digest_authenticate(
		struct sip_msg *_m, str *_realm, hdr_types_t _hftype, str *_method)
{
	struct hdr_field *h;
	...
	while(secret_struct != NULL) {
		ret = do_auth(_m, h, _realm, _method, &secret_struct->secret_key);
		if(ret == AUTH_OK) {
			break;
		}
...
int ki_autheph_check(sip_msg_t *_m, str *srealm)
{
	...
	if(_m->REQ_METHOD == METHOD_REGISTER) {
		...
		return digest_authenticate(
				_m, srealm, HDR_PROXYAUTH_T, &_m->first_line.u.request.method);
	}
}
...
int ki_autheph_www(sip_msg_t *_m, str *srealm)
{
	...
	return digest_authenticate(
			_m, srealm, HDR_AUTHORIZATION_T, &_m->first_line.u.request.method);
}
...
int ki_autheph_authenticate(sip_msg_t *_m, str *susername, str *spassword)
{
	...
	while(secret_struct != NULL) {
		LM_DBG("trying secret: %.*s (%i)\n", secret_struct->secret_key.len,
				secret_struct->secret_key.s, secret_struct->secret_key.len);
		if(get_pass(susername, &secret_struct->secret_key, &sgenerated_password)
				== 0) {
			LM_DBG("generated password: %.*s (%i)\n", sgenerated_password.len,
					sgenerated_password.s, sgenerated_password.len);
