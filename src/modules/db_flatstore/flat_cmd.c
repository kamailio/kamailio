...
int flat_cmd(db_cmd_t *cmd)
{
	...
	if(db_drv_init(&fcmd->gen, flat_cmd_free) < 0)
		goto error;

	/* FIXME */
	con = cmd->ctx->con[db_payload_idx];
	if(flat_open_table(&fcmd->file_index, con, &cmd->table) < 0)
		goto error;
...
int flat_put(db_res_t *res, db_cmd_t *cmd)
{
	...
	for(i = 0; !DB_FLD_EMPTY(cmd->vals) && !DB_FLD_LAST(cmd->vals[i]); i++) {
		if(i) {
			if(fprintf(f, "%c", flat_delimiter.s[0]) < 0)
				goto error;
		}

		/* TODO: how to distinguish NULL from empty */
		if(cmd->vals[i].flags & DB_NULL)
			continue;

		switch(cmd->vals[i].type) {
			case DB_INT:
				if(fprintf(f, "%d", cmd->vals[i].v.int4) < 0)
					goto error;
				break;

			case DB_FLOAT:
				if(fprintf(f, "%f", cmd->vals[i].v.flt) < 0)
					goto error;
				break;
			...
			case DB_DATETIME:
				if(fprintf(f, "%llu", (unsigned long long)cmd->vals[i].v.time)
						< 0)
					goto error;
				break;
			...
			case DB_CSTR:
				...
				while(*s) {
					...
					if(fprintf(f, "%.*s", (int)len, s) < 0)
						goto error;
					s += len;
					if(*s) {
						/* FIXME: do not use the escaped value for easier parsing */
						if(fprintf(f, "%c%c", flat_escape.s[0], *s) < 0)
							goto error;
						s++;
					}
				}
			...
			case DB_BLOB:
				...
				while(len > 0) {
					char *c;
					for(c = s; len > 0 && *c != flat_delimiter.s[0]
							   && *c != flat_record_delimiter.s[0]
							   && *c != flat_escape.s[0];
							c++, len--)
						;
					if(fprintf(f, "%.*s", (int)(c - s), s) < 0)
						goto error;
					s = c;
					if(len > 0) {
						if(fprintf(f, "%c%c", flat_escape.s[0], *s) < 0)
			...
			case DB_BITMAP:
				if(fprintf(f, "%u", cmd->vals[i].v.bitmap) < 0)
					goto error;
				break;
	...
	if(fprintf(f, "%c", flat_record_delimiter.s[0]) < 0)
		goto error;
