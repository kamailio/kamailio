		...
		int cdp_avp_add_Framed_IP_Address(AAA_AVP_LIST *list, ip_address ip)
{
	...
	return cdp_avp_add_to_list(
			list, cdp_avp_new(AVP_Framed_IP_Address, AAA_AVP_FLAG_MANDATORY, 0,
						  s, AVP_DUPLICATE_DATA));
}
		...
		AAA_AVP_LIST list, ip_address *ip, AAA_AVP **avp_ptr)
{
	...
	AAA_AVP *avp = cdp_avp_get_next_from_list(
			list, AVP_Framed_IP_Address, 0, avp_ptr ? *avp_ptr : 0);
	if(avp_ptr)
		*avp_ptr = avp;
	if(!avp) {
	...
	if(avp->data.len < 4) {
		LOG(L_ERR,
				"Error decoding Framed IP Address from AVP data of length %d < "
				"4",
				avp->data.len);
		bzero(ip, sizeof(ip_address));
		return 0;
...
int cdp_avp_add_Framed_IPv6_Prefix(AAA_AVP_LIST *list, ip_address_prefix ip)
{
	...
	return cdp_avp_add_to_list(
			list, cdp_avp_new_OctetString(AVP_Framed_IPv6_Prefix,
						  AAA_AVP_FLAG_MANDATORY, 0, data, AVP_DUPLICATE_DATA));
}
		...
		AAA_AVP_LIST list, ip_address_prefix *ip, AAA_AVP **avp_ptr)
{
	if(!ip)
		return 0;
	AAA_AVP *avp = cdp_avp_get_next_from_list(
			list, AVP_Framed_IPv6_Prefix, 0, avp_ptr ? *avp_ptr : 0);
	if(avp_ptr)
		*avp_ptr = avp;
	if(!avp) {
	...
	if(avp->data.len < 18) {
		LOG(L_ERR, "Error decoding Framed-IPv6-Prefix from data len < 18 "
				   "bytes!\n");
		bzero(ip, sizeof(ip_address_prefix));
		return 0;
	}
...
int CDP_AVP_MODULE.add_Framed_IPv6_Prefix(
		AAA_AVP_LIST *list, ip_address_prefix ip);
int CDP_AVP_MODULE.get_Framed_IPv6_Prefix(
		AAA_AVP_LIST list, ip_address_prefix *ip, AAA_AVP **avp_ptr);
...
#define _CDP_AVP_NASAPP_H_2
												...
												int cdp_avp_add_Framed_IP_Address(
														AAA_AVP_LIST *list,
														ip_address ip);
typedef int (*cdp_avp_add_Framed_IP_Address_f)(
		AAA_AVP_LIST *list, ip_address ip);
...
int cdp_avp_add_Framed_IPv6_Prefix(AAA_AVP_LIST *list, ip_address_prefix ip);
typedef int (*cdp_avp_add_Framed_IPv6_Prefix_f)(
		AAA_AVP_LIST *list, ip_address_prefix ip);

int cdp_avp_get_Framed_IPv6_Prefix(
		AAA_AVP_LIST list, ip_address_prefix *ip, AAA_AVP **avp_ptr);
typedef int (*cdp_avp_get_Framed_IPv6_Prefix_f)(
		AAA_AVP_LIST list, ip_address_prefix *ip, AAA_AVP **avp_ptr);
