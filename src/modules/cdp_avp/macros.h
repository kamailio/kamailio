...
#define cdp_avp_add(avp_name, vendor_id, flags, avp_type, data_type)        \
	...
	{                                                                       \
		return cdp_avp_add_to_list(                                         \
				list, cdp_avp_new_##avp_type(CDP_AVP_NAME(avp_name), flags, \
							  vendor_id, data));                            \
	}

#define cdp_avp_add_ptr(avp_name, vendor_id, flags, avp_type, data_type)    \
	int cdp_avp_add_##avp_name(                                             \
			AAA_AVP_LIST *list, data_type data, AVPDataStatus data_do)      \
	{                                                                       \
		return cdp_avp_add_to_list(                                         \
				list, cdp_avp_new_##avp_type(CDP_AVP_NAME(avp_name), flags, \
							  vendor_id, data, data_do));                   \
	}

#define cdp_avp_add2(avp_name, vendor_id, flags, avp_name_1, data_type_1, \
		avp_name_2, data_type_2)                                          \
	int cdp_avp_add_##avp_name##_Group(AAA_AVP_LIST *list,                \
			data_type_1 avp_name_1, data_type_2 avp_name_2)               \
	{                                                                     \
		...
		return cdp_avp_add_to_list(                                       \
				list, cdp_avp_new_Grouped(CDP_AVP_NAME(avp_name), flags,  \
							  vendor_id, &list_grp, AVP_FREE_DATA));      \
	}

#define cdp_avp_add3(avp_name, vendor_id, flags, avp_name_1, data_type_1, \
		avp_name_2, data_type_2, avp_name_3, data_type_3)                 \
	int cdp_avp_add_##avp_name##_Group(AAA_AVP_LIST *list,                \
			data_type_1 avp_name_1, data_type_2 avp_name_2,               \
			data_type_3 avp_name_3)                                       \
	{                                                                     \
		AAA_AVP_LIST list_grp = {0, 0};                                   \
		if(!cdp_avp_add_##avp_name_1(&list_grp, avp_name_1)               \
				|| !cdp_avp_add_##avp_name_2(&list_grp, avp_name_2)       \
...
#define cdp_avp_get(avp_name, vendor_id, flags, avp_type, data_type)        \
	int cdp_avp_get_##avp_name(                                             \
			AAA_AVP_LIST list, data_type *data, AAA_AVP **avp_ptr)          \
	{                                                                       \
		AAA_AVP *avp = cdp_avp_get_next_from_list(list,                     \
				CDP_AVP_NAME(avp_name), vendor_id, avp_ptr ? *avp_ptr : 0); \
		if(avp_ptr)                                                         \
			*avp_ptr = avp;                                                 \
		if(!avp) {                                                          \
			if(data)                                                        \
				bzero(data, sizeof(data_type));                             \
			return 0;                                                       \
		}                                                                   \
...
#define cdp_avp_get2(avp_name, vendor_id, flags, avp_name_1, data_type_1,   \
		avp_name_2, data_type_2)                                            \
	int cdp_avp_get_##avp_name##_Group(AAA_AVP_LIST list,                   \
			data_type_1 *avp_name_1, data_type_2 *avp_name_2,               \
			AAA_AVP **avp_ptr)                                              \
	{                                                                       \
		int cnt = 0;                                                        \
		...
		if(!avp || !cdp_avp_get_Grouped(avp, &list_grp)) {                  \
			if(avp_name_1)                                                  \
				bzero(avp_name_1, sizeof(data_type_1));                     \
			if(avp_name_2)                                                  \
				bzero(avp_name_2, sizeof(data_type_2));                     \
			return 0;                                                       \
		}                                                                   \
...
#define cdp_avp_get3(avp_name, vendor_id, flags, avp_name_1, data_type_1,   \
		avp_name_2, data_type_2, avp_name_3, data_type_3)                   \
	int cdp_avp_get_##avp_name##_Group(AAA_AVP_LIST list,                   \
			data_type_1 *avp_name_1, data_type_2 *avp_name_2,               \
			data_type_3 *avp_name_3, AAA_AVP **avp_ptr)                     \
	{                                                                       \
		int cnt = 0;                                                        \
		AAA_AVP_LIST list_grp = {0, 0};                                     \
		AAA_AVP *avp = cdp_avp_get_next_from_list(list,                     \
		...
		if(!avp || !cdp_avp_get_Grouped(avp, &list_grp)) {                  \
			...
			if(avp_name_2)                                                  \
				bzero(avp_name_2, sizeof(data_type_2));                     \
			if(avp_name_3)                                                  \
				bzero(avp_name_3, sizeof(data_type_3));                     \
			return 0;                                                       \
		}                                                                   \
		cnt += cdp_avp_get_##avp_name_1(list_grp, avp_name_1, 0);           \
		cnt += cdp_avp_get_##avp_name_2(list_grp, avp_name_2, 0);           \
...
#define cdp_avp2(avp_name, vendor_id, flags, avp_name_1, data_type_1,         \
		avp_name_2, data_type_2)                                              \
	cdp_avp_add2(avp_name, vendor_id, flags, avp_name_1, data_type_1,         \
			avp_name_2, data_type_2) cdp_avp_get2(avp_name, vendor_id, flags, \
			avp_name_1, data_type_1, avp_name_2, data_type_2)

#define cdp_avp3(avp_name, vendor_id, flags, avp_name_1, data_type_1,         \
		avp_name_2, data_type_2, avp_name_3, data_type_3)                     \
	cdp_avp_add3(avp_name, vendor_id, flags, avp_name_1, data_type_1,         \
			avp_name_2, data_type_2, avp_name_3, data_type_3)                 \
			cdp_avp_get3(avp_name, vendor_id, flags, avp_name_1, data_type_1, \
					avp_name_2, data_type_2, avp_name_3, data_type_3)
...
#define cdp_avp_add_ptr(avp_name, vendor_id, flags, avp_type, data_type) \
	int CDP_AVP_MODULE.add_##avp_name(                                   \
			AAA_AVP_LIST *list, data_type data, AVPDataStatus data_do);

#define cdp_avp_add2(avp_name, vendor_id, flags, avp_name_1, data_type_1, \
		avp_name_2, data_type_2)                                          \
	int CDP_AVP_MODULE.add_##avp_name##_Group(AAA_AVP_LIST *list,         \
			data_type_1 avp_name_1, data_type_2 avp_name_2);
...
#define cdp_avp_get(avp_name, vendor_id, flags, avp_type, data_type) \
	int CDP_AVP_MODULE.get_##avp_name(                               \
			AAA_AVP_LIST list, data_type *data, AAA_AVP **avp_ptr);

#define cdp_avp_get2(avp_name, vendor_id, flags, avp_name_1, data_type_1, \
		avp_name_2, data_type_2)                                          \
	int CDP_AVP_MODULE.get_##avp_name##_Group(AAA_AVP_LIST list,          \
			data_type_1 *avp_name_1, data_type_2 *avp_name_2,             \
			AAA_AVP **avp_ptr);
...
#define cdp_avp2(avp_name, vendor_id, flags, avp_name_1, data_type_1, \
	...
	cdp_avp_add2(avp_name, vendor_id, flags, avp_name_1, data_type_1, \
			avp_name_2, data_type_2)                                  \
			| cdp_avp_get2(avp_name, vendor_id, flags, avp_name_1,    \
					data_type_1, avp_name_2, data_type_2)

#define cdp_avp3(avp_name, vendor_id, flags, avp_name_1, data_type_1, \
		avp_name_2, data_type_2, avp_name_3, data_type_3)             \
	cdp_avp_add3(avp_name, vendor_id, flags, avp_name_1, data_type_1, \
			avp_name_2, data_type_2, avp_name_3, data_type_3)         \
			| cdp_avp_get3(avp_name, vendor_id, flags, avp_name_1,    \
					data_type_1, avp_name_2, data_type_2, avp_name_3, \
					data_type_3)
...
#define cdp_avp_add(avp_name, vendor_id, flags, avp_type, data_type) \
	int cdp_avp_add_##avp_name(AAA_AVP_LIST *list, data_type data);  \
	typedef int (*cdp_avp_add_##avp_name##_f)(                       \
			AAA_AVP_LIST * list, data_type data);
...
#define cdp_avp_add2(avp_name, vendor_id, flags, avp_name_1, data_type_1, \
	...
	int cdp_avp_add_##avp_name##_Group(AAA_AVP_LIST *list,                \
			data_type_1 avp_name_1, data_type_2 avp_name_2);              \
	typedef int (*cdp_avp_add_##avp_name##_Group_f)(AAA_AVP_LIST * list,  \
			data_type_1 avp_name_1, data_type_2 avp_name_2);
...
#define cdp_avp_get(avp_name, vendor_id, flags, avp_type, data_type) \
	int cdp_avp_get_##avp_name(                                      \
			AAA_AVP_LIST list, data_type *data, AAA_AVP **avp_ptr);  \
	typedef int (*cdp_avp_get_##avp_name##_f)(                       \
			AAA_AVP_LIST list, data_type * data, AAA_AVP * *avp_ptr);
...
#define cdp_avp_get2(avp_name, vendor_id, flags, avp_name_1, data_type_1, \
	...
	int cdp_avp_get_##avp_name##_Group(AAA_AVP_LIST list,                 \
			data_type_1 *avp_name_1, data_type_2 *avp_name_2,             \
			AAA_AVP **avp_ptr);                                           \
	typedef int (*cdp_avp_get_##avp_name##_Group_f)(AAA_AVP_LIST list,    \
			data_type_1 * avp_name_1, data_type_2 * avp_name_2,           \
			AAA_AVP * *avp_ptr);
