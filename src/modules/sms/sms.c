...
int set_modem_arg(struct modem *mdm, char *arg, char *arg_end)
{
	...
	switch(*arg) {
		...
		case 'p': /* pin */
			...
			break;
		case 'm': /* mode */
			if(!strncasecmp(arg + 2, "OLD", 3) && arg_end - arg - 2 == 3) {
				mdm->mode = MODE_OLD;
		...
		case 's': /* scan */
			foo = str2s(arg + 2, arg_end - arg - 2, &err);
			if(err) {
				LM_WARN("cannot convert [s] arg to integer!, assume default "
						"mode s=%d (SCAN)\n",
						SMS_BODY_SCAN);
				foo = SMS_BODY_SCAN;
			}
			switch(foo) {
				...
				case SMS_BODY_SCAN_MIX:
					break;
				default:
					LM_WARN("unsupported value s=%d for [s] arg!, assume "
							"default mode s=%d (SCAN)\n",
							foo, SMS_BODY_SCAN);
					foo = SMS_BODY_SCAN;
			}
			mdm->scan = foo;
...
parse_error:
	LM_ERR("SMS %s config: parse error before  chr %d [%.*s]\n",
			...
			(int)(p
					- ((step == 1) ? modems_config
								   : (step == 2 ? networks_config
												: links_config))),
			(*p == 0) ? 4 : 1, (*p == 0) ? "NULL" : p);
error:
	return -1;
