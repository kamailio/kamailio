...
#define ERR_NUMBER_TEXT                                                   \
	...
	"using a number in +(country code)(area code)(local number) format. " \
	"Thanks"                                                              \
	...
	*/
	...
	if(!msg->new_uri.s || parse_uri(msg->new_uri.s, msg->new_uri.len, &uri)
		...
		if(parse_uri(msg->first_line.u.request.uri.s,
				   msg->first_line.u.request.uri.len, &uri)
				|| !uri.user.len) {
			...
			if((!msg->to
					...
					|| parse_uri(get_to(msg)->uri.s, get_to(msg)->uri.len, &uri)
							   == -1
					|| !uri.user.len) {
				LM_ERR("unable to extract user name from RURI and To "
					   "header!\n");
				goto error;
			}
		}
	...
	len = SMS_HDR_BF_ADDR_LEN + from->uri.len + SMS_HDR_AF_ADDR_LEN + body.len
		  + SMS_FOOTER_LEN			/*text to send*/
		  + from->uri.len			/* from */
	...
	if(write(net_pipes_in[net], &sms_messg, sizeof(sms_messg))
		...
		LM_ERR("error when writing for net %d to pipe [%d] : %s\n", net,
				net_pipes_in[net], strerror(errno));
		shm_free(sms_messg);
		(*queued_msgs)--;
		goto error;
	}
...
static inline int send_error(struct sms_msg *sms_messg, char *msg1_s,
		int msg1_len, char *msg2_s, int msg2_len)
{
	str body;
	char *p;
	int foo;
...
int send_as_sms(struct sms_msg *sms_messg, struct modem *mdm)
{
	...
	sms_messg->ref = 1;
	for(i = 0, p = text.s; i < nr_chunks && i < max_sms_parts;
			p += len_array[i++]) {
		if(use_nice) {
			q = buf;
			if(nr_chunks > 1 && i) {
		...
		if(i + 1 == max_sms_parts && i + 1 < nr_chunks) {
			...
			p += buf_len - SMS_TRUNCATED_LEN - SMS_FOOTER_LEN
				 - SMS_EDGE_PART_LEN;
			send_error(sms_messg, ERR_TRUNCATE_TEXT, ERR_TRUNCATE_TEXT_LEN, p,
					text.len - (p - text.s) - SMS_FOOTER_LEN);
		}
...
error:
	if(ret_code == -1)
		...
		send_error(sms_messg, ERR_MODEM_TEXT, ERR_MODEM_TEXT_LEN,
				text.s + SMS_HDR_BF_ADDR_LEN + sms_messg->from.len
						+ SMS_HDR_AF_ADDR_LEN,
				text.len - SMS_FOOTER_LEN - SMS_HDR_BF_ADDR_LEN
						- sms_messg->from.len - SMS_HDR_AF_ADDR_LEN);
	...
	if(*p == SMS_HDR_BF_ADDR[0]) {
		...
		do {
			if((p[0] == 's' || p[0] == 'S') && (p[1] == 'i' || p[1] == 'I')
				...
				sip_addr.s = p;
				/* goes to the end of the address */
				while(p < sms->ascii + sms->userdatalength
						&& is_in_sip_addr(*p))
					p++;
				if(p == sms->ascii + sms->userdatalength) {
					LM_ERR("failed to find sip address end in sms body [%s]!\n",
				...
				p++;
				if(p + 9 >= sms->ascii + sms->userdatalength) {
					LM_ERR("unable to find sip address start in sms body "
						   "[%s]!\n",
							sms->ascii);
					goto error;
				}
...
int send_sms_as_sip_scan_no(struct incame_sms *sms, char *to)
{
	...
	LM_DBG("SMS from: [%.*s], to: [%.*s], body: [%.*s]\n", sip_from.len,
			sip_from.s, sip_to.len, sip_to.s, sip_body.len, sip_body.s);
...
int check_sms_report(struct incame_sms *sms)
{
	...
	} else if(res == 1 && sms->ascii[0] == 48
			  && old != 48) { /* provisional 48 */
	...
	while(1) {
		...
		for(i = 0; i < nr_of_networks && mdm->net_list[i] != -1; i++) {
			...
			while(counter < net->max_sms_per_call && !empty_pipe) {
				...
				len = read(net->pipe_out, &sms_messg, sizeof(sms_messg));
				...
				LM_DBG("%s processing sms for net %s:"
					   " \n\tTo:[%.*s]\n\tBody=<%d>[%.*s]\n",
						mdm->device, net->name, sms_messg->to.len,
						sms_messg->to.s, sms_messg->text.len,
						sms_messg->text.len, sms_messg->text.s);
		...
		if(!cpms_unsupported)
			if((used_mem = check_memory(mdm, USED_MEM)) == -1) {
				LM_ERR("CPMS command failed! cannot get used mem -> using "
					   "10\n");
				used_mem = 10;
			}
		...
		for(i = 1, k = 1; k <= used_mem && i <= max_mem; i++) {
			if(getsms(&sms, mdm, i) != -1) {
				...
				LM_DBG("SMS RECEIVED:\n\rFrom: %s %s\n\r%.*s %.*s"
					   "\n\r\"%.*s\"\n\r",
						sms.sender, sms.name, DATE_LEN, sms.date, TIME_LEN,
						sms.time, sms.userdatalength, sms.ascii);
