...
#define optz(_n, _l) \
	(buf + buf_len - (((_n) + (_l) > buf_len) ? buf_len : (_n) + (_l)))
		...
		int max, int timeout, char *exp_end)
{
	...
	do {
		...
		if(available > 0) {
			...
			n = (available > MAX_BUF - buf_len - 1) ? MAX_BUF - buf_len - 1
													: available;
			/* read data */
			...
			if(n) {
				...
				foo = pos = 0;
				if((!exp_end
						   && ((pos = strstr(optz(n, 4), "OK\r\n"))
								   || (foo = strstr(optz(n, 5), "ERROR"))))
						|| (exp_end
								&& (pos = strstr(
											optz(n, exp_end_len), exp_end)))) {
					/* we found the end */
					//LM_DBG("end found = %s\n",
	...
	if(mdm->mode == MODE_DIGICOM)
		...
		do {
			...
			if(strchr(answer, '1')) {
				...
				LM_WARN("Ignoring that modem does not support +CREG "
						"command.\n");
				success = 1;
			} else {
	...
	for(n = 0; n < 2 + 2 * (sms_report_type == CDS_REPORT); n++) {
		...
		switch(n) {
			case 0:
				...
				command[8] +=
						(mdm->mode == MODE_ASCII || mdm->mode == MODE_DIGICOM);
				clen = 10;
				break;
			case 1:
