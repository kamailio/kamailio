 ...
 */
#ifdef USE_CONFIRM_CALLBACK
static void sst_dialog_confirmed_CB(
		struct dlg_cell *did, int type, struct dlg_cb_params *params);
#endif /* USE_CONFIRM_CALLBACK */
static void sst_dialog_terminate_CB(
		struct dlg_cell *did, int type, struct dlg_cb_params *params);
static void sst_dialog_request_within_CB(
		struct dlg_cell *did, int type, struct dlg_cb_params *params);
static void sst_dialog_response_fwded_CB(
		struct dlg_cell *did, int type, struct dlg_cb_params *params);
...
static inline int sst_build_minse_hdr(int seval, str *sehdr)
{
	...
	sehdr->len = snprintf(sst_se_buf, SST_SE_BUF_SIZE, "Min-SE: %d\r\n", seval);
	sehdr->s = sst_se_buf;
	return 0;
}
static inline int sst_build_se_hdr(int seval, str *sehdr, char *refresher)
{
	...
	if(refresher == NULL) {
		sehdr->len = snprintf(
				sst_se_buf, SST_SE_BUF_SIZE, "Session-Expires: %d\r\n", seval);
	} else {
 ...
 */
void sst_handler_init(pv_spec_t *timeout_avp_p, unsigned int min_se, int flag,
		unsigned int reject)
{
		...
		struct dlg_cell *did, int type, struct dlg_cb_params *params)
{
	...
	if(msg->first_line.type == SIP_REPLY) {
		 ...
		 */
		if(!msg->cseq
				&& ((parse_headers(msg, HDR_CSEQ_F, 0) == -1) || !msg->cseq)) {
		...
		if(msg->first_line.u.reply.statuscode > 199
			...
			if(minfo.se != 0) {
				...
				if(info->requester == SST_PXY || info->supported == SST_UAC) {
					...
					LM_DBG("appending the Session-Expires: header to the 2XX "
						   "reply."
	...
	if(msg->first_line.type == SIP_REQUEST
		...
		if(sst_min_se < MIN(minse, se.interval)) {
			...
			if(flag) {
				...
				if(send_response(
						   msg, 422, &sst_422_rpl, msehdr.s, msehdr.len)) {
	...
	if(timeout_avp) {
		if((result = pv_get_spec_value(msg, timeout_avp, &pv_val)) == 0) {
			...
			if(pv_val.flags & PV_VAL_INT && pv_val.ri == value) {
				...
				pv_val.ri = value;
				if(timeout_avp->setf(msg, &timeout_avp->pvp, EQ_T, &pv_val)
						!= 0) {
...
static void setup_dialog_callbacks(struct dlg_cell *did, sst_info_t *info)
{
	...
	dlg_binds->register_dlgcb(
			did, DLGCB_CONFIRMED_NA, sst_dialog_confirmed_CB, info, NULL);
#endif /* USE_CONFIRM_CALLBACK */
	...
	dlg_binds->register_dlgcb(
			did, DLGCB_REQ_WITHIN, sst_dialog_request_within_CB, info, NULL);
	/*
	...
	dlg_binds->register_dlgcb(did, DLGCB_RPC_CONTEXT, sst_dialog_rpc_context_CB,
			(void *)info, NULL);
}
