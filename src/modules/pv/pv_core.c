...
int is_uri_enclosed(struct sip_msg *msg, struct to_body *tb)
{
	...
	if(tb->display.len == 0) {
		/* 	Display name not found, URI might be alone */
		char *start_uri = tb->body.s;
		int len_uri = tb->body.len;
		/* -2 is needed because last character of header is /r/n */
		if(start_uri[0] == '<' && start_uri[len_uri - 2] == '>') {
			return 1;
		}
		return 0;
	}
	/* Display name found, URI is enclosed */
	return 1;
}
