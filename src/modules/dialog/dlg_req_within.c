...
dlg_t *build_dlg_t(struct dlg_cell * cell, int dir)
{
    ...
    if (cell->route_set[dir].s && cell->route_set[dir].len) {
        ...
        if (parse_rr_body(cell->route_set[dir].s, cell->route_set[dir].len, &td->route_set) != 0) {
...
static inline int build_extra_hdr(struct dlg_cell *cell, str * extra_hdrs, str * str_hdr)
{
    char *p;
    int blen;
...
static inline int send_bye(struct dlg_cell *cell, int dir, str * hdrs)
{
    ...
    if (dir == DLG_CALLEE_LEG && dlg_lreq_callee_headers.len > 0) {
        ...
        memcpy(lhdrs.s + lhdrs.len, dlg_lreq_callee_headers.s, dlg_lreq_callee_headers.len);
    ...
    set_uac_req(&uac_r, &met, &lhdrs, NULL, dialog_info, TMCB_LOCAL_COMPLETED, bye_reply_cb, (void *)iuid);
