...
int _ip_is_in_subnet_v4(
		struct in_addr *ip, char *net, size_t netlen, int netmask);
int _ip_is_in_subnet_v6(
		struct in6_addr *ip, char *net, size_t netlen, int netmask);
int _ip_is_in_subnet_str(void *ip, enum enum_ip_type type, char *s, int slen);
int _ip_is_in_subnet_str_trimmed(
		void *ip, enum enum_ip_type type, char *b, char *e);
static int _detailed_ip_type(
		unsigned int _type, sip_msg_t *_msg, char *_s, char *_dst);
		...
		size_t len2, enum enum_ip_type ip2_type)
{
	...
	switch(ip1_type) {
		...
		case(ip_type_ipv4):
			if(inet_pton(AF_INET, _ip1, &in_addr1) == 0)
				return 0;
			if(inet_pton(AF_INET, _ip2, &in_addr2) == 0)
				return 0;
	...
	switch(ip1_type) {
		...
		case(ip_type_ipv4):
			...
			if(netmask == 32)
				...
				ipv4_mask = htonl(~(0xFFFFFFFF >> netmask));
		...
		case(ip_type_ipv6):
			...
			for(i = 0; i < 16; i++)
				in6_addr1.s6_addr[i] &= ipv6_mask[i];
			if(memcmp(in6_addr1.s6_addr, in6_addr2.s6_addr,
					   sizeof(in6_addr1.s6_addr))
					== 0)
				return 1;
		...
		struct in_addr *ip, char *net, size_t netlen, int netmask)
{
	...
	if(inet_pton(AF_INET, _net, &net_addr) == 0)
		return 0;
	if(netmask < 0 || netmask > 32)
		return 0;
		...
		struct in6_addr *ip, char *net, size_t netlen, int netmask)
{
	...
	if(inet_pton(AF_INET6, _net, &net_addr) != 1)
		return 0;
	if(netmask < 0 || netmask > 128)
		return 0;
	for(i = 0; i < 16; i++) {
	...
	for(i = 0; i < 16; i++)
		ip->s6_addr[i] &= ipv6_mask[i];
	for(i = 0; i < 16; i++)
		net_addr.s6_addr[i] &= ipv6_mask[i];
...
static int _detailed_ip_type_helper(
		unsigned int _type, sip_msg_t *_msg, str *sval, pv_spec_t *dst)
{
	str string;
	pv_value_t val;
	char *res;
	...
	assumed_type = (ip_type_error == _type)
						   ? ip_parser_execute(string.s, string.len)
						   : _type;
...
static int ki_detailed_ip_type_helper(
		unsigned int _type, sip_msg_t *_msg, str *_sval, str *_dpv)
{
	pv_spec_t *dst;
...
static int ki_ip_is_in_subnet(sip_msg_t *_msg, str *_sval1, str *_sval2)
{
	...
	switch(ip1_type = ip_parser_execute(string1.s, string1.len)) {
		...
		case(ip_type_ipv6):
			memcpy(ip_addr_str, string1.s, string1.len);
			ip_addr_str[string1.len] = '\0';
			if(inet_pton(AF_INET6, ip_addr_str, &ip_addr6) != 1)
				return 0;
			ip = &ip_addr6;
			break;
		default:
	...
	for(; e != NULL; b = e + 1, e = strchr(b, ',')) {
		if(b == e)
			continue;
		if((ret = _ip_is_in_subnet_str_trimmed(ip, ip1_type, b, e)) > 0)
			return ret;
	}
