...
int parse_rlsubs_did(char *str_did, str *callid, str *from_tag, str *to_tag)
{
	...
	to_tag->len =
			strlen(str_did) - 2 * RLS_DID_SEP_LEN - callid->len - from_tag->len;
	...
	if(dbmode == RLS_DB_ONLY) {
		...
		*hash_code = core_hash(&callid, &to_tag, hash_size);
		...
		s = pres_search_shtable(
				rls_table, callid, to_tag, from_tag, *hash_code);
...
int send_notify(xmlDocPtr *rlmi_doc, char *buf, int buf_len, const str bstr,
		subs_t *dialog, unsigned int hash_code)
{
	int result = 0;
	...
	xmlDocDumpFormatMemory(
			*rlmi_doc, (xmlChar **)(void *)&rlmi_cont.s, &rlmi_cont.len, 0);
...
static void send_notifies(db1_res_t *result, int did_col, int resource_uri_col,
		int auth_state_col, int reason_col, int pres_state_col,
		int content_type_col)
{
	...
	for(i = 0; i < result->n; i++) {
		...
		if(prev_did == NULL || strcmp(prev_did, curr_did) != 0) {
			...
			len_est = create_empty_rlmi_doc(&rlmi_doc, &list_node,
					&dialog->pres_uri, dialog->version, 0);
		...
		len_est += strlen(resource_uri)
				   + 35; /* <resource uri="[uri]"></resource>/r/n */
		resource_added = 1;
		...
		while(1) {
			...
			if(auth_state_flag & ACTIVE_STATE) {
				...
				chunk_len = 4 + bstr.len + 35 + 16 + cid.len + 18
							+ content_type.len + 4 + pres_state.len + 8;
				len_est += chunk_len;
			...
			if(rls_max_notify_body_len > 0
				...
				if(resource_added == 1) {
					...
					LM_DBG("timer_send_notify hit the size limit. len_est = "
						   "%d\n",
							len_est);
					if(send_notify(&rlmi_doc, buf, buf_len, bstr, dialog,
							   hash_code)) {
						LM_ERR("in send_notify\n");
						goto error;
					}
					...
					LM_DBG("timer_send_notify hit the size limit. NO RESOURCE "
						   "ADDED len_est = %d\n",
							len_est);
				}
				len_est = 0;
			...
			if(auth_state_flag & ACTIVE_STATE) {
				xmlNewProp(
						instance_node, BAD_CAST "state", BAD_CAST auth_state);
			} else if(auth_state_flag & TERMINATED_STATE) {
	...
	if(rlmi_doc) {
		LM_DBG("timer notify at end len_est = %d resource_added = %d dlg = "
			   "%p\n",
...
int ki_rls_handle_notify(sip_msg_t *msg)
{
	...
	if(pua_get_record_id(&dialog, &res_id)
			< 0) /* verify if within a stored dialog */
	{
	...
	if(res_id == 0) {
		...
		if(auth_flag == TERMINATED_STATE)
			goto done;
		LM_INFO("no presence dialog record for non-TERMINATED state uri "
				"pres_uri = %.*s watcher_uri = %.*s\n",
	...
	if(rlpres_dbf.replace != NULL) {
		if(rlpres_dbf.replace(
				   rlpres_db, query_cols, query_vals, n_query_cols, 2, 0)
				< 0) {
		...
		if(rlpres_dbf.update(rlpres_db, query_cols, 0, query_vals,
				   query_cols + 2, query_vals + 2, 2, n_query_cols - 2)
				< 0) {
		...
		if(rlpres_dbf.affected_rows(rlpres_db) == 0) {
			if(rlpres_dbf.insert(
					   rlpres_db, query_cols, query_vals, n_query_cols)
					< 0) {
...
static void timer_send_full_state_notifies(int round)
{
	...
	db_query_f query_fn =
			rls_dbf.query_lock ? rls_dbf.query_lock : rls_dbf.query;
	...
	if(query_fn(rls_db, query_cols, 0, query_vals, result_cols, 1,
	...
	for(i = 0; i < RES_ROW_N(result); i++) {
		...
		if(VAL_INT(&values[expires_col]) > now + rls_expires_offset) {
			...
			if(rls_get_service_list(&sub.pres_uri, &sub.watcher_user,
					   &sub.watcher_domain, &service_node, &doc)
					< 0) {
...
static void timer_send_update_notifies(int round)
{
	...
	int did_col, resource_uri_col, auth_state_col, reason_col, pres_state_col,
			content_type_col;
	int n_result_cols = 0;
	db1_res_t *result = NULL;
	db_query_f query_fn =
	...
	if(query_fn(rlpres_db, query_cols, 0, query_vals, result_cols, 1,
			   n_result_cols, &str_rlsubs_did_col, &result)
			< 0) {
		LM_ERR("in sql query\n");
		goto done;
	}
...
void timer_send_notify(unsigned int ticks, void *param)
{
	if(dbmode == RLS_DB_ONLY) {
		...
		int round =
				subset + (waitn_time * rls_notifier_poll_rate * process_num);
		if(++subset > (waitn_time * rls_notifier_poll_rate) - 1)
			subset = 0;
