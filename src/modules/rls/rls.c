...
extern int rls_insert_shtable(
		shtable_t htable, unsigned int hash_code, subs_t *subs);
extern subs_t *rls_search_shtable(shtable_t htable, str callid, str to_tag,
		str from_tag, unsigned int hash_code);
extern int rls_delete_shtable(
		shtable_t htable, unsigned int hash_code, subs_t *subs);
extern int rls_update_shtable(
		shtable_t htable, unsigned int hash_code, subs_t *subs, int type);
extern void rls_update_db_subs_timer(db1_con_t *db, db_func_t *dbf,
		shtable_t hash_table, int htable_size, int no_lock,
...
static param_export_t params[] = {
		...
		{"max_backend_subs", INT_PARAM, &rls_max_backend_subs}, {0, 0, 0}};
...
static int mod_init(void)
{
	...
	if(!pres_contains_event || !pres_get_ev_list || !pres_new_shtable
			|| !pres_destroy_shtable || !pres_insert_shtable
			|| !pres_delete_shtable || !pres_update_shtable
			|| !pres_search_shtable || !pres_copy_subs
	...
	LM_DBG("db_url=%s/%d/%p\n", ZSW(rlpres_db_url.s), rlpres_db_url.len,
			rlpres_db_url.s);
	...
	if(db_check_table_version(&rls_dbf, rls_db, &rlsubs_table, W_TABLE_VERSION)
			< 0) {
	...
	if(db_check_table_version(
			   &rlpres_dbf, rlpres_db, &rlpres_table, P_TABLE_VERSION)
			< 0) {
	...
	/* verify table version */
	if(db_check_table_version(
			   &rls_xcap_dbf, rls_xcap_db, &rls_xcap_table, X_TABLE_VERSION)
			< 0) {
	...
	if(dbmode == RLS_DB_ONLY) {
		if((rls_notifier_id = shm_malloc(sizeof(int) * rls_notifier_processes))
				== NULL) {
...
static int child_init(int rank)
{
	...
	if(rank == PROC_MAIN && dbmode == RLS_DB_ONLY) {
		...
		for(i = 0; i < rls_notifier_processes; i++) {
			...
			if(fork_basic_utimer(PROC_TIMER, tmp, 1, timer_send_notify,
...
void rlsubs_table_update(unsigned int ticks, void *param)
{
	...
	pres_update_db_subs_timer(rls_db, &rls_dbf, rls_table, hash_size, no_lock,
...
static const char *rls_rpc_cleanup_doc[2] = {"Trigger rls cleanup.", 0};
...
static const char *rls_rpc_update_subs_doc[2] = {"Trigger rls cleanup.", 0};
