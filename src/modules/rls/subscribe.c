...
subs_t *constr_new_subs(
		struct sip_msg *msg, struct to_body *pto, pres_ev_t *event);
...
xmlNodePtr rls_get_by_service_uri(xmlDocPtr doc, str *service_uri)
{
	...
	for(node = root->children; node; node = node->next) {
		if(xmlStrcasecmp(node->name, (unsigned char *)"service") == 0) {
			...
			xmlFree(uri.s);
			if(parse_uri(normalized_uri->s, normalized_uri->len, &sip_uri)
					< 0) {
	...
	*service_node = rls_get_by_service_uri(xmldoc, service_uri);
	if(*service_node == NULL) {
		LM_DBG("service uri %.*s not found in rl document for user"
			   " sip:%.*s@%.*s\n",
				service_uri->len, service_uri->s, user->len, user->s,
				domain->len, domain->s);
		if(xmldoc != NULL)
			xmlFreeDoc(xmldoc);
		*rootdoc = NULL;
...
int reply_200(struct sip_msg *msg, str *contact, int expires)
{
	...
	hdr_append.len = snprintf(hdr_append.s, contact->len + 70,
			"Expires: %d" CRLF "Contact: <%.*s>" CRLF "Require: eventlist" CRLF,
			expires, contact->len, contact->s);
		...
		struct sip_msg *msg, str watcher_user, str watcher_domain)
{
	...
	if(get_from(msg)->tag_value.s == NULL
			|| get_from(msg)->tag_value.len == 0) {
	...
	if(pres_extract_sdialog_info(&subs, msg, rls_max_expires, &to_tag_gen,
			   rls_server_address, watcher_user, watcher_domain)
			< 0) {
	...
	if(get_to(msg)->tag_value.s == NULL
		...
		if(dbmode == RLS_DB_ONLY) {
			...
			if(pres_search_shtable(rls_table, subs.callid, subs.to_tag,
	...
	if(get_to(msg)->tag_value.s == NULL || get_to(msg)->tag_value.len == 0) {
		...
		if(send_full_notify(&subs, service_node, &subs.pres_uri, hash_code)
				< 0) {
...
int remove_expired_rlsubs(subs_t *subs, unsigned int hash_code)
{
	...
	s = pres_search_shtable(
			rls_table, subs->callid, subs->to_tag, subs->from_tag, hash_code);
	if(s == NULL) {
...
int send_resource_subs(char *uri, void *param)
{
	...
	LM_DBG("pres uri [%.*s] - normalized uri [%.*s]\n", pres_uri.len,
			pres_uri.s, normalized_uri->len, normalized_uri->s);
	...
	if(check_self(&parsed_pres_uri.host, 0, PROTO_NONE) != 1
		...
		LM_WARN("Unable to subscribe to remote contact %.*s for watcher %.*s\n",
				normalized_uri->len, normalized_uri->s, s->watcher_uri->len,
				s->watcher_uri->s);
		return 1;
	}
	...
	if((tmp_str->s = (char *)pkg_malloc(sizeof(char) * normalized_uri->len))
			== NULL) {
		pkg_free(tmp_str);
		LM_ERR("out of private memory\n");
		return -1;
	...
	*rls_contact_list = list_insert(tmp_str, *rls_contact_list, &duplicate);
	if(duplicate != 0) {
		LM_WARN("%.*s has %.*s multiple times in the same resource list\n",
				s->watcher_uri->len, s->watcher_uri->s, s->pres_uri->len,
				s->pres_uri->s);
		pkg_free(tmp_str->s);
		pkg_free(tmp_str);
		return 1;
...
int resource_subscriptions(subs_t *subs, xmlNodePtr xmlnode)
{
	...
	if(rls_max_backend_subs > 0 && counter > rls_max_backend_subs)
		LM_WARN("%.*s has too many contacts.  Max: %d, has: %d\n", wuri.len,
				wuri.s, rls_max_backend_subs, counter);
...
void update_a_sub(subs_t *subs_copy)
{
	...
	if(rls_get_service_list(&subs_copy->pres_uri, &subs_copy->watcher_user,
			   &subs_copy->watcher_domain, &service_node, &doc)
			< 0) {
	...
	if(doc == NULL) {
		LM_WARN("no document returned for: %.*s\n", subs_copy->pres_uri.len,
				subs_copy->pres_uri.s);
		goto done;
	}
...
int ki_rls_update_subs(struct sip_msg *msg, str *uri, str *event)
{
	...
	LM_DBG("watcher username: %.*s, watcher domain: %.*s\n",
			parsed_uri.user.len, parsed_uri.user.s, parsed_uri.host.len,
			parsed_uri.host.s);

	if(dbmode == RLS_DB_ONLY) {
		int ret;
		lock_get(rls_update_subs_lock);
		ret = (update_all_subs_rlsdb(
				&parsed_uri.user, &parsed_uri.host, event));
	...
	for(i = 0; i < hash_size; i++) {
		...
		while(subs != NULL) {
			if(subs->watcher_user.len == parsed_uri.user.len
					&& strncmp(subs->watcher_user.s, parsed_uri.user.s,
							   parsed_uri.user.len)
							   == 0
					&& subs->watcher_domain.len == parsed_uri.host.len
					&& strncmp(subs->watcher_domain.s, parsed_uri.host.s,
							   parsed_uri.host.len)
							   == 0
