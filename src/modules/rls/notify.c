...
xmlDocPtr constr_rlmi_doc(db1_res_t *result, str *rl_uri, int version,
		xmlNodePtr rl_node, char ***cid_array, str username, str domain);
void constr_multipart_body(const str *const content_type, const str *const body,
		str *cid, int boundary_len, char *boundary_string);
...
int send_full_notify(
		subs_t *subs, xmlNodePtr rl_node, str *rl_uri, unsigned int hash_code)
{
	...
	multipart_body = NULL;
	db_query_f query_fn =
			rlpres_dbf.query_lock ? rlpres_dbf.query_lock : rlpres_dbf.query;
	...
	if(query_fn(rlpres_db, query_cols, 0, query_vals, result_cols, 1,
			   n_result_cols, NULL, &result)
			< 0) {
		LM_ERR("in sql query\n");
		goto error;
	}
	...
	len_est = create_empty_rlmi_doc(
			&rlmi_body, &list_node, rl_uri, subs->version, 1);
	xmlDocSetRootElement(rlmi_body, list_node);

	/* Find all the uri's to which we are subscribed */
	param.next = &uri_list_head;
	if(process_list_and_exec(rl_node, subs->watcher_user, subs->watcher_domain,
			   add_resource_to_list, (void *)(&param))
			< 0) {
	...
	while(uri_list_head) {
		...
		if(add_resource(uri_list_head->uri, list_node, boundary_string, result,
			...
			if(resource_added == 0) {
				...
				xmlDocDumpFormatMemory(rlmi_body,
						(xmlChar **)(void *)&rlmi_cont->s, &rlmi_cont->len, 0);
				xmlFreeDoc(rlmi_body);

				if(agg_body_sendn_update(rl_uri, boundary_string, rlmi_cont,
						   multipart_body, subs, hash_code)
						< 0) {
				...
				len_est = create_empty_rlmi_doc(
						&rlmi_body, &list_node, rl_uri, subs->version, 0);
				xmlDocSetRootElement(rlmi_body, list_node);
				multipart_body->len = 0;
				resource_added = 0;
			}
	...
	if(agg_body_sendn_update(rl_uri, boundary_string, rlmi_cont, multipart_body,
			   subs, hash_code)
			< 0) {
		LM_ERR("in function agg_body_sendn_update\n");
		goto error;
...
int add_resource_instance(char *uri, xmlNodePtr resource_node,
		db1_res_t *result, char *boundary_string, int *len_est)
{
	...
	for(i = 0; i < result->n; i++) {
		...
		cmp_code = strncmp(
				row_vals[resource_uri_col].val.string_val, uri, strlen(uri));
		if(cmp_code > 0)
			break;
		...
		if(cmp_code == 0) {
			...
			*len_est +=
					strlen(auth_state)
					+ 38; /* <instance id="12345678" state="[auth_state]" />r/n */
			...
			if(auth_state_flag & ACTIVE_STATE) {
				...
				*len_est += 4 + boundary_len + 35 + 16 + cid.len + 18
							+ content_type.len + 4 + body.len + 8;
			...
			instance_node =
					xmlNewChild(resource_node, NULL, BAD_CAST "instance", NULL);
			 ...
			 * be fixed properly. */
			xmlNewProp(instance_node, BAD_CAST "id", BAD_CAST instance_id);
			xmlNewProp(instance_node, BAD_CAST "state", BAD_CAST auth_state);

			if(auth_state_flag & ACTIVE_STATE) {
				constr_multipart_body(&content_type, &body, &cid, boundary_len,
						boundary_string);
		...
		db1_res_t *result, int *len_est)
{
	...
	if(rls_max_notify_body_len > 0) {
		*len_est +=
				strlen(uri) + 35; /* <resource uri="[uri]"></resource>/r/n */
	...
	res = add_resource_instance(
			uri, resource_node, result, boundary_string, len_est);
	if(res < 0) {
...
int create_empty_rlmi_doc(xmlDocPtr *rlmi_doc, xmlNodePtr *list_node, str *uri,
		int version, int full_state)
{
		...
		str *cid, int boundary_len, char *boundary_string)
{
	...
	chunk_len = 4 + boundary_len + 35 + 16 + cid->len + 18 + content_type->len
				+ 4 + body->len + 8;
	while(length + chunk_len >= multipart_body_size) {
		multipart_body_size += BUF_REALLOC_SIZE;
		multipart_body->s =
				(char *)pkg_reallocxf(multipart_body->s, multipart_body_size);
		if(multipart_body->s == NULL) {
...
str *rls_notify_extra_hdr(subs_t *subs, char *start_cid, char *boundary_string)
{
	...
	str_hdr->len +=
			sprintf(str_hdr->s + str_hdr->len, "Require: eventlist\r\n");
...
int rls_send_notify(
		subs_t *subs, str *body, char *start_cid, char *boundary_string)
{
	dlg_t *td = NULL;
	str met = {"NOTIFY", 6};
	str *str_hdr = NULL;
	...
	size = sizeof(dialog_id_t)
		   + (subs->to_tag.len + subs->callid.len + subs->from_tag.len)
					 * sizeof(char);
...
void rls_notify_callback(struct cell *t, int type, struct tmcb_params *ps)
{
	...
	LM_DBG("completed with status %d [to_tag:"
		   "%.*s]\n",
			ps->code, ((dialog_id_t *)(*ps->param))->to_tag.len,
		...
		list_func_t function, void *param)
{
	...
	for(node = list_node->children; node; node = node->next) {
		if(xmlStrcasecmp(node->name, (unsigned char *)"resource-list") == 0) {
			...
			if(parse_xcap_uri(unescaped_uri.s, &hostname, &port, &rl_uri) > 0) {
				if(rls_integrated_xcap_server == 1
					...
					LM_DBG("fetching local <resource-list - %.*s>\n", uri.len,
							uri.s);
					if(rls_get_resource_list(
							   &rl_uri, &username, &domain, &rl_node, &rl_doc)
							> 0) {
						LM_DBG("calling myself for rl_node\n");
						res = process_list_and_exec(
								rl_node, username, domain, function, param);
						xmlFree(uri.s);
						xmlFreeDoc(rl_doc);
					} else {
					...
					LM_ERR("<resource-list - %.*s> is not local - unsupported "
						   "at this time\n",
							uri.len, uri.s);
					xmlFree(uri.s);
					return -1;
			...
			xmlFree(uri.s);
		} else if(xmlStrcasecmp(node->name, (unsigned char *)"list") == 0)
			res = process_list_and_exec(
					node, username, domain, function, param);
	}
	return res;
}
...
int parse_xcap_uri(char *uri, str *host, unsigned short *port, str *path)
{
	...
	*port = 0;
	...
	} else if(strncmp(uri, HTTPS_PREFIX, HTTPS_PREFIX_LEN) == 0) {
		host->s = &uri[HTTPS_PREFIX_LEN];
		*port = HTTPS_PORT_DEFAULT;
		LM_DBG("resource list is on https server\n");
	} else if(strncmp(uri, LOCAL_PREFIX, LOCAL_PREFIX_LEN) == 0) {
		path->s = &uri[0];
	...
	if(host->s != NULL) {
		while(host->s[host->len] != '\0' && host->s[host->len] != ':'
				&& host->s[host->len] != '/')
			host->len++;
		...
		if(host->s[host->len] == ':') {
			...
			while(host->s[cur_pos] != '\0' && host->s[cur_pos] != '/')
				cur_pos++;
			strncpy(char_port, &host->s[host->len + 1],
					MIN(cur_pos - host->len - 1, CHAR_PORT_LEN));
			*port = atoi(char_port);
	...
	while(path->s != NULL && path->s[path->len] != '\0')
		path->len++;
		...
		xmlNodePtr *rl_node, xmlDocPtr *xmldoc)
{
	...
	while(checked < rl_uri->len) {
		if(checked < rl_uri->len - 3
				&& strncmp(rl_uri->s + checked, "/~~", 3) == 0) {
			root.len = checked;
			checked += 3;
			break;
		}
	...
	*xmldoc = xmlParseMemory(body.s, body.len);
	...
	if(path.len == 0) {
		...
		*rl_node = XMLDocGetNodeByName(*xmldoc, "resource-lists", NULL);
		...
		if(xmlXPathRegisterNs(xpathCtx, BAD_CAST "xmlns",
				   BAD_CAST "urn:ietf:params:xml:ns:resource-lists")
				!= 0) {
