...
/* ---- exported commands: */
static int lookup_matrix(struct sip_msg *msg, struct multiparam_t *_first,
		struct multiparam_t *_second, struct multiparam_t *_dstavp);
...
static int lookup_matrix(struct sip_msg *msg, struct multiparam_t *_srctree,
		struct multiparam_t *_second, struct multiparam_t *_dstavp)
{
	int first;
	int second;
	...
	switch(_srctree->type) {
		...
		case MP_AVP:
			avp = search_first_avp(
					_srctree->u.a.flags, _srctree->u.a.name, &avp_val, 0);
			if(!avp) {
	...
	switch(_second->type) {
		...
		case MP_AVP:
			avp = search_first_avp(
					_second->u.a.flags, _second->u.a.name, &avp_val, 0);
			if(!avp) {
...
{
	db_key_t columns[3] = {
			&matrix_first_col, &matrix_second_col, &matrix_res_col};
	db1_res_t *res;
	int i;
	int n = 0;
	...
	if(matrix_dbf.query(matrix_dbh, NULL, NULL, NULL, columns, 0, 3, NULL, &res)
			< 0) {
		...
		for(i = 0; i < RES_ROW_N(res); i++) {
			if((!RES_ROWS(res)[i].values[0].nul)
				...
				if((RES_ROWS(res)[i].values[0].type == DB1_INT)
					...
					matrix_insert(RES_ROWS(res)[i].values[0].val.int_val,
							RES_ROWS(res)[i].values[1].val.int_val,
							RES_ROWS(res)[i].values[2].val.int_val);
					n++;
				} else {
...
static const char *matrix_rpc_reload_doc[2] = {
		"Reload matrix records from database", 0};
