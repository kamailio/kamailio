...
static str _dbg_state_list[] = {str_init("unknown"), str_init("init"),
		str_init("wait"), str_init("next"), {0, 0}};
...
static str _dbg_status_list[] = {str_init("cfgtrace-on"),
		str_init("cfgtrace-off"), str_init("abkpoint-on"),
		str_init("abkpoint-off"), str_init("lbkpoint-on"),
		str_init("lbkpoint-off"), str_init("cfgtest-on"),
		str_init("cfgtest-off"), {0, 0}};
...
static str _dbg_cmd_list[] = {str_init("nop"), str_init("err"),
		str_init("read"), str_init("next"), str_init("move"), str_init("show"),
		str_init("pveval"), str_init("pvlog"), {0, 0}};
...
int dbg_cfg_trace(sr_event_param_t *evp)
{
		...
		if(is_printable(_dbg_cfgtrace_level)) {
			if(unlikely(_dbg_cfgtrace_format & DBG_CFGTRACE_NOCFGFILE)) {
				LOG_FL(_dbg_cfgtrace_facility, _dbg_cfgtrace_level,
						...
						"%s=[%s] l=%d a=%d n=%.*s\n",
						get_current_route_type_name(), ZSW(a->rname), a->cline,
	...
	if(_dbg_pid_list[process_no].state == DBG_STATE_INIT) {
		LOG(_dbg_cfgtrace_level,
				...
				_dbg_pid_list[process_no].pid, ZSW(a->cfile), a->cline, a->type,
				an->len, ZSW(an->s));
	...
	while(loop) {
		switch(_dbg_pid_list[process_no].in.cmd) {
			...
			case DBG_CMD_NEXT:
				...
				_dbg_pid_list[process_no].in.cmd = DBG_CMD_NOP;
				olen = snprintf(_dbg_pid_list[process_no].out.buf, DBG_CMD_SIZE,
						"exec [%s:%d] a=%d n=%.*s", ZSW(a->cfile), a->cline,
						a->type, an->len, ZSW(an->s));
				if(olen < 0) {
			...
			case DBG_CMD_PVLOG:
				...
				if(val.flags & PV_TYPE_INT) {
					if(loop == DBG_CMD_PVEVAL) {
						olen = snprintf(_dbg_pid_list[process_no].out.buf,
								DBG_CMD_SIZE, "%s : t=int v=%d", pvn.s, val.ri);
						if(olen < 0) {
							_dbg_pid_list[process_no].out.cmd = DBG_CMD_ERR;
							break;
						}
				...
				if(loop == DBG_CMD_PVEVAL) {
					olen = snprintf(_dbg_pid_list[process_no].out.buf,
							DBG_CMD_SIZE, "%s : t=str v=%.*s", pvn.s,
							val.rs.len, val.rs.s);
					if(olen < 0) {
						_dbg_pid_list[process_no].out.cmd = DBG_CMD_ERR;
						break;
			...
			case DBG_CMD_SHOW:
				...
				olen = snprintf(_dbg_pid_list[process_no].out.buf, DBG_CMD_SIZE,
						"at bkp [%s:%d] a=%d n=%.*s", a->cfile, a->cline,
						a->type, an->len, an->s);
				if(olen < 0) {
					_dbg_pid_list[process_no].out.cmd = DBG_CMD_ERR;
					break;
 ...
 *
 */
static const char *dbg_rpc_bp_doc[2] = {"Breakpoint command", 0};
 ...
 *
 */
static const char *dbg_rpc_list_doc[2] = {"List debugging process array", 0};
...
static void dbg_rpc_list(rpc_t *rpc, void *ctx)
{
	...
	for(; i < limit; i++) {
		...
		if(rpc->struct_add(th, "dddddd", "entry", i, "pid",
				   _dbg_pid_list[i].pid, "set", _dbg_pid_list[i].set, "state",
				   _dbg_pid_list[i].state, "in.pid", _dbg_pid_list[i].in.pid,
 ...
 *
 */
static const char *dbg_rpc_trace_doc[2] = {"Config trace command", 0};
 ...
 *
 */
static const char *dbg_rpc_set_mod_level_doc[2] = {"Set module log level", 0};
 ...
 *
 */
static const char *dbg_rpc_get_mod_level_doc[2] = {"Get module log level", 0};
 ...
 *
 */
rpc_export_t dbg_rpc[] = {{"dbg.bp", dbg_rpc_bp, dbg_rpc_bp_doc, 0},
		{"dbg.ls", dbg_rpc_list, dbg_rpc_list_doc, 0},
		{"dbg.trace", dbg_rpc_trace, dbg_rpc_trace_doc, 0},
		{"dbg.set_mod_level", dbg_rpc_set_mod_level, dbg_rpc_set_mod_level_doc,
				0},
		{"dbg.set_mod_facility", dbg_rpc_set_mod_facility,
				dbg_rpc_set_mod_facility_doc, 0},
		{"dbg.get_mod_level", dbg_rpc_get_mod_level, dbg_rpc_get_mod_level_doc,
				0},
		{"dbg.get_mod_facility", dbg_rpc_get_mod_facility,
				dbg_rpc_get_mod_facility_doc, 0},
...
int dbg_init_mod_levels(int dbg_mod_hash_size)
{
	...
	_dbg_mod_table = (dbg_mod_slot_t *)shm_malloc(
			_dbg_mod_table_size * sizeof(dbg_mod_slot_t));
	if(_dbg_mod_table == NULL) {
		LM_ERR("no more shm.\n");
		return -1;
	}
...
static inline unsigned int dbg_compute_hash(char *s1, int s1len)
{
	...
	for(p = s1; p <= (end - 4); p += 4) {
		v = (dbg_ch_icase(*p) << 24) + (dbg_ch_icase(p[1]) << 16)
			+ (dbg_ch_icase(p[2]) << 8) + dbg_ch_icase(p[3]);
		dbg_ch_h_inc;
	}
	v = 0;
	for(; p < end; p++) {
...
int dbg_set_mod_debug_level(char *mname, int mnlen, int *mlevel)
{
	...
	while(it != NULL && it->hashid == hid) {
		if(mnlen == it->name.len && strncmp(mname, it->name.s, mnlen) == 0) {
			/* found */
			if(mlevel == NULL) {
				/* remove */
				if(itp != NULL) {
	...
	itn = (dbg_mod_level_t *)shm_malloc(
			sizeof(dbg_mod_level_t) + (mnlen + 1) * sizeof(char));
	if(itn == NULL) {
		LM_ERR("no more shm\n");
		return -1;
	}
...
int dbg_set_mod_debug_facility(char *mname, int mnlen, int *mfacility)
{
	...
	itn = (dbg_mod_facility_t *)shm_malloc(
			sizeof(dbg_mod_facility_t) + (mnlen + 1) * sizeof(char));
	if(itn == NULL) {
		LM_ERR("no more shm\n");
		return -1;
	}
...
{
	_dbg_pvcache = (dbg_pvcache_t **)pkg_malloc(
			sizeof(dbg_pvcache_t *) * DBG_PVCACHE_SIZE);
...
int _dbg_log_assign_action_avp(struct sip_msg *msg, struct lvalue *lv)
{
	...
	avp = search_avp_by_index(avp_s->type, avp_s->name, &avp_val, avp_s->index);
	if(likely(avp)) {
		if(avp->flags & (AVP_VAL_STR)) {
			LM_DBG("%.*s:\"%.*s\"\n", avp_s->name.s.len, avp_s->name.s.s,
					avp_val.s.len, avp_val.s.s);
		} else {
			LM_DBG("%.*s:%d\n", avp_s->name.s.len, avp_s->name.s.s, avp_val.n);
		}
	}
	return 0;
}
