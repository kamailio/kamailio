...
int th_get_uri_param_value(str *uri, str *name, str *value)
{
	...
	LM_DBG("uri params: [%.*s] - sip uri params: [%.*s]\n", puri.params.len,
			(puri.params.s) ? puri.params.s : "", puri.sip_params.len,
			(puri.sip_params.s) ? puri.sip_params.s : "");
...
int th_mask_via(sip_msg_t *msg, str *via_prefix)
{
	...
	for(hdr = msg->h_via1; hdr; hdr = next_sibling_hdr(hdr)) {
		for(via = (struct via_body *)hdr->parsed; via; via = via->next) {
			...
			if(i != 1) {
				out.s = th_mask_encode(via->name.s, vlen, via_prefix, &out.len);
				if(out.s == NULL) {
		...
		for(via = (struct via_body *)hdr->parsed; via; via = via->next) {
			...
			if(i != 1) {
				...
				if(i == 2) {
					...
					if(parse_via(out.s, out.s + out.len + CRLF_LEN + 1, via2)
							== NULL) {
...
int th_unmask_callid(sip_msg_t *msg)
{
	...
	if((msg->callid->body.len < th_callid_prefix.len)
			|| (strncasecmp(msg->callid->body.s, th_callid_prefix.s,
						th_callid_prefix.len)
					!= 0)) {
		LM_DBG("call-id [%.*s] not encoded", msg->callid->body.len,
				msg->callid->body.s);
		return 0;
	}
...
int th_mask_callid_str(str *icallid, str *ocallid)
{
	...
	out.s = th_mask_encode(
			icallid->s, icallid->len, &th_callid_prefix, &out.len);
	if(out.s == NULL) {
...
int th_unmask_callid_str(str *icallid, str *ocallid)
{
	...
	out.s = th_mask_decode(
			icallid->s, icallid->len, &th_callid_prefix, 0, &out.len);
	if(out.s == NULL) {
	...
	while(hdr != NULL) {
		...
		while(rr) {
			...
			switch(utype) {
				...
				case 2: /* decode */
					if(mode == 0) {
						out.s = th_mask_decode(pval.s, pval.len,
		...
		while(rr) {
			...
			if(i != 1) {
				...
				if(th_get_uri_param_value(
						   &rr->nameaddr.uri, &th_uparam_name, &eval)
								< 0
						|| eval.len <= 0) {
				...
				out.s = th_mask_decode(
						eval.s, eval.len, &th_uparam_prefix, 0, &out.len);
...
int th_unmask_refer_to(sip_msg_t *msg, str *uri_prefix)
{
	...
	if(th_get_uri_param_value(uri, &th_uparam_name, &eval) < 0 || eval.len <= 0)
		return -1;
...
int th_update_hdr_replaces(sip_msg_t *msg)
{
	...
	if(rcallid.len > th_callid_prefix.len
			&& strncmp(rcallid.s, th_callid_prefix.s, th_callid_prefix.len)
					   == 0) {
		/* value encoded - decode it */
		out.s = th_mask_decode(
				rcallid.s, rcallid.len, &th_callid_prefix, 0, &out.len);
	} else {
		/* value decoded - encode it */
		out.s = th_mask_encode(
				rcallid.s, rcallid.len, &th_callid_prefix, &out.len);
	}
	if(out.s == NULL) {
...
int th_add_via_cookie(sip_msg_t *msg, struct via_body *via)
{
	...
	memcpy(out.s + th_cookie_name.len + 2, th_cookie_value.s,
			th_cookie_value.len);
	out.s[out.len - 1] = 'v';
	out.s[out.len] = '\0';
	if(insert_new_lump_after(l, out.s, out.len, 0) == 0) {
...
int th_add_hdr_cookie(sip_msg_t *msg)
{
	...
	memcpy(h.s + th_cookie_name.len + 2, th_cookie_value.s,
			th_cookie_value.len);
	memcpy(h.s + th_cookie_name.len + 2 + th_cookie_value.len + 1, CRLF,
			CRLF_LEN);
...
struct via_param *th_get_via_cookie(sip_msg_t *msg, struct via_body *via)
{
	...
	for(p = via->param_lst; p; p = p->next) {
		if(p->name.len == th_cookie_name.len
				&& strncasecmp(p->name.s, th_cookie_name.s, th_cookie_name.len)
						   == 0)
			return p;
	}
	return NULL;
...
hdr_field_t *th_get_hdr_cookie(sip_msg_t *msg)
{
	...
	for(hf = msg->headers; hf; hf = hf->next) {
		if(hf->name.len == th_cookie_name.len
				&& strncasecmp(hf->name.s, th_cookie_name.s, th_cookie_name.len)
						   == 0)
			return hf;
	}
	return NULL;
