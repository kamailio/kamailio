...
int reload_address_table(void)
{
	...
	for (i = 0; i < RES_ROW_N(res); i++) {
		...
		{
			LM_DBG("failure during checks of database value 1 (trust) in address table\n");
			goto dberror;
		}
...
int check_address(struct dest_info* dst, int trust)
{
	...
	if (addr_hash_table && match_addr_hash_table(*addr_hash_table, trust, &ip, (unsigned int)port) == 1) {
	        	LM_DBG("Destination has been found in IP table\n");
			return 1;
        } else {
	        if(subnet_table) {
	                if(match_subnet_table(*subnet_table, trust, &ip, (unsigned int)port) == 1) {
	                        LM_DBG("Destination has been found in subnet table\n");
	                        return 1;
                        }	                
		}
	} 
