		...
		struct sip_msg *msg, unsigned int flags, void *param)
{
	...
	if(group >= 0 && group <= 5) {
		switch(method) {
			case METHOD_INVITE:
				update_stat(VAR_NAME(invite)[group], 1);
				break;
			case METHOD_CANCEL:
				update_stat(VAR_NAME(cancel)[group], 1);
				break;
			case METHOD_BYE:
				update_stat(VAR_NAME(bye)[group], 1);
				break;
			case METHOD_REGISTER:
				update_stat(VAR_NAME(reg)[group], 1);
				break;
			case METHOD_MESSAGE:
				update_stat(VAR_NAME(message)[group], 1);
				break;
			case METHOD_PRACK:
				update_stat(VAR_NAME(prack)[group], 1);
				break;
			case METHOD_UPDATE:
				update_stat(VAR_NAME(update)[group], 1);
				break;
			case METHOD_REFER:
...
int register_core_stats(void)
{
	...
	if(stats_proc_stats_init_rpc() < 0)
		return -1;
...
static void rpc_get_grp_vars_cbk(void *p, str *g, str *n, counter_handle_t h)
{
	...
	rpc->rpl_printf(ctx, "%.*s:%.*s = %lu", g->len, g->s, n->len, n->s,
			counter_get_val(h));
}
...
static void stats_get_all(rpc_t *rpc, void *ctx, char *stat)
{
	...
	if(len == 3 && strcmp("all", stat) == 0) {
		...
		if(s_stat) {
			rpc->rpl_printf(ctx, "%s:%s = %lu", ZSW(get_stat_module(s_stat)),
...
static void rpc_fetch_add_stat(rpc_t *rpc, void *ctx, void *hst, char *g,
		char *n, unsigned long val, int numeric)
{
	char nbuf[128];
	int res;
...
static void rpc_fetch_grp_vars_cbk(void *p, str *g, str *n, counter_handle_t h)
{
	...
	rpc_fetch_add_stat(packed_params->rpc, packed_params->ctx,
			packed_params->hst, g->s, n->s, counter_get_val(h),
			packed_params->numeric);
}
 ...
 * All statistic getter RPC callback.
 */
static void stats_fetch_all(
		rpc_t *rpc, void *ctx, void *th, char *stat, int numeric)
{
	int len = strlen(stat);
	struct rpc_list_params packed_params;
	str s_statistic;
	...
	if(len == 3 && strcmp("all", stat) == 0) {
		...
		s_stat = get_stat(&s_statistic);
		if(s_stat) {
			rpc_fetch_add_stat(rpc, ctx, th, ZSW(get_stat_module(s_stat)),
					ZSW(get_stat_name(s_stat)), get_stat_val(s_stat), numeric);
		} else {
			n = strchr(stat, '.');
			if(n == NULL) {
				n = strchr(stat, ':');
			...
			if(s_stat) {
				...
				rpc_fetch_add_stat(rpc, ctx, th, ZSW(m),
						ZSW(get_stat_name(s_stat)), get_stat_val(s_stat),
						numeric);
			}
		}
 ...
 */
static void rpc_reset_or_clear_grp_vars_cbk(
		void *p, str *g, str *n, counter_handle_t h)
{
	struct rpc_list_params *packed_params;
	rpc_t *rpc;
	void *ctx;
		...
		if(clear) {
			...
			if(old_val == new_val) {
				rpc->rpl_printf(ctx, "%s:%s = %lu",
						...
						ZSW(get_stat_name(s_stat)), new_val);
			} else {
				rpc->rpl_printf(ctx, "%s:%s = %lu (%lu)",
						ZSW(get_stat_module(s_stat)),
						ZSW(get_stat_name(s_stat)), new_val, old_val);
			}
		} else {
			reset_stat(s_stat);
		}
 ...
 */
static void stats_reset_or_clear_all(
		rpc_t *rpc, void *ctx, char *stat, int clear)
{
	int len = strlen(stat);
	struct rpc_list_params packed_params;
	str s_statistic;
	...
	if(len == 3 && strcmp("all", stat) == 0) {
		...
		counter_iterate_grp_names(
				rpc_reset_or_clear_all_grps_cbk, &packed_params);
	} else if(stat[len - 1] == ':') {
		packed_params.rpc = rpc;
		packed_params.ctx = ctx;
		packed_params.clear = clear;
		stat[len - 1] = '\0';
		counter_iterate_grp_vars(
				stat, rpc_reset_or_clear_grp_vars_cbk, &packed_params);
		stat[len - 1] = ':';
	} else {
		s_statistic.s = stat;
		s_statistic.len = strlen(stat);
