...
static void process_dialog_peers(struct str_list *peers, pstate_t new_state,
		int early_enabled, str *callid);
static struct str_list *peer_at_host_list(struct sip_uri *uri);
static inline void free_dlginfo_cell(void *param);
...
static struct dlginfo_cell *get_dialog_data(struct dlg_cell *dlg, int type,
...
static void trigger_event_route(
		pstate_t pstate, pstate_t prev_state, str *peer, str *uniq_id)
{
	...
	avp_name.s.len = 8;
	avp_val.s.s = (char *)PSTATE_TO_STR(pstate);
	...
	avp_name.s.len = 13;
	avp_val.s.s = (char *)PSTATE_TO_STR(prev_state);
	avp_val.s.len = strlen(avp_val.s.s);
	if(add_avp(AVP_NAME_STR | AVP_VAL_STR, avp_name, avp_val) < 0) {
	...
	LM_DBG("event_route[%s] executed: peer=%.*s, state=%s, id=%.*s\n",
			PEERSTATE_EVENT_ROUTE, peer->len, peer->s, PSTATE_TO_STR(pstate),
			uniq_id->len, uniq_id->s);
	return;
 ...
 */
static inline void notify_with_callbacks(pstate_t pstate, pstate_t prev_state,
		str *peer, str *uniq_id, peerstate_event_type_t event_type)
{
	peerstate_cb_ctx_t ctx;
	int call_count = 0, is_registered = 0;

	LM_DBG("[%.*s] notify for peer: %.*s with state: %s (event_type=%d)\n",
			uniq_id->len, uniq_id->s, peer->len, peer->s, PSTATE_TO_STR(pstate),
			event_type);
...
static void __usrloc_changed(ucontact_t *c, int type, void *param)
{
	...
	if(valid_contact_counter == 0) {
		state_changed = update_peer_state_usrloc(&peer, pstate, &prev_state);
		if(state_changed > 0) {
			LM_INFO("Peer '%.*s' state changed from %s to %s\n", peer.len,
					peer.s, PSTATE_TO_STR(prev_state), PSTATE_TO_STR(pstate));
			notify_with_callbacks(pstate, prev_state, &peer, &cur_ruid,
		...
		int disable_caller_notify, int disable_callee_notify)
{
	...
	dlginfo->callid.s = (char *)dlginfo + sizeof(struct dlginfo_cell);
		...
		int early_enabled, str *callid)
{
	...
	while(peer) {
		...
		if(state_changed > 0) {
			LM_DBG("[%.*s] '%.*s': %s -> %s\n", callid->len, callid->s,
					peer->s.len, peer->s.s, PSTATE_TO_STR(prev_state),
					PSTATE_TO_STR(new_state));
...
static void rpc_get_peer(rpc_t *rpc, void *ctx)
{
	...
	if(rpc->add(ctx, "{", &ctx) < 0)
		return;
	rpc->struct_add(ctx, "Ssds", "peer", &peer, "state", PSTATE_TO_STR(state),
...
static void rpc_list(rpc_t *rpc, void *ctx)
{
	...
	for(i = 0; i < count; i++) {
		...
		if(get_peer_info(&peers[i], &pstate, &call_count, &is_registered) < 0) {
			rpc->struct_add(peer_ctx, "Ssds", "name", &peers[i], "state",
					"UNKNOWN", "call_count", 0, "registered", "no");
		} else {
			rpc->struct_add(peer_ctx, "Ssds", "name", &peers[i], "state",
					PSTATE_TO_STR(pstate), "call_count", call_count,
...
static void rpc_dump(rpc_t *rpc, void *ctx)
{
	...
	for(i = 0; i < count; i++) {
		...
		if(get_peer_info(&peers[i], &state, &call_count, &is_registered) < 0) {
			...
			rpc->struct_add(peer_ctx, "Ssds", "name", &peers[i], "state",
					PSTATE_TO_STR(state), "call_count", call_count,
					"registered", is_registered ? "yes" : "no");
		}
	}
