...
static struct peer_state **peer_state_hash_table = NULL;
...
int init_peerstate_cache(int expire_time, int cleanup_interval, int hash_size)
{
	...
	peer_state_hash_table = (struct peer_state **)shm_malloc(
			peer_state_hash_size * sizeof(struct peer_state *));
		...
		int early_enabled, pstate_t *prev_state)
{
	...
	if(!p) {
		p = (struct peer_state *)shm_malloc(
				sizeof(struct peer_state) + peer->len);
		if(!p) {
			SHM_MEM_ERROR;
			lock_release(peer_state_lock);
			return -1;
		...
		p->peer.s = (char *)p + sizeof(struct peer_state);
		p->peer.len = peer->len;
	...
	if(candidate_state == RINGING) {
		...
		if(old_state == INUSE)
			new_state = INUSE; // Don't downgrade from INUSE to RINGING
		else if(old_state == RINGING)
			new_state = RINGING; // Already ringing, stay ringing
		else
			new_state =
					RINGING; // NOT_INUSE or UNAVAILABLE -> upgrade to RINGING
		...
		LM_DBG("EARLY: peer='%.*s', old_state=%s, new_state=%s, old_count=%d, "
			   "new_count=%d, is_registered=%d\n",
				...
				PSTATE_TO_STR(new_state), old_count, new_count,
				p->is_registered);
		...
		LM_DBG("CONFIRMED: peer='%.*s', old_state=%s, new_state=%s, "
			   ...
			   "is_registered=%d\n",
				peer->len, peer->s, PSTATE_TO_STR(old_state),
				PSTATE_TO_STR(new_state), old_count, new_count, early_enabled,
				p->is_registered);
		...
		str *peer, pstate_t candidate_state, pstate_t *prev_state)
{
	...
	if(candidate_state == NOT_INUSE) {
		...
		LM_DBG("REGISTERED: peer='%.*s', old_state=%s, new_state=%s, count=%d, "
			   "is_registered=%d\n",
				peer->len, peer->s, PSTATE_TO_STR(old_state),
				PSTATE_TO_STR(new_state), call_count, p->is_registered);
	} else if(candidate_state == UNAVAILABLE) {
		// UNREGISTERED event occurred
		// Update registration flag
		p->is_registered = 0;
		if(old_state == NOT_INUSE)
			new_state = UNAVAILABLE; // Peer was idle, now unavailable
		else if(old_state == INUSE)
			new_state =
					INUSE; // Peer is in active call, Don't mark unavailable during call
		else if(old_state == RINGING)
			new_state =
					RINGING; // Peer is ringing, Don't mark unavailable during ringing
		else
			new_state = UNAVAILABLE; // Already unavailable
 ...
 */
int get_peers_by_state(pstate_t target_state, str **peers, int *count)
{
	int i, found = 0;
	struct peer_state *p;
	str *result = NULL;
