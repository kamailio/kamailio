...
static inline int add_contacts_to_loc_set(
		struct sip_msg *msg, struct location **loc_set)
{
	struct sip_uri uri;
	...
	if(msg->contact->parsed) {
		...
		for(; contacts; contacts = contacts->next) {
			...
			if(add_location(loc_set, &contacts->uri, 0, prio, CPL_LOC_DUPL)
					!= 0) {
...
static void reply_callback(struct cell *t, int type, struct tmcb_params *ps)
{
	...
	if(intr->proxy.last_to_proxy && !(no_new_branches(t))) {
		...
		if(intr->ip == 0)
			intr->ip = (intr->proxy.default_) ? intr->proxy.default_
											  : DEFAULT_ACTION;
...
static inline char *run_proxy(struct cpl_interpreter *intr)
{
		...
		switch(attr_name) {
			case TIMEOUT_ATTR:
				if(cpl_env.timer_avp.n || cpl_env.timer_avp.s.s) {
					is_val.n = n;
					if(add_avp(cpl_env.timer_avp_type, cpl_env.timer_avp,
							   is_val)
							< 0) {
			...
			case RECURSE_ATTR:
				...
				break;
			case ORDERING_ATTR:
				if(n != PARALLEL_VAL && n != SEQUENTIAL_VAL
						&& n != FIRSTONLY_VAL) {
					LM_ERR("invalid value (%u) found"
	...
	if(!(intr->flags & CPL_PROXY_DONE)) {
		...
		if(intr->accept_language != STR_NOT_FOUND) {
			search_and_duplicate_hdr(
					intr, accept_language, HDR_ACCEPTLANGUAGE_F, s);
			if(intr->accept_language != STR_NOT_FOUND)
		...
		if(cpl_fct.tmb.register_tmcb(intr->msg, 0,
				   TMCB_ON_FAILURE | TMCB_RESPONSE_OUT, reply_callback,
				   (void *)intr, 0)
				<= 0) {
