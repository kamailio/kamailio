...
#define append_double_str_attr(_p_, _s1_, _s2_, _end_, _error_)                \
	do {                                                                       \
		check_overflow(_p_,                                                    \
				(_s1_).len + (_s2_).len                                        \
						+ 1 * ((((_s2_).len + (_s2_).len) & 0x0001) == 1),     \
				_end_, _error_);                                               \
		*((length_type_ptr)(_p_)) =                                            \
				htons((length_type)((_s1_).len) + (_s2_).len);                 \
		(_p_) += sizeof(length_type);                                          \
...
static inline char *decode_mail_url(
		char *p, char *p_end, char *url, unsigned char *nr_attr)
{
				...
				switch(status) {
					case EMAIL_HDR_NAME:
						...
						if(hdr_len == BODY_EMAILHDR_LEN
								&& strncasecmp(buf, BODY_EMAILHDR_STR, hdr_len)
										   == 0) {
							/* BODY hdr found */
							...
							max_len = MAX_EMAIL_BODY_SIZE;
								  ...
								  && strncasecmp(
											 buf, SUBJECT_EMAILHDR_STR, hdr_len)
											 == 0) {
							/* SUBJECT hdr found */
 ...
 */
static inline int encode_address_attr(
		xmlNodePtr node, char *node_ptr, char *buf_end)
{
	xmlAttrPtr attr;
	char *p, *p_orig;
 ...
 */
static inline int encode_address_switch_attr(
		xmlNodePtr node, char *node_ptr, char *buf_end)
{
	xmlAttrPtr attr;
	char *p, *p_orig;
		...
		switch(attr->name[0]) {
			...
			case 'f':
				...
				if(val.s[0] == 'D' || val.s[0] == 'd')
					...
					append_short_attr(
							p, ORIGINAL_DESTINATION_VAL, buf_end, error);
				else if(!val.s[6])
 ...
 */
static inline int encode_lang_attr(
		xmlNodePtr node, char *node_ptr, char *buf_end)
 ...
 */
static inline int encode_priority_attr(
		xmlNodePtr node, char *node_ptr, char *buf_end)
{
	xmlAttrPtr attr;
	char *p, *p_orig;
 ...
 */
static inline int encode_string_switch_attr(
		xmlNodePtr node, char *node_ptr, char *buf_end)
{
	xmlAttrPtr attr;
	char *p, *p_orig;
 ...
 *  [| attr2_t(2) attr2_len(2) attr_val(2*x) |]?  CONTAINS attr (NT)
 */
static inline int encode_string_attr(
		xmlNodePtr node, char *node_ptr, char *buf_end)
{
	xmlAttrPtr attr;
	char *p, *p_orig;
 ...
 */
static inline int encode_time_switch_attr(
		xmlNodePtr node, char *node_ptr, char *buf_end)
{
 ...
 */
static inline int encode_time_attr(
		xmlNodePtr node, char *node_ptr, char *buf_end)
{
	xmlAttrPtr attr;
	char *p, *p_orig;
 ...
 */
static inline int encode_lookup_attr(
		xmlNodePtr node, char *node_ptr, char *buf_end)
{
	xmlAttrPtr attr;
	char *p, *p_orig;
 ...
 */
static inline int encode_location_attr(
		xmlNodePtr node, char *node_ptr, char *buf_end)
{
	struct sip_uri uri;
	xmlAttrPtr attr;
 ...
 */
static inline int encode_rmvloc_attr(
		xmlNodePtr node, char *node_ptr, char *buf_end)
{
	struct sip_uri uri;
	xmlAttrPtr attr;
 ...
 */
static inline int encode_proxy_attr(
		xmlNodePtr node, char *node_ptr, char *buf_end)
{
	xmlAttrPtr attr;
	char *p, *p_orig;
 ...
 */
static inline int encode_reject_attr(
		xmlNodePtr node, char *node_ptr, char *buf_end)
{
	xmlAttrPtr attr;
	char *p, *p_orig;
 ...
 * [| attr2_t(2) attr2_len(2) attr2_val(2*x)|]?      REASON attr (NT)
 */
static inline int encode_redirect_attr(
		xmlNodePtr node, char *node_ptr, char *buf_end)
{
	xmlAttrPtr attr;
	char *p, *p_orig;
 ...
 */
static inline int encode_log_attr(
		xmlNodePtr node, char *node_ptr, char *buf_end)
{
	xmlAttrPtr attr;
	char *p, *p_orig;
	...
	*nr_attr = 0;
	...
	FOR_ALL_ATTR(node, attr)
	{
		...
		switch(attr->name[0]) {
			case 'n':
			case 'N':
				if(val.len > MAX_NAME_SIZE)
					val.len = MAX_NAME_SIZE;
				set_attr_type(p, NAME_ATTR, buf_end, error);
				break;
			case 'c':
			case 'C':
				if(val.len > MAX_COMMENT_SIZE)
					val.len = MAX_COMMENT_SIZE;
 ...
 */
static inline int encode_mail_attr(
		xmlNodePtr node, char *node_ptr, char *buf_end)
{
	xmlAttrPtr attr;
	char *p, *p_orig;
	...
	*nr_attr = 0;
	...
	FOR_ALL_ATTR(node, attr)
	{
		...
		p = decode_mail_url(
				p, buf_end, (char *)xmlGetProp(node, attr->name), nr_attr);
		if(p == 0)
			goto error;
	}
 ...
 */
static inline int encode_subaction_attr(
		xmlNodePtr node, char *node_ptr, char *buf_end)
{
	xmlAttrPtr attr;
	str val;
 ...
 */
static inline int encode_sub_attr(
		xmlNodePtr node, char *node_ptr, char *buf_end)
{
	xmlAttrPtr attr;
	char *p, *p_orig;
	...
	*nr_attr = 0;
	...
	FOR_ALL_ATTR(node, attr)
	{
		...
		append_short_attr(
				p, (unsigned short)(node_ptr - sub_ptr), buf_end, error);
	}
