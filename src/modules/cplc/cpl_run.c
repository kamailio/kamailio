...
#define get_first_child(_node_)                 \
	((NR_OF_KIDS(_node_) == 0) ? DEFAULT_ACTION \
							   : (_node_) + KID_OFFSET(_node_, 0))
			...
			if(contact) {
				...
				do {
					...
					if(add_location(&(intr->loc_set), &contact->c,
							   ...
							   CPL_LOC_DUPL
									   | ((contact->cflags
												  & cpl_fct.ulb.nat_flag)
													   ? CPL_LOC_NATED
													   : 0))
							== -1) {
						LM_ERR("unable to add location to set :-(\n");
						cpl_fct.ulb.unlock_udomain(
								cpl_env.lu_domain, &intr->user);
						goto runtime_error;
					}
					contact = contact->next;
...
static inline char *run_redirect(struct cpl_interpreter *intr)
{
			...
			case PERMANENT_ATTR:
				if(n != YES_VAL && n != NO_VAL) {
					LM_ERR("unsupported value (%d)"
						   " in attribute PERMANENT for REDIRECT node",
							n);
					goto script_error;
				}
				permanent = n;
	...
	for(loc = intr->loc_set; loc; loc = loc->next)
		lump_str.len += 1 /*"<"*/ + loc->addr.uri.len
						+ 7 /*">;q=x.x"*/ + 2 * (loc->next != 0) /*" ,"*/;
