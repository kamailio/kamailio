			...
			case ADDRESS_NODE:
				...
				if(!msg_val) {
					switch(field) {
						...
						case ORIGINAL_DESTINATION_VAL: /* TO */
							if(!intr->to) {
								...
								if(!intr->msg->to
										&& (parse_headers(intr->msg, HDR_TO_F,
													0) == -1
												|| !intr->msg->to)) {
					...
					switch(subfield) {
						...
						case TEL_VAL:
							...
							if(parsed_uri.user_param_val.len == 5
									&& memcmp(parsed_uri.user_param_val.s,
											   "phone", 5)
											   == 0)
								msg_val = &(parsed_uri.user);
							break;
					...
					LM_DBG("extracted val. is <%.*s>\n",
							(msg_val == 0) ? 0 : msg_val->len,
							(msg_val == 0) ? 0 : msg_val->s);
				}
				...
				switch(attr_name) {
					case IS_ATTR:
						if((!msg_val && !cpl_val.s)
								|| (msg_val && msg_val->len == cpl_val.len
										&& strncasecmp(msg_val->s, cpl_val.s,
												   cpl_val.len)
												   == 0)) {
					...
					case CONTAINS_ATTR:
						if(subfield != DISPLAY_VAL) {
							LM_WARN("operator "
									"CONTAINS applies only to DISPLAY -> "
									"ignored\n");
						} else {
					...
					case SUBDOMAIN_OF_ATTR:
						switch(subfield) {
							case HOST_VAL:
								...
								if(k >= 0
										&& (k == 0 || msg_val->s[k - 1] == '.')
										&& !strncasecmp(cpl_val.s,
												msg_val->s + k, cpl_val.len)) {
									LM_DBG("matching on "
							...
							case TEL_VAL:
								...
								if(msg_val->len >= cpl_val.len
										&& !strncasecmp(cpl_val.s, msg_val->s,
												cpl_val.len)) {
									LM_DBG("matching on "
										   "ADDRESS node (SUBDOMAIN_OF)\n");
									return get_first_child(kid);
					...
					switch(field) {
						case SUBJECT_VAL: /* SUBJECT */
							...
							if(!intr->subject) {
								...
								intr->subject = &(intr->msg->subject->body);
							}
			...
			case PRIORITY_NODE:
				...
				if(attr_val == UNKNOWN_PRIO_VAL) {
					if(attr_name != EQUAL_ATTR) {
						LM_ERR("bad PRIORITY"
							   " branch: attr=EQUAL doesn't match "
							   "val=UNKNOWN\n");
						goto script_error;
					}
					...
					get_str_attr(
							p, cpl_val.s, cpl_val.len, intr, script_error, 1);
				}

				LM_DBG("testing PRIORITY branch "
					   "(attr=%d,val=%d) [%.*s](%d)..\n",
						attr_name, attr_val, cpl_val.len, cpl_val.s,
						cpl_val.len);
				if(!msg_val.s) {
					if(!intr->priority) {
						/* get the PRIORITY header from message */
						if(!intr->msg->priority) {
							if(parse_headers(intr->msg, HDR_PRIORITY_F, 0)
									== -1) {
								...
								intr->priority = &(intr->msg->priority->body);
							}
						} else {
							intr->priority = &(intr->msg->priority->body);
						}
				...
				if(msg_attr_val == UNKNOWN_PRIO_VAL
						&& attr_name != EQUAL_ATTR) {
					LM_NOTICE(
							"UNKNOWN "
...
static inline char *run_time_switch(struct cpl_interpreter *intr)
{
	...
	LM_DBG("checking recv. time stamp <%llu>\n",
			(unsigned long long)intr->recv_time);
			...
			case TIME_NODE:
				...
				for(j = 0; j < nr_attrs; j++) {
					...
					switch(attr_name) {
						case DTSTART_ATTR:
							if(!attr_str
									|| cpl_tr_parse_dtstart(&trt, attr_str))
								goto parse_err;
							flags ^= (1 << 0);
							break;
						case DTEND_ATTR:
						...
						case DURATION_ATTR:
							if(!attr_str
									|| cpl_tr_parse_duration(&trt, attr_str))
								goto parse_err;
							flags ^= (1 << 1);
							break;
						case FREQ_ATTR:
						...
						case INTERVAL_ATTR:
							if(attr_str
									&& cpl_tr_parse_interval(&trt, attr_str))
								goto parse_err;
							break;
						case BYDAY_ATTR:
						...
						case BYWEEKNO_ATTR:
							if(attr_str
									&& cpl_tr_parse_byweekno(&trt, attr_str))
								goto parse_err;
							break;
						case WKST_ATTR:
...
inline static int is_lang_tag_matching(
		str *range, str *cpl_tag, str *cpl_subtag)
{
	char *c;
	char *end;
	str tag = {0, 0};
	...
	while(c < end) {
		...
		if(c == end || *c == ',') {
			...
			LM_DBG("testing range [%.*s]-[%.*s] against tag [%.*s]-[%.*s]\n",
					tag.len, tag.s, subtag.len, subtag.s, cpl_tag->len,
					cpl_tag->s, cpl_subtag->len, cpl_subtag->s);
			/* language range of "*" is ignored for the purpose of matching*/
			if(!(tag.len == 1 && *tag.s == '*')) {
				/* does the language tag matches ? */
				if(tag.len == cpl_tag->len
					...
					if(subtag.len == cpl_subtag->len
							&& !strncasecmp(
									subtag.s, cpl_subtag->s, subtag.len))
						return 1;
				}
			}
				...
				if(!msg_val.s) {
					...
					if(!intr->accept_language) {
						...
						if(!intr->msg->accept_language) {
							if(parse_headers(intr->msg, HDR_ACCEPTLANGUAGE_F, 0)
									== -1) {
						...
						intr->subject = &(intr->msg->accept_language->body);
					}
				}
				trim_len(msg_val.len, msg_val.s, *(intr->subject));
				LM_DBG("extracted msg string is "
