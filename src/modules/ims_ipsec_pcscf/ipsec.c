...
static int delsa_data_cb(const struct nlmsghdr *nlh, void *data)
{
	...
	if(delmsg_buf->offset + new_delmsg_len
			> sizeof(delmsg_buf->buf) / sizeof(delmsg_buf->buf[0])) {
		LM_ERR("Not enough memory allocated for delete SAs netlink command\n");
		return MNL_CB_ERROR;
	}

	struct nlmsghdr *new_delmsg =
			(struct nlmsghdr *)&delmsg_buf->buf[delmsg_buf->offset];
	...
	mnl_attr_put(
			new_delmsg, XFRMA_SRCADDR, sizeof(xsid->daddr), &xsinfo->saddr);
...
static int delpolicy_data_cb(const struct nlmsghdr *nlh, void *data)
{
	...
	if(delmsg_buf->offset + new_delmsg_len
			> sizeof(delmsg_buf->buf) / sizeof(delmsg_buf->buf[0])) {
		LM_ERR("Not enough memory allocated for delete policies netlink "
			   "command\n");
		return MNL_CB_ERROR;
	}
...
int clean_sa(struct mnl_socket *mnl_socket)
{
	...
	while(ret > 0) {
		ret = mnl_cb_run(buf, ret, req.n.nlmsg_seq,
				mnl_socket_get_portid(mnl_socket), delsa_data_cb, &delmsg_buf);
		if(ret <= MNL_CB_STOP) {
	...
	// DELETE SAs
	if(mnl_socket_sendto(mnl_socket, &delmsg_buf.buf, delmsg_buf.offset)
			== -1) {
...
int clean_policy(struct mnl_socket *mnl_socket)
{
	...
	if(mnl_socket_sendto(mnl_socket, &req, req.n.nlmsg_len) == -1) {
		LM_ERR("Error sending get all policies command via netlink socket: "
			   "%s\n",
				strerror(errno));
		return 1;
	}
	...
	while(ret > 0) {
		ret = mnl_cb_run(buf, ret, req.n.nlmsg_seq,
				mnl_socket_get_portid(mnl_socket), delpolicy_data_cb,
				&delmsg_buf);
		if(ret <= MNL_CB_STOP) {
...
static int delete_unused_sa_cb(const struct nlmsghdr *nlh, void *data)
{
	...
	if(xsinfo->sel.family == AF_INET6) {
		memcpy(sa_src_addr.u.addr32, xsinfo->sel.saddr.a6,
				sizeof(xsinfo->sel.saddr.a6));
		memcpy(sa_dst_addr.u.addr32, xsinfo->sel.daddr.a6,
				sizeof(xsinfo->sel.daddr.a6));
	...
	while(1) {
		...
		if(ip_addr_cmp(&sa_src_addr, &received_host_addr)
				&& ip_addr_cmp(&sa_dst_addr, proxy_ip_addr)) {
			// check for SA1
		...
		if(ip_addr_cmp(&sa_src_addr, proxy_ip_addr)
				&& ip_addr_cmp(&sa_dst_addr, &received_host_addr)) {
			// check for SA3
	...
	LM_DBG("The SA is not used and will be deleted: spi:%u | saddr:%s:%u | "
		   "daddr:%s:%u\n",
...
static int delete_unused_sa(struct mnl_socket *mnl_socket, void *contacts)
{
	...
	while(ret > 0) {
		ret = mnl_cb_run(buf, ret, req.n.nlmsg_seq,
				mnl_socket_get_portid(mnl_socket), delete_unused_sa_cb,
				&del_data);
		if(ret <= MNL_CB_STOP) {
			break;
		}
	...
	// DELETE unused SAs
	if(mnl_socket_sendto(
			   mnl_socket, &del_data.delmsg_buf.buf, del_data.delmsg_buf.offset)
			== -1) {
		LM_ERR("Error sending delete unused SAs command via netlink socket: "
			   "%s\n",
				strerror(errno));
		return 1;
	}
...
static int delete_unused_policy_cb(const struct nlmsghdr *nlh, void *data)
{
	...
	if(xpinfo->sel.family == AF_INET6) {
		memcpy(sa_src_addr.u.addr32, xpinfo->sel.saddr.a6,
				sizeof(xpinfo->sel.saddr.a6));
		memcpy(sa_dst_addr.u.addr32, xpinfo->sel.daddr.a6,
				sizeof(xpinfo->sel.daddr.a6));
		sa_src_addr.len = sa_dst_addr.len = 16;
	...
	LM_DBG("The POLICY is not used and will be deleted: saddr:%s:%u | "
		   "daddr:%s:%u\n",
...
static int delete_unused_policy(struct mnl_socket *mnl_socket, void *contacts)
{
	...
	while(ret > 0) {
		ret = mnl_cb_run(buf, ret, req.n.nlmsg_seq,
				mnl_socket_get_portid(mnl_socket), delete_unused_policy_cb,
				&del_data);
		if(ret <= MNL_CB_STOP) {
			break;
		}
	...
	if(mnl_socket_sendto(
			   mnl_socket, &del_data.delmsg_buf.buf, del_data.delmsg_buf.offset)
			== -1) {
		LM_ERR("Error sending delete unused policies command via netlink "
			   "socket: %s\n",
				strerror(errno));
		return 1;
	}
