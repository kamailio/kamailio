...
int init_port_gen(
		uint32_t sport_start_val, uint32_t cport_start_val, uint32_t range)
{
	if(sport_start_val < 1 || cport_start_val < 1) {
		return 1;
	}

	if((UINT32_MAX - range < sport_start_val)
			|| (UINT32_MAX - range < cport_start_val)) {
		return 2;
	}
...
uint32_t acquire_port(spi_list_t *used_ports, pthread_mutex_t *port_mut,
		uint32_t *port_val, uint32_t min_port, uint32_t max_port)
{
...
uint32_t acquire_sport()
{
	...
	return acquire_port(&port_data->used_sports, &port_data->sport_mut,
			&port_data->sport_val, port_data->min_sport, port_data->max_sport);
}
...
uint32_t acquire_cport()
{
	...
	return acquire_port(&port_data->used_cports, &port_data->cport_mut,
			&port_data->cport_val, port_data->min_cport, port_data->max_cport);
}
