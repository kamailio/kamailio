...
static int alter_rtcp(
		struct sip_msg *msg, str *body, str *oldport, str *newport);
		...
		for(crt_rtpp = rtpp_list->rn_first; crt_rtpp != NULL;
			...
			if(crt_rtpp->rn_url.len == rtpp_url.len) {
				...
				if(strncmp(crt_rtpp->rn_url.s, rtpp_url.s, rtpp_url.len) == 0) {
					...
					found = 1;
					crt_rtpp->rn_recheck_ticks = enable ? RPC_MIN_RECHECK_TICKS
														: RPC_MAX_RECHECK_TICKS;
...
static void rtpproxy_rpc_list(rpc_t *rpc, void *ctx)
{
	...
	for(rtpp_list = rtpp_set_list->rset_first; rtpp_list != NULL;
		...
		for(crt_rtpp = rtpp_list->rn_first; crt_rtpp != NULL;
			...
			rpc->struct_add(vh, "dSdddd", "setid", rtpp_list->id_set, "url",
					&crt_rtpp->rn_url, "index", crt_rtpp->idx, "disabled",
					crt_rtpp->rn_disabled, "weight", crt_rtpp->rn_weight,
...
static const char *rtpproxy_rpc_enable_doc[2] = {
		"Set state (enable/disable) for a rtp proxy.", 0};

static const char *rtpproxy_rpc_list_doc[2] = {"List rtp proxies.", 0};
...
static int mod_init(void)
{
	...
	if(ice_candidate_priority_avp_param) {
		s.s = ice_candidate_priority_avp_param;
		s.len = strlen(s.s);
		...
		if(pv_get_avp_name(
				   0, &(avp_spec.pvp), &ice_candidate_priority_avp, &avp_flags)
				!= 0) {
...
static int child_init(int rank)
{
	...
	for(rtpp_list = rtpp_set_list->rset_first; rtpp_list != 0;
		...
		for(pnode = rtpp_list->rn_first; pnode != 0; pnode = pnode->rn_next) {
			...
			rtpp_socks[pnode->idx] = socket(
					(pnode->rn_umode == 6) ? AF_INET6 : AF_INET, SOCK_DGRAM, 0);
...
static int alter_mediaport(struct sip_msg *msg, str *body, str *oldport,
		str *newport, int preserve)
{
	char *buf;
	int offset;
	struct lump *anchor;
	...
	if(preserve != 0) {
		...
		if(insert_new_lump_after(
				   anchor, buf, AOLDMEDPRT_LEN + oldport->len + CRLF_LEN, 0)
				== NULL) {
			LM_ERR("insert_new_lump_after failed\n");
			pkg_free(buf);
			return -1;
...
static char *append_filtered_ip(char *at, str *ip)
{
	int i;
	for(i = 0; i < ip->len; i++) {
		if(isdigit(ip->s[i])) {
...
static int insert_candidates(struct sip_msg *msg, char *where, str *ip,
		unsigned int port, int priority)
{
	char *buf, *at;
	struct lump *anchor;
	str rtp_port;
	...
	buf = pkg_malloc(
			24 + 78 + 14 + 24 + 2 * ip->len + 2 + 2 * rtcp_port.len + 24);
	if(buf == NULL) {
...
static struct rtpp_set *select_rtpp_set(int id_set)
{
	...
	for(rtpp_list = rtpp_set_list->rset_first;
			rtpp_list != NULL && rtpp_list->id_set != id_set;
			rtpp_list = rtpp_list->rset_next)
		;
...
static int unforce_rtp_proxy(struct sip_msg *msg, char *flags)
{
...
static int rtpproxy_manage(struct sip_msg *msg, char *flags, char *ip)
{
	...
	if(!(method == METHOD_INVITE || method == METHOD_ACK
			   || method == METHOD_CANCEL || method == METHOD_BYE
			   || method == METHOD_UPDATE || method == METHOD_PRACK))
		return -1;
...
static int rtpproxy_offer1_f(struct sip_msg *msg, char *str1, char *str2)
{
	str flags;
...
static int rtpproxy_offer2_f(struct sip_msg *msg, char *param1, char *param2)
{
	str flags, new_ip;
...
static int rtpproxy_answer1_f(struct sip_msg *msg, char *str1, char *str2)
{
	str flags;
...
static int append_opts(struct options *op, char ch)
{
	void *p;
...
static void free_opts(
		struct options *op1, struct options *op2, struct options *op3)
{
...
static int force_rtp_proxy(
		struct sip_msg *msg, char *str1, char *str2, int offer, int forcedIP)
{
	...
	flookup = force = real = orgip = commip = via = autobridge_ipv4v6 = extra =
			0;
	...
	if(nortpproxy_str.len) {
		for(cp = body.s;
				(len = body.s + body.len - cp) >= nortpproxy_str.len;) {
	...
	if(ice_candidate_priority_avp_param) {
		if(search_first_avp(ice_candidate_priority_avp_type,
				   ice_candidate_priority_avp, &ice_candidate_priority_val, 0)
				== NULL) {
	...
	for(;;) {
		...
		for(;;) {
			...
			if(!sdp_stream
					|| (ice_candidate_priority_val.n
							&& sdp_stream->remote_candidates.len))
				break;
...
#ifdef EXTRA_DEBUG
			LM_DBG("STR2IOVEC(newip[%.*s], v[9])", newip.strip.len,
					newip.strip.s);
			...
			for(ap = argv; cp < cpend;
					cp = next + 1, next = eat_token_end(cp, cpend)) {
				*next = 0;
				if(*cp != '\0') {
					*ap = cp;
					argc++;
			...
			if(sdp_stream->rtcp_port.s && sdp_stream->rtcp_port.len) {
				newrtcp.s = int2str(
						port + 1, &newrtcp.len); /* beware static buffer */
				/* Alter port. */
...
#endif
				if(alter_rtcp(msg, &body1, &sdp_stream->rtcp_port, &newrtcp)
						== -1) {
					FORCE_RTP_PROXY_RET(-1);
				}
			}
			...
			if(ice_candidate_priority_val.n && sdp_stream->ice_attrs_num > 0) {
				...
				body1.len = bodylimit - body1.s;
				if(insert_candidates(msg,
						   sdp_stream->ice_attr->foundation.s - 12,
						   &newip.strip, port, ice_candidate_priority_val.n)
						== -1) {
					FORCE_RTP_PROXY_RET(-1);
				}
			}
...
#endif
				if(alter_mediaip(
						   msg, &body1, &oldip, pf, &newip.strip, newip.pf, 0)
						== -1) {
					FORCE_RTP_PROXY_RET(-1);
				}
				if(!c2p)
			...
			if(commip && c1p && !c1p_altered) {
				...
				if(alter_mediaip(msg, &body1, &sdp_session->ip_addr,
						   sdp_session->pf, &newip.strip, newip.pf, 0)
						== -1) {
					FORCE_RTP_PROXY_RET(-1);
				}
				c1p_altered = 1;
			...
			if(o1p) {
				...
				if(alter_mediaip(msg, &body1, &sdp_session->o_ip_addr,
						   sdp_session->o_pf, &newip.strip, newip.pf, 0)
						== -1) {
					FORCE_RTP_PROXY_RET(-1);
				}
				o1p = 0;
...
{
	return ki_start_recording(msg);
}

/*
 * Returns the current RTP-Statistics from the RTP-Proxy
 */
static int pv_get_rtppstat_f(
...
{
	return rtpproxy_manage(
			msg, ((flags && flags->len > 0) ? flags->s : NULL), NULL);
}
...
static int ki_set_rtp_proxy_set(sip_msg_t *msg, int rset)
{
	selected_rtpp_set = select_rtpp_set(rset);
	if(selected_rtpp_set == NULL) {
...
static int ki_rtpproxy_destroy0(sip_msg_t *msg)
{
	return unforce_rtp_proxy(msg, NULL);
}

static int ki_rtpproxy_destroy(sip_msg_t *msg, str *flags)
{
	if(flags == NULL || flags->len <= 0)
		return unforce_rtp_proxy(msg, NULL);
	return unforce_rtp_proxy(msg, flags->s);
}

static int ki_rtpproxy_offer0(sip_msg_t *msg)
{
	return rtpproxy_offer1_helper_f(msg, NULL);
}

static int ki_rtpproxy_offer(sip_msg_t *msg, str *flags)
{
	if(flags == NULL || flags->len <= 0)
		return rtpproxy_offer1_helper_f(msg, NULL);
	return rtpproxy_offer1_helper_f(msg, flags->s);
...
static int ki_rtpproxy_offer_ip(sip_msg_t *msg, str *flags, str *mip)
{
	return force_rtp_proxy(msg, flags->s, mip->s, 1, 1);
}

static int ki_rtpproxy_answer0(sip_msg_t *msg)
{
	return rtpproxy_answer1_helper_f(msg, NULL);
}

static int ki_rtpproxy_answer(sip_msg_t *msg, str *flags)
{
	if(flags == NULL || flags->len <= 0)
		return rtpproxy_answer1_helper_f(msg, NULL);
	return rtpproxy_answer1_helper_f(msg, flags->s);
...
static int ki_rtpproxy_answer_ip(sip_msg_t *msg, str *flags, str *mip)
{
	return force_rtp_proxy(msg, flags->s, mip->s, 0, 1);
}

static int ki_rtpproxy_stream2uac(sip_msg_t *msg, str *pname, int count)
{
	return rtpproxy_stream(msg, pname, count, 1);
}

static int ki_rtpproxy_stream2uas(sip_msg_t *msg, str *pname, int count)
{
	return rtpproxy_stream(msg, pname, count, 0);
}

static int ki_rtpproxy_stop_stream2uac(sip_msg_t *msg)
{
	return rtpproxy_stop_stream(msg, 1);
}

static int ki_rtpproxy_stop_stream2uas(sip_msg_t *msg)
{
...
static sr_kemi_t sr_kemi_rtpproxy_exports[] = {
	...
	{ str_init("rtpproxy"), str_init("set_rtpproxy_set"),
		SR_KEMIP_INT, ki_set_rtp_proxy_set,
		{ SR_KEMIP_INT, SR_KEMIP_NONE, SR_KEMIP_NONE,
			SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE }
	},
	{ str_init("rtpproxy"), str_init("rtpproxy_destroy0"),
		SR_KEMIP_INT, ki_rtpproxy_destroy0,
		{ SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE,
			SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE }
	},
	{ str_init("rtpproxy"), str_init("rtpproxy_destroy"),
		SR_KEMIP_INT, ki_rtpproxy_destroy,
		{ SR_KEMIP_STR, SR_KEMIP_NONE, SR_KEMIP_NONE,
			SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE }
	},
	{ str_init("rtpproxy"), str_init("start_recording"),
		SR_KEMIP_INT, ki_start_recording,
		{ SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE,
			SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE }
	},
	{ str_init("rtpproxy"), str_init("rtpproxy_offer0"),
		SR_KEMIP_INT, ki_rtpproxy_offer0,
		{ SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE,
			SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE }
	},
	{ str_init("rtpproxy"), str_init("rtpproxy_offer"),
		SR_KEMIP_INT, ki_rtpproxy_offer,
		{ SR_KEMIP_STR, SR_KEMIP_NONE, SR_KEMIP_NONE,
			SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE }
	},
	{ str_init("rtpproxy"), str_init("rtpproxy_offer_ip"),
		SR_KEMIP_INT, ki_rtpproxy_offer_ip,
		{ SR_KEMIP_STR, SR_KEMIP_STR, SR_KEMIP_NONE,
			SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE }
	},
	{ str_init("rtpproxy"), str_init("rtpproxy_answer0"),
		SR_KEMIP_INT, ki_rtpproxy_answer0,
		{ SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE,
			SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE }
	},
	{ str_init("rtpproxy"), str_init("rtpproxy_answer"),
		SR_KEMIP_INT, ki_rtpproxy_answer,
		{ SR_KEMIP_STR, SR_KEMIP_NONE, SR_KEMIP_NONE,
			SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE }
	},
	{ str_init("rtpproxy"), str_init("rtpproxy_answer_ip"),
		SR_KEMIP_INT, ki_rtpproxy_answer_ip,
		{ SR_KEMIP_STR, SR_KEMIP_STR, SR_KEMIP_NONE,
			SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE }
	},
	{ str_init("rtpproxy"), str_init("rtpproxy_stream2uac"),
		SR_KEMIP_INT, ki_rtpproxy_stream2uac,
		{ SR_KEMIP_STR, SR_KEMIP_INT, SR_KEMIP_NONE,
			SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE }
	},
	{ str_init("rtpproxy"), str_init("rtpproxy_stream2uas"),
		SR_KEMIP_INT, ki_rtpproxy_stream2uas,
		{ SR_KEMIP_STR, SR_KEMIP_INT, SR_KEMIP_NONE,
			SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE }
	},
	{ str_init("rtpproxy"), str_init("rtpproxy_stop_stream2uac"),
		SR_KEMIP_INT, ki_rtpproxy_stop_stream2uac,
		{ SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE,
			SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE }
	},
	{ str_init("rtpproxy"), str_init("rtpproxy_stop_stream2uas"),
		SR_KEMIP_INT, ki_rtpproxy_stop_stream2uas,
		{ SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE,
			SR_KEMIP_NONE, SR_KEMIP_NONE, SR_KEMIP_NONE }
	},
