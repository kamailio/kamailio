...
int rtpproxy_stream(struct sip_msg *msg, str *pname, int count, int stream2uac)
{
	...
	char cbuf[16];
	struct iovec v[] = {{NULL, 0}, {cbuf, 0}, /* 1 P<count> */
			{" ", 1}, {NULL, 0},			  /* 3 callid */
			{" ", 1}, {NULL, 0},			  /* 5 pname */
			{" session ", 9}, {NULL, 0},	  /* 7 from tag */
			{";1 ", 3}, {NULL, 0},			  /* 9 to tag */
			{";1", 2}};
	...
	if(node->rn_ptl_supported == 0) {
		LM_ERR("required functionality is not "
			   "supported by the version of the RTPproxy running on the "
			   "selected "
...
static int rtpproxy_stream2_f(
		struct sip_msg *msg, char *str1, int count, int stream2uac)
{
	str pname;
...
int rtpproxy_stop_stream(struct sip_msg *msg, int stream2uac)
{
	int nitems;
	str callid, from_tag, to_tag;
	struct rtpp_node *node;
	struct iovec v[] = {{NULL, 0}, {"S", 1}, /* 1 */
			{" ", 1}, {NULL, 0},			 /* 3 callid */
			{" ", 1}, {NULL, 0},			 /* 5 from tag */
			{";1 ", 3}, {NULL, 0},			 /* 7 to tag */
			{";1", 2}};
