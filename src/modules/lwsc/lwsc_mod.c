...
static int w_lwsc_request_proto(
	...
	switch(reason) {
		...
		case LWS_CALLBACK_CLIENT_RECEIVE:
			...
			LM_DBG("LWS_CALLBACK_RECEIVE - wsi: %p len: %lu, first: %d, "
				   "final = %d, remains = %lu\n",
					wsi, (unsigned long)len, first, final,
			...
			if(len > 0) {
				...
				if(rbuf.s == NULL) {
					...
					if(ep->rbuf.s != NULL) {
						LM_DBG("append to read buffer of %d bytes more %d "
							   "bytes\n",
								ep->rbuf.len, (int)len);
...
static int ksr_shoud_reconnect(unsigned int *last, unsigned int secs)
{
	...
	if((unsigned long)tv.tv_sec - (unsigned long)(*last)
			< (unsigned long)secs) {
		return 0;
	}
		...
		str *wsurl, str *wsproto, str *sdata, str *rdata, int rtimeout)
{
	...
	if(wsurl == NULL || wsurl->s == NULL || wsurl->len <= 0 || sdata == NULL
			|| sdata->s == NULL || sdata->len <= 0 || rdata == NULL) {
...
static int ki_lwsc_request(sip_msg_t *msg, str *wsurl, str *data)
{
	...
	return lwsc_api_request(
			wsurl, NULL, data, &_lwsc_rdata_buf, _lwsc_timeout_read);
}
 ...
 *
 */
static int ki_lwsc_request_proto(
		sip_msg_t *msg, str *wsurl, str *wsproto, str *data)
{
	/* clear global per-process receive buffer */
	if(_lwsc_rdata_buf.s != NULL) {
		pkg_free(_lwsc_rdata_buf.s);
	...
	return lwsc_api_request(
			wsurl, wsproto, data, &_lwsc_rdata_buf, _lwsc_timeout_read);
}
...
static int lwsc_api_notify(str *wsurl, str *wsproto, str *data)
{
	...
	if(wsurl == NULL || wsurl->s == NULL || wsurl->len <= 0 || data == NULL
			|| data->s == NULL || data->len <= 0) {
		LM_ERR("invalid parameters\n");
		return -1;
	}
 ...
 *
 */
static int ki_lwsc_notify_proto(
		sip_msg_t *msg, str *wsurl, str *wsproto, str *data)
{
