		...
		const db_val_t *vals, const int n)
{
	...
	for(i = 0; i < n; i++) {
		if(ops) {
			if(*(ops + i))
				element = cond2perlcond(*(keys + i), *(ops + i), vals + i);
		} else {
	...
	switch(VAL_TYPE(val)) {
		...
		case DB1_BLOB:
			if(VAL_BLOB(val).len > 0)
				data = newSVpv(VAL_BLOB(val).s, VAL_BLOB(val).len);
			else
				data = &PL_sv_undef;
			break;
...
SV *val2perlval(const db_val_t *val)
{
	...
	retval = perlvdb_perlmethod(
			class, PERL_CONSTRUCTOR_NAME, p_type, p_data, NULL, NULL);
...
SV *pair2perlpair(const db_key_t key, const db_val_t *val)
{
	...
	retval = perlvdb_perlmethod(
			class, PERL_CONSTRUCTOR_NAME, p_key, p_type, p_data, NULL);
...
SV *cond2perlcond(const db_key_t key, const db_op_t op, const db_val_t *val)
{
	...
	retval = perlvdb_perlmethod(
			class, PERL_CONSTRUCTOR_NAME, p_key, p_op, p_type, p_data);
...
int perlresult2dbres(SV *perlres, db1_res_t **r)
{
	...
	*r = (db1_res_t *)pkg_malloc(sizeof(db1_res_t));
	...
	colarrayref = perlvdb_perlmethod(
			perlres, PERL_VDB_COLDEFSMETHOD, NULL, NULL, NULL, NULL);
	if(colarrayref == NULL || !(SvROK(colarrayref)))
		goto error;
	colarray = (AV *)SvRV(colarrayref);
	if(colarray == NULL || !(SvTYPE(colarray) == SVt_PVAV))
		goto error;
	...
	for(i = colcount - 1; i >= 0; i--) {
		...
		d1 = perlvdb_perlmethod(
				acol, PERL_VDB_TYPEMETHOD, NULL, NULL, NULL, NULL);
		if(!SvIOK(d1))
			goto error;
		...
		d1 = perlvdb_perlmethod(
				acol, PERL_VDB_NAMEMETHOD, NULL, NULL, NULL, NULL);
		if(!SvPOK(d1))
			goto error;
	...
	rowarrayref = perlvdb_perlmethod(
			perlres, PERL_VDB_ROWSMETHOD, NULL, NULL, NULL, NULL);
	...
	if(rowarray == NULL || !(SvTYPE(rowarray) == SVt_PVAV))
		goto error;
...
#define cur_val (((*r)->rows)[i].values)[j]
			...
			atype = SvIV(atypesv = perlvdb_perlmethod(aelement,
								 PERL_VDB_TYPEMETHOD, NULL, NULL, NULL, NULL));
			aval = perlvdb_perlmethod(
					aelement, PERL_VDB_DATAMETHOD, NULL, NULL, NULL, NULL);
			...
			if(!SvOK(aval)) {
				...
				switch(atype) {
					case DB1_INT:
						cur_val.val.int_val = SvIV(aval);
						cur_val.nul = 0;
						break;
					case DB1_DOUBLE:
						cur_val.val.double_val = SvNV(aval);
						cur_val.nul = 0;
						break;
					case DB1_STRING:
					case DB1_STR:
				 ...
				 * Set DB1_STRING instead */
						...
						charbuf = pkg_malloc(len + 1);
						strncpy(charbuf, currentstring, len + 1);
						cur_val.val.string_val = charbuf;
						cur_val.nul = 0;
						break;
					case DB1_DATETIME:
						cur_val.val.time_val = (time_t)SvIV(aval);
						cur_val.nul = 0;
						break;
					case DB1_BLOB:
						currentstring = SvPV(aval, len);
						charbuf = pkg_malloc(len + 1);
						strncpy(charbuf, currentstring, len + 1);
						cur_val.val.blob_val.s = charbuf;
						cur_val.val.blob_val.len = len;
						cur_val.nul = 0;
						break;
					case DB1_BITMAP:
						cur_val.val.bitmap_val = SvIV(aval);
						cur_val.nul = 0;
						break;
					default:
