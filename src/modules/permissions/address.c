...
int reload_address_db_table(address_tables_group_t *atg)
{
	...
	for(i = 0; i < RES_ROW_N(res); i++) {
		...
		//convert tag DB field value to AVP string for supported DB field types
		switch(VAL_TYPE(val + 4)) {
			...
			case DB1_BIGINT:
				if(VAL_NULL(val + 4)) {
					...
					tagv.len = TAG_BUF_SIZE;
					if(db_longlong2str(VAL_BIGINT(val + 4), tag_buf, &tagv.len) < 0) {
						LM_DBG("address table row [%d]: error while converting "
			...
			case DB1_UBIGINT:
				if(VAL_NULL(val + 4)) {
					...
					tagv.len = TAG_BUF_SIZE;
					if(db_ulonglong2str(VAL_UBIGINT(val + 4), tag_buf, &tagv.len) < 0) {
						LM_DBG("address table row [%d]: error while converting "
