...
static char tag_buf[TAG_BUF_SIZE];
...
int reload_trusted_table(void)
{
	...
	for(i = 0; i < RES_ROW_N(res); i++) {
		...
		switch(VAL_TYPE(val + 4)) {
			...
			case DB1_INT:
				if(VAL_NULL(val + 4)) {
					...
					ret = snprintf(tag_buf, TAG_BUF_SIZE, "%-d", VAL_INT(val + 4));
			...
			case DB1_UINT:
				if(VAL_NULL(val + 4)) {
					...
					ret = snprintf(tag_buf, TAG_BUF_SIZE, "%u", VAL_UINT(val + 4));
					if(ret < 0 || ret >= TAG_BUF_SIZE) {
						LM_ERR("address table row [%d]: error while converting "
			...
			case DB1_BIGINT:
				if(VAL_NULL(val + 4)) {
					...
					ret = snprintf(tag_buf, TAG_BUF_SIZE, "%-lld", VAL_BIGINT(val + 4));
					if(ret < 0 || ret >= TAG_BUF_SIZE) {
						LM_ERR("address table row [%d]: error while converting "
			...
			case DB1_UBIGINT:
				if(VAL_NULL(val + 4)) {
					...
					ret = snprintf(tag_buf, TAG_BUF_SIZE, "%llu", VAL_UBIGINT(val + 4));
					if(ret < 0 || ret >= TAG_BUF_SIZE) {
						LM_ERR("address table row [%d]: error while converting "
		...
		if(!VAL_NULL(val + 5) &&
			!(VAL_TYPE(val + 5) == DB1_INT || VAL_TYPE(val + 5) == DB1_UINT)) {
			LM_DBG("failure during checks of database field (%s) "
				   "in trusted table\n", perm_priority_col.s);
			goto dberror;
		}
