...
static int w_t_relay_cancel(struct sip_msg *p_msg, char *_foo, char *_bar);
...
static int w_t_set_no_e2e_cancel_reason(
...
static cmd_export_t cmds[] = {
		{"t_newtran", w_t_newtran, 0, 0, 0, REQUEST_ROUTE},
		{"t_lookup_request", w_t_check, 0, 0, 0, REQUEST_ROUTE},
		{"t_lookup_cancel", w_t_lookup_cancel, 0, 0, 0, REQUEST_ROUTE},
		...
		{"t_release", w_t_release, 0, 0, 0, REQUEST_ROUTE},
		...
		{"t_relay_to_sctp", w_t_relay_to_sctp_uri, 0, 0, 0,
				REQUEST_ROUTE | FAILURE_ROUTE},
#endif
		{"t_replicate", w_t_replicate_uri, 0, 0, 0, REQUEST_ROUTE},
		...
		{"t_replicate_to", w_t_replicate_to, 2, fixup_proto_hostport2proxy, 0,
				REQUEST_ROUTE},
		{"t_relay", w_t_relay, 0, 0, 0, REQUEST_ROUTE | FAILURE_ROUTE},
		{"t_relay", w_t_relay2, 2, fixup_hostport2proxy, 0,
				REQUEST_ROUTE | FAILURE_ROUTE},
		{"t_relay_to_avp", w_t_relay_to_avp, 2, fixup_proto_hostport2proxy, 0,
				REQUEST_ROUTE},
		{"t_relay_to", w_t_relay_to, 0, 0, 0, REQUEST_ROUTE | FAILURE_ROUTE},
		...
		{"t_forward_nonack_uri", w_t_forward_nonack_uri, 0, 0, 0,
				REQUEST_ROUTE},
		{"t_forward_nonack_udp", w_t_forward_nonack_udp, 2,
				fixup_hostport2proxy, 0, REQUEST_ROUTE},
#ifdef USE_TCP
		{"t_forward_nonack_tcp", w_t_forward_nonack_tcp, 2,
				fixup_hostport2proxy, 0, REQUEST_ROUTE},
#endif
#ifdef USE_TLS
		{"t_forward_nonack_tls", w_t_forward_nonack_tls, 2,
				fixup_hostport2proxy, 0, REQUEST_ROUTE},
#endif
#ifdef USE_SCTP
		{"t_forward_nonack_sctp", w_t_forward_nonack_sctp, 2,
				fixup_hostport2proxy, 0, REQUEST_ROUTE},
#endif
		{"t_forward_nonack_to", w_t_forward_nonack_to, 2,
				fixup_proto_hostport2proxy, 0, REQUEST_ROUTE},
		{"t_relay_cancel", w_t_relay_cancel, 0, 0, 0, REQUEST_ROUTE},
		...
		{"t_grep_status", w_t_grep_status, 1, fixup_var_int_1, 0,
				REQUEST_ROUTE | TM_ONREPLY_ROUTE | FAILURE_ROUTE
						| BRANCH_ROUTE},
		{"t_drop_replies", w_t_drop_replies, 0, 0, 0, FAILURE_ROUTE},
		{"t_drop_replies", w_t_drop_replies, 1, 0, 0, FAILURE_ROUTE},
		{"t_save_lumps", w_t_save_lumps, 0, 0, 0, REQUEST_ROUTE},
		{"t_check_trans", w_t_check_trans, 0, 0, 0,
				REQUEST_ROUTE | ONREPLY_ROUTE | BRANCH_ROUTE},
		{"t_is_set", w_t_is_set, 1, fixup_t_is_set, 0, ANY_ROUTE},
		{"t_use_uac_headers", w_t_use_uac_headers, 0, 0, 0, ANY_ROUTE},
...
static param_export_t params[] = {
		...
		{"callid_cseq_matching", PARAM_INT,
				&default_tm_cfg.callid_cseq_matching},
		...
		{"auto_inv_100_reason", PARAM_STRING,
				&default_tm_cfg.tm_auto_inv_100_r},
		...
		{"blst_methods_lookup", PARAM_INT,
				&default_tm_cfg.tm_blst_methods_lookup},
...
static int fixup_proto_hostport2proxy(void **param, int param_no)
{
	...
	if(ret <= 0)
		return ret;
	if(fix_param(FPARAM_STRING, param) != 0)
		return -1;
	return 0;
}
...
static int fixup_t_check_status(void **param, int param_no)
{
	...
	if(fix_param(FPARAM_REGEX, param) != 0)
		return -1;
	return 0;
}
...
static int mod_init(void)
{
	 ...
	 * configuration */
	if(cancel_b_flags_get(
			   &default_tm_cfg.cancel_b_flags, default_tm_cfg.cancel_b_flags)
			< 0) {
	...
	*failover_reply_codes_cnt = 0;
	...
	if(cfg_declare(
			   "tm", tm_cfg_def, &default_tm_cfg, cfg_sizeof(tm), &tm_cfg)) {
...
static int t_check_status(struct sip_msg *msg, char *p1, char *foo)
{
	...
	switch(fp->type) {
		...
		default:
			...
			if(get_str_fparam(&tmp, msg, fp) < 0)
				goto error;
	...
	if(backup)
		status[msg->first_line.u.reply.status.len] = backup;
	if(s)
		pkg_free(s);
	if(re0) {
		regfree(re0);
...
static int ki_t_lookup_cancel_flags(sip_msg_t *msg, int flags)
{
	...
	if(msg->REQ_METHOD == METHOD_CANCEL) {
		...
		LM_WARN("script error - t_lookup_cancel() called for non-CANCEL "
				"request\n");
	}
	return -1;
}
...
static int w_t_lookup_cancel(struct sip_msg *msg, char *str, char *str2)
{
	...
	if(str) {
		if(get_int_fparam(&i, msg, (fparam_t *)str) < 0)
			return -1;
	}
...
static int _w_t_forward_nonack(
		struct sip_msg *msg, struct proxy_l *proxy, int proto)
{
	struct cell *t;
	if(t_check(msg, 0) == -1) {
...
static int w_t_forward_nonack(struct sip_msg *msg, char *proxy, char *foo)
{
	return _w_t_forward_nonack(msg, (struct proxy_l *)proxy, PROTO_NONE);
}


static int w_t_forward_nonack_uri(struct sip_msg *msg, char *foo, char *bar)
{
	return _w_t_forward_nonack(msg, 0, PROTO_NONE);
}


static int w_t_forward_nonack_udp(struct sip_msg *msg, char *proxy, char *foo)
{
...
#ifdef USE_TCP
static int w_t_forward_nonack_tcp(struct sip_msg *msg, char *proxy, char *foo)
{
...
#ifdef USE_TLS
static int w_t_forward_nonack_tls(struct sip_msg *msg, char *proxy, char *foo)
{
...
#ifdef USE_SCTP
static int w_t_forward_nonack_sctp(struct sip_msg *msg, char *proxy, char *foo)
{
...
static int w_t_forward_nonack_to(
		struct sip_msg *p_msg, char *proto_par, char *addr_par)
{
	struct proxy_l *proxy;
	int r = -1;
	...
	if(is_route_type(FAILURE_ROUTE)) {
		...
		if(likely(t->uas.request)) {
			if(is_route_type(CORE_ONREPLY_ROUTE))
				ret = t_reply_str(
						t, t->uas.request, (unsigned int)code, reason);
			else
				ret = t_reply_str_unsafe(
						t, t->uas.request, (unsigned int)code, reason);
		} else
			ret = -1;
		/* t_check() above has the side effect of setting T and
...
static int fixup_t_is_set(void **param, int param_no)
{
	...
	if(param_no == 1) {
		...
		if((len == 13 && strncmp((char *)*param, "failure_route", 13) == 0)
				|| (len == 13
						&& strncmp((char *)*param, "onreply_route", 13) == 0)
				|| (len == 12
						&& strncmp((char *)*param, "branch_route", 12) == 0)) {
			return 0;
		}
...
static int _w_t_relay_to(
		struct sip_msg *p_msg, struct proxy_l *proxy, int force_proto)
{
	struct cell *t;
	int res;
...
static int w_t_relay_to_udp_uri(struct sip_msg *p_msg, char *_foo, char *_bar)
{
...
static int w_t_relay_to_tcp_uri(struct sip_msg *p_msg, char *_foo, char *_bar)
{
...
static int w_t_relay_to_tls_uri(struct sip_msg *p_msg, char *_foo, char *_bar)
{
...
/* forward to uri, but force tcp as transport */
static int w_t_relay_to_sctp_uri(struct sip_msg *p_msg, char *_foo, char *_bar)
{
 ...
 * as the received msg */
static int w_t_relay2(struct sip_msg *p_msg, char *proxy, char *_foo)
{
...
static int t_set_fr_all(struct sip_msg *msg, char *p1, char *p2)
{
	...
	if(get_int_fparam(&fr_inv, msg, (fparam_t *)p1) < 0)
		return -1;
	if(p2) {
		if(get_int_fparam(&fr, msg, (fparam_t *)p2) < 0)
			return -1;
	} else {
		fr = 0;
	}
...
static int w_t_set_retr(struct sip_msg *msg, char *p1, char *p2)
{
	...
	if(get_int_fparam(&t1, msg, (fparam_t *)p1) < 0)
		return -1;
	if(p2) {
		if(get_int_fparam(&t2, msg, (fparam_t *)p2) < 0)
			return -1;
	} else {
		t2 = 0;
	}
...
#define W_T_SET_FLAG_GEN_FUNC(fname, T_FLAG_NAME)            \
	...
	{                                                        \
		int state;                                           \
		if(get_int_fparam(&state, msg, (fparam_t *)p1) < 0)  \
			return -1;                                       \
...
int w_t_grep_status(struct sip_msg *msg, char *status, char *bar)
{
	...
	if(get_int_fparam(&code, msg, (fparam_t *)status) < 0)
		return -1;
			...
			case 1: /* found */
				...
				if(msg->REQ_METHOD == METHOD_ACK) {
					...
					if(unlikely(has_tran_tmcbs(t, TMCB_ACK_NEG_IN)))
						run_trans_callbacks(
								TMCB_ACK_NEG_IN, t, msg, 0, msg->REQ_METHOD);
					t_release_transaction(t);
				} else {
					/* is a retransmission */
					if(unlikely(has_tran_tmcbs(t, TMCB_REQ_RETR_IN)))
						run_trans_callbacks(
								TMCB_REQ_RETR_IN, t, msg, 0, msg->REQ_METHOD);
				...
				*(param) = 0;
				...
				if(str2int(&s, &flags) == 0) {
					...
					*(param) = 0;
					...
					/* try proxy */
					if(parse_phostport(s.s, &host.s, &host.len, &port, &proto)
							!= 0) {
						LM_CRIT("invalid proxy addr parameter <%s>\n", s.s);
						return E_UNSPEC;
					}
		...
		char *pnexthop, char *psock, char *phdrs, char *pbody)
{
	...
	if(t_uac_send(&method, &ruri, &nexthop, &send_socket, &headers, &body)
			< 0) {
		return -1;
	}
	return 1;
...
static int ki_t_uac_send(sip_msg_t *msg, str *method, str *ruri, str *nexthop,
		str *ssock, str *hdrs, str *body)
{
...
static const char *rpc_cancel_doc[2] = {"Cancel a pending transaction", 0};

static const char *rpc_reply_doc[2] = {"Reply transaction", 0};

static const char *rpc_reply_callid_doc[2] = {
		"Reply transaction by call-id", 0};

static const char *tm_rpc_stats_doc[2] = {"Print transaction statistics.", 0};
...
static const char *rpc_t_uac_wait_block_doc[2] = {
		"starts a tm uac and waits for the final reply in blocking mode, using "
		"a"
		" list of string parameters: method, ruri, dst_uri send_sock, headers"
		" (CRLF separated) and body (optional)",
		0};

static const char *tm_rpc_list_doc[2] = {"List transactions.", 0};

static const char *tm_rpc_clean_doc[2] = {
		"Clean expired (lifetime exceeded) transactions.", 0};


/* rpc exports */
static rpc_export_t tm_rpc[] = {{"tm.cancel", rpc_cancel, rpc_cancel_doc, 0},
		...
		{"tm.t_uac_wait_block", rpc_t_uac_wait_block, rpc_t_uac_wait_block_doc,
				0},
		{"tm.list", tm_rpc_list, tm_rpc_list_doc, RET_ARRAY},
		{"tm.clean", tm_rpc_clean, tm_rpc_clean_doc, 0}, {0, 0, 0, 0}};
...
static int ki_t_on_failure(sip_msg_t *msg, str *rname)
{
	...
	if(rname == NULL || rname->s == NULL || rname->len <= 0
			|| rname->s[0] == '\0') {
		ridx = 0;
	} else {
		keng = sr_kemi_eng_get();
 ...
 *
 */
static int ki_t_relay_to_proto_addr(
		sip_msg_t *msg, str *sproto, str *host, int port)
{
...
static int ki_t_exists(sip_msg_t *msg)
{
	...
	if(parse_headers(msg, HDR_VIA1_F | HDR_CSEQ_F | HDR_CALLID_F | HDR_TO_F, 0)
			== -1) {
