...
void tm_log_transaction(tm_cell_t *tcell, int llev, char *ltext)
{
	...
	LOG(llev,
			"%s - tindex=%u tlabel=%u method='%.*s' from='%.*s'"
			...
			tcell->method.len, tcell->method.s, tcell->from.len, tcell->from.s,
			tcell->to.len, tcell->to.s, tcell->callid.len, tcell->callid.s,
			tcell->cseq_n.len, tcell->cseq_n.s,
			(tcell->uas.request) ? "yes" : "no", (unsigned)tcell->flags,
