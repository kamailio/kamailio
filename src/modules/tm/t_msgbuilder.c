		...
		char *method, int method_len, str *to, struct cancel_reason *reason)
{
...
#ifdef USE_TCP
	if(!is_local(Trans)
			&& ((Trans->uas.request->rcv.proto == PROTO_TCP)
#ifdef USE_TLS
					|| (Trans->uas.request->rcv.proto == PROTO_TLS)
#endif /* USE_TLS */
							)) {
		if((via_id.s = id_builder(
					Trans->uas.request, (unsigned int *)&via_id.len))
				== 0) {
	...
	*len += via_len;
	/*headers*/
	*len += Trans->from.len + Trans->callid.len + to->len + +Trans->cseq_n.len
			+ 1 + method_len + CRLF_LEN + MAXFWD_HEADER_LEN;
	...
	if(reason && reason->cause != CANCEL_REAS_UNKNOWN) {
		if(likely(reason->cause > 0
			...
			reason_len = REASON_PREFIX_LEN + USHORT2SBUF_MAX_LEN
						 + (reason->u.text.s ? REASON_TEXT_LEN + 1
													   + reason->u.text.len + 1
											 : 0)
						 + CRLF_LEN;
	...
	*len += CRLF_LEN; /* end of msg. */
	...
	if(reason_len) {
		if(likely(reason->cause > 0)) {
			...
			code_len =
					ushort2sbuf(reason->cause, p, *len - (int)(p - cancel_buf));
				...
				if(reason_len) {
					...
					if(likely(reason->cause > 0)) {
						...
						if(reason->u.text.s) {
							...
							*d = '"';
							d++;
							append_str(d, reason->u.text.s, reason->u.text.len);
							*d = '"';
							d++;
						}
						append_str(d, CRLF, CRLF_LEN);
				...
				if(likely(hadded == 0)) {
					if(cfg_get(tm, tm_cfg, ac_extra_hdrs).len
							...
							&& (strncasecmp(s1,
										...
										cfg_get(tm, tm_cfg, ac_extra_hdrs).len)
									== 0)) {
						append_str(d, s1, s - s1);
					}
				}
		...
		switch(puri->type) {
			...
			case TEL_URI_T:
				if(puri->transport_val.len == sizeof("TLS") - 1) {
					unsigned trans;
					trans = puri->transport_val.s[0] | 0x20;
					trans <<= 8;
					trans |= puri->transport_val.s[1] | 0x20;
					trans <<= 8;
...
#define PARSE_URI(_str_, _uri_)                                             \
	do {                                                                    \
		...
		if(parse_uri((_str_)->s, (_str_)->len, _uri_) < 0) {                \
...
#define URI_PORT(_puri_, _port)         \
	...
	if((uri_ia = str2ip(&topr_uri.host))
		...
		if((he = rev_resolvehost(&ia))) {
			if((strlen(he->h_name) == topr_uri.host.len)
					&& (memcmp(he->h_name, topr_uri.host.s, topr_uri.host.len)
							== 0))
				return F_RB_NH_LOOSE;
			for(alias = he->h_aliases; *alias; alias++)
				if((strlen(*alias) == topr_uri.host.len)
						&& (memcmp(*alias, topr_uri.host.s, topr_uri.host.len)
								== 0))
					return F_RB_NH_LOOSE;
			return F_RB_NH_STRICT;
		} else {
	...
	if((cont_port == inv_port) && (cont_uri.host.len == inv_ruri.host.len)
			&& (memcmp(cont_uri.host.s, inv_ruri.host.s, cont_uri.host.len)
					== 0))
		return F_RB_NH_LOOSE;
	...
	if((cont_port == lastr_port) && (cont_uri.host.len == lastr_uri.host.len)
			&& (memcmp(cont_uri.host.s, lastr_uri.host.s, lastr_uri.host.len)
					== 0))
		return F_RB_NH_STRICT;
		...
		str *contact, struct rte **list, str *ruri, str *next_hop)
{
	...
	eval_flags:
		switch(route_flags & (F_RB_NH_LOOSE | F_RB_NH_STRICT)) {
			case 0:
				LM_WARN("calculate_hooks() not called when built the local UAC "
						"of "
			 ...
			 * (strict/loose) by reading the original invite */
				if((route_flags = nhop_type(
							&orig_inv, rtset, &inv_rb->dst, contact))) {
					LM_DBG("original request's next hop type evaluated to: "
						   "0x%x.\n",
							(unsigned int)route_flags);
					goto eval_flags;
				} else {
		...
		struct dest_info *dst)
{
	...
	if(eval_uac_routing(rpl, &Trans->uac[branch].request, &contact, &list,
			   &ruri, &next_hop)
			< 0) {
	...
	*len += CONTENT_LENGTH_LEN + body_len.len + CRLF_LEN + CRLF_LEN;
	...
	if(body) {
		...
		append_str(
				p, /*end crr. header*/ CRLF /*EoH*/ CRLF, CRLF_LEN + CRLF_LEN);
 ...
 * Create Via header
 */
static inline int assemble_via(
		str *dest, struct cell *t, struct dest_info *dst, int branch)
{
 ...
 * Print Request-URI
 */
static inline char *print_request_uri(
		char *w, str *method, dlg_t *dialog, struct cell *t, int branch)
{
		...
		unsigned int branch, unsigned int *len, struct dest_info *dst)
{
	...
	LM_DBG("sing FROM=<%.*s>, TO=<%.*s>, CSEQ_N=<%.*s>\n", cancelledT->from.len,
			cancelledT->from.s, cancelledT->to.len, cancelledT->to.s,
			cancelledT->cseq_n.len, cancelledT->cseq_n.s);
	...
	*(p++) = ' ';
	memapp(p, cancelledT->uac[branch].uri.s, cancelledT->uac[branch].uri.len);
