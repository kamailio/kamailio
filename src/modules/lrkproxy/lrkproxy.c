...
static int lrkproxy_force(struct sip_msg *msg, const char *flags,
		enum lrk_operation op, int more);
static int lrkproxy_unforce(struct sip_msg *msg, const char *flags,
		enum lrk_operation op, int more);
...
static int change_media_sdp(sip_msg_t *msg, struct lrkproxy_hash_entry *e,
		const char *flags, enum lrk_operation op);
...
static cmd_export_t cmds[] = {
		...
		{"set_lrkproxy_set", (cmd_function)set_lrkproxy_set_f, 1, fixup_set_id,
				0, ANY_ROUTE},
		{"lrkproxy_manage", (cmd_function)lrkproxy_manage0, 0, 0, 0, ANY_ROUTE},
		{"lrkproxy_manage", (cmd_function)lrkproxy_manage1, 1, fixup_spve_null,
				fixup_free_spve_null, ANY_ROUTE},
		{"lrkproxy_manage", (cmd_function)lrkproxy_manage2, 2, fixup_spve_spve,
...
static int lrkproxy_set_store(modparam_t type, void *val)
{
	...
	if(lrkp_sets == 0) {
		...
		lrkp_strings = (char **)pkg_reallocxf(
				lrkp_strings, (lrkp_sets + 1) * sizeof(char *));
		if(!lrkp_strings) {
			LM_ERR("no pkg memory left\n");
			return -1;
		}
...
static int mod_init(void)
{
	...
	if(!lrkproxy_hash_table_init(hash_table_size)) {
		...
		LM_INFO(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>lrkproxy_hash_table_init(%d) "
				"success!\n",
				hash_table_size);
	}
	...
	for(lrkp_list = lrkp_set_list->lset_first; lrkp_list != 0;
		...
		for(pnode = lrkp_list->ln_first; pnode != 0; pnode = pnode->ln_next) {
			...
			hostname = (char *)pkg_malloc(
					sizeof(char) * (strlen(pnode->ln_address) + 1));
			if(hostname == NULL) {
				LM_ERR("no more pkg memory\n");
				return -1;
			}
			...
			lrkp_socks[pnode->idx] = socket(
					(pnode->ln_umode == 6) ? AF_INET6 : AF_INET, SOCK_DGRAM, 0);
			...
			if(connect(lrkp_socks[pnode->idx], res->ai_addr, res->ai_addrlen)
					== -1) {
...
char *send_lrkp_command(
		struct lrkp_node *node, struct iovec *v, int vcnt, int more)
{
	...
	if(node->ln_umode == 0) {
		...
		strncpy(addr.sun_path, node->ln_address, sizeof(addr.sun_path) - 1);
...
static struct lrkp_set *select_lrkp_set(int id_set)
{
	...
	for(lrkp_list = lrkp_set_list->lset_first;
			lrkp_list != NULL && lrkp_list->id_set != id_set;
			lrkp_list = lrkp_list->lset_next)
		;
...
retry:
	for(node = selected_lrkp_set->ln_first; node != NULL;
			node = node->ln_next) {
	...
	retry3:
		...
		for(node = selected_lrkp_node->ln_next; node != NULL;
				node = node->ln_next)
		...
		const char *flags, enum lrk_operation op)
{
	...
	if(flags) {
		if(strstr(flags, "ei")) {
			ip_selected =
					e->node->lrkp_n_c->internal_ip; // lrk_node->internal_ip;
		} else if(strstr(flags, "ie")) {
			ip_selected =
					e->node->lrkp_n_c->external_ip; //lrk_node->external_ip;
		} else {
	...
	if(op == OP_OFFER) {
		...
		str current_port;
		current_port.s =
				int2str(e->node->lrkp_n_c->current_port, &current_port.len);

		if(shm_str_dup(&e->dst_port, &current_port) < 0) {
			LM_ERR("lrkproxy fail to insert dst_port, calllen=%d "
				   "dst_port=%.*s\n",
		...
		if(shm_str_dup(&e->snat_port, &current_port) < 0) {
			LM_ERR("lrkproxy fail to insert snat_port, calllen=%d "
				   "snat_port=%.*s\n",
					e->callid.len, current_port.len, current_port.s);
			lrkproxy_hash_table_free_entry(e);
			return 0;
	...
	while(*off != EOB) //while end of body.
	{
		...
		if((int)(start_sdp_o - off) == 0) {
			...
			snprintf(sdp_new_o, 128, "o=lrkproxy %s %s IN IP4 %s\r",
					SUP_CPROTOVER, REQ_CPROTOVER, ip_selected);
		...
		if((int)(start_sdp_m - off) == 0) {
			...
			avp_flags++;
			if(op == OP_OFFER)
				snprintf(sdp_new_m, 128, "m=audio %.*s %.*s\r", e->dst_port.len,
						e->dst_port.s, (int)(len - (avp_flags - off)),
						avp_flags);
			//                snprintf(sdp_new_m, 128, "m=audio %d %.*s\r",e->node->lrkp_n_c->current_port, (int)(len - (avp_flags-off)), avp_flags);
			else if(op == OP_ANSWER)
				snprintf(sdp_new_m, 128, "m=audio %.*s %.*s\r",
						e->snat_port.len, e->snat_port.s,
...
static int lrkproxy_manage(struct sip_msg *msg, char *flags, char *ip)
{
	int method;
	int nosdp;
	tm_cell_t *t = NULL;
...
static int lrkproxy_force(
		struct sip_msg *msg, const char *flags, enum lrk_operation op, int more)
{
	...
	if(op == OP_OFFER) {
		...
		if(!entry) {
			LM_ERR("lrkproxy hash table fail to create entry for calllen=%d "
				   "callid=%.*s viabranch=%.*s\n",
					call_id.len, call_id.len, call_id.s, viabranch.len,
					viabranch.s);
			return 0;
		}
		...
		if(viabranch.s && viabranch.len > 0) {
			if(shm_str_dup(&entry->viabranch, &viabranch) < 0) {
				LM_ERR("lrkproxy hash table fail to insert viabranch, "
					   "calllen=%d viabranch=%.*s\n",
		...
		if(src_ipv4.s && src_ipv4.len > 0) {
			...
			if(shm_str_dup(&entry->src_ipv4, &src_ipv4) < 0) {
				LM_ERR("lrkproxy hash table fail to insert src_ipv4, "
					   "calllen=%d src_ipv4=%.*s\n",
		...
		if(src_port.s && src_port.len > 0) {
			...
			if(shm_str_dup(&entry->src_port, &src_port) < 0) {
				LM_ERR("lrkproxy hash table fail to insert src_port, "
					   "calllen=%d src_port=%.*s\n",
		...
		if(!lrkproxy_hash_table_insert(call_id, viabranch, entry)) {
			LM_ERR("lrkproxy hash table fail to insert node=%.*s for "
				   "calllen=%d callid=%.*s viabranch=%.*s\n",
					node->ln_url.len, node->ln_url.s, call_id.len, call_id.len,
					call_id.s, viabranch.len, viabranch.s);
			lrkproxy_hash_table_free_entry(entry);
			return 0;
		} else {
			LM_INFO("lrkproxy hash table insert node=%.*s for calllen=%d "
					"callid=%.*s viabranch=%.*s\n",
					node->ln_url.len, node->ln_url.s, call_id.len, call_id.len,
					call_id.s, viabranch.len, viabranch.s);

			LM_DBG("lrkproxy hash table insert node=%.*s for calllen=%d "
				   "callid=%.*s viabranch=%.*s\n",
					node->ln_url.len, node->ln_url.s, call_id.len, call_id.len,
					call_id.s, viabranch.len, viabranch.s);
		}
		...
		if(dnat_ipv4.s && dnat_ipv4.len > 0) {
			...
			if(shm_str_dup(&entry->dnat_ipv4, &dnat_ipv4) < 0) {
				LM_ERR("lrkproxy hash table fail to insert dnat_ipv4, "
					   "calllen=%d dnat_ipv4=%.*s\n",
		...
		if(dnat_port.s && dnat_port.len > 0) {
			...
			if(shm_str_dup(&entry->dnat_port, &dnat_port) < 0) {
				LM_ERR("lrkproxy hash table fail to insert dnat_port, "
					   "calllen=%d dnat_port=%.*s\n",
		...
		struct sip_msg *msg, const char *flags, enum lrk_operation op, int more)
{
	...
	if(op == OP_DELETE) {
		...
		if(!lrkproxy_hash_table_remove(call_id, viabranch, op)) {
			LM_ERR("lrkproxy hash table failed to remove entry for callen=%d "
				   "callid=%.*s viabranch=%.*s\n",
					call_id.len, call_id.len, call_id.s, viabranch.len,
					viabranch.s);
		} else {
			LM_DBG("lrkproxy hash table remove entry for callen=%d callid=%.*s "
				   "viabranch=%.*s\n",
					call_id.len, call_id.len, call_id.s, viabranch.len,
					viabranch.s);
		}
	}
	LM_INFO("lrkproxy hash table remove entry for callen=%d callid=%.*s "
			"viabranch=%.*s successfully\n",
			call_id.len, call_id.len, call_id.s, viabranch.len, viabranch.s);
	return 1;
}
