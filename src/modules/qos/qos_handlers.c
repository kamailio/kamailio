...
static void qos_dialog_destroy_CB(
		struct dlg_cell *did, int type, struct dlg_cb_params *params);
static void qos_dialog_request_CB(
		struct dlg_cell *did, int type, struct dlg_cb_params *params);
static void qos_dialog_response_CB(
		struct dlg_cell *did, int type, struct dlg_cb_params *params);

static void qos_dialog_rpc_context_CB(
		struct dlg_cell *did, int type, struct dlg_cb_params *params);
...
static void setup_dialog_callbacks(struct dlg_cell *did, qos_ctx_t *ctx)
{
	dlg_binds->register_dlgcb(
			did, DLGCB_REQ_WITHIN, qos_dialog_request_CB, (void *)ctx, NULL);

	dlg_binds->register_dlgcb(did, DLGCB_RESPONSE_FWDED | DLGCB_RESPONSE_WITHIN,
			qos_dialog_response_CB, (void *)ctx, NULL);

	dlg_binds->register_dlgcb(
			did, DLGCB_DESTROY, qos_dialog_destroy_CB, (void *)ctx, NULL);

	dlg_binds->register_dlgcb(did, DLGCB_RPC_CONTEXT, qos_dialog_rpc_context_CB,
			(void *)ctx, NULL);
	return;
}
...
/********************************* RPC *********************************/
static inline void internal_rpc_print_qos_stream_payloads(
		rpc_t *rpc, void *c, sdp_stream_cell_t *stream)
{
	int i;
	...
	for(i = stream->payloads_num - 1; i >= 0; i--) {
		...
		rpc->rpl_printf(c, "\t\t\t\tpayload[%d]=%.*s codec=%.*s", i,
				sdp_payload->rtp_payload.len, sdp_payload->rtp_payload.s,
				sdp_payload->rtp_enc.len, sdp_payload->rtp_enc.s);
		sdp_payload = sdp_payload->next;
	}
}
static inline void internal_rpc_print_qos_stream(
		rpc_t *rpc, void *c, sdp_session_cell_t *session)
{
	int i;
	...
	for(i = session->streams_num - 1; i >= 0; i--) {
		...
		rpc->rpl_printf(c,
				"\t\t\tmedia=%.*s IP:port=%.*s:%.*s trans=%.*s sendrecv=%.*s "
				"ptime=%.*s payload:%d",
				...
				stream->ptime.len, stream->ptime.s, stream->payloads_num);
		internal_rpc_print_qos_stream_payloads(rpc, c, stream);
		stream = stream->next;
	}
}
static inline void internal_rpc_print_qos(
		rpc_t *rpc, void *c, qos_sdp_t *qos_sdp)
{
	int i;
	sdp_session_cell_t *session;

	rpc->rpl_printf(c,
			"\t\tm_dir=%u m_id=%u method=%.*s cseq=%.*s negotiation=%u",
			qos_sdp->method_dir, qos_sdp->method_id, qos_sdp->method.len,
			qos_sdp->method.s, qos_sdp->cseq.len, qos_sdp->cseq.s,
			qos_sdp->negotiation);
	...
	for(i = 1; i >= 0; i--) {
		...
		if(session) {
			rpc->rpl_printf(c,
					"\t\tcalle%s: cnt_disp=%.*s bw_type=%.*s bw_width=%.*s",
					i ? "e" : "r", session->cnt_disp.len, session->cnt_disp.s,
...
void qos_dialog_rpc_context_CB(
		struct dlg_cell *did, int type, struct dlg_cb_params *params)
{
