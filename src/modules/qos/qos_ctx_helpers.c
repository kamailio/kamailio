...
void print_qos_sdp(qos_sdp_t *qos_sdp)
{
	...
	LM_DBG("[%p] prev->%p next->%p method_dir=%d method_id=%d method='%.*s' "
		   "cseq='%.*s' negotiation=%d sdp[0:QOS_CALLER]=%p "
		   "sdp[1:QOS_CALLEE]=%p\n",
			qos_sdp, qos_sdp->prev, qos_sdp->next, qos_sdp->method_dir,
			qos_sdp->method_id, qos_sdp->method.len, qos_sdp->method.s,
			qos_sdp->cseq.len, qos_sdp->cseq.s, qos_sdp->negotiation,
			qos_sdp->sdp_session[0], qos_sdp->sdp_session[1]);
 ...
 */
int find_qos_sdp(qos_ctx_t *qos_ctx, unsigned int dir, unsigned int other_role,
		str *cseq_number, int cseq_method_id, sdp_session_cell_t *session,
		struct sip_msg *_m, qos_sdp_t **_qos_sdp)
{
	qos_sdp_t *qos_sdp;
		...
		case SIP_REQUEST:
			switch(cseq_method_id) {
				...
				case METHOD_ACK:
					...
					while(qos_sdp) {
						if(METHOD_INVITE == qos_sdp->method_id
								&& dir != qos_sdp->method_dir
								&& qos_sdp->negotiation == N_200OK_ACK
								&& cseq_number->len == qos_sdp->cseq.len
								&& 0
										   == strncmp(cseq_number->s,
												   qos_sdp->cseq.s,
												   cseq_number->len)) {
							LM_DBG("method_id, dir and cseq match with "
								   "previous session %p->%p\n",
							...
							if(qos_sdp->sdp_session[other_role] != NULL) {
								...
								if(local_cnt_disp->len
										== received_cnt_disp->len) {
									if(local_cnt_disp->len == 0) {
										...
										*_qos_sdp = qos_sdp;
										return PENDING_MATCH;
											  ...
											  == strncmp(local_cnt_disp->s,
													  received_cnt_disp->s,
													  local_cnt_disp->len)) {
										LM_DBG("'%.*s' => %p\n",
												local_cnt_disp->len,
												local_cnt_disp->s, qos_sdp);
										*_qos_sdp = qos_sdp;
										return PENDING_MATCH;
									}
								} else if(received_cnt_disp->len == 0
										  && local_cnt_disp->len == 7
										  && 0
													 == strncmp(
															 local_cnt_disp->s,
															 "session", 7)) {
			...
			switch(cseq_method_id) {
				case METHOD_INVITE:
					...
					while(qos_sdp) {
						...
						if(cseq_method_id == qos_sdp->method_id
								&& dir != qos_sdp->method_dir
								&& qos_sdp->negotiation == N_INVITE_200OK
								&& cseq_number->len == qos_sdp->cseq.len
								&& 0
										   == strncmp(cseq_number->s,
												   qos_sdp->cseq.s,
...
{
	if(qos_sdp->prev != NULL)
		LM_ERR("got qos_sdp->prev=%p\n", qos_sdp->prev);
	if(qos_sdp->next != NULL)
		LM_ERR("got qos_sdp->next=%p\n", qos_sdp->next);

	if(qos_ctx->pending_sdp) {
		LM_DBG("Adding pending qos_sdp: %p\n", qos_sdp);
		if(qos_ctx->pending_sdp->prev != NULL)
			LM_ERR("got qos_ctx->pending_sdp->prev=%p\n",
...
void link_negotiated_qos_sdp_and_run_cb(qos_ctx_t *qos_ctx, qos_sdp_t *qos_sdp,
		unsigned int role, struct sip_msg *_m)
{
	...
	if(temp_qos_sdp) {
		while(temp_qos_sdp) {
			...
			if(qos_sdp->negotiation == temp_qos_sdp->negotiation) {
				LM_DBG("run_qos_callbacks(QOSCB_REMOVE_SDP, qos_ctx=%p, "
					   "temp_qos_sdp=%p, role=%d, _m=%p)\n",
						qos_ctx, temp_qos_sdp, role, _m);
				run_qos_callbacks(
						QOSCB_REMOVE_SDP, qos_ctx, temp_qos_sdp, role, _m);
		...
		if(qos_ctx->negotiated_sdp) {
			...
			if(qos_ctx->negotiated_sdp->prev != NULL)
				LM_ERR("got qos_ctx->negotiated_sdp->prev=%p\n",
	...
	LM_DBG("run_qos_callbacks(QOSCB_UPDATE_SDP, qos_ctx=%p, qos_sdp=%p, "
		   "role=%d, _m=%p)\n",
			qos_ctx, qos_sdp, role, _m);
	run_qos_callbacks(QOSCB_UPDATE_SDP, qos_ctx, qos_sdp, role, _m);
}

int add_pending_sdp_session(qos_ctx_t *qos_ctx, unsigned int dir,
		str *cseq_number, str *cseq_method, int cseq_method_id,
		unsigned int role, unsigned int negotiation,
		sdp_session_cell_t *session, struct sip_msg *_m)
{
	unsigned int len;
	...
	if(qos_sdp == NULL) {
		...
		LM_DBG("run_qos_callbacks(QOSCB_ADD_SDP, qos_ctx=%p, qos_sdp=%p, "
			   "role=%d, _m=%p)\n",
				qos_ctx, qos_sdp, role, _m);
		...
		unsigned int role, unsigned int other_role)
{
	...
	if((!_m->cseq && parse_headers(_m, HDR_CSEQ_F, 0) < 0) || !_m->cseq
			|| !_m->cseq->parsed) {
	...
	while(recv_session) {
		qos_sdp = NULL;
		sdp_match = find_qos_sdp(qos_ctx, dir, other_role, cseq_number,
				cseq_method_id, recv_session, _m, &qos_sdp);

		switch(sdp_match) {
			case NO_INVITE_REQ_MATCH:
				if(0
						!= add_pending_sdp_session(qos_ctx, dir, cseq_number,
								cseq_method, cseq_method_id, role,
								N_INVITE_200OK, recv_session, _m)) {
			...
			case NO_INVITE_RESP_MATCH:
				if(0
						!= add_pending_sdp_session(qos_ctx, dir, cseq_number,
								cseq_method, cseq_method_id, role, N_200OK_ACK,
								recv_session, _m)) {
					LM_ERR("Unable to add new sdp session\n");
					goto error;
				}
			...
			case PENDING_MATCH:
				...
				qos_sdp->sdp_session[role] =
						clone_sdp_session_cell(recv_session);
				if(qos_sdp->sdp_session[role] == NULL) {
					LM_ERR("PENDING_MATCH:oom: Unable to add new sdp "
						   "session\n");
					return;
				}
			...
			case NEGOTIATED_MATCH:
				...
				qos_sdp->sdp_session[role] =
						clone_sdp_session_cell(recv_session);
				if(qos_sdp->sdp_session[role] == NULL) {
					LM_ERR("NEGOTIATED_MATCH:oom: Unable to add new sdp "
						   "session\n");
					return;
				}

				LM_DBG("run_qos_callbacks(QOSCB_UPDATE_SDP, qos_ctx=%p, "
					   "qos_sdp=%p, role=%d, _m=%p)\n",
						qos_ctx, qos_sdp, role, _m);
				run_qos_callbacks(QOSCB_UPDATE_SDP, qos_ctx, qos_sdp, role, _m);
					...
					if(cseq_method_id == qos_sdp->method_id
							...
							&& 0
									   == strncmp(cseq_number->s,
											   qos_sdp->cseq.s,
											   cseq_number->len)) {
						LM_DBG("method_id, dir and cseq match with previous "
							   "session %p->%p\n",
								qos_sdp, qos_sdp->sdp_session[other_role]);
						/* print_sdp_session(qos_sdp->sdp_session[other_role]); */
						if(qos_sdp->sdp_session[other_role] != NULL) {
							LM_DBG("run_qos_callbacks(QOSCB_REMOVE_SDP, "
								   "qos_ctx=%p, qos_sdp=%p, role=%d, _m=%p)\n",
									qos_ctx, qos_sdp, role, _m);
							run_qos_callbacks(QOSCB_REMOVE_SDP, qos_ctx,
									qos_sdp, role, _m);
							...
							LM_ERR("skipping search for null sdp for %s\n",
									(other_role == QOS_CALLER) ? "QOS_CALLER"
															   : "QOS_CALLEE");
						}
					}
				} /* end while (qos_sdp) */
