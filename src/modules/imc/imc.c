...
/** header variables */
str imc_hdrs =
		str_init("Content-Type: text/plain\r\nSupported: kamailio/imc\r\n");
char hdr_buf[1024];
str all_hdrs;
...
static int mod_init(void)
{
	...
	if(extra_hdrs.s) {
		...
		memcpy(all_hdrs.s + imc_hdrs.len, extra_hdrs.s, extra_hdrs.len);
...
static void imc_rpc_list_rooms(rpc_t *rpc, void *ctx)
{
	...
	for(i = 0; i < imc_hash_size; i++) {
		...
		while(irp) {
			...
			rpc->struct_add(vh, "SdS", "room", &irp->uri, "members",
					irp->nr_of_members, "owner",
					(irp->nr_of_members > 0) ? &irp->members->uri : &unknown);
...
static void imc_rpc_list_members(rpc_t *rpc, void *ctx)
{
	...
	if(room_name.s == NULL || room_name.len == 0 || *room_name.s == '\0'
			|| *room_name.s == '.') {
	...
	rpc->struct_add(vh, "S[d", "room", &room->uri, "members", &ih, "count",
			room->nr_of_members);
...
static const char *imc_rpc_list_rooms_doc[2] = {"List imc rooms.", 0};
