...
static int w_app_lua_run(
		struct sip_msg *msg, char *func, char *p1, char *p2, char *p3);
static int w_app_lua_run0(
		struct sip_msg *msg, char *func, char *p1, char *p2, char *p3);
static int w_app_lua_run1(
		struct sip_msg *msg, char *func, char *p1, char *p2, char *p3);
static int w_app_lua_run2(
		struct sip_msg *msg, char *func, char *p1, char *p2, char *p3);
static int w_app_lua_run3(
		struct sip_msg *msg, char *func, char *p1, char *p2, char *p3);
...
static param_export_t params[] = {
	...
	{"reload", PARAM_INT | USE_FUNC_PARAM, (void *)app_lua_reload_param},
		...
		sip_msg_t *msg, int rtype, str *rname, str *rparam)
{
	...
	if(rtype == REQUEST_ROUTE) {
		if(rname != NULL && rname->s != NULL) {
			...
			ret = app_lua_run_ex(
					msg, kemi_request_route_callback.s, NULL, NULL, NULL, 1);
		}
...
static int ki_app_lua_dostring(sip_msg_t *msg, str *script)
{
	if(script == NULL || script->s == NULL
			|| script->len >= LUA_BUF_STACK_SIZE - 1) {
