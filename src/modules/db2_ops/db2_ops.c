...
static struct dbops_action *find_action_by_name(char *name, int len)
{
	...
	for(a = dbops_actions; a; a = a->next) {
		if(a->query_name && strlen(a->query_name) == len
				&& strncmp(name, a->query_name, len) == 0)
			return a;
	}
	return NULL;
}
...
static struct dbops_handle *find_handle_by_name(char *name, int len)
{
	...
	for(a = dbops_handles; a; a = a->next) {
		if(a->handle_name && strlen(a->handle_name) == len
				&& strncmp(name, a->handle_name, len) == 0)
			return a;
	}
	return NULL;
}
...
static int parse_ops(char *act_s, struct dbops_action **action, int has_name)
{
	...
	*action = pkg_malloc(sizeof(**action));
	...
	if(has_name) {
		...
		if(c != s && *c2 == '=') {
			*c = '\0';
			...
			ERR(MODULE_NAME ": parse_ops: query_no: %d, valid query name not "
							"found in '%s'\n%s\n%s\n",
	...
	if(c[0] == ':' && c[1] == '/'
		...
		while(*s == '/')
			s++;
		res = get_next_part(&s, &part, PART_DELIM,
				1); /* type://_user:host_/database_name/ */
		if(res < 0)
			return res;


		res = get_next_part(&s, &part, PART_DELIM,
				0); /* type://user:host/_database_name_/ */
		if(res < 0)
			return res;
	}
	...
	if(strcasecmp(part, "select") == 0)
		...
		if((*action)->is_raw_query)
			*c = ' ';
		ERR(MODULE_NAME
				": parse_ops: query: %s(%d), unknown type of query '%s'\n",
	...
	if(!(*action)->is_raw_query) {
		...
		if(!*part) {
			ERR(MODULE_NAME ": parse_ops: query: %s(%d), table not specified "
							"near '%s' in '%s'\n",
		...
		switch((*action)->operation) {
			...
			case INSERT_OPS:
				res = split_fields(
						part, &(*action)->field_count, &(*action)->fields);
				if(res < 0)
					return res;
				if((*action)->field_count == 0) {
					ERR(MODULE_NAME ": parse_ops: query: %s(%d), no field "
									"specified near '%s' ?n '%s'\n",
							(*action)->query_name, (*action)->query_no, part,
							act_s);
					return E_CFG;
				}
			...
			case DELETE_OPS:
				res = split_fields(
						part, &(*action)->where_count, &(*action)->wheres);
				if(res < 0)
					return res;
				res = get_next_part(&s, &part, PART_DELIM, 0);
				if(res < 0)
	...
	if((*action)->value_count) {
		(*action)->value_types =
				(int *)pkg_malloc(sizeof(int) * (*action)->value_count);
		...
		for(i = 0; i < (*action)->value_count; i++) {
			(*action)->value_types[i] =
					DB_CSTR; // DB_NONE; /* let decide db driver itself, FIXME: until jjanak changes then default type is string */
	...
	if((*action)->extra_ops_count > 0) {
		(*action)->extra_ops = pkg_malloc(
				(*action)->extra_ops_count * sizeof(*(*action)->extra_ops));
		if(!(*action)->extra_ops) {
			ERR(MODULE_NAME ": parse_ops: not enough pkg memory\n");
			return E_OUT_OF_MEM;
		}
		memset((*action)->extra_ops, 0,
				(*action)->extra_ops_count * sizeof(*(*action)->extra_ops));
		...
		while(*c) {
			...
			eat_alphanum(fld);
			if(*fld != '=') {
				ERR(MODULE_NAME ": parse_ops: query: %s(%d), bad extra "
								"parameter format in '%s'\n",
						(*action)->query_name, (*action)->query_no,
						(*action)->extra_ops[i].name);
				return E_CFG;
			}
			*fld = '\0';
			fld++;
			...
			DEBUG(MODULE_NAME ": extra_ops #%d, name='%s', type=%d, val='%s'\n",
					i, (*action)->extra_ops[i].name,
					(*action)->extra_ops[i].type,
					(*action)->extra_ops[i].value);
			i++;
		}
	}

	if(*s) {
		ERR(MODULE_NAME ": parse_ops: query: %s(%d), too many "
						"parameters/parts, remaining '%s' in '%s'\n",
				(*action)->query_name, (*action)->query_no, s, act_s);
		return E_CFG;
	}
	if((*action)->is_raw_query) {
		DEBUG(MODULE_NAME ": query: %s(%d) oper:%d database:'%s' query:'%s' "
						  "value#:%d extra_ops#:%d\n",
				(*action)->query_name, (*action)->query_no,
				(*action)->operation, (*action)->db_url, (*action)->raw.s,
				(*action)->value_count, (*action)->extra_ops_count);
	} else {
		/* check num of fields */
		if((((*action)->operation == OPEN_QUERY_OPS) ? 0
													 : (*action)->field_count)
						+ (*action)->where_count
				!= (*action)->value_count) {
			ERR(MODULE_NAME
					": parse_ops: query: %s(%d), number of values does not "
					"correspond to number of fields (%d+%d!=%d) in '%s'\n",
					(*action)->query_name, (*action)->query_no,
					((*action)->operation == OPEN_QUERY_OPS)
							? 0
							: (*action)->field_count,
					(*action)->where_count, (*action)->value_count, act_s);
			return E_CFG;
		}
		DEBUG(MODULE_NAME
				": query_no:%d oper:%d database:'%s' table:'%s' 'field#:'%d' "
				"where#:'%d' order:'%s' value#:%d extra_ops#:%d\n",
				(*action)->query_no, (*action)->operation, (*action)->db_url,
				(*action)->table.s, (*action)->field_count,
				(*action)->where_count, (*action)->order.s,
				(*action)->value_count, (*action)->extra_ops_count);
	}
	return 0;
}
...
static int parse_xlstr(struct xlstr *s)
{
	...
	if(!xl_print) {
		...
		if(!xl_print) {
			ERR(MODULE_NAME
					": cannot find \"xprint\", is module xprint loaded?\n");
			return E_UNSPEC;
		}
	}
	...
	if(!xl_parse) {
		...
		if(!xl_parse) {
			ERR(MODULE_NAME
					": cannot find \"xparse\", is module xprint loaded?\n");
			return E_UNSPEC;
		}
	}
	...
	if(!xl_nul) {
		...
		if(!xl_nul) {
			ERR(MODULE_NAME
					": cannot find \"xnulstr\", is module xprint loaded?\n");
			return E_UNSPEC;
		} else
...
static int eval_xlstr(struct sip_msg *msg, struct xlstr *s)
{
	...
	if(s->xlfmt) {
		...
		if((xl_nul) && (xl_nul->len == len)
				&& strncmp(xl_nul->s, xlbuf_tail, len) == 0) {
			s->s = null_str;
		} else {
			s->s = xlbuf_tail;
			s->s[len] = '\0';
...
*/
	if(action->is_raw_query) {
		...
		*/
		...
		DEBUG(MODULE_NAME ": dbops_func(%s, %d, %d, '%s', %d, %d, %d)\n",
				action->query_name, action->query_no, action->operation,
				action->table.s, action->field_count, action->where_count,
				action->value_count);
	...
	for(i = 0; i < action->value_count; i++) {
			...
			switch(action->operation) {
				...
				case UPDATE_OPS:
					if(i < action->field_count)
						...
						cmd_params =
								action->cmd->match + i - action->field_count;
					break;
				case REPLACE_OPS:
				case INSERT_OPS:
					cmd_params = action->cmd->vals + i;
		...
		if(!action->values[i].s || !action->values[i].s[0])
	...
	if(db_exec((action->operation == OPEN_QUERY_OPS ? &action->result : NULL),
			   action->cmd)
			< 0)
		goto err;
	res = 1;
...
err:
	ERR(MODULE_NAME
			": query: %s(%d), database operation (%d) error, table: '%s'\n",
			action->query_name, action->query_no, action->operation,
			action->table.s);
	res = -1;
	goto cleanup;
...
static int sel_get_field(
		str *res, int *cur_row_no, int field_no, db_res_t *result)
{
	...
	if(field_no == -2) { /* cur_row_no */
		...
		if(!(rec->fld[field_no].flags & DB_NULL)) {
			switch(rec->fld[field_no].type) {
				case DB_INT:
					res->len = snprintf(
							res->s, len, "%d", rec->fld[field_no].v.int4);
					break;
				case DB_FLOAT:
					res->len = snprintf(
							res->s, len, "%f", rec->fld[field_no].v.flt);
					break;
				case DB_DOUBLE:
					res->len = snprintf(
							res->s, len, "%f", rec->fld[field_no].v.dbl);
					break;
				case DB_STR:
				...
				case DB_CSTR:
					res->len = snprintf(
							res->s, len, "%s", rec->fld[field_no].v.cstr);
					break;
				case DB_DATETIME:
					res->len = snprintf(res->s, len, "%u",
							(unsigned int)rec->fld[field_no].v.time);
					break;
				case DB_BITMAP:
					res->len = snprintf(res->s, len, "%u",
							(unsigned int)rec->fld[field_no].v.bitmap);
					break;
				default:
					break;
			}
...
static int sel_do_select(str *result, str *query_name, int row_no, int field_no,
		struct sip_msg *msg)
{
	struct dbops_action *a;
	int cur_row_no, res;
	...
	if(!a) {
		ERR(MODULE_NAME ": select: query: %.*s not declared using "
						"declare_query param\n",
...
static inline int check_query_opened(struct dbops_handle *handle, char *f)
{
	if(!handle->result) {
		ERR(MODULE_NAME
				": %s: handle '%s' is not opened. Use db_query() first\n",
...
static int sel_do_fetch(
		str *result, str *handle_name, int field_no, struct sip_msg *msg)
{
	struct dbops_handle *a;
	...
	if(check_query_opened(a, "fetch") < 0)
		return -1;
	return sel_get_field(result, &a->cur_row_no, field_no, a->result);
}

static int sel_dbops(str *res, select_t *s, struct sip_msg *msg)
{ /* dummy */
	return 0;
}
...
{
	return sel_do_select(
			res, &s->params[2].v.s, s->params[4].v.i, s->params[6].v.i, msg);
}
...
select_row_t sel_declaration[] = {
		{NULL, SEL_PARAM_STR, STR_STATIC_INIT(MODULE_NAME2), sel_dbops,
				SEL_PARAM_EXPECTED},

		{sel_dbops, SEL_PARAM_STR, STR_STATIC_INIT("query"), sel_select,
				CONSUME_NEXT_STR},
		{sel_select, SEL_PARAM_STR, STR_STATIC_INIT("is_empty"),
				sel_select_is_empty},
		{sel_select, SEL_PARAM_STR, STR_STATIC_INIT("count"), sel_select_count},
		{sel_select, SEL_PARAM_STR, STR_STATIC_INIT("nameaddr"),
				select_any_nameaddr, NESTED | CONSUME_NEXT_STR},
		{sel_select, SEL_PARAM_STR, STR_STATIC_INIT("uri"), select_any_uri,
				NESTED | CONSUME_NEXT_STR},
		{sel_select, SEL_PARAM_STR, STR_STATIC_INIT("field"), sel_select_field,
				CONSUME_NEXT_INT},
		{sel_select_field, SEL_PARAM_STR, STR_STATIC_INIT("nameaddr"),
				select_any_nameaddr, NESTED | CONSUME_NEXT_STR},
		{sel_select_field, SEL_PARAM_STR, STR_STATIC_INIT("uri"),
				select_any_uri, NESTED | CONSUME_NEXT_STR},

		{sel_select, SEL_PARAM_STR, STR_STATIC_INIT("row"), sel_select_row,
				CONSUME_NEXT_INT},
		{sel_select_row, SEL_PARAM_STR, STR_STATIC_INIT("field"),
				sel_select_row_field, CONSUME_NEXT_INT},
		{sel_select_row_field, SEL_PARAM_STR, STR_STATIC_INIT("nameaddr"),
				select_any_nameaddr, NESTED | CONSUME_NEXT_STR},
		{sel_select_row_field, SEL_PARAM_STR, STR_STATIC_INIT("uri"),
				select_any_uri, NESTED | CONSUME_NEXT_STR},

		{sel_dbops, SEL_PARAM_STR, STR_STATIC_INIT("fetch"), sel_fetch,
				CONSUME_NEXT_STR},
		{sel_fetch, SEL_PARAM_STR, STR_STATIC_INIT("nameaddr"),
				select_any_nameaddr, NESTED | CONSUME_NEXT_STR},
		{sel_fetch, SEL_PARAM_STR, STR_STATIC_INIT("uri"), select_any_uri,
				NESTED | CONSUME_NEXT_STR},
		{sel_fetch, SEL_PARAM_STR, STR_STATIC_INIT("row_no"),
				sel_fetch_cur_row_no, 0},
		{sel_fetch, SEL_PARAM_STR, STR_STATIC_INIT("count"), sel_fetch_count,
				0},
		{sel_fetch, SEL_PARAM_STR, STR_STATIC_INIT("is_empty"),
				sel_fetch_is_empty, 0},
		{sel_fetch, SEL_PARAM_STR, STR_STATIC_INIT("field"), sel_fetch_field,
				CONSUME_NEXT_INT},
		{sel_fetch_field, SEL_PARAM_STR, STR_STATIC_INIT("nameaddr"),
				select_any_nameaddr, NESTED | CONSUME_NEXT_STR},
		{sel_fetch_field, SEL_PARAM_STR, STR_STATIC_INIT("uri"), select_any_uri,
				NESTED | CONSUME_NEXT_STR},
...
static int dbops_pre_script_cb(
...
static int dbops_post_script_cb(
		struct sip_msg *msg, unsigned int flags, void *param)
{
	struct dbops_handle *a;
	for(a = dbops_handles; a; a = a->next) {
...
static int dbops_fixup_func(void **param, int init_act)
{
	...
	*param = (void *)*p;
	if(init_act)
		return init_action(
				*p); /* fixup is acquired after init_mod() therefore initialize new action */
	else
		return 0;
}
...
static int mod_init(void)
{
	...
	if(register_script_cb(
			   dbops_pre_script_cb, REQUEST_CB | ONREPLY_CB | PRE_SCRIPT_CB, 0)
			< 0) {
...
static int build_params(db_fld_t **params, struct dbops_action *p)
{
	...
	newp = (db_fld_t *)pkg_malloc(
			sizeof(db_fld_t) * (p->value_count - p->where_count + 1));
	if(newp == NULL) {
		ERR(MODULE_NAME ": No memory left\n");
		return -1;
	}
	...
	for(i = 0; i < p->value_count - p->where_count; i++) {
		newp[i].name = (i < p->field_count)
							   ? p->fields[i].s
							   : ""; /* in case of raw query it's empty */
		newp[i].type = p->value_types[i];
	}
	newp[i].name = NULL;
...
static int init_db(struct dbops_action *p)
{
	...
	if(db_add_db(p->ctx, p->db_url) < 0)
		return -1;
	if(db_connect(p->ctx) < 0)
		return -1;

	if(p->is_raw_query) {
		type = DB_SQL;
		if(build_params(&matches, p) < 0)
			return -1;
	} else {
		switch(p->operation) {
			case INSERT_OPS:
	...
	if(p->cmd == NULL) {
		ERR(MODULE_NAME ": init_db: query: %s(%d), error while compiling "
						"database query\n",
				p->query_name, p->query_no);
		if(values)
			pkg_free(values);
	...
	for(i = 0; i < p->extra_ops_count; i++) {
		char *end;
		DEBUG(MODULE_NAME
				": init_db: query_no: %s(%d), setopt('%s', %i, '%s'\n",
				p->query_name, p->query_no, p->extra_ops[i].name,
				p->extra_ops[i].type, p->extra_ops[i].value);
		switch(p->extra_ops[i].type) {
			...
			case DB_DATETIME: {
				...
				if(db_setopt(p->cmd, p->extra_ops[i].name, v) < 0)
					return -1;
				break;
			}
			case DB_INT: {
			...
			case DB_CSTR:
				if(db_setopt(
						   p->cmd, p->extra_ops[i].name, p->extra_ops[i].value)
						< 0)
					return -1;
				break;
			default:
...
static int child_init(int rank)
{
	...
	if(rank != PROC_INIT && rank != PROC_MAIN && rank != PROC_TCP_MAIN) {
		for(p = dbops_actions; p; p = p->next) {
			for(p2 = dbops_actions; p != p2;
					p2 = p2->next) { /* check if database is already opened */
	...
	if(param_no == 1) {
		...
		if(((struct dbops_action *)*param)->operation == OPEN_QUERY_OPS) {
			if(fixup_get_param_count(param, param_no) != 2) {
				ERR(MODULE_NAME
						": query_fixup: SELECT query requires 2 parameters\n");
				return E_CFG;
			}
		} else {
			if(fixup_get_param_count(param, param_no) != 1) {
				ERR(MODULE_NAME ": query_fixup: non SELECT query requires only "
								"1 parameter\n");
				return E_CFG;
			}
		}
...
static int dbops_query_func(struct sip_msg *m, char *dbops_action, char *handle)
{
	if(((struct dbops_action *)dbops_action)->operation == OPEN_QUERY_OPS) {
		...
		res = do_seek(((struct dbops_action *)dbops_action)->result,
				&a->cur_row_no, 0);
		if(res < 0)
			return res;
		return 1;
...
static int dbops_foreach_func(struct sip_msg *m, char *handle, char *route_no)
{
	...
	res = -1;
	a->cur_row_no = 0;
	for(rec = db_first(a->result); rec != NULL;
			rec = db_next(a->result), a->cur_row_no++) {
