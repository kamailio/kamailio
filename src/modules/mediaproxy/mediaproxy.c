...
#define STR_HAS_PREFIX(str, prefix) \
	((str).len >= (prefix).len      \
			&& memcmp((prefix).s, (str).s, (prefix).len) == 0)
...
static void *strfind(
		const void *haystack, size_t len, const void *needle, size_t nlen)
{
	char *sp;
...
static void *strcasefind(
		const char *haystack, size_t len, const char *needle, size_t nlen)
{
	char *sp;
...
static char *find_line_starting_with(str *block, char *start, int ignoreCase)
{
	char *ptr, *bend;
	str zone;
	int tlen;
...
static unsigned int count_lines_starting_with(
		str *block, char *start, int ignoreCase)
{
	char *ptr, *bend;
	str zone;
	int tlen;
...
static int get_tokens(char *string, str *tokens, int limit)
{
	int i, len, size;
	char *ptr;
...
static int get_str_tokens(str *string, str *tokens, int limit)
{
	int count;
	char c;
...
static Bool get_callid(struct sip_msg *msg, str *cid)
{
	if(msg->callid == NULL) {
...
static Bool get_cseq_number(struct sip_msg *msg, str *cseq)
{
	if(msg->cseq == NULL) {
...
static str get_from_uri(struct sip_msg *msg)
{
...
static str get_to_uri(struct sip_msg *msg)
{
	static str unknown = str_init("unknown");
	str uri;
	char *ptr;
...
static str get_to_tag(struct sip_msg *msg)
{
	static str undefined = str_init("");
	str tag;
...
static str get_user_agent(struct sip_msg *msg)
{
	...
	server.s = ptr + 7;
	server.len =
			findendline(server.s, block.s + block.len - server.s) - server.s;
...
static int find_content_type_application_sdp(struct sip_msg *msg, str *sdp)
{
...
//   -2 - empty message
static int get_sdp_message(struct sip_msg *msg, str *sdp)
{
	sdp->s = get_body(msg);
	if(sdp->s == NULL) {
...
static str get_sdp_line_separator(str *sdp)
{
...
static str get_direction_attribute(str *block, str *default_direction)
{
	...
	for(zone = *block;;) {
		...
		if(line.len == 8) {
			if(strncasecmp(line.s, "sendrecv", 8) == 0
					|| strncasecmp(line.s, "sendonly", 8) == 0
					|| strncasecmp(line.s, "recvonly", 8) == 0
					|| strncasecmp(line.s, "inactive", 8) == 0) {
				return line;
			}
		}
...
static str get_rtcp_port_attribute(str *block)
{
...
// if defined by the stream, otherwise will return {NULL, 0}.
static str get_rtcp_ip_attribute(str *block)
{
...
static Bool has_ice_attributes(str *block)
{
	char *ptr;
...
// session level.
static Bool has_session_ice_attributes(str *sdp)
{
	str block;
	char *ptr;
...
static Bool has_ice_candidates(str *block)
{
	char *ptr;
...
static Bool has_ice_candidate_component(str *block, int id)
{
	char *ptr, *block_end;
...
// for the candidate(s) inserted
static unsigned int get_ice_candidate_priority(str priority)
{
	int type_pref;
...
static int get_media_ip_from_block(str *block, str *mediaip)
{
	str tokens[3], zone;
	char *ptr;
	int count;
...
static Bool get_sdp_session_ip(str *sdp, str *mediaip, str *ip_line)
{
	char *ptr, *end_ptr;
	str block;
...
static str get_session_direction(str *sdp)
{
...
static int get_method_from_reply(struct sip_msg *reply)
{
...
static Bool supported_transport(str transport)
{
...
static int get_session_info(str *sdp, SessionInfo *session)
{
	...
	for(i = 0, block = *sdp; i < MAX_STREAMS; i++) {
		...
		session->streams[i].next_line =
				zone.s + zone.len + session->separator.len;
		...
		if(supported_transport(tokens[2])) {
			...
			if(ptr != NULL) {
				...
				if(!(port_nr.len == 1 && port_nr.s[0] == '1')) {
					LM_ERR("unsupported number of ports specified in `m=' "
						   "line\n");
					return -1;
				}
	...
	for(i = 0; i < session->stream_count; i++) {
		...
		if(result == -1) {
			...
			if(session->streams[i].transport == TSupported) {
				ptr = memchr(ip.s, '/', ip.len);
				if(ptr) {
					LM_ERR("unsupported multicast IP specification in stream "
						   "nr %d: %.*s\n",
							i + 1, ip.len, ip.s);
					return -1;
				}
		...
		session->streams[i].direction =
				get_direction_attribute(&block, &session->direction);
		session->streams[i].has_ice =
				((has_ice_attributes(&block) || has_session_ice_attributes(sdp))
						&& has_ice_candidates(&block));
		session->streams[i].has_rtcp_ice =
				has_ice_candidate_component(&block, 2);
		session->streams[i].first_ice_candidate =
				find_line_starting_with(&block, "a=candidate:", False);
	}
...
static Bool insert_element(struct sip_msg *msg, char *position, char *element)
{
	struct lump *anchor;
	char *buf;
	int len;
...
static Bool replace_element(
		struct sip_msg *msg, str *old_element, str *new_element)
{
	struct lump *anchor;
	char *buf;
...
static Bool remove_element(struct sip_msg *msg, str *element)
{
...
static Bool mediaproxy_connect(void)
{
	...
	if(connect(mediaproxy_socket.sock, (struct sockaddr *)&addr, sizeof(addr))
			< 0) {
...
static void mediaproxy_disconnect(void)
{
	if(mediaproxy_socket.sock < 0)
		return;
...
static char *send_command(char *command)
{
	...
	for(sent = 0, tries = 0; sent < cmd_len && tries < 3;
			tries++, sent += bytes) {
		do
			bytes = send(mediaproxy_socket.sock, command + sent, cmd_len - sent,
		...
		if(bytes == -1) {
			switch(errno) {
				...
				case EPIPE:
					...
					mediaproxy_socket.last_failure =
							0; // we want to reconnect immediately
					if(mediaproxy_connect()) {
						sent = bytes = 0;
						continue;
					} else {
	...
	while(True) {
		...
		do
			count = select(
					mediaproxy_socket.sock + 1, &rset, NULL, NULL, &timeout);
		...
		if(count == -1) {
			...
			do
				bytes = recv(mediaproxy_socket.sock,
						mediaproxy_socket.data + received,
						BUFFER_SIZE - 1 - received, 0);
			while(bytes == -1 && errno == EINTR);
			if(bytes == -1) {
...
static int use_media_proxy(
		struct sip_msg *msg, char *dialog_id, ice_candidate_data *ice_data)
{
	...
	if(have_sdp) {
		...
		str_buf.s = media_str + len;
			...
			if(stream.type.len + stream.ip.len + stream.port.len
							+ stream.direction.len + 4
					> str_buf.len) {
	...
	len = snprintf(request, sizeof(request),
			...
			signaling_ip.len, signaling_ip.s, media_relay.len, media_relay.s,
			media_str);
	...
	if(session.ip.s && !isnulladdr(session.ip)) {
		if(session.stream_count == session.supported_count) {
			if(!replace_element(msg, &session.ip, &tokens[0])) {
				LM_ERR("failed to replace session-level media IP in the SDP "
					   "body\n");
				return -1;
			}
		} else {
			if(!remove_element(msg, &session.ip_line)) {
				LM_ERR("failed to remove session-level media IP in the SDP "
					   "body\n");
				return -1;
			}
	...
	for(i = 0, j = 1; i < session.stream_count; i++) {
		...
		if(stream.transport != TSupported) {
			if(!stream.local_ip && removed_session_ip) {
				...
				if(!insert_element(msg, stream.next_line, buf)) {
					LM_ERR("failed to insert IP address in media stream number "
						   "%d\n",
							i + 1);
					return -1;
				}
		...
		if(stream.rtcp_port.len > 0 && !isnullport(stream.rtcp_port)) {
			...
			if(!replace_element(msg, &stream.rtcp_port, &rtcp_port)) {
				LM_ERR("failed to replace RTCP port in media stream number "
					   "%d\n",
						i + 1);
				return -1;
			}
		...
		if(stream.local_ip && !isnulladdr(stream.ip)) {
			if(!replace_element(msg, &stream.ip, &tokens[0])) {
				LM_ERR("failed to replace IP address in media stream number "
					   "%d\n",
						i + 1);
				return -1;
			}
		...
		if(stream.has_ice && stream.first_ice_candidate
				&& !STR_IMATCH(priority_str, "none")) {
			// add some pseudo-random string to the foundation
			struct in_addr hexip;
			inet_aton(tokens[0].s, &hexip);

			priority = (ice_data == NULL)
							   ? get_ice_candidate_priority(priority_str)
							   : ice_data->priority;
			port = strtoint(&tokens[j]);
			candidate.s = buf;
			...
			if(!insert_element(msg, stream.first_ice_candidate, candidate.s)) {
				LM_ERR("failed to insert ICE candidate in media stream number "
					   "%d\n",
						i + 1);
				return -1;
			}
			...
			if(stream.has_rtcp_ice) {
				...
				if(!insert_element(
						   msg, stream.first_ice_candidate, candidate.s)) {
					LM_ERR("failed to insert ICE candidate in media stream "
						   "number %d\n",
							i + 1);
					return -1;
				}
...
static int end_media_session(str callid, str from_tag, str to_tag)
{
...
static void __free_dialog_data(void *data)
{
	shm_free((ice_candidate_data *)data);
}


static void __dialog_requests(
		...
		struct dlg_cell *dlg, int type, struct dlg_cb_params *_params)
{
	...
	if(dlg_api.register_dlgcb(dlg, DLGCB_REQ_WITHIN | DLGCB_CONFIRMED,
			   __dialog_requests, (void *)ice_data, __free_dialog_data)
			!= 0)
	...
	if(dlg_api.register_dlgcb(dlg, DLGCB_RESPONSE_FWDED | DLGCB_RESPONSE_WITHIN,
			   __dialog_replies, (void *)ice_data, NULL)
			!= 0)
	...
	if(dlg_api.register_dlgcb(dlg,
			   ...
			   __dialog_ended, (void *)MPActive, NULL)
			!= 0)
...
static int EngageMediaProxy(struct sip_msg *msg)
{
	...
	if(!have_dlg_api) {
		LM_ERR("engage_media_proxy requires the dialog module to be loaded and "
			   "configured\n");
		return -1;
	}
...
static int mod_init(void)
{
	...
	if(pv_parse_spec(&(signaling_ip_avp.spec), &avp_spec) == 0
			|| avp_spec.type != PVT_AVP) {
		LM_CRIT("invalid AVP specification for signaling_ip_avp: `%s'\n",
				signaling_ip_avp.spec.s);
		return -1;
	}
	if(pv_get_avp_name(0, &(avp_spec.pvp), &(signaling_ip_avp.name),
			   &(signaling_ip_avp.type))
			!= 0) {
		LM_CRIT("invalid AVP specification for signaling_ip_avp: `%s'\n",
				signaling_ip_avp.spec.s);
		return -1;
	...
	if(pv_parse_spec(&(media_relay_avp.spec), &avp_spec) == 0
			|| avp_spec.type != PVT_AVP) {
		LM_CRIT("invalid AVP specification for media_relay_avp: `%s'\n",
				media_relay_avp.spec.s);
		return -1;
	}
	if(pv_get_avp_name(0, &(avp_spec.pvp), &(media_relay_avp.name),
			   &(media_relay_avp.type))
			!= 0) {
		LM_CRIT("invalid AVP specification for media_relay_avp: `%s'\n",
				media_relay_avp.spec.s);
		return -1;
	...
	if(!ice_candidate_avp.spec.s || ice_candidate_avp.spec.len <= 0) {
		LM_WARN("missing/empty ice_candidate_avp parameter. will use "
				"default.\n");
		ice_candidate_avp.spec.s = ICE_CANDIDATE_AVP_SPEC;
		ice_candidate_avp.spec.len = strlen(ice_candidate_avp.spec.s);
	}

	if(pv_parse_spec(&(ice_candidate_avp.spec), &avp_spec) == 0
			|| avp_spec.type != PVT_AVP) {
		LM_CRIT("invalid AVP specification for ice_candidate_avp: `%s'\n",
				ice_candidate_avp.spec.s);
		return -1;
	}
	if(pv_get_avp_name(0, &(avp_spec.pvp), &(ice_candidate_avp.name),
			   &(ice_candidate_avp.type))
			!= 0) {
		LM_CRIT("invalid AVP specification for ice_candidate_avp: `%s'\n",
				ice_candidate_avp.spec.s);
		return -1;
	...
	if(!STR_IMATCH(ice_candidate, "none")
			&& !STR_IMATCH(ice_candidate, "low-priority")
			&& !STR_IMATCH(ice_candidate, "high-priority")) {
	...
	if(load_dlg_api(&dlg_api) == 0) {
		...
		LM_NOTICE("engage_media_proxy() will not work because the dialog "
