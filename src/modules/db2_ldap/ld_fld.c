...
static void ld_fld_free(db_fld_t *fld, struct ld_fld *payload)
{
	...
	if(payload->values)
		ldap_value_free_len(payload->values);
	payload->values = NULL;
	if(payload->filter)
		pkg_free(payload->filter);
	payload->filter = NULL;
	pkg_free(payload);
}
...
int ld_fld(db_fld_t *fld, char *table)
{
	...
	if(db_drv_init(&res->gen, ld_fld_free) < 0)
		goto error;
...
int ld_resolve_fld(db_fld_t *fld, struct ld_cfg *cfg)
{
	...
	for(i = 0; !DB_FLD_EMPTY(fld) && !DB_FLD_LAST(fld[i]); i++) {
		...
		if(lfld->attr.s == NULL)
			lfld->attr.s = fld[i].name;
		if(lfld->attr.s)
			lfld->attr.len = strlen(lfld->attr.s);
	}
	return 0;
}
...
static inline int ldap_gentime2db_datetime(time_t *dst, str *src)
{
	...
	strptime(src->s, "%Y%m%d%H%M%S",
			&time); /* Note: frac of seconds are lost in time_t representation */

	if(src->s[src->len - 1] == 'Z' || src->s[src->len - 5] == '-'
			|| src->s[src->len - 5] == '+') {
...
int ld_ldap2fldex(db_fld_t *fld, LDAP *ldap, LDAPMessage *msg, int init)
{
	...
	for(i = 0; !DB_FLD_EMPTY(fld) && !DB_FLD_LAST(fld[i]); i++) {
		...
		if(init) {
			...
			if(lfld->values)
				ldap_value_free_len(lfld->values);
			...
			if(lfld->values == NULL || lfld->values[0] == NULL) {
				...
				if((lfld->valuesnum > 1 || lfld->client_side_filtering)
						&& lfld->filter) {
					...
					do {
						...
						if(passed) {
							...
							for(i = lfld->index + 1; i < lfld->valuesnum; i++) {
								lfld->values[i - 1]->bv_val =
										lfld->values[i]->bv_val;
								lfld->values[i - 1]->bv_len =
										lfld->values[i]->bv_len;
							}
							lfld->values[lfld->valuesnum - 1]->bv_val =
									save_bvval;
							lfld->values[lfld->valuesnum - 1]->bv_len =
									save_bvlen;
							lfld->valuesnum--;
						}
...
static inline int ld_db2ldap(struct sbuf *buf, db_fld_t *fld)
{
	...
	switch(fld->type) {
		case DB_CSTR:
			if(sb_add_esc(
					   buf, fld->v.cstr, fld->v.cstr ? strlen(fld->v.cstr) : 0)
					!= 0)
				goto error;
			break;
...
inline static void skip_client_side_filtering_fields(
		db_cmd_t *cmd, db_fld_t **fld)
{
	struct ld_fld *lfld;
	db_fld_t *f;
try_next:
	if(DB_FLD_EMPTY(*fld) || DB_FLD_LAST(**fld))
		return;
...
int ld_prepare_ldap_filter(char **filter, db_cmd_t *cmd, str *add)
{
	...
	struct ld_fld *lfld;
	int rv = 0;
	struct sbuf buf = {.s = NULL, .len = 0, .size = 0, .increment = 128};
	...
	for(; !DB_FLD_EMPTY(fld) && !DB_FLD_LAST(*fld);
			fld++, skip_client_side_filtering_fields(cmd, &fld)) {
		int op;
		lfld = DB_GET_PAYLOAD(fld);
