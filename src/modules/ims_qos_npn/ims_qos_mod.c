		...
		str sessionId)
{
	...
	if(visited_net_id.len > 0 && visited_net_id.s) {
		avp_name.s.s = "visited_net";
		avp_name.s.len = 11;
		avp_val.s = visited_net_id;
		add_avp(AVP_NAME_STR | AVP_VAL_STR, avp_name, avp_val);
	}

	if(access_network_information.len > 0 && access_network_information.s) {
		avp_name.s.s = "pani";
		avp_name.s.len = 4;
		avp_val.s = access_network_information;
		add_avp(AVP_NAME_STR | AVP_VAL_STR, avp_name, avp_val);
	}

	if(access_network_charging_info.len > 0 && access_network_charging_info.s) {
		avp_name.s.s = "anci";
		avp_name.s.len = 4;
