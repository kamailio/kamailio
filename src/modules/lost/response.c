...
void lost_delete_response_type(p_type_t type)
{
	...
	if(type->target != NULL) {
		pkg_free(type->target);
	}
	if(type->source != NULL) {
		pkg_free(type->source);
	}
	if(type->info != NULL) {
		lost_delete_response_info(type->info);
	}
	if(type != NULL) {
		pkg_free(type);
		type = NULL;
...
void lost_delete_response_data(p_data_t m)
{
	...
	if(m->name != NULL) {
		lost_delete_response_info(m->name);
	}
	if(m->number != NULL) {
		pkg_free(m->number);
	}
...
/*
 * lost_delete_findServiceResponse(response)
 * removes findServiceResponse object from private memory
 */
void lost_free_findServiceResponse(p_fsr_t res)
{
	if(res->mapping != NULL) {
		lost_delete_response_data(res->mapping);
	}
	if(res->path != NULL) {
		lost_delete_response_list(res->path);
	}
	if(res->warnings != NULL) {
		lost_delete_response_issues(res->warnings);
	}
	if(res->errors != NULL) {
		lost_delete_response_issues(res->errors);
	}
	if(res->redirect != NULL) {
		lost_delete_response_type(res->redirect);
	}
	if(res->uri != NULL) {
		lost_delete_response_list(res->uri);
	}
	if(res != NULL) {
		pkg_free(res);
		res = NULL;
