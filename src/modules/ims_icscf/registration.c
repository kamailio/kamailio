		...
		struct sip_msg *msg, char *route, char *str1, char *str2)
{
	...
	if(!b || (!b->contacts && !b->star)) {
		...
		for(c = b->contacts; c; c = c->next) {
			...
			if(sos_reg == -1) {
				//error case
				LM_ERR("ERR:I_UAR: MSG_400_MALFORMED_CONTACT, responding with "
					   "400\n");
				cscf_reply_transactional(msg, 400, MSG_400_MALFORMED_CONTACT);
				return CSCF_RETURN_BREAK;
			} else if(sos_reg == -2) {
				LM_ERR("ERR:I_UAR: MSG_500_SERVER_ERROR_OUT_OF_MEMORY, "
					   "responding with 500\n");
				cscf_reply_transactional(
						msg, 500, MSG_500_SERVER_ERROR_OUT_OF_MEMORY);
				return CSCF_RETURN_BREAK;
			}
		}
	}
	...
	if(!visited_network_id.len) {
		LM_ERR("ERR:I_UAR: Visited Network Identity not found, responding with "
			   "400\n");
	...
	if(atoi(str1))
		authorization_type = AVP_IMS_UAR_REGISTRATION_AND_CAPABILITIES;
	else {
		expires = cscf_get_max_expires(msg, 0);
		if(expires == 0)
			authorization_type = AVP_IMS_UAR_DE_REGISTRATION;
	}

	LM_DBG("SENDING UAR: PI: [%.*s], PU: [%.*s], VNID: [%.*s]\n",
			private_identity.len, private_identity.s, public_identity.len,
			public_identity.s, visited_network_id.len, visited_network_id.s);
	...
	if(cxdx_send_uar(msg, private_identity, public_identity, visited_network_id,
			   authorization_type, sos_reg, saved_t)
			!= 0) {
