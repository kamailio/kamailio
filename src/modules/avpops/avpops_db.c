...
int avp_add_db_scheme(modparam_t type, void *val)
{
	...
	LM_DBG("new scheme <%.*s> added\n"
		   ...
		   "\t\tdomain_col=<%.*s>\n\t\tvalue_col=<%.*s>\n"
		   "\t\tdb_flags=%d\n\t\ttable=<%.*s>\n",
			scheme->name.len, scheme->name.s, scheme->uuid_col.len,
			scheme->uuid_col.s, scheme->username_col.len,
			scheme->username_col.s, scheme->domain_col.len,
			scheme->domain_col.s, scheme->value_col.len, scheme->value_col.s,
			scheme->db_flags, scheme->table.len, scheme->table.s);
		...
		char *attr, struct db_scheme *scheme)
{
	...
	if(uuid) {
		/* uuid column */
		keys_cmp[nr_keys_cmp] = (scheme && scheme->uuid_col.s)
										? &scheme->uuid_col
										: db_columns[0];
		...
		if(username) {
			/* username column */
			keys_cmp[nr_keys_cmp] = (scheme && scheme->username_col.s)
											? &scheme->username_col
											: db_columns[4];
			vals_cmp[nr_keys_cmp].type = DB1_STR;
			vals_cmp[nr_keys_cmp].nul = 0;
		...
		if(domain) {
			/* domain column */
			keys_cmp[nr_keys_cmp] = (scheme && scheme->domain_col.s)
											? &scheme->domain_col
											: db_columns[5];
			vals_cmp[nr_keys_cmp].type = DB1_STR;
			vals_cmp[nr_keys_cmp].nul = 0;
		...
		const str *table, struct db_scheme *scheme)
{
	...
	if(avpops_dbf.query(db_hdl, keys_cmp, 0 /*op*/, vals_cmp, keys_ret,
			   nr_keys_cmp, nr_keys_ret, 0 /*order*/, &res)
			< 0)
		return 0;
		...
		for(j = 0; j < RES_COL_N(db_res); j++) {
			...
			switch(RES_ROWS(db_res)[i].values[j].type) {
				case DB1_STRING:
					...
					avp_val.s.s = (char *)RES_ROWS(db_res)[i]
										  .values[j]
										  .val.string_val;
				...
				case DB1_BLOB:
					...
					break;
				case DB1_INT:
					avp_val.n = (int)RES_ROWS(db_res)[i].values[j].val.int_val;
					break;
				case DB1_BIGINT:
					avp_val.n = (int)RES_ROWS(db_res)[i].values[j].val.ll_val;
					break;
				case DB1_DATETIME:
					avp_val.n = (int)RES_ROWS(db_res)[i].values[j].val.time_val;
					break;
				case DB1_BITMAP:
					avp_val.n =
