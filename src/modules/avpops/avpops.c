...
static str *db_columns[6] = {&uuid_col, &attribute_col, &value_col, &type_col,
		&username_col, &domain_col};
...
static cmd_export_t cmds[] = {
		...
		{"avp_db_load", (cmd_function)w_dbload_avps, 2, fixup_db_load_avp, 0,
				REQUEST_ROUTE | FAILURE_ROUTE | BRANCH_ROUTE | ONREPLY_ROUTE
						| LOCAL_ROUTE},
		{"avp_db_delete", (cmd_function)w_dbdelete_avps, 2, fixup_db_delete_avp,
				0,
				REQUEST_ROUTE | FAILURE_ROUTE | BRANCH_ROUTE | ONREPLY_ROUTE
						| LOCAL_ROUTE},
		{"avp_db_store", (cmd_function)w_dbstore_avps, 2, fixup_db_store_avp, 0,
				REQUEST_ROUTE | FAILURE_ROUTE | BRANCH_ROUTE | ONREPLY_ROUTE
						| LOCAL_ROUTE},
		{"avp_db_query", (cmd_function)w_dbquery1_avps, 1, fixup_db_query_avp,
				0,
				REQUEST_ROUTE | FAILURE_ROUTE | BRANCH_ROUTE | ONREPLY_ROUTE
						| LOCAL_ROUTE},
		{"avp_db_query", (cmd_function)w_dbquery2_avps, 2, fixup_db_query_avp,
				0,
				REQUEST_ROUTE | FAILURE_ROUTE | BRANCH_ROUTE | ONREPLY_ROUTE
						| LOCAL_ROUTE},
...
struct module_exports exports = {
		"avpops", DEFAULT_DLFLAGS, /* dlopen flags */
...
static int fixup_db_avp(void **param, int param_no, int allow_scheme)
{
	...
	flags = 0;
	if(db_url.s == 0) {
		LM_ERR("you have to configure a db_url for using avp_db_xxx "
			   "functions\n");
		return E_UNSPEC;
	}
	...
	if(param_no == 1) {
		...
		if(*s.s != '$') {
			...
			sp->opd =
					((flags == 0) ? AVPOPS_FLAG_UUID0 : flags) | AVPOPS_VAL_STR;
			...
			if(sp->u.sval == 0 || sp->u.sval->type == PVT_NULL
					|| sp->u.sval->type == PVT_EMPTY) {
				LM_ERR("bad param 1; "
			...
			if(sp->u.sval->type == PVT_RURI || sp->u.sval->type == PVT_FROM
					|| sp->u.sval->type == PVT_TO
					|| sp->u.sval->type == PVT_OURI) {
				sp->opd = ((flags == 0) ? AVPOPS_FLAG_URI0 : flags)
						  | AVPOPS_VAL_PVAR;
			} else {
				sp->opd = ((flags == 0) ? AVPOPS_FLAG_UUID0 : flags)
						  | AVPOPS_VAL_PVAR;
			}
		}
		*param = (void *)sp;
...
static int fixup_db_query_avp(void **param, int param_no)
{
	...
	if(db_url.s == 0) {
		LM_ERR("you have to configure db_url for using avp_db_query "
			   "function\n");
		return E_UNSPEC;
	}
...
{
	return ops_copy_avp(
			msg, (struct fis_param *)name1, (struct fis_param *)name2);
}

static int w_pushto_avps(struct sip_msg *msg, char *destination, char *param)
{
	return ops_pushto_avp(
			msg, (struct fis_param *)destination, (struct fis_param *)param);
}

static int w_check_avps(struct sip_msg *msg, char *param, char *check)
{
	return ops_check_avp(
			msg, (struct fis_param *)param, (struct fis_param *)check);
}

static int w_op_avps(struct sip_msg *msg, char *param, char *op)
{
	return ops_op_avp(msg, (struct fis_param **)param, (struct fis_param *)op);
}
...
static int ki_check_avps(struct sip_msg *msg, str *param, str *check)
{
	...
	if(fcheck->ops & AVPOPS_OP_RE) {
		...
		if(!(fcheck->opd & AVPOPS_VAL_PVAR
				   || (!(fcheck->opd & AVPOPS_VAL_PVAR)
						   && fcheck->opd & AVPOPS_VAL_STR))) {
			LM_ERR("fast_match operation requires string value or "
				   "avp name/alias (%d/%d)\n",
