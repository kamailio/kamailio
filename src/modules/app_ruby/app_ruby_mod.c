...
static int w_app_ruby_run(
		sip_msg_t *msg, char *func, char *p1, char *p2, char *p3);
static int w_app_ruby_run0(
		sip_msg_t *msg, char *func, char *p1, char *p2, char *p3);
static int w_app_ruby_run1(
		sip_msg_t *msg, char *func, char *p1, char *p2, char *p3);
static int w_app_ruby_run2(
		sip_msg_t *msg, char *func, char *p1, char *p2, char *p3);
static int w_app_ruby_run3(
		sip_msg_t *msg, char *func, char *p1, char *p2, char *p3);
...
#define RTLD_NOW DL_LAZY
	...
	/* launch register */
	bind_f = (app_ruby_proc_bind_f)dlsym(
			_app_ruby_dlhandle, "app_ruby_proc_bind");
	...
	if(_app_ruby_papi.AppRubyOptSetP(
			   "ReloadVersionPtr", _app_ruby_reload_version)
			!= 0) {
		return -1;
	}
 ...
 *
 */
int sr_kemi_config_engine_ruby(
		sip_msg_t *msg, int rtype, str *rname, str *rparam)
{
	...
	if(rtype == REQUEST_ROUTE) {
		if(rname != NULL && rname->s != NULL) {
			...
			ret = _app_ruby_papi.AppRubyRunEx(
...
static void app_ruby_rpc_reload(rpc_t *rpc, void *ctx)
{
	...
	*_app_ruby_reload_version += 1;
	...
	LM_INFO("marking for reload ruby script file: %.*s (%d / %d => %d)\n",
			_app_ruby_load_file.len, _app_ruby_load_file.s, lversion, v,
			*_app_ruby_reload_version);

	if(rpc->add(ctx, "{", &vh) < 0) {
		rpc->fault(ctx, 500, "Server error");
		return;
	}
	rpc->struct_add(vh, "dd", "old", v, "new", *_app_ruby_reload_version);
}
