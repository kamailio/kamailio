...
int diameter_is_user_in(struct sip_msg *_m, char *_hf, char *_group)
{
	...
	/* Usergroup AVP */
	if((avp = AAACreateAVP(
				AVP_User_Group, 0, 0, grp->s, grp->len, AVP_DUPLICATE_DATA))
			== 0) {
		LM_ERR("no more pkg memory!\n");
		goto error;
	}
	...
	LM_DBG("******* m_id=%d\n", _m->id);
	tmp = _m->id;
	if((avp = AAACreateAVP(AVP_SIP_MSGID, 0, 0, (char *)(&tmp), sizeof(tmp),
				AVP_DUPLICATE_DATA))
			== 0) {
		LM_ERR("no more pkg memory!\n");
		goto error;
	...
	/* ServiceType AVP */
	if((avp = AAACreateAVP(AVP_Service_Type, 0, 0, SIP_GROUP_CHECK, SERVICE_LEN,
				AVP_DUPLICATE_DATA))
			== 0) {
		LM_ERR("no more pkg memory!\n");
		goto error;
