...
auth_diam_result_t diam_pre_auth(
		struct sip_msg *_m, str *_realm, int _hftype, struct hdr_field **_h)
{
	int ret;
	struct sip_uri uri;
	str realm;
	...
	if(ret < 0) {
		...
		if(send_resp(_m, (ret == -2) ? 500 : 400,
				   (ret == -2) ? &dia_500_err : &dia_400_err, 0, 0)
				== -1) {
			LM_ERR("failed to send 400 reply\n");
		}
		return ERROR;
		...
		sip_uri_t *ruri, unsigned int m_id, rd_buf_t *rb)
{
	...
	if(!cred) {
		...
		if(user_name.len > 0) {
			...
			if(uri->user.len > 0) {
				...
				memcpy(user_name.s + uri->user.len + 1, uri->host.s,
						uri->host.len);
			} else
	...
	} else /* it is a SIP message with credentials */
	{
		...
		if(cred->username.domain.len > 0) {
			if((avp = AAACreateAVP(AVP_User_Name, 0, 0, cred->username.whole.s,
						cred->username.whole.len, AVP_DUPLICATE_DATA))
					== 0) {
				LM_ERR("could not create AVP\n");
				goto error;
			}
	...
	LM_DBG("******* m_id=%d\n", m_id);
	tmp = m_id;
	if((avp = AAACreateAVP(AVP_SIP_MSGID, 0, 0, (char *)(&tmp), sizeof(m_id),
				AVP_DUPLICATE_DATA))
			== 0) {
		LM_ERR("could not create AVP\n");
		goto error;
	...
	if((avp = AAACreateAVP(AVP_Service_Type, 0, 0, SIP_AUTHENTICATION,
				SERVICE_LEN, AVP_DUPLICATE_DATA))
			== 0) {
		LM_ERR("could not create AVP\n");
		goto error;
	}
	...
	if((avp = AAACreateAVP(AVP_Destination_Realm, 0, 0, uri->host.s,
				uri->host.len, AVP_DUPLICATE_DATA))
			== 0) {
		LM_ERR("could not create AVP\n");
		goto error;
	}
	...
	memcpy(user_name.s + ruri->user.len + name_flag, ruri->host.s,
			ruri->host.len);
	...
	if(ruri->port.s) {
		...
		memcpy(user_name.s + ruri->user.len + ruri->host.len + name_flag
						+ port_flag,
	...
	if((avp = AAACreateAVP(
				AVP_Resource, 0, 0, user_name.s, user_name.len, AVP_FREE_DATA))
			== 0) {
		LM_ERR("could not create AVP\n");
		if(user_name.s)
			pkg_free(user_name.s);
	...
	{
		/* Response AVP */
		if((avp = AAACreateAVP(AVP_Response, 0, 0, hdr->body.s, hdr->body.len,
					AVP_DUPLICATE_DATA))
				== 0) {
			LM_ERR("could not create AVP\n");
			goto error;
		...
		/* Method AVP */
		if((avp = AAACreateAVP(AVP_Method, 0, 0, p_method->s, p_method->len,
					AVP_DUPLICATE_DATA))
				== 0) {
			LM_ERR("could not create AVP\n");
			goto error;
