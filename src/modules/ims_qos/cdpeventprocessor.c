...
cdp_cb_event_t *new_cdp_cb_event(
		int event, str *rx_session_id, rx_authsessiondata_t *session_data)
{
	...
	if((rx_session_id->len > 0) && rx_session_id->s) {
		...
		memcpy(new_event->rx_session_id.s, rx_session_id->s,
				rx_session_id->len);
...
void push_cdp_cb_event(cdp_cb_event_t *event)
{
	...
	cdp_event_list->size++;
	if(cdp_event_list_size_threshold > 0
			&& cdp_event_list->size > cdp_event_list_size_threshold) {
		LM_WARN("cdp_event_list is size [%d] and has exceed "
				"cdp_event_list_size_threshold of [%d]",
				cdp_event_list->size, cdp_event_list_size_threshold);
	}
	...
	for(;;) {
		...
		if(cdp_event_latency) { //track delays
			...
			if(diff > cdp_event_threshold) {
				switch(cdp_event_latency_loglevel) {
					case 0:
						LM_ERR("Took to long to pickup CDP callback event [%d] "
							   "> [%d]\n",
								diff, cdp_event_threshold);
						break;
					case 1:
						LM_WARN("Took to long to pickup CDP callback event "
								"[%d] > [%d]\n",
								diff, cdp_event_threshold);
						break;
					case 2:
						LM_INFO("Took to long to pickup CDP callback event "
								"[%d] > [%d]\n",
								diff, cdp_event_threshold);
						break;
					case 3:
						LM_DBG("Took to long to pickup CDP callback event [%d] "
							   "> [%d]\n",
								diff, cdp_event_threshold);
						break;
					default:
		...
		switch(ev->event) {
			...
			case AUTH_EV_RECV_ASR:
				LM_DBG("Received notification of ASR from transport plane or "
					   "CDP timeout for CDP session with Rx session ID: [%.*s] "
					   "and associated contact [%.*s]"
					   " and domain [%.*s]\n",
			...
			case AUTH_EV_SERVICE_TERMINATED:
				LM_DBG("Received notification of CDP TERMINATE of CDP session "
					   "with Rx session ID: [%.*s] and associated contact "
					   "[%.*s]"
					   " and domain [%.*s]\n",
						rx_session_id->len, rx_session_id->s,
						p_session_data->registration_aor.len,
				...
				if(p_session_data->subscribed_to_signaling_path_status) {
					...
					if(p_session_data->session_has_been_opened) {
						if(delete_contact_on_rx_failure) {
							if(ul.register_udomain(
									   p_session_data->domain.s, &domain)
									< 0) {
								LM_DBG("Unable to register usrloc "
									   "domain....aborting\n");
								return;
							}
							ul.lock_udomain(domain, &p_session_data->via_host,
									p_session_data->via_port,
									p_session_data->via_proto);

							contact_info.received_host = p_session_data->ip;
							contact_info.received_port =
									p_session_data->recv_port;
							contact_info.received_proto =
									p_session_data->recv_proto;
							...
							if(ul.get_pcontact(
									   domain, &contact_info, &pcontact, 0)
									!= 0) {
								LM_DBG("no contact found for terminated Rx reg "
									   "session..... ignoring\n");
							} else {
								LM_DBG("Updating contact [%.*s] after Rx reg "
									   "session terminated, setting state to "
							...
							ul.unlock_udomain(domain, &p_session_data->via_host,
									p_session_data->via_port,
									p_session_data->via_proto);
						}
						counter_add(
					...
					LM_DBG("This is a media bearer session session");
					if(p_session_data->session_has_been_opened) {
						LM_DBG("Session was opened so decrementing "
							   "active_media_rx_sessions\n");
						counter_add(
								ims_qos_cnts_h.active_media_rx_sessions, -1);
					}
