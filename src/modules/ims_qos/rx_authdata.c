		...
		rx_authsessiondata_t **session_data)
{
	...
	int len = (domain->len + 1) + ip->len + aor->len + via_host->len
			  + sizeof(rx_authsessiondata_t);
	...
	p_session_data->must_terminate_dialog =
			0; /*irrelevent for reg session data this will always be 0 */

	p_session_data->session_has_been_opened =
			0; /*0 has not been opened 1 has been opened*/
		...
		rx_authsessiondata_t **session_data)
{
	...
	if(!call_session_data) {
		LM_ERR("no more shm mem trying to create call_session_data of size "
			   "%d\n",
				len);
		return -1;
	}
	...
	call_session_data->session_has_been_opened =
			0; /*0 has not been opened 1 has been opened*/
		...
		int current)
{
	...
	int len = media->len + req_sdp_ip_addr->len + req_sdp_port->len
			  + rpl_sdp_ip_addr->len + rpl_sdp_port->len
			  + rpl_sdp_transport->len + req_sdp_raw_stream->len
			  + rpl_sdp_raw_stream->len + sizeof(flow_description_t);
	fd = shm_malloc(len);
	if(!fd) {
		LM_ERR("no more shm mem trying to create new flow description of size "
			   "%d\n",
				len);
		return -1;
	}
	...
	if(rpl_sdp_transport && rpl_sdp_transport->len > 0
			&& rpl_sdp_transport->s) {
		...
		memcpy(fd->rpl_sdp_transport.s, rpl_sdp_transport->s,
				rpl_sdp_transport->len);
		fd->rpl_sdp_transport.len = rpl_sdp_transport->len;
		p += rpl_sdp_transport->len;
	}
	if(req_sdp_raw_stream && req_sdp_raw_stream->len > 0
			&& req_sdp_raw_stream->s) {
		...
		memcpy(fd->req_sdp_raw_stream.s, req_sdp_raw_stream->s,
				req_sdp_raw_stream->len);
		fd->req_sdp_raw_stream.len = req_sdp_raw_stream->len;
		p += req_sdp_raw_stream->len;
	}
	if(rpl_sdp_raw_stream && rpl_sdp_raw_stream->len > 0
			&& rpl_sdp_raw_stream->s) {
		...
		memcpy(fd->rpl_sdp_raw_stream.s, rpl_sdp_raw_stream->s,
				rpl_sdp_raw_stream->len);
...
void free_callsessiondata(rx_authsessiondata_t *session_data)
{
	...
	LM_DBG("Freeing session data for [%.*s]\n", session_data->via_host.len,
			session_data->via_host.s);
	LM_DBG("Destroy current flow description\n");
...
void show_callsessiondata(rx_authsessiondata_t *session_data)
{
	...
	LM_DBG("Session data:\n");
	LM_DBG("=====================\n");
	LM_DBG("Call id [%.*s]\n", session_data->callid.len,
			session_data->callid.s);
	...
	LM_DBG("Identifier [%.*s]\n", session_data->identifier.len,
			session_data->identifier.s);
	LM_DBG("Registration AOR [%.*s]\n", session_data->registration_aor.len,
			session_data->registration_aor.s);
	...
	while(flow_description) {
		...
		LM_DBG("\tReq_sdp_raw_stream [%.*s]\n",
				flow_description->req_sdp_raw_stream.len,
				flow_description->req_sdp_raw_stream.s);
		...
		LM_DBG("\tRpl_sdp_raw_stream [%.*s]\n",
				flow_description->rpl_sdp_raw_stream.len,
				flow_description->rpl_sdp_raw_stream.s);
		LM_DBG("\tRpl_sdp_transport [%.*s]\n",
				flow_description->rpl_sdp_transport.len,
				flow_description->rpl_sdp_transport.s);
