...
inline void free_dp_config(dp_config *x)
{
	...
	if(x->fqdn.s)
		shm_free(x->fqdn.s);
	if(x->identity.s)
		shm_free(x->identity.s);
	if(x->realm.s)
		shm_free(x->realm.s);
	if(x->product_name.s)
		shm_free(x->product_name.s);
	if(x->peers) {
	...
	if(x->applications)
		shm_free(x->applications);

	if(x->supported_vendors)
		shm_free(x->supported_vendors);
...
void log_dp_config(dp_config *x)
{
	...
	for(i = 0; i < x->applications_cnt; i++)
		LM_DBG("\t\t%s ID:  %d \t Vendor: %d \n",
				...
				x->applications[i].id, x->applications[i].vendor);
