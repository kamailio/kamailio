...
typedef struct _cdp_auth_session_t
{
	...
	cdp_auth_session_class_t class
			; /**< useful to know if this is Rx reg or Rx media for example */
	...
	unsigned int
			last_requested_timeout; /**<requesitng in a request, if the answer does not have anything */
	unsigned int
...
{
	ACC_EV_START =
			101, /**< Client or device "requests access" (SIP session establishment) */
	ACC_EV_EVENT =
			102, /**< Client or device requests a one-time service (e.g. SIP MESSAGE) */
	ACC_EV_BUFFEREDSTART = 103, /**< Records in storage */
	ACC_EV_RCV_SUC_ACA_START =
			104,		  /**< Successful accounting start answer received */
	ACC_EV_SNDFAIL = 105, /**< Failure to send */
	ACC_EV_RCV_FAILED_ACA_START =
			106,		  /**< Failed accounting start answer received */
	ACC_EV_STOP = 107,	  /**< User service terminated */
	ACC_EV_INTERIM = 108, /**< Interim interval elapses */
	ACC_EV_RCV_SUC_ACA_INTERIM =
			109, /**< Successful accounting interim answer received */
	ACC_EV_RCV_FAILED_ACA_INTERIM =
			110, /**< Failed accounting interim answer received */
	ACC_EV_RCV_SUC_ACA_EVENT =
			111, /**< Successful accounting event answer received */
	ACC_EV_RCV_FAILED_ACA_EVENT =
			112, /**< Failed accounting event answer received */
	ACC_EV_RCV_SUC_ACA_STOP =
			113, /**< Successful accounting stop answer received */
	ACC_EV_RCV_FAILED_ACA_STOP =
			114, /**< Failed accounting stop answer received */
} cdp_acc_event_t;
...
typedef enum
{
	ACC_CC_EV_SESSION_START =
			0,				/**< Client or device requests access to service */
	...
	ACC_CC_EV_SESSION_CREATED =
			5, /**< used for callbacks - new CC session created */
	...
	ACC_CC_EV_SESSION_STALE =
			10, /**< Session is being removed from memory after stale expiry */
} cdp_cc_acc_event_t;
...
{
	ACC_CC_TYPE_EVENT =
			0, /**< Immediate Event Charge (IEC - see TS32.299 6.3.2.1)*/
	ACC_CC_TYPE_SESSION =
			1, /**< Session-based charging (SCUR/ECUR - see TS32.299 6.3.2.1)*/
} cdp_cc_acc_type_t;
...
typedef struct _acc_session
{
	...
	unsigned int
			acct_record_number; /**< number of last accounting record within this session */
...
typedef struct _cc_acc_session
{
	...
	cdp_cc_acc_type_t
			type; /**< type of CC, event or session (each have a different state machine - see RFC 4006 */
	cdp_cc_acc_ccfh_t
			ccfh; /**< credit control failure handling type set for this session */
...
typedef struct _cdp_session_t
{
	...
	unsigned int
			application_id; /**< specific application id associated with this session */
	unsigned int vendor_id; /**< specific vendor id for this session */
	cdp_session_type_t type;
	str dest_host,
			dest_realm; /*the destination host and realm, used only for auth, for the moment*/
...
void cdp_session_cleanup(cdp_session_t *s,
		AAAMessage *msg); /**< Session cleanup for all session types */


cdp_session_t *cdp_get_session(str id);
cdp_session_t *cdp_new_session(str id,
		cdp_session_type_t
				type); //this function is needed in the peerstatemachine

void cdp_add_session(cdp_session_t *x);
cdp_session_t *cdp_new_auth_session(str id, int is_client, int is_statefull);
cdp_session_t *cdp_new_cc_acc_session(
		str id, int is_statefull); //new redit control acct session
...
AAASession *AAACreateCCAccSession(AAASessionCallback_f *cb, int is_session,
		void *generic_data); /*create CreditControl accounting session*/
typedef AAASession *(*AAACreateCCAccSession_f)(
...
void AAADropCCAccSession(
		AAASession *s); /*drop CreditControl accounting session*/
