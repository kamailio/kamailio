...
int verify_destination(struct sip_msg *_msg, char *s1, char *s2)
{
	...
	if(!rc_avpair_add(rh, &send, attrs[A_USER_NAME].v, _msg->parsed_uri.host.s,
	...
	if(!rc_avpair_add(rh, &send, attrs[A_SIP_URI_USER].v,
			   _msg->parsed_uri.user.s, _msg->parsed_uri.user.len, 0)) {
	...
	if(!rc_avpair_add(rh, &send, attrs[A_SIP_FROM_TAG].v,
			   get_from(_msg)->tag_value.s, get_from(_msg)->tag_value.len, 0)) {
		LM_ERR("error adding PW_SIP_FROM_TAG\n");
		goto err;
	}

	/* Add Call-Id */
	if((parse_headers(_msg, HDR_CALLID_F, 0) == -1) || (_msg->callid == NULL)
			|| (_msg->callid->body.s == NULL)
			|| (_msg->callid->body.len <= 0)) {
		LM_ERR("error while accessing Call-Id\n");
		goto err;
	}

	if(!rc_avpair_add(rh, &send, attrs[A_SIP_CALL_ID].v, _msg->callid->body.s,
			   _msg->callid->body.len, 0)) {
...
int verify_source(struct sip_msg *_msg, char *s1, char *s2)
{
	...
	for(hf = _msg->headers; hf; hf = hf->next) {
		if(cmp_hdrname_strzn(
				   &hf->name, "P-Request-Hash", sizeof("P-Request-Hash") - 1)
				== 0)
			break;
	}
	if(!hf) {
	...
	if(!rc_avpair_add(rh, &send, attrs[A_SIP_REQUEST_HASH].v, hf->body.s,
			   hf->body.len, 0)) {
