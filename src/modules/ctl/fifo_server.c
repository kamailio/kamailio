...
#define DEFAULT_REPLY_WAIT \
	80000 /* How long we should wait for the client, in micro seconds */
#define DEFAULT_FIFO_DIR "/tmp/" /* Where reply pipes may be opened */
#define MAX_MSG_CHUNKS 1024		 /* maximum message pieces */
#define FIFO_TX_TIMEOUT \
	200 /* maximum time to block writing to
                                          the fifo */
...
enum text_flags
{
	CHUNK_SEEN = (1 << 0),
	CHUNK_POSITIONAL =
			(1 << 1), /* Positional parameter, should be followed by \n */
	CHUNK_MEMBER_NAME =
			(1 << 2), /* Struct member name, should be followed by : */
	CHUNK_MEMBER_VALUE =
			(1 << 3) /* Struct member value, should be followed by , if
...
typedef struct rpc_context
{
	...
	struct text_chunk
			*strs; /* Strings to be collected at the end of processing */
	struct rpc_struct
			*structs; /* Structures to be collected at the end of processing */
...
static int rpc_scan(
		rpc_ctx_t *ctx, char *fmt, ...); /* Retrieve request parameters */
static int rpc_rpl_printf(rpc_ctx_t *ctx, char *fmt,
		...); /* Add printf-like formatted data to the result set */
static int rpc_struct_add(
		struct text_chunk *s, char *fmt, ...); /* Create a new structure */
static int rpc_struct_scan(struct rpc_struct *s, char *fmt,
		...); /* Scan attributes of a structure */
...
static struct rpc_struct *new_struct(rpc_ctx_t *ctx, str *line)
{
	...
	if(!line->len) {
		rpc_fault(ctx, 400, "Line %d Empty - Structure Expected", ctx->line_no);
		return 0;
	}
	...
	do {
		...
		if(!n) {
			rpc_fault(ctx, 400,
					"Error while processing struct member '%.*s' "
					"on line %d",
					name.len, ZSW(name.s), ctx->line_no);
			goto err;
		}
		n->next = s->names;
		s->names = n;
 ...
 */
int init_fifo_fd(
		char *fifo, int fifo_mode, int fifo_uid, int fifo_gid, int *fifo_write)
{
		...
		switch(*fmt) {
			...
			case 'd': /* Integer */
				if(!line.len) {
					if(nofault == 0)
						rpc_fault(ctx, 400,
								"Invalid parameter value on line %d",
								ctx->line_no);
					goto error;
				}
			...
			default:
				...
				rpc_fault(ctx, 500,
						"Server Internal Error (Invalid Formatting Character "
						"'%c')",
						*fmt);
				goto error;
		}
...
static int rpc_struct_add(struct text_chunk *s, char *fmt, ...)
{
	...
	while(*fmt) {
		...
		st.len = strlen(st.s);
		m = new_chunk_escape(
				&st, 1); /* Escape all characters, including : and , */
		if(!m) {
			rpc_fault(ctx, 500, "Internal Server Error");
			goto err;
		}
		...
		if(*fmt == '{' || *fmt == '[') {
			...
			*void_ptr = m;
			...
			switch(*fmt) {
				...
				default:
					rpc_fault(ctx, 500,
							"Bug In SER (Invalid formatting character %c)",
							*fmt);
					ERR("Invalid formatting character\n");
					goto err;
			}
...
static int find_member(
		struct text_chunk **value, struct rpc_struct *s, str *member_name)
{
	struct text_chunk *n, *v;
...
static int rpc_struct_scan(struct rpc_struct *s, char *fmt, ...)
{
	...
	while(*fmt) {
		...
		switch(*fmt) {
			...
			default:
				rpc_fault(s->ctx, 500,
						"Invalid character in formatting string '%c'", *fmt);
				ERR("Invalid parameter type in formatting string: %c\n", *fmt);
				goto error;
		}
		fmt++;
