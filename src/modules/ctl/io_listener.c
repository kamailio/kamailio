...
#define IO_LISTEN_TIMEOUT \
	10 /* in s,  how often the timer
										  will be run */
...
static struct stream_connection *s_conn_new(
		int sock, struct ctrl_socket *cs, union sockaddr_u *from)
{
 ...
 */
inline static int sendv_disc(
		struct send_handle *sh, struct iovec *v, size_t count)
{
...
again:
	bytes = recvfrom(
			cs->fd, buf, DGRAM_BUF_SIZE, 0, &sh.from.sa_in.s, &sh.from_len);
	if(bytes == -1) {
...
again:
	...
	do {
		...
		if(s_c->p_proto == P_FIFO)
			bytes_processed =
					fifo_process((char *)r->proc, (int)(r->end - r->proc),
