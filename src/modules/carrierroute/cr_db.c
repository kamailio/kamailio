...
str *columns[COLUMN_NUM] = {&carrierroute_id_col, &carrierroute_carrier_col,
		&carrierroute_domain_col, &carrierroute_scan_prefix_col,
		&carrierroute_flags_col, &carrierroute_mask_col, &carrierroute_prob_col,
		&carrierroute_rewrite_host_col, &carrierroute_strip_col,
		&carrierroute_rewrite_prefix_col, &carrierroute_rewrite_suffix_col,
		&carrierroute_description_col};

str *carrier_name_columns[CARRIER_NAME_COLUMN_NUM] = {
		&carrier_name_id_col, &carrier_name_carrier_col};

str *domain_name_columns[DOMAIN_NAME_COLUMN_NUM] = {
		&domain_name_id_col, &domain_name_domain_col};

str *failure_columns[FAILURE_COLUMN_NUM] = {&carrierfailureroute_id_col,
		&carrierfailureroute_carrier_col, &carrierfailureroute_domain_col,
		&carrierfailureroute_scan_prefix_col,
		&carrierfailureroute_host_name_col, &carrierfailureroute_reply_code_col,
		&carrierfailureroute_flags_col, &carrierfailureroute_mask_col,
...
void set_load_comments_params(int lc)
{
	...
	failure_columns_load_num =
			lc ? FAILURE_COLUMN_NUM : FAILURE_COLUMN_NUM_NO_COMMENT;
}
...
static int load_carrier_map(struct route_data_t *rd)
{
	...
	if(carrierroute_dbf.query(carrierroute_dbh, 0, 0, 0,
			   (db_key_t *)carrier_name_columns, 0, CARRIER_NAME_COLUMN_NUM, 0,
			   &res)
			< 0) {
	...
	count = RES_ROW_N(res);
	if(count == 0) {
		LM_ERR("empty %.*s table", carrier_name_table.len,
				carrier_name_table.s);
	...
	for(i = 0; i < count; i++) {
		rd->carrier_map[i].id =
				res->rows[i].values[CARRIER_NAME_ID_COL].val.int_val;
		rd->carrier_map[i].name.len = strlen(
				res->rows[i].values[CARRIER_NAME_NAME_COL].val.string_val);
		...
		memcpy(rd->carrier_map[i].name.s,
				res->rows[i].values[CARRIER_NAME_NAME_COL].val.string_val,
				rd->carrier_map[i].name.len);
	}
...
static int load_domain_map(struct route_data_t *rd)
{
	...
	if(carrierroute_dbf.query(carrierroute_dbh, 0, 0, 0,
			   (db_key_t *)domain_name_columns, 0, DOMAIN_NAME_COLUMN_NUM, 0,
			   &res)
			< 0) {
		LM_ERR("couldn't query table\n");
		return -1;
	...
	for(i = 0; i < count; i++) {
		rd->domain_map[i].id =
				res->rows[i].values[DOMAIN_NAME_ID_COL].val.int_val;
		rd->domain_map[i].name.len = strlen(
				res->rows[i].values[DOMAIN_NAME_NAME_COL].val.string_val);
		...
		memcpy(rd->domain_map[i].name.s,
				res->rows[i].values[DOMAIN_NAME_NAME_COL].val.string_val,
				rd->domain_map[i].name.len);
	}
...
int load_user_carrier(str *user, str *domain)
{
	...
	if(carrierroute_dbf.query(carrierroute_dbh, keys, op, vals, cols,
			   use_domain ? 2 : 1, 1, NULL, &res)
			< 0) {
...
int load_route_data_db(struct route_data_t *rd)
{
	...
	if((strlen("SELECT DISTINCT  FROM  WHERE = ") + carrierroute_table.len
			   + columns[COL_DOMAIN]->len + columns[COL_CARRIER]->len + 20)
			> QUERY_LEN) {
	...
	if((rd->carriers = shm_malloc(
				sizeof(struct carrier_data_t *) * rd->carrier_num))
			== NULL) {
		SHM_MEM_ERROR;
		goto errout;
	}
	...
	for(i = 0; i < rd->carrier_num; i++) {
		...
		ret = snprintf(query, QUERY_LEN,
				"SELECT DISTINCT %.*s FROM %.*s WHERE %.*s=%i",
				columns[COL_DOMAIN]->len, columns[COL_DOMAIN]->s,
				carrierroute_table.len, carrierroute_table.s,
				columns[COL_CARRIER]->len, columns[COL_CARRIER]->s,
				rd->carrier_map[i].id);
		if(ret < 0) {
		...
		LM_INFO("carrier '%.*s' (id %i) has %i domains\n",
				...
				rd->carrier_map[i].id, RES_ROW_N(res));
		tmp_carrier_data = create_carrier_data(rd->carrier_map[i].id,
				&rd->carrier_map[i].name, RES_ROW_N(res));
	...
	if(carrierroute_dbf.use_table(carrierroute_dbh, &carrierroute_table) < 0) {
		LM_ERR("Cannot set database table '%.*s'.\n", carrierroute_table.len,
				carrierroute_table.s);
		return -1;
	}

	if(DB_CAPABILITY(carrierroute_dbf, DB_CAP_FETCH)) {
		if(carrierroute_dbf.query(carrierroute_dbh, NULL, NULL, NULL,
				   (db_key_t *)columns, 0, columns_load_num, NULL, NULL)
				< 0) {
			LM_ERR("Failed to query database to prepare fetch row.\n");
			return -1;
		}
		if(carrierroute_dbf.fetch_result(carrierroute_dbh, &res,
				   cfg_get(carrierroute, carrierroute_cfg, fetch_rows))
				< 0) {
		...
		if(carrierroute_dbf.query(carrierroute_dbh, NULL, NULL, NULL,
				   (db_key_t *)columns, 0, columns_load_num, NULL, &res)
				< 0) {
			LM_ERR("Failed to query database.\n");
			return -1;
		}
	...
	do {
		...
		for(i = 0; i < RES_ROW_N(res); ++i) {
			row = &RES_ROWS(res)[i];
			tmp_scan_prefix.s =
					(char *)row->values[COL_SCAN_PREFIX].val.string_val;
			tmp_rewrite_host.s =
					(char *)row->values[COL_REWRITE_HOST].val.string_val;
			tmp_rewrite_prefix.s =
					(char *)row->values[COL_REWRITE_PREFIX].val.string_val;
			tmp_rewrite_suffix.s =
					(char *)row->values[COL_REWRITE_SUFFIX].val.string_val;
			if(tmp_scan_prefix.s == NULL)
				tmp_scan_prefix.s = "";
			if(tmp_rewrite_host.s == NULL)
				tmp_rewrite_host.s = "";
			...
			if(add_route(rd, row->values[COL_CARRIER].val.int_val,
					   row->values[COL_DOMAIN].val.int_val, &tmp_scan_prefix,
					   row->values[COL_FLAGS].val.int_val,
					   row->values[COL_MASK].val.int_val, 0,
					   row->values[COL_PROB].val.double_val, &tmp_rewrite_host,
					   row->values[COL_STRIP].val.int_val, &tmp_rewrite_prefix,
					   &tmp_rewrite_suffix, 1, 0, -1, NULL, p_tmp_comment)
					== -1) {
				goto errout;
			}
		}
	...
	for(i = 0; i < rd->carrier_num; ++i) {
		for(j = 0; j < rd->carriers[i]->domain_num; ++j) {
			if(rd->carriers[i]->domains[j]->sum_prob == 0.0) {
				LM_ERR("All routes with carrier id %d (%.*s) and domain id %d "
					   "(%.*s) have probability 0.\n",
						rd->carriers[i]->id, rd->carriers[i]->name->len,
						rd->carriers[i]->name->s,
						rd->carriers[i]->domains[j]->id,
	...
	if(carrierroute_dbf.use_table(carrierroute_dbh, &carrierfailureroute_table)
			< 0) {
		LM_ERR("cannot set database table '%.*s'.\n",
	...
	if(carrierroute_dbf.query(carrierroute_dbh, NULL, NULL, NULL,
			   (db_key_t *)failure_columns, 0, failure_columns_load_num, NULL,
			   &res)
			< 0) {
		LM_ERR("failed to query database.\n");
		return -1;
	...
	for(i = 0; i < RES_ROW_N(res); ++i) {
		row = &RES_ROWS(res)[i];
		tmp_scan_prefix.s =
				(char *)row->values[FCOL_SCAN_PREFIX].val.string_val;
		...
		if(tmp_scan_prefix.s == NULL)
			tmp_scan_prefix.s = "";
		if(tmp_host_name.s == NULL)
			tmp_host_name.s = "";
		if(tmp_reply_code.s == NULL)
			tmp_reply_code.s = "";
		tmp_scan_prefix.len = strlen(tmp_scan_prefix.s);
		...
		if(add_failure_route(rd, row->values[FCOL_CARRIER].val.int_val,
				   row->values[COL_DOMAIN].val.int_val, &tmp_scan_prefix,
				   &tmp_host_name, &tmp_reply_code,
				   row->values[FCOL_FLAGS].val.int_val,
				   row->values[FCOL_MASK].val.int_val,
				   row->values[FCOL_NEXT_DOMAIN].val.int_val, p_tmp_comment)
				== -1) {
			goto errout;
		}
	}
