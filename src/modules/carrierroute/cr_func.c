 ...
 */
static inline int cr_gp2id(
		struct sip_msg *_msg, gparam_t *gp, struct name_map_t *map, int size)
{
	int id;
	struct usr_avp *avp;
	int_str avp_val;
		...
		case GPARAM_TYPE_PVE:
			...
			if(gp->v.pve->spec->type == PVT_AVP) {
				...
				if(!avp) {
					...
					else if(gp->v.pve->spec->pvp.pvn.u.isname.type
							& AVP_NAME_RE)
				...
				if((avp->flags & AVP_VAL_STR) == 0) {
					...
					if(id < 0) {
						if(gp->v.pve->spec->pvp.pvn.u.isname.type
								& AVP_NAME_STR)
							LM_ERR("cannot map carrier with id %.*s from  AVP "
								   "'%.*s'\n",
									avp_val.s.len, avp_val.s.s,
									gp->v.pve->spec->pvp.pvn.u.isname.name.s
											.len,
									gp->v.pve->spec->pvp.pvn.u.isname.name.s.s);
						else if(gp->v.pve->spec->pvp.pvn.u.isname.type
								& AVP_NAME_RE)
		...
		*/
		if(((rr->mask & flags) == rr->flags)
			...
			if(add_avp(dstavp->v.pve->spec->pvp.pvn.u.isname.type,
					   dstavp->v.pve->spec->pvp.pvn.u.isname.name, avp_val)
					< 0) {
...
static int set_next_domain_recursor(struct dtrie_node_t *failure_node,
		const str *uri, const str *host, const str *reply_code,
		const flag_t flags, const gparam_t *dstavp)
{
	str re_uri = *uri;
	void **ret;
	...
	if(ret == NULL) {
		...
		return set_next_domain_on_rule(*ret, host, reply_code, flags, dstavp);
}
	...
	*p = '\0';
	...
	if(descavp) {
		...
		if(add_avp(AVP_VAL_STR | descavp->v.pve->spec->pvp.pvn.u.isname.type,
				   descavp->v.pve->spec->pvp.pvn.u.isname.name, avp_val)
				< 0) {
			LM_ERR("set AVP failed\n");
			return -1;
		}
...
static int rewrite_on_rule(struct route_flags *rf_head, flag_t flags, str *dest,
		struct sip_msg *msg, const str *user,
 ...
 * @return 0 on success, -1 on failure, 1 on no more matching child node and no rule list
 */
static int rewrite_uri_recursor(struct dtrie_node_t *node, const str *pm,
		flag_t flags, str *dest, struct sip_msg *msg, const str *user,
		const enum hash_source hash_source, const enum hash_algorithm alg,
		gparam_t *descavp)
{
	str re_pm = *pm;
	...
	if(ret == NULL) {
		...
		return rewrite_on_rule(
				*ret, flags, dest, msg, user, hash_source, alg, descavp);
}
 ...
 */
int cr_do_route(struct sip_msg *_msg, gparam_t *_carrier, gparam_t *_domain,
		gparam_t *_prefix_matching, gparam_t *_rewrite_user,
		enum hash_source _hsrc, enum hash_algorithm _halg, gparam_t *_dstavp)
{
	...
	if(rewrite_uri_recursor(domain_data->tree, &prefix_matching, flags, &dest,
			   _msg, &rewrite_user, _hsrc, _halg, _dstavp)
			!= 0) {
	...
	LM_INFO("uri %.*s was rewritten to %.*s, carrier %d, domain %d\n",
			rewrite_user.len, rewrite_user.s, dest.len, dest.s, carrier_id,
			domain_id);
		...
		gparam_t *_domain, gparam_t *_dstavp)
{
	...
	if((avp_val.n = load_user_carrier(&user, &domain)) < 0) {
		...
		if(add_avp(_dstavp->v.pve->spec->pvp.pvn.u.isname.type,
				   _dstavp->v.pve->spec->pvp.pvn.u.isname.name, avp_val)
				< 0) {
 ...
 * @return 1 on success, -1 on failure
 */
int cr_route(struct sip_msg *_msg, gparam_t *_carrier, gparam_t *_domain,
		gparam_t *_prefix_matching, gparam_t *_rewrite_user,
		enum hash_source _hsrc, gparam_t *_descavp)
{
	return cr_do_route(_msg, _carrier, _domain, _prefix_matching, _rewrite_user,
			_hsrc, alg_crc32, _descavp);
}

int cr_route5(struct sip_msg *_msg, gparam_t *_carrier, gparam_t *_domain,
		gparam_t *_prefix_matching, gparam_t *_rewrite_user,
...
int cr_nofallback_route(struct sip_msg *_msg, gparam_t *_carrier,
		gparam_t *_domain, gparam_t *_prefix_matching, gparam_t *_rewrite_user,
		enum hash_source _hsrc, gparam_t *_dstavp)
{
	return cr_do_route(_msg, _carrier, _domain, _prefix_matching, _rewrite_user,
			_hsrc, alg_crc32_nofallback, _dstavp);
}
		...
		gparam_t *_reply_code, gparam_t *_dstavp)
{
	...
	if(set_next_domain_recursor(domain_data->failure_tree, &prefix_matching,
			   &host, &reply_code, flags, _dstavp)
			!= 0) {
