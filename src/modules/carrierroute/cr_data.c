...
{
	if(global_data == NULL) {
		global_data = (struct route_data_t **)shm_malloc(
				sizeof(struct route_data_t *));
		if(global_data == NULL) {
			SHM_MEM_ERROR;
			return -1;
		}
...
void clear_route_data(struct route_data_t *data)
{
	...
	if(data->carrier_map) {
		for(i = 0; i < data->carrier_num; ++i) {
			if(data->carrier_map[i].name.s)
				shm_free(data->carrier_map[i].name.s);
		}
		shm_free(data->carrier_map);
	}
	if(data->domain_map) {
		for(i = 0; i < data->domain_num; ++i) {
			if(data->domain_map[i].name.s)
				shm_free(data->domain_map[i].name.s);
		}
...
int reload_route_data(void)
{
	...
	qsort(new_data->carriers, new_data->carrier_num,
			sizeof(new_data->carriers[0]), compare_carrier_data);

	/* sort domains by id for faster access */
	for(i = 0; i < new_data->carrier_num; i++) {
		qsort(new_data->carriers[i]->domains, new_data->carriers[i]->domain_num,
				sizeof(new_data->carriers[i]->domains[0]), compare_domain_data);
	}
...
struct carrier_data_t *get_carrier_data(struct route_data_t *rd, int carrier_id)
{
	...
	key.id = carrier_id;
	ret = bsearch(&pkey, rd->carriers, rd->carrier_num, sizeof(rd->carriers[0]),
			compare_carrier_data);
	if(ret)
		return *ret;
	return NULL;
}
 ...
 */
static int binary_search(void *base, unsigned int len, int elemsize, void *key,
		cmpfunc_t cmpfunc, int *index)
{
	int left, right, mid;
	...
	if(cmpfunc(base + elemsize * left, key) > 0) {
		...
		if(index)
			*index =
					0; /* not found, must be inserted at the beginning of array */
		return 0;
	}
	if(cmpfunc(base + elemsize * right, key) < 0) {
		LM_DBG("not found (out of right bound)\n");
		if(index)
			*index = len; /* not found, must be inserted at the end of array */
		return 0;
	}

	while(left < right) {
		mid = left + ((right - left) / 2);
		if(cmpfunc(base + elemsize * mid, key) < 0)
			left = mid + 1;
		else
			right = mid;
	}
		...
		struct carrier_data_t *carrier_data, int domain_id)
{
	...
	key.id = domain_id;
	res = binary_search(carrier_data->domains, carrier_data->first_empty_domain,
			sizeof(struct domain_data_t *), &pkey, compare_domain_data, &i);
	if(res < 0) {
		...
		if((domain_name = map_id2name(
					rd->domain_map, rd->domain_num, domain_id))
				== NULL) {
		...
		LM_INFO("added domain %d '%.*s' to carrier %d '%.*s'", domain_id,
				domain_name->len, domain_name->s, carrier_data->id,
		...
		const str *comment)
{
	...
	if((domain_data = get_domain_data_or_add(rd, carrier_data, domain_id))
			== NULL) {
	...
	return add_route_to_tree(domain_data->tree, scan_prefix, flags, mask,
			scan_prefix, max_targets, prob, rewrite_hostpart, strip,
		...
		flag_t flags, flag_t mask, int next_domain_id, const str *comment)
{
	...
	LM_INFO("adding prefix %.*s, reply code %.*s\n", scan_prefix->len,
			scan_prefix->s, reply_code->len, reply_code->s);
	...
	return add_failure_route_to_tree(domain_data->failure_tree, scan_prefix,
			scan_prefix, host, reply_code, flags, mask, next_domain_id,
			comment);
}
...
static int fixup_rule_backup(struct route_flags *rf, struct route_rule *rr)
{
	...
	if(!rr->status && rr->backup) {
		if((rr->backup->rr = find_rule_by_hash(rf, rr->backup->hash_index))
				== NULL) {
	...
	for(rf = (struct route_flags *)(node->data); rf != NULL; rf = rf->next) {
		...
		if(rf->rule_list) {
			...
			if(rf->rule_num != rf->max_targets) {
				LM_ERR("number of rules(%i) differs from max_targets(%i), "
					   "maybe your config is wrong?\n",
			...
			if((rf->rules = shm_malloc(
						sizeof(struct route_rule *) * rf->rule_num))
					== NULL) {
				SHM_MEM_ERROR;
				return -1;
			}
			...
			while(rr && i < rf->rule_num) {
				if(!rr->hash_index) {
					if(rf->rules[i]) {
						...
						rf->rules[i] = rr;
						rr->hash_index = i + 1;
						LM_INFO("hashless rule with host %.*s hash, hash_index "
								"%i\n",
