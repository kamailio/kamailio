...
{
	memset(&(target_options[PO_MAX_TARGETS].value), '\0',
			sizeof(union opt_data));
...
static int backup_config(void)
{
	...
	if((backup_file = pkg_malloc(strlen(config_file) + strlen(".bak") + 1))
			== NULL) {
		PKG_MEM_ERROR;
		return -1;
	}
...
int load_config(struct route_data_t *rd)
{
	...
	char domain_buf[CR_MAX_LINE_SIZE], prefix_buf[CR_MAX_LINE_SIZE],
			rewrite_buf[CR_MAX_LINE_SIZE];
	...
	domain_name.s = domain_buf;
	domain_name.len = CR_MAX_LINE_SIZE;
	prefix_name.s = prefix_buf;
	prefix_name.len = CR_MAX_LINE_SIZE;
	rewrite_host.s = rewrite_buf;
	rewrite_host.len = CR_MAX_LINE_SIZE;
	...
	memcpy(rd->carrier_map[0].name.s, default_tree.s,
			rd->carrier_map[0].name.len);

	/* Create domain map */
	if((rd->domain_map = shm_malloc(
				sizeof(struct name_map_t) * allocated_domain_num))
			== NULL) {
		SHM_MEM_ERROR;
		goto errclose;
	}
	...
	tmp_carrier_data = create_carrier_data(
			1, &rd->carrier_map[0].name, allocated_domain_num);
	...
	while((ret_domain = parse_struct_header(file, "domain", &domain_name))
		...
		if(rd->domain_num > allocated_domain_num) {
			...
			if((p_realloc = shm_realloc(rd->carriers[0]->domains,
						sizeof(struct domain_data_t *) * allocated_domain_num))
					== NULL) {
				SHM_MEM_ERROR;
				goto errclose;
			}
		...
		rd->domain_map[domain_id - 1].name.s =
				shm_malloc(rd->domain_map[domain_id - 1].name.len);
		if(rd->domain_map[domain_id - 1].name.s == NULL) {
			SHM_MEM_ERROR;
			goto errclose;
		}
		memcpy(rd->domain_map[domain_id - 1].name.s, domain_name.s,
				rd->domain_map[domain_id - 1].name.len);

		/* create new domain data */
		if((domain_data = create_domain_data(
					domain_id, &(rd->domain_map[domain_id - 1].name)))
				== NULL) {
		...
		LM_DBG("added domain %d '%.*s' to carrier %d '%.*s'\n", domain_id,
				domain_name.len, domain_name.s, tmp_carrier_data->id,
				tmp_carrier_data->name->len, tmp_carrier_data->name->s);
		...
		while((ret_prefix = parse_struct_header(file, "prefix", &prefix_name))
			...
			if((ret_prefix_opts = parse_options(
						file, prefix_options, PO_MAX_IDS, "target"))
					!= SUCCESSFUL_PARSING) {
			...
			for(k = 0; k < max_targets; k++) {
				...
				ret_target_opts =
						parse_options(file, target_options, TO_MAX_IDS, "}");
				...
				rewrite_prefix.s =
						target_options[TO_ID_REWR_PREFIX].value.string_data.s;
				rewrite_prefix.len =
						target_options[TO_ID_REWR_PREFIX].value.string_data.len;
				rewrite_suffix.s =
						target_options[TO_ID_REWR_SUFFIX].value.string_data.s;
				rewrite_suffix.len =
						target_options[TO_ID_REWR_SUFFIX].value.string_data.len;
				hash_index = target_options[TO_ID_HASH_INDEX].value.int_data;
				comment.s = target_options[TO_ID_COMMENT].value.string_data.s;
				comment.len =
						target_options[TO_ID_COMMENT].value.string_data.len;
				status = target_options[TO_ID_STATUS].value.int_data;

				if((backed_up_size = target_options[TO_ID_BACKED_UP].no_elems)
						> 0) {
					if((backed_up = pkg_malloc(
								sizeof(int) * (backed_up_size + 1)))
							== NULL) {
						PKG_MEM_ERROR;
						goto errclose;
					}
					for(l = 0; l < backed_up_size; l++) {
						backed_up[l] = target_options[TO_ID_BACKED_UP]
											   .value.int_list[l];
					}
				...
				LM_DBG("\n Adding route to tree <'%.*s'>: prefix_name:%s\n,"
					   ...
					   " status=%i\n, hash_index=%i\n, comment=%s \n",
						...
						prefix_name.s, max_targets, prob, rewrite_host.s, strip,
						rewrite_prefix.s, rewrite_suffix.s, status, hash_index,
						comment.s);
...
int save_config(struct route_data_t *rd)
{
	...
	if(rd->carrier_num >= 1) {
		for(j = 0; j < rd->carriers[i]->domain_num; j++) {
			...
			if(save_route_data_recursor(
					   rd->carriers[i]->domains[j]->tree, outfile)
					< 0) {
				goto errout;
			}
			fprintf(outfile, "}\n\n");
