		...
		uint32_t *sip_response_timestamp_fraction)
{
	...
	if(sip_response_timestamp_fraction
			&& *sip_response_timestamp_fraction > 0) {
		...
		str *app_provided_party)
{
	...
	*(x->role_of_node) = node_role;
	...
	if(app_provided_party && app_provided_party->s
			&& (app_provided_party->len > 0)) {
...
service_information_t *new_service_information(
	...
	if(x->mscc->final_unit_action) {
		if(x->mscc->final_unit_action->redirect_server) {
			if(x->mscc->final_unit_action->redirect_server->server_address) {
				p_str = x->mscc->final_unit_action->redirect_server
								->server_address;
