...
int init_ro_session_table(unsigned int size)
{
	...
	ro_session_table = (struct ro_session_table *)shm_malloc(
			sizeof(struct ro_session_table)
			+ size * sizeof(struct ro_session_entry));
	...
	ro_session_table->entries =
			(struct ro_session_entry *)(ro_session_table + 1);
	...
	for(i = 0; i < size; i++) {
		memset(&(ro_session_table->entries[i]), 0,
				sizeof(struct ro_session_entry));
		...
		str *app_provided_party)
{
	...
	char *p;
	unsigned int len = /*session_id->len + */ callid->len
					   + asserted_identity->len + called_asserted_identity->len
					   + mac->len + incoming_trunk_id->len
					   + outgoing_trunk_id->len + pani->len
					   + app_provided_party->len + sizeof(struct ro_session);
	...
	new_ro_session->called_asserted_identity.len =
			called_asserted_identity->len;
		...
		unsigned int h_entry, str *callid, int direction, unsigned int *del)
{
	...
	for(ro_session = ro_session_entry->first; ro_session;
			ro_session = ro_session->next) {
		if((direction == 0 || direction == ro_session->direction)
				&& (strncmp(ro_session->callid.s, callid->s, callid->len)
						== 0)) {
