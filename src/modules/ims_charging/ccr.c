...
int Ro_write_event_type_avps(AAA_AVP_LIST *avp_list, event_type_t *x)
{
	...
	if(x->sip_method) {
		if(!cdp_avp->epcapp.add_SIP_Method(
				   &aList, *(x->sip_method), AVP_DUPLICATE_DATA))
			goto error;
	}
	...
	if(!cdp_avp->epcapp.add_Event_Type(
			   avp_list, &aList, AVP_FREE_DATA)) //TODO: used to be DON'T FREE
		goto error;
...
int Ro_write_time_stamps_avps(AAA_AVP_LIST *avp_list, time_stamps_t *x)
{
	...
	if(x->sip_request_timestamp)
		if(!cdp_avp->epcapp.add_SIP_Request_Timestamp(
				   &aList, *(x->sip_request_timestamp)))
			goto error;

	if(x->sip_request_timestamp_fraction)
		if(!cdp_avp->epcapp.add_SIP_Request_Timestamp_Fraction(
				   &aList, *(x->sip_request_timestamp_fraction)))
			goto error;

	if(x->sip_response_timestamp)
		if(!cdp_avp->epcapp.add_SIP_Response_Timestamp(
				   &aList, *(x->sip_response_timestamp)))
			goto error;

	if(x->sip_response_timestamp_fraction)
		if(!cdp_avp->epcapp.add_SIP_Response_Timestamp_Fraction(
				   &aList, *(x->sip_response_timestamp_fraction)))
			goto error;

	if(!cdp_avp->epcapp.add_Time_Stamps(
			   avp_list, &aList, AVP_FREE_DATA)) //used to be DON'T FREE
		goto error;
...
int Ro_write_ims_information_avps(AAA_AVP_LIST *avp_list, ims_information_t *x)
{
	...
	if(x->event_type)
		if(!Ro_write_event_type_avps(&aList2, x->event_type))
			goto error;
	if(x->role_of_node)
		if(!cdp_avp->epcapp.add_Role_Of_Node(&aList2, *(x->role_of_node)))
			goto error;
	...
	if(x->user_session_id)
		if(!cdp_avp->epcapp.add_User_Session_Id(
				   &aList2, *(x->user_session_id), 0))
			goto error;
	...
	if(x->called_party_address) {
		if(!cdp_avp->epcapp.add_Called_Party_Address(
				   &aList2, *(x->called_party_address), 0))
			goto error;
	...
	if(x->incoming_trunk_id && (x->incoming_trunk_id->len > 0)
		...
		if(!cdp_avp->epcapp.add_Outgoing_Trunk_Group_Id(
				   &aList, *(x->outgoing_trunk_id), 0))
			goto error;

		if(!cdp_avp->epcapp.add_Incoming_Trunk_Group_Id(
				   &aList, *(x->incoming_trunk_id), 0))
			goto error;
	...
	if(x->access_network_info) {
		cdp_avp->imsapp.add_Access_Network_Information(
				&aList2, *(x->access_network_info), 0);
	}
	...
	if(x->requested_party_address)
		if(!cdp_avp->epcapp.add_Requested_Party_Address(
				   &aList2, *(x->requested_party_address), 0))
			goto error;
	if(x->time_stamps)
	...
	for(ioi_elem = x->ioi.head; ioi_elem; ioi_elem = ioi_elem->next) {
		...
		if(ioi_elem->info.originating_ioi)
			if(!cdp_avp->epcapp.add_Originating_IOI(
					   &aList, *(ioi_elem->info.originating_ioi), 0))
				goto error;

		if(ioi_elem->info.terminating_ioi)
			if(!cdp_avp->epcapp.add_Terminating_IOI(
					   &aList, *(ioi_elem->info.terminating_ioi), 0))
				goto error;
	...
	for(info = x->service_specific_info.head; info; info = info->next) {
		...
		if(info->info.data)
			if(!cdp_avp->epcapp.add_Service_Specific_Data(
					   &aList, *(info->info.data), 0))
				goto error;
		if(info->info.type)
			if(!cdp_avp->epcapp.add_Service_Specific_Type(
					   &aList, *(info->info.type)))
				goto error;
	...
	if(x->app_provided_party) {
		if(!cdp_avp->epcapp.add_Application_Provided_Called_Party_Address(
				   &aList, *(x->app_provided_party), 0))
			goto error;
		if(!cdp_avp->epcapp.add_Application_Server_Information(
				   &aList2, &aList, 0))
			goto error;
		cdp_avp->cdp->AAAFreeAVPList(&aList);
		aList.head = aList.tail = 0;
	...
	if(!cdp_avp->epcapp.add_IMS_Information(avp_list, &aList2,
			   AVP_FREE_DATA)) //TODO check why not DON'T FREE DATA
		goto error;
...
int Ro_write_service_information_avps(
		AAA_AVP_LIST *avp_list, service_information_t *x)
{
	...
	for(elem = x->subscription_id.head; elem; elem = elem->next) {
		...
		if(!cdp_avp->ccapp.add_Subscription_Id_Group(
				   &aList, elem->s.type, elem->s.id, 0))
			goto error;
	}
	...
	if(!cdp_avp->epcapp.add_Service_Information(
			   avp_list, &aList, AVP_FREE_DATA)) //TODO: use to be don't free
		goto error;
...
AAAMessage *Ro_write_CCR_avps(AAAMessage *ccr, Ro_CCR_t *x)
{
	...
	if(x->acct_interim_interval)
		if(!cdp_avp->base.add_Acct_Interim_Interval(
				   &(ccr->avpList), *(x->acct_interim_interval)))
			goto error;

	if(x->origin_state_id)
		if(!cdp_avp->base.add_Origin_State_Id(
				   &(ccr->avpList), *(x->origin_state_id)))
			goto error;

	if(x->event_timestamp)
		if(!cdp_avp->base.add_Event_Timestamp(
				   &(ccr->avpList), *(x->event_timestamp)))
			goto error;

	if(x->service_context_id)
		if(!cdp_avp->ccapp.add_Service_Context_Id(
				   &(ccr->avpList), *(x->service_context_id), 0))
			goto error;

	if(x->service_information)
		if(!Ro_write_service_information_avps(
				   &(ccr->avpList), x->service_information))
			goto error;
	return ccr;
error:
...
AAAMessage *Ro_new_ccr(AAASession *session, Ro_CCR_t *ro_ccr_data)
{
	...
	ccr = cdp_avp->cdp->AAACreateRequest(
			IMS_Ro, Diameter_CCR, Flag_Proxyable, session);
	if(!ccr) {
							...
							while(z) {
								switch(z->code) {
									case AVP_CC_Time:
										...
										break;
									default:
										LM_ERR("Unsupported Granted Service "
											   "Unit with code:[%d]\n",
												z->code);
								}
								z = z->next;
							...
							while(z) {
								switch(z->code) {
									...
									case AVP_Redirect_Server:
										...
										redirect_server_t
												*redirect_server_info = 0;
										mem_new(redirect_server_info,
												sizeof(redirect_server_t), pkg);
										mscc->final_unit_action
												->redirect_server =
												redirect_server_info;

										AAA_AVP_LIST yy;
										AAA_AVP *zz;
										yy = cdp_avp->cdp->AAAUngroupAVPS(
												z->data);
										zz = yy.head;
										while(zz) {
											switch(zz->code) {
