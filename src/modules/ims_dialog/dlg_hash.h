...
#define DLG_FLAG_INSERTED \
	(1 << 10) /*!< DLG already written to DB - could have been put in by early media or confirmed */
...
typedef struct dlg_cell
{
	...
	struct cell *
			transaction; /*!< ptr to associated transaction for this dialog TM module cell ptr */
 ...
 */
static inline void unlink_unsafe_dlg(
		struct dlg_entry *d_entry, struct dlg_cell *dlg)
{
	if(dlg->next)
 ...
 */
static inline int match_dialog(struct dlg_cell *dlg, str *callid, str *ftag,
		str *ttag, unsigned int *dir)
{
	...
	if(d_entry_out->first == 0) {
		...
		if(*dir == DLG_DIR_DOWNSTREAM) {
			if(dlg->callid.len == callid->len && dlg->from_tag.len == ftag->len
					&& strncmp(dlg->callid.s, callid->s, callid->len) == 0
					&& strncmp(dlg->from_tag.s, ftag->s, ftag->len) == 0) {
				return 1;
			}
		} else if(*dir == DLG_DIR_UPSTREAM) {
			if(dlg->callid.len == callid->len && dlg->from_tag.len == ttag->len
					&& strncmp(dlg->callid.s, callid->s, callid->len) == 0
		...
		if(*dir == DLG_DIR_DOWNSTREAM) {
			if(dlg->callid.len == callid->len && dlg->from_tag.len == ftag->len
				...
				while(dlg_out) {
					if(dlg_out->to_tag.len == ttag->len
							&& memcmp(dlg_out->to_tag.s, ttag->s,
									   dlg_out->to_tag.len)
									   == 0) {
						return 1;
					}
			...
			if(dlg->callid.len == callid->len && dlg->from_tag.len == ttag->len
				...
				while(dlg_out) {
					if(dlg_out->to_tag.len == ftag->len
							&& memcmp(dlg_out->to_tag.s, ftag->s,
									   dlg_out->to_tag.len)
									   == 0) {
						return 1;
					}
			...
			LM_DBG("p: %p ft[%.*s] tt [%.*s]", d_entry_out->first,
					dlg->from_tag.len, dlg->from_tag.s, ttag->len, ttag->s);
			if(dlg->from_tag.len == ttag->len
					&& strncmp(dlg->from_tag.s, ttag->s, ttag->len) == 0
					&& strncmp(dlg->callid.s, callid->s, callid->len) == 0) {
				...
				dlg_out = d_entry_out->first;
				while(dlg_out) {
					LM_DBG("dout: tt[%.*s]", dlg_out->to_tag.len,
							dlg_out->to_tag.s);
					if(dlg_out->to_tag.len == ftag->len
							&& memcmp(dlg_out->to_tag.s, ftag->s,
									   dlg_out->to_tag.len)
 ...
 */
static inline int match_downstream_dialog(
		struct dlg_cell *dlg, str *callid, str *ftag)
{
