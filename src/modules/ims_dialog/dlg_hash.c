	...
	do {                                                                     \
		...
		if((_dlg)->ref < 0) {                                                \
			LM_CRIT("bogus ref %d with cnt %d for dlg %p [%u:%u] "           \
					...
					(_dlg)->from_tag.len, (_dlg)->from_tag.s);               \
		}                                                                    \
		if((_dlg)->ref <= 0) {                                               \
...
struct dlg_cell *build_new_dlg(
		str *callid, str *from_uri, str *from_tag, str *req_uri)
{
	struct dlg_cell *dlg;
	int len;
	char *p;
		...
		if(d_entry_out) {
			...
			while(dlg_out) {
				LM_DBG("Searching out dialog with to_tag '%.*s' (looking for "
					   "%.*s\n",
						dlg_out->to_tag.len, dlg_out->to_tag.s, tag->len,
						tag->s);

				if(dlg_out->to_tag.len == tag->len
						&& memcmp(dlg_out->to_tag.s, tag->s,
								   dlg_out->to_tag.len)
								   == 0) {
					LM_DBG("Found dialog_out entry with correct to_tag - "
						   "updating leg info\n");

					if(contact->len > 0) {
						dlg_out->callee_contact.s =
								(char *)shm_malloc(contact->len);
						if(!dlg_out->callee_contact.s) {
						...
						memcpy(dlg_out->callee_contact.s, contact->s,
								contact->len);
					}
					if(rr->len > 0) {
						dlg_out->callee_route_set.s =
								(char *)shm_malloc(rr->len);
						if(!dlg_out->callee_route_set.s) {
							LM_ERR("no more shm mem\n");
							lock_release(dlg->dlg_out_entries_lock);
							return -1; //if we're out of mem we don't really care about cleaning up - prob going to crash anyway
 ...
 */
struct dlg_cell_out *build_new_dlg_out(
		struct dlg_cell *dlg, str *to_uri, str *to_tag, str *branch)
{
		...
		if(pdlg_out->deleted) {
			...
			if(pdlg_out->prev) {
				...
				if(pdlg_out->next) {
					...
					LM_ERR("dlg out entry has prev set to null and next set to "
						   "null too\n");
					only = 1;
				}
			}
...
void dlg_remove_dlg_out(struct dlg_cell_out *dlg_out_do_not_remove,
		struct dlg_cell *dlg, int only_mark)
{
	...
	dlg_out = d_entry_out->first;
	LM_DBG("Scanning dlg_entry_out list for dlg_out entry with did: [%s]",
			dlg->did.s);
	...
	while(dlg_out) {
		...
		if(dlg_out->to_tag.len == dlg_out_do_not_remove->to_tag.len
				&& memcmp(dlg_out->to_tag.s, dlg_out_do_not_remove->to_tag.s,
						   dlg_out->to_tag.len)
						   == 0) {
			...
			if(d_entry_out->first == d_entry_out->last) {
				...
				if(!only_mark) {
					...
					LM_DBG("Marking dlg_out structure for deletion - it should "
						   "be deleted by tm callback instead to_tag: %.*s\n",
 ...
 */
int dlg_update_cseq(
		struct dlg_cell *dlg, unsigned int leg, str *cseq, str *to_tag)
{
 ...
 * \return 0 on success, -1 on failure
 */
int dlg_update_contact(
		struct dlg_cell *dlg, unsigned int leg, str *contact, str *to_tag)
{
 ...
 * \see next_state_dlg
 */
static inline void log_next_state_dlg(
		const int event, const struct dlg_cell *dlg)
{

	LM_CRIT("bogus event %d in state %d for dlg %p [%u:%u] with clid '%.*s' "
			"and tags "
			"'%.*s'\n",
			event, dlg->state, dlg, dlg->h_entry, dlg->h_id, dlg->callid.len,
 ...
 * \param to_tag dialog to tag
 */
void next_state_dlg(struct dlg_cell *dlg, int event, int *old_state,
		int *new_state, int *unref, str *to_tag)
{
	struct dlg_entry *d_entry;
			...
			switch(dlg->state) {
				...
				case DLG_STATE_CONFIRMED:
					if(to_tag) {
						...
						if(dlg_out->to_tag.len == to_tag->len
								&& memcmp(dlg_out->to_tag.s, to_tag->s,
										   dlg_out->to_tag.len)
										   == 0) {
							...
							LM_ERR("It looks like this is a concurrently "
								   "confirmed call!!\n");
							LM_ERR("Error checking now so not putting into "
								   "DLG_STATE_CONCURRENTLY_CONFIRMED\n");

							LM_ERR("This is event DLG_EVENT_RPL2XX and the "
								   "current dlg state is "
								   "DLG_STATE_CONFIRMED\n");
							LM_ERR("There should only be one dlg out here as "
								   "the state is CONFIRMED but we are checking "
								   "anyway!\n");
							LM_ERR("To tag passed in the 2XX: [%.*s]",
									to_tag->len, to_tag->s);
							LM_ERR("Now printing dlgouts totags - there should "
								   "be only one!\n");
							while(dlg_out) {
 ...
 * \return 0 on success, -1 on failure
 */
static inline int internal_mi_print_dlg_out(
		struct mi_node *rpl, struct dlg_cell_out *dlg_out)
{
	...
	attr = addf_mi_attr(
			node, 0, "hash", 4, "%u:%u", dlg_out->h_entry, dlg_out->h_id);
	if(attr == 0)
		goto error;
	...
	node1 = add_mi_node_child(
			node, MI_DUP_VALUE, "did", 3, dlg_out->did.s, dlg_out->did.len);
	if(node1 == 0)
		goto error;


	node1 = add_mi_node_child(node, MI_DUP_VALUE, "callee_contact", 14,
			dlg_out->callee_contact.s, dlg_out->callee_contact.len);
	if(node1 == 0)
		goto error;

	node1 = add_mi_node_child(node, MI_DUP_VALUE, "caller_cseq", 11,
			dlg_out->caller_cseq.s, dlg_out->caller_cseq.len);
	if(node1 == 0)
		goto error;

	node1 = add_mi_node_child(node, MI_DUP_VALUE, "callee_cseq", 11,
			dlg_out->callee_cseq.s, dlg_out->callee_cseq.len);
	if(node1 == 0)
		goto error;


	node1 = add_mi_node_child(node, MI_DUP_VALUE, "callee_route_set", 16,
			dlg_out->callee_route_set.s, dlg_out->callee_route_set.len);
	if(node1 == 0)
		goto error;

	if(dlg_out->callee_bind_addr) {
		node1 = add_mi_node_child(node, 0, "callee_bind_addr", 16,
				dlg_out->callee_bind_addr->sock_str.s,
				dlg_out->callee_bind_addr->sock_str.len);
	} else {
		node1 = add_mi_node_child(node, 0, "callee_bind_addr", 16, 0, 0);
	}
...
static inline int internal_mi_print_dlg(
		struct mi_node *rpl, struct dlg_cell *dlg, int with_context)
{
	struct mi_node *node = NULL;
	struct mi_node *node1 = NULL;
	struct mi_attr *attr = NULL;
	...
	attr = addf_mi_attr(node, 0, "hash", 4, "%u:%u", dlg->h_entry, dlg->h_id);
	if(attr == 0)
		goto error;
	...
	node1 = add_mi_node_child(
			node, MI_DUP_VALUE, "callid", 6, dlg->callid.s, dlg->callid.len);
	if(node1 == 0)
		goto error;
	...
	if(dlg->caller_bind_addr) {
		...
		node1 = add_mi_node_child(node, 0, "caller_bind_addr", 16, 0, 0);
	}
	...
	if(with_context) {
		...
		if(node1 == 0)
			goto error;
		run_dlg_callbacks(
				DLGCB_MI_CONTEXT, dlg, NULL, NULL, DLG_DIR_NONE, (void *)node1);
	}
...
static inline struct mi_root *process_mi_params(
		struct mi_root *cmd_tree, struct dlg_cell **dlg_p)
{
	struct mi_node *node;
	struct dlg_entry *d_entry;
	struct dlg_cell *dlg;
