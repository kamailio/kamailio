 ...
 */
static void unref_dlg_from_cb(
		struct cell *t, int type, struct tmcb_params *param)
{
	dlg_cell_t *dlg = NULL;
		...
		unsigned int leg, str *tag)
{
	...
	if(!msg->contact
		...
		if(msg->first_line.type == SIP_REQUEST)
			LM_ERR("bad sip message or missing Contact hdr for message "
				   "[%.*s]\n",
			...
			LM_ERR("bad sip message or missing Contact hdr for message "
				   "[%.*s]\n",
					...
					msg->first_line.u.reply.status.s);
		goto error0;
	}
	...
	LM_DBG("route_set %.*s, contact %.*s, cseq %.*s and bind_addr %.*s [%p]\n",
			rr_set.len, rr_set.s, contact.len, contact.s, cseq.len, cseq.s,
			msg->rcv.bind_address->sock_str.len,
			msg->rcv.bind_address->sock_str.s, msg->rcv.bind_address);

	if(dlg_set_leg_info(
			   dlg, tag, &rr_set, &contact, &cseq, callee_bind_address, leg)
			!= 0) {
...
static void dlg_cancel_confirmed(
		struct cell *t, int type, struct tmcb_params *params)
{
	...
	/* dialog termination confirmed (BYE reply) */
	run_dlg_callbacks(DLGCB_TERMINATED_CONFIRMED, dlg, params->req, params->rpl,
			DLG_DIR_UPSTREAM, 0);
}
 ...
 */
static void dlg_terminated(
		struct sip_msg *req, struct dlg_cell *dlg, unsigned int dir)
{
	if(!req) {
		...
		struct sip_msg *req, struct dlg_cell *dlg, unsigned int dir)
{
	...
	if(d_tmb.register_tmcb(
			   req, 0, TMCB_RESPONSE_OUT, dlg_cancel_confirmed, (void *)dlg, 0)
			<= 0) {
		...
		struct tmcb_params *param, const int direction)
{
	...
	if(type == TMCB_RESPONSE_FWDED) {
		run_dlg_callbacks(DLGCB_RESPONSE_WITHIN, dlg, param->req, param->rpl,
				direction, 0);
	}
	dlg_release(dlg);
 ...
 * \param param saved dialog structure inside the callback
 */
static void dlg_seq_up_onreply(
		struct cell *t, int type, struct tmcb_params *param)
{
 ...
 * \param param saved dialog structure inside the callback
 */
static void dlg_seq_down_onreply(
		struct cell *t, int type, struct tmcb_params *param)
{
 ...
 * \return 0 on success, -1 on failure
 */
static inline int pre_match_parse(
		struct sip_msg *req, str *callid, str *ftag, str *ttag, int with_ttag)
{
...
static inline int parse_dlg_rr_param(
		char *p, char *end, int *h_entry, int *h_id)
{
	char *s;
		...
		unsigned int dir, str *to_tag)
{
	...
	if(dir == DLG_DIR_UPSTREAM) {
		return dlg_update_cseq(
				dlg, DLG_CALLEE_LEG, &((get_cseq(req))->number), to_tag);
	} else if(dir == DLG_DIR_DOWNSTREAM) {
		return dlg_update_cseq(
				dlg, DLG_CALLER_LEG, &((get_cseq(req))->number), to_tag);
	} else {
...
void dlg_onreq(struct cell *t, int type, struct tmcb_params *param)
{
	...
	if(req->first_line.u.request.method_value
			!= METHOD_INVITE /* && req->first_line.u.request.method_value != METHOD_CANCEL*/) {
		return;
	}
	...
	if(dlg != NULL) {
		if(dlg_set_tm_callbacks(t, req, dlg, spiral_detected) != 0) {
			LM_ERR("Failed to register TM Callbacks for dialog... this is "
				   "bad!\n");
		...
		if(d_rrb.get_route_param(req, &rr_param, &val) != 0) {
			...
			if(dlg == 0) {
				LM_WARN("unable to find dialog for %.*s "
						...
						req->first_line.u.request.method.len,
						req->first_line.u.request.method.s, val.len, val.s,
						h_entry, h_id);
				if(seq_match_mode == SEQ_MATCH_STRICT_ID)
					return;
			} else {
				...
				if(match_dialog(dlg, &callid, &ftag, &ttag, &dir) == 0) {
					LM_WARN("tight matching failed for %.*s with "
							...
							req->first_line.u.request.method.len,
							req->first_line.u.request.method.s, callid.len,
							callid.s, callid.len, ftag.len, ftag.s, ftag.len,
	...
	if(req->first_line.u.request.method_value != METHOD_ACK) {
		...
		if(iuid != NULL) {
			...
			if(d_tmb.register_tmcb(req, 0, TMCB_RESPONSE_IN | TMCB_ON_FAILURE,
					   dlg_onreply, (void *)iuid, dlg_iuid_sfree)
					< 0) {
	...
	if(event == DLG_EVENT_REQCANCEL && new_state == DLG_STATE_DELETED
		...
		if(ret < 0) {
			LM_CRIT("unable to unlink the timer on dlg %p [%u:%u] "
					...
					dlg->callid.s, dlg->from_tag.len, dlg->from_tag.s);
		...
		} else if(old_state == DLG_STATE_CONFIRMED
...
void dlg_onreply(struct cell *t, int type, struct tmcb_params *param)
{
	...
	if(type & TMCB_REQUEST_OUT) {
		LM_DBG("Received Request out for request [%.*s]\n", param->send_buf.len,
				param->send_buf.s);
		if(param->send_buf.s && (param->send_buf.len > 6)
				&& !memcmp(param->send_buf.s, "CANCEL", 6)) {
	...
	if(type == TMCB_RESPONSE_FWDED) {
		...
		run_dlg_callbacks(
				DLGCB_RESPONSE_FWDED, dlg, req, rpl, DLG_DIR_UPSTREAM, 0);
		goto done;
	}
	...
	if(type == TMCB_RESPONSE_READY) {
		if(rpl == FAKED_REPLY) {
			...
			if(!rpl->to
					&& ((parse_headers(rpl, HDR_TO_F, 0) < 0) || !rpl->to)) {
	...
	if(type == TMCB_RESPONSE_READY) {
		LM_DBG("Checking if there is an existing dialog_out entry with same "
			   "to-tag");
		if(rpl == FAKED_REPLY) {
			LM_DBG("Faked reply\n");
			...
			if(new_state == DLG_STATE_DELETED
				...
				run_dlg_callbacks(
						DLGCB_FAILED, dlg, req, rpl, DLG_DIR_UPSTREAM, 0);
				/* do unref */
				if(unref)
					unref_dlg(dlg, unref);
		...
		if(rpl == FAKED_REPLY
			...
			to_tag = get_to(rpl)->tag_value;
			if(to_tag.s == 0 || to_tag.len == 0) {
				LM_DBG("missing TAG param in TO hdr :-/ - probably a failure "
					   "response\n");
				to_tag.s = 0;
				to_tag.len = 0;
				//Here we assume that the transaction module timer will remove any early dialogs
		...
		while(dlg_out) {
			...
			if(dlg_out->to_tag.len == to_tag.len
					&& memcmp(dlg_out->to_tag.s, to_tag.s, dlg_out->to_tag.len)
							   == 0) {
		...
		if(!dlg_out) {
			if(rpl->first_line.u.reply.statuscode
					< 299) { /*we don't care about failure responses to dialog - not necessary to create dialog out...*/
				if(rpl->via1->branch && (rpl->via1->branch->value.s)
						&& (rpl->via1->branch->value.len > 0)) {
					branch = rpl->via1->branch->value;
				}

				LM_DBG("No dlg_out entry found - creating a new dialog_out "
					   "entry on dialog [%p]\n",
						dlg);
				...
				if(populate_leg_info(dlg, rpl, t, DLG_CALLEE_LEG, &to_tag)
						!= 0) {
			...
			LM_DBG("dlg_out entry found - updating cseq's for dialog out [%p] "
				   "for to-tag [%.*s] \n",
					dlg_out, dlg_out->to_tag.len, dlg_out->to_tag.s);

			if((!rpl->cseq && parse_headers(rpl, HDR_CSEQ_F, 0) < 0)
					|| !rpl->cseq || !rpl->cseq->parsed) {
				LM_ERR("bad sip message or missing CSeq hdr :-/\n");
			}
			dlg_update_cseq(dlg, DLG_CALLEE_LEG, &((get_cseq(rpl))->number),
					&(dlg_out->to_tag));


			/* extract the contact address to update if present*/
			if(!rpl->contact
					&& (parse_headers(rpl, HDR_CONTACT_F, 0) < 0
							|| !rpl->contact)) {
				LM_DBG("Can not update callee contact: bad sip message or "
					   "missing Contact hdr\n");
			} else if(parse_contact(rpl->contact) < 0
					  || ((contact_body_t *)rpl->contact->parsed)->contacts
								 == NULL
					  || ((contact_body_t *)rpl->contact->parsed)
										 ->contacts->next
								 != NULL) {
				...
				str contact;
				contact =
						((contact_body_t *)rpl->contact->parsed)->contacts->uri;
				dlg_update_contact(
						dlg, DLG_CALLEE_LEG, &contact, &(dlg_out->to_tag));
			}
		}
	}
	if(new_state == DLG_STATE_EARLY) {
	...
	if(new_state == DLG_STATE_CONFIRMED_NA
		...
		run_dlg_callbacks(
				DLGCB_CONFIRMED_NA, dlg, req, rpl, DLG_DIR_UPSTREAM, 0);
	...
	if(new_state == DLG_STATE_CONCURRENTLY_CONFIRMED
			&& (old_state == DLG_STATE_CONFIRMED
					|| old_state == DLG_STATE_CONCURRENTLY_CONFIRMED)) {
		...
		new_dlg = build_new_dlg(&(dlg->callid) /*callid*/,
				&(dlg->from_uri) /*from uri*/, &(dlg->from_tag) /*from_tag*/,
				&(dlg->req_uri) /*r-uri*/);
		...
		struct sip_msg *req, struct cell *t, const int run_initial_cbs)
{
	...
	LM_DBG("SIP Method: %.*s  \n", req->first_line.u.request.method.len,
			req->first_line.u.request.method.s);
	dlg = build_new_dlg(&callid /*callid*/, &(get_from(req)->uri) /*from uri*/,
			&ftag /*from_tag*/, &req_uri /*r-uri*/);
	...
	if(smode == 0) {
		...
		if(d_tmb.register_tmcb(req, t,
				   TMCB_RESPONSE_IN | TMCB_RESPONSE_READY | TMCB_RESPONSE_FWDED
						   | TMCB_ON_FAILURE | TMCB_E2ECANCEL_IN
						   | TMCB_REQUEST_OUT,
				   dlg_onreply, (void *)iuid, dlg_iuid_sfree)
				< 0) {
		...
		LM_DBG("Not registering for TM callbacks because this is a spiraled "
			   "request and we should have already registered callbacks on "
			   "this transaction....\n");
	}
...
void dlg_ontimeout(struct dlg_tl *tl)
{
	...
	if(dlg->state == DLG_STATE_CONFIRMED_NA
		...
		if(dlg->state == DLG_STATE_CONFIRMED) {
			if(dlg_bye_all(dlg, &reason_hdr_s)
					< 0) /* function only supports DLG_STATE_CONFIRMED*/
...
void internal_print_all_dlg(struct dlg_cell *dlg)
{
	...
	LM_DBG("Dialog bind_address: %.*s\n",
			dlg->caller_bind_addr ? dlg->caller_bind_addr->sock_str.len : 0,
			dlg->caller_bind_addr ? dlg->caller_bind_addr->sock_str.s : 0);
