...
int pg_fld(db_fld_t *fld, char *table)
{
	...
	if(db_drv_init(&res->gen, pg_fld_free) < 0)
		goto error;
...
static inline void db_int2pg_int4(struct pg_params *dst, int i, db_fld_t *src)
{
...
static inline void db_int2pg_int2(struct pg_params *dst, int i, db_fld_t *src)
{
	struct pg_fld *pfld = DB_GET_PAYLOAD(src);
...
static inline void db_int2pg_timestamp(
		struct pg_params *dst, int i, db_fld_t *src, unsigned int flags)
{
	struct pg_fld *pfld = DB_GET_PAYLOAD(src);
...
static inline void db_int2pg_int8(struct pg_params *dst, int i, db_fld_t *src)
{
	struct pg_fld *pfld = DB_GET_PAYLOAD(src);
	pfld->v.int4[0] = 0;
...
static inline void db_float2pg_float4(
		struct pg_params *dst, int i, db_fld_t *src)
{
	struct pg_fld *pfld = DB_GET_PAYLOAD(src);
	pfld->v.int4[0] = htonl(src->v.int4);
...
static inline void db_float2pg_float8(
		struct pg_params *dst, int i, db_fld_t *src)
{
	struct pg_fld *pfld = DB_GET_PAYLOAD(src);
...
static inline void db_double2pg_float8(
		struct pg_params *dst, int i, db_fld_t *src)
{
	struct pg_fld *pfld = DB_GET_PAYLOAD(src);
...
static inline void db_double2pg_float4(
		struct pg_params *dst, int i, db_fld_t *src)
{
	struct pg_fld *pfld = DB_GET_PAYLOAD(src);
...
static inline void db_str2pg_string(struct pg_params *dst, int i, db_fld_t *src)
{
	dst->fmt[i] = 1;
...
static inline void db_cstr2pg_string(
		struct pg_params *dst, int i, db_fld_t *src)
{
	dst->fmt[i] = 0;
	...
	for(i = 0; !DB_FLD_EMPTY(fld) && !DB_FLD_LAST(fld[i]); i++) {
		...
		switch(fld[i].type) {
			case DB_INT:
				...
				if(pfld->oid == types[PG_INT4].oid)
					continue;
				if(pfld->oid == types[PG_INT8].oid)
					continue;
				if(pfld->oid == types[PG_BOOL].oid)
					continue;
				if(pfld->oid == types[PG_INET].oid)
					continue;
				if(pfld->oid == types[PG_TIMESTAMP].oid)
					continue;
				if(pfld->oid == types[PG_TIMESTAMPTZ].oid)
					continue;
				if(pfld->oid == types[PG_BIT].oid)
					continue;
				if(pfld->oid == types[PG_VARBIT].oid)
					continue;
				break;
			...
			case DB_DOUBLE:
				if(pfld->oid == types[PG_FLOAT4].oid)
					continue;
				if(pfld->oid == types[PG_FLOAT8].oid)
					continue;
				break;
			...
			case DB_STR:
				if(pfld->oid == types[PG_BYTE].oid)
					continue;
				if(pfld->oid == types[PG_CHAR].oid)
					continue;
				if(pfld->oid == types[PG_TEXT].oid)
					continue;
				if(pfld->oid == types[PG_BPCHAR].oid)
					continue;
				if(pfld->oid == types[PG_VARCHAR].oid)
					continue;
				break;
			...
			default:
				BUG("postgres: Unsupported field type %d, bug in postgres "
					"module\n",
						fld[i].type);
				return -1;
		}
...
int pg_resolve_param_oids(
		db_fld_t *vals, db_fld_t *match, int n1, int n2, PGresult *types)
{
	struct pg_fld *pfld;
	int i;
...
static inline int pg_timestamp2db_int(
		db_fld_t *fld, char *val, int len, unsigned int flags)
{
	if(flags & PG_INT8_TIMESTAMP) {
		/* int8 format */
		fld->v.int4 = (int64_t)_ntohll(((int64_t *)val)[0]) / (int64_t)1000000
					  + PG_EPOCH_TIME;
	} else {
		/* double format */
		...
		unsigned int flags)
{
	...
	for(i = 0; !DB_FLD_EMPTY(dst) && !DB_FLD_LAST(dst[i]); i++) {
		...
		switch(dst[i].type) {
			...
			case DB_CSTR:
				if((type == types[PG_CHAR].oid) || (type == types[PG_TEXT].oid)
			...
			case DB_STR:
			case DB_BLOB:
				if((type == types[PG_BYTE].oid) || (type == types[PG_CHAR].oid)
