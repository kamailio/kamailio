		...
		case PRES_DMQ_UPDATE_PRESENTITY:
			if(presentity == NULL
					|| update_presentity(NULL, presentity, &p_body, t_new,
							   &sent_reply, sphere, &cur_etag, &ruid, 0, pres_skip_notify_dmq)
							   < 0) {
				goto error;
			}
			break;
...
int pres_dmq_replicate_subscription(subs_t *subscription, dmq_node_t *node)
{
	...
	LM_DBG("replicating subscription record - pres_uri %.*s, watcher_user %.*s, watcher_domain "
		   "%.*s\n",
	...
	// action
	srjson_AddNumberToObject(&jdoc, jdoc.root, "action", PRES_DMQ_UPDATE_SUBSCRIPTION);
	// subscription
	s_json = srjson_CreateObject(&jdoc);
	srjson_AddStrToObject(&jdoc, s_json, "pres_uri", subscription->pres_uri.s, subscription->pres_uri.len);
	srjson_AddStrToObject(&jdoc, s_json, "contact", subscription->contact.s, subscription->contact.len);
	srjson_AddStrToObject(&jdoc, s_json, "local_contact", subscription->local_contact.s, subscription->local_contact.len);
	srjson_AddStrToObject(&jdoc, s_json, "watcher_domain", subscription->watcher_domain.s, subscription->watcher_domain.len);
	srjson_AddStrToObject(&jdoc, s_json, "watcher_user", subscription->watcher_user.s, subscription->watcher_user.len);
	srjson_AddStrToObject(&jdoc, s_json, "from_domain", subscription->from_domain.s, subscription->from_domain.len);
	srjson_AddStrToObject(&jdoc, s_json, "from_user", subscription->from_user.s, subscription->from_user.len);
	srjson_AddStrToObject(&jdoc, s_json, "to_domain", subscription->to_domain.s, subscription->to_domain.len);
	srjson_AddStrToObject(&jdoc, s_json, "to_user", subscription->to_user.s, subscription->to_user.len);
	srjson_AddStrToObject(&jdoc, s_json, "from_tag", subscription->from_tag.s, subscription->from_tag.len);
	srjson_AddStrToObject(&jdoc, s_json, "to_tag", subscription->to_tag.s, subscription->to_tag.len);
	srjson_AddStrToObject(&jdoc, s_json, "user_agent", subscription->user_agent.s, subscription->user_agent.len);
	srjson_AddStrToObject(&jdoc, s_json, "callid", subscription->callid.s, subscription->callid.len);
	srjson_AddStrToObject(&jdoc, s_json, "reason", subscription->reason.s, subscription->reason.len);
	srjson_AddStrToObject(&jdoc, s_json, "socket_info", subscription->sockinfo_str.s, subscription->sockinfo_str.len);
	srjson_AddStrToObject(&jdoc, s_json, "record_route", subscription->record_route.s, subscription->record_route.len);
	srjson_AddStrToObject(&jdoc, s_json, "event_id", subscription->event_id.s, subscription->event_id.len);
	if (subscription->event)
		srjson_AddStrToObject(&jdoc, s_json, "event", subscription->event->name.s, subscription->event->name.len);
	srjson_AddNumberToObject(&jdoc, s_json, "local_cseq", subscription->local_cseq);
	srjson_AddNumberToObject(&jdoc, s_json, "remote_cseq", subscription->remote_cseq);
	if (subscription->expires != 0 && subscription->expires < (int)time(NULL)) {
		srjson_AddNumberToObject(&jdoc, s_json, "expires", subscription->expires+(int)time(NULL));
	} else {
		srjson_AddNumberToObject(&jdoc, s_json, "expires", subscription->expires);
	}
	...
	srjson_AddNumberToObject(&jdoc, s_json, "updated_winfo", subscription->updated_winfo);
...
int pres_dmq_cache_send_all_presentities(dmq_node_t *dmq_node)
{
	...
	for(i=0; i<ps_ptable->ssize; i++) {
		...
		while(ptn != NULL) {
			...
			presentity = new_presentity(&ptn->domain, &ptn->user, ptn->expires - ptn->received_time, ev, &ptn->etag, (ptn->sender.s)?&ptn->sender:NULL);
			if (presentity != NULL) {
				presentity->priority = ptn->priority;
				presentity->received_time = ptn->received_time;
				pres_dmq_replicate_presentity(presentity, (ptn->body.s)?&ptn->body:&pempty,
						1, &pempty, NULL, (ptn->ruid.s)?&ptn->ruid:&pempty, dmq_node);
				pkg_free(presentity);
			}
			ptn = ptn->next;
		}
...
int pres_dmq_cache_send_all_subscriptions(dmq_node_t *dmq_node)
{
	...
	for(i = 0; i < shtable_size; i++) {
		...
		while(s) {
			LM_DBG("send_all_subscriptions found subs to pres_uri %.*s with callid %.*s\n",
