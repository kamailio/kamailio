...
static int ht_pack_values(
		ht_t *ht, db1_res_t *db_res, int row, int cols, str *hvalue)
{
...
int ht_db_load_table(ht_t *ht, str *dbtable, int mode)
{
	...
	if(DB_CAPABILITY(ht_dbf, DB_CAP_FETCH)) {
		...
		if((ret = ht_dbf.query(ht_db_con, NULL, NULL, NULL, db_cols, 0, ncols,
					db_ord, &db_res))
						!= 0
				|| RES_ROW_N(db_res) <= 0) {
			if(ret == 0) {
		...
		for(i = 0; i < RES_ROW_N(db_res); i++) {
			...
			switch(RES_ROWS(db_res)[i].values[0].type) {
				...
				case DB1_BLOB:
					...
					break;
				case DB1_STRING:
					kname.s = (char *)(RES_ROWS(db_res)[i]
											   .values[0]
											   .val.string_val);
					if(kname.s == NULL) {
						LM_ERR("htable [%.*s] row [%d] has NULL key\n",
			...
			if(ht->ncols > 0) {
				...
				if(vtype == 1) {
					switch(RES_ROWS(db_res)[i].values[3].type) {
						...
						case DB1_BLOB:
							...
							break;
						case DB1_STRING:
							kvalue.s = (char *)(RES_ROWS(db_res)[i]
														.values[3]
														.val.string_val);
							if(kvalue.s == NULL) {
								LM_ERR("null value in row %d\n", i);
					...
					switch(RES_ROWS(db_res)[i].values[3].type) {
						...
						case DB1_INT:
							kvalue.s = int2str(
									RES_ROWS(db_res)[i].values[3].val.int_val,
									&kvalue.len);
							val.s = kvalue;
							break;
						case DB1_BIGINT:
							kvalue.s = int2str(
									RES_ROWS(db_res)[i].values[3].val.ll_val,
									&kvalue.len);
							val.s = kvalue;
							break;
						default:
				...
				if(ht_set_cell(
						   ht, &hname, (vtype) ? 0 : AVP_VAL_STR, &val, mode)) {
					LM_ERR("error adding to hash table\n");
					goto error;
				}
	...
	if(last_ktype == 1) {
		snprintf(ht_name_buf, HT_NAME_BUF_SIZE, "%.*s%.*s", pname.len, pname.s,
				ht_array_size_suffix.len, ht_array_size_suffix.s);
		hname.s = ht_name_buf;
		hname.len = strlen(ht_name_buf);
		val.n = n;
		if(ht_set_cell(ht, &hname, 0, &val, mode)) {
...
int ht_db_save_table(ht_t *ht, str *dbtable)
{
	...
	if(ht->ncols > 0) {
		LM_WARN("saving htable [%.*s] with custom db columns is not "
				"available\n",
		...
		while(it) {
			if(it->flags & AVP_VAL_STR) {
				LM_DBG("entry key: [%.*s] value: [%.*s] (str)\n", it->name.len,
						it->name.s, it->value.s.len, it->value.s.s);
			} else {
				LM_DBG("entry key: [%.*s] value: [%ld] (int)\n", it->name.len,
						it->name.s, it->value.n);
			}
			...
			if(ht_dbf.insert(ht_db_con, db_cols, db_vals, ncols) < 0) {
				LM_ERR("failed to store key [%.*s] in table [%.*s]\n",
						it->name.len, it->name.s, dbtable->len, dbtable->s);
			}
			it = it->next;
		}
		ht_slot_unlock(ht, i);
