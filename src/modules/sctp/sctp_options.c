...
static int fix_sinit_max_init_timeo(
		void *cfg_h, str *gname, str *name, void **val);
static void set_sinit_max_init_timeo(str *gname, str *name);
static int fix_sinit_max_attempts(
		void *cfg_h, str *gname, str *name, void **val);
...
static cfg_def_t sctp_cfg_def[] = {
		...
		{"send_retries", CFG_VAR_INT | CFG_ATOMIC, 0, MAX_SCTP_SEND_RETRIES, 0,
				0, "re-send attempts on failure"},
		{"assoc_tracking", CFG_VAR_INT | CFG_ATOMIC, 0, 1, fix_assoc_tracking,
				0, "connection/association tracking (see also assoc_reuse)"},
		...
		{"srto_initial", CFG_VAR_INT | CFG_CB_ONLY_ONCE, 0, 1 << 30,
				...
				" in msecs"},
		{"srto_max", CFG_VAR_INT | CFG_CB_ONLY_ONCE, 0, 1 << 30, fix_srto_max,
				set_srto_max,
				"maximum value of the retransmission timeout (RTO), in msecs"},
		{"srto_min", CFG_VAR_INT | CFG_CB_ONLY_ONCE, 0, 1 << 30, fix_srto_min,
				set_srto_min,
...
#define SCTP_SET_SOCKOPT_BODY_NRET(lev, opt_name, val, err_prefix)     \
	...
	for(si = sctp_listen; si; si = si->next) {                         \
		err += (sctp_setsockopt(si->socket, (lev), (opt_name),         \
						(void *)(&(val)), sizeof((val)), (err_prefix)) \
				< 0);                                                  \
	}
...
static void set_srto_initial(str *gname, str *name)
{
	...
	SCTP_SET_SOCKOPT_BODY_NRET(
			IPPROTO_SCTP, SCTP_RTOINFO, rto, "cfg: setting SCTP_RTOINFO");
#else
	ERR("no SCTP_RTOINFO support, please upgrade your sctp library\n");
#endif /* SCTP_RTOINFO */
}
...
static void set_asocmaxrxt(str *gname, str *name)
{
	...
	SCTP_SET_SOCKOPT_BODY_NRET(
			IPPROTO_SCTP, SCTP_ASSOCINFO, ap, "cfg: setting SCTP_ASSOCINFO");
#else
	ERR("no SCTP_ASSOCINFO support, please upgrade your sctp library\n");
#endif /* SCTP_ASSOCINFO */
}
...
static void set_sinit_max_init_timeo(str *gname, str *name)
{
	...
	SCTP_SET_SOCKOPT_BODY_NRET(
			IPPROTO_SCTP, SCTP_INITMSG, im, "cfg: setting SCTP_INITMSG");
#else
	ERR("no SCTP_INITMSG support, please upgrade your sctp library\n");
#endif /* SCTP_INITMSG */
}
...
/* setting SCTP_DELAYED_SACK failed or no lib support for 
		...
		SCTP_SET_SOCKOPT_BODY_NRET(IPPROTO_SCTP, SCTP_DELAYED_ACK_TIME,
				sack_val, "cfg: setting SCTP_DELAYED_ACK_TIME");
		if(err == 0)
			return;
...
static void set_max_burst(str *gname, str *name)
{
	...
	SCTP_SET_SOCKOPT_BODY_NRET(
			IPPROTO_SCTP, SCTP_MAX_BURST, av, "cfg: setting SCTP_MAX_BURST");
#else
	ERR("no SCTP_MAX_BURST support, please upgrade your sctp library\n");
#endif /* SCTP_MAX_BURST */
}
