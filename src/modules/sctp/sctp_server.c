...
/** get a socket option (wrapper over getsockopt).
  ...
  *                     it will be prepended to the error message.
  * @return 0 on success, -1 on error */
int sctp_getsockopt(int s, int level, int optname, void *optval,
		socklen_t *optlen, char *err_prefix)
{
...
int sctp_get_cfg_from_sock(int s, struct cfg_group_sctp *cfg)
{
	...
	optlen = sizeof(int);
	if(sctp_getsockopt(
			   s, SOL_SOCKET, SO_RCVBUF, (void *)&optval, &optlen, "SO_RCVBUF")
			== 0) {
/* success => hack to set the "default" values*/
	...
	optlen = sizeof(int);
	if(sctp_getsockopt(
			   s, SOL_SOCKET, SO_SNDBUF, (void *)&optval, &optlen, "SO_SNDBUF")
			== 0) {
/* success => hack to set the "default" values*/
#ifdef __OS_linux
		optval /= 2; /* in linux getsockopt() returns 2*set_value */
...
/* SCTP_AUTOCLOSE */
	...
	optlen = sizeof(rto);
	rto.srto_assoc_id = 0;
	if(sctp_getsockopt(s, IPPROTO_SCTP, SCTP_RTOINFO, (void *)&rto, &optlen,
			   "SCTP_RTOINFO")
			== 0) {
		/* success => hack to set the "default" values*/
	...
	optlen = sizeof(ap);
	ap.sasoc_assoc_id = 0;
	if(sctp_getsockopt(s, IPPROTO_SCTP, SCTP_ASSOCINFO, (void *)&ap, &optlen,
			   "SCTP_ASSOCINFO")
			== 0) {
		/* success => hack to set the "default" values*/
...
#ifdef SCTP_INITMSG
	optlen = sizeof(im);
	if(sctp_getsockopt(s, IPPROTO_SCTP, SCTP_INITMSG, (void *)&im, &optlen,
			   "SCTP_INITMSG")
			== 0) {
		/* success => hack to set the "default" values*/
...
#endif /* SCTP_DELAYED_SACK */
	{
		...
		if(sctp_getsockopt(s, IPPROTO_SCTP, SCTP_DELAYED_ACK_TIME,
				   (void *)&sack_val, &optlen, "SCTP_DELAYED_ACK_TIME")
				== 0) {
			/* success => hack to set the "default" values*/
	...
	optlen = sizeof(av);
	av.assoc_id = 0;
	if(sctp_getsockopt(s, IPPROTO_SCTP, SCTP_MAX_BURST, (void *)&av, &optlen,
			   "SCTP_MAX_BURST")
			== 0) {
		/* success => hack to set the "default" values*/
...
static int sctp_init_sock_opt_common(int s, int af)
{
	...
	optval = 1;
	if(_sr_ip_free_bind
			&& setsockopt(s, IPPROTO_IP, IP_FREEBIND, (void *)&optval,
					   sizeof(optval))
					   == -1) {
...
#error SCTP_AUTOCLOSE not supported, please upgrade your sctp library
	...
	if(rto.srto_initial || rto.srto_min || rto.srto_max) {
		...
		if(sctp_setsockopt(s, IPPROTO_SCTP, SCTP_RTOINFO, (void *)&rto,
				   sizeof(rto), "setsockopt: SCTP_RTOINFO")
				!= 0) {
			sctp_err++;
			/* non critical, try to continue */
		}
...
#warning no sctp lib support for SCTP_RTOINFO, consider upgrading
	...
	if(ap.sasoc_asocmaxrxt) {
		...
		if(sctp_setsockopt(s, IPPROTO_SCTP, SCTP_ASSOCINFO, (void *)&ap,
				   sizeof(ap), "setsockopt: SCTP_ASSOCINFO")
				!= 0) {
			sctp_err++;
			/* non critical, try to continue */
		}
...
#warning no sctp lib support for SCTP_ASSOCINFO, consider upgrading
	...
	if(im.sinit_max_attempts || im.sinit_max_init_timeo) {
		...
		if(sctp_setsockopt(s, IPPROTO_SCTP, SCTP_INITMSG, (void *)&im,
				   sizeof(im), "setsockopt: SCTP_INITMSG")
				!= 0) {
			sctp_err++;
			/* non critical, try to continue */
		}
...
#warning no sctp lib support for SCTP_INITMSG, consider upgrading
	...
	if(pp.spp_hbinterval || pp.spp_pathmaxrxt) {
		...
		if(sctp_setsockopt(s, IPPROTO_SCTP, SCTP_PEER_ADDR_PARAMS, (void *)&pp,
				   sizeof(pp), "setsockopt: SCTP_PEER_ADDR_PARAMS")
				!= 0) {
			sctp_err++;
			/* non critical, try to continue */
		}
...
#warning no sctp lib support for SCTP_DELAYED_SACK, consider upgrading
	...
	if(av.assoc_value) {
		if(sctp_setsockopt(s, IPPROTO_SCTP, SCTP_MAX_BURST, (void *)&av,
				   sizeof(av), "setsockopt: SCTP_MAX_BURST")
				!= 0) {
			sctp_err++;
			/* non critical, try to continue */
		}
...
static int sctp_bind_sock(struct socket_info *sock_info)
{
	...
	if(bind(sock_info->socket, &addr->s, sockaddru_len(*addr)) == -1) {
		LOG(L_ERR, "ERROR: sctp_bind_sock: bind(%x, %p, %d) on %s: %s\n",
				sock_info->socket, &addr->s, (unsigned)sockaddru_len(*addr),
...
int sctp_init_sock(struct socket_info *sock_info)
{
	...
	if(sctp_init_su(sock_info) != 0)
		goto error;
	sock_info->socket =
			socket(AF2PF(addr->s.sa_family), SOCK_SEQPACKET, IPPROTO_SCTP);
...
/* make socket non-blocking */
	...
	if(listen(sock_info->socket, 1) < 0) {
		LOG(L_ERR, "ERROR: sctp_init_sock: listen(%x, 1) on %s: %s\n",
				sock_info->socket, sock_info->address_str.s, strerror(errno));
		goto error;
	}
	return 0;
error:
...
int sctp_init_sock_oo(struct socket_info *sock_info)
{
	...
	if(sctp_init_su(sock_info) != 0)
		goto error;
	sock_info->socket =
			socket(AF2PF(addr->s.sa_family), SOCK_STREAM, IPPROTO_SCTP);
	if(sock_info->socket == -1) {
...
#define UNLOCK_SCTP_ASSOC_H(h) lock_set_release(sctp_con_assoc_h_lock_set, (h))
...
#ifdef SCTP_ADDR_HASH
static inline unsigned get_sctp_con_addr_hash(
		union sockaddr_union *remote, struct socket_info *si)
{
	struct ip_addr ip;
	unsigned short port;
	unsigned h;
	...
	if(likely(ip.len == 4))
		...
		h = ip.u.addr32[0] ^ ip.u.addr32[1] ^ ip.u.addr32[2] ^ ip.u.addr32[3]
			^ port;
	else
		h = 0; /* error */
...
int init_sctp_con_tracking()
{
	...
	sctp_con_id_hash =
			shm_malloc(SCTP_ID_HASH_SIZE * sizeof(*sctp_con_id_hash));
	sctp_con_assoc_hash =
			shm_malloc(SCTP_ASSOC_HASH_SIZE * sizeof(*sctp_con_assoc_hash));
#ifdef SCTP_ADDR_HASH
	sctp_con_addr_hash =
			shm_malloc(SCTP_ADDR_HASH_SIZE * sizeof(*sctp_con_addr_hash));
#endif /* SCTP_ADDR_HASH */
...
inline static int _sctp_con_del_assoc_locked(
		unsigned h, struct sctp_con_elem *e)
{
	unsigned id_hash;
	int deref; /* delayed de-reference counter */
	int locked;
...
inline static int _sctp_con_del_addr_locked(unsigned h, struct sctp_con_elem *e)
{
	unsigned id_hash;
	unsigned assoc_id_h;
	int deref; /* delayed de-reference counter */
		...
		if(e->con.id == id) {
			...
			*remote = e->con.remote;
			if(del) {
				...
				e->con.expire =
						now + S_TO_TICKS(cfg_get(sctp, sctp_cfg, autoclose));
			break;
		}
#if 0
...
int sctp_con_addr_get_id_assoc(union sockaddr_union *remote,
		struct socket_info *si, int *assoc_id, int del)
{
	unsigned h;
	ticks_t now;
	struct sctp_con_elem *e;
...
struct sctp_con_elem *sctp_con_new(unsigned id, unsigned assoc_id,
		struct socket_info *si, union sockaddr_union *remote)
{
	struct sctp_con_elem *e;
	...
	e->con.expire =
			e->con.start + S_TO_TICKS(cfg_get(sctp, sctp_cfg, autoclose));
	return e;
error:
	return 0;
}
		...
		union sockaddr_union *remote, int ev)
{
	...
	clist_foreach_safe(&sctp_con_assoc_hash[assoc_hash], e, tmp, l.next_assoc)
	{
			...
			if(ev == SCTP_CON_DOWN_SEEN) {
				if(e->con.flags & SCTP_CON_UP_SEEN) {
					...
					BUG("unexpected flags: %x for assoc_id %d, id %d"
						", sctp con %p\n",
							e->con.flags, assoc_id, e->con.id, e);
					/* do delete */
					if(_sctp_con_del_assoc_locked(assoc_hash, e) == 0)
						goto found; /* skip unlock */
				}
	...
	again:
		...
		if(likely(e)) {
			...
			LOCK_SCTP_ADDR_H(hash);
			clist_insert(
					&sctp_con_addr_hash[hash], e, l.next_addr, l.prev_addr);
			UNLOCK_SCTP_ADDR_H(hash);
#endif /* SCTP_ADDR_HASH */
			atomic_inc(sctp_conn_tracked);
		}
...
static int sctp_raw_send(int socket, char *buf, unsigned len,
		union sockaddr_union *to, struct sctp_sndrcvinfo *sndrcv_info,
		int flags);
...
static int sctp_handle_send_failed(struct socket_info *si,
		union sockaddr_union *su, char *buf, unsigned len)
{
...
static int sctp_handle_assoc_change(struct socket_info *si,
		union sockaddr_union *su, union sctp_notification *snp)
{
	int ret;
	int state;
	int assoc_id;
...
again:
			...
			if(unlikely((unsigned)atomic_get(sctp_conn_no)
				...
				ret = sctp_raw_send(si->socket, ABORT_REASON_MAX_ASSOCS,
						sizeof(ABORT_REASON_MAX_ASSOCS) - 1, su, &sinfo, 0);
				if(ret < 0) {
					su2ip_addr(&ip, su);
					WARN("failed to ABORT new sctp association %d (%s:%d):"
						 " %s (%d)\n",
							assoc_id, ip_addr2a(&ip), su_getport(su),
							strerror(errno), errno);
				} else {
...
#define ERR_LEN_TOO_SMALL(length, val, bind_addr, from_su, text)            \
	if(unlikely((length) < (val))) {                                        \
		SNOT("ERROR: sctp notification from %s on %.*s:%d: " text           \
			 " too short (%d bytes instead of %d bytes)\n",                 \
				su2a((from_su), sizeof(*(from_su))), (bind_addr)->name.len, \
				(bind_addr)->name.s, (bind_addr)->port_no, (int)(length),   \
				(int)(val));                                                \
		goto error;                                                         \
	}
	...
	switch(snp->sn_header.sn_type) {
		case SCTP_REMOTE_ERROR:
			...
			SNOT("sctp notification from %s on %.*s:%d: SCTP_REMOTE_ERROR:"
				 " %d, len %d\n, assoc_id %d",
					...
					si->port_no, ntohs(snp->sn_remote_error.sre_error),
		...
		case SCTP_SENDER_DRY_EVENT:
			ERR_LEN_TOO_SMALL(len, sizeof(struct sctp_sender_dry_event), si, su,
	...
	for(;;) {
		...
		for(cmsg = CMSG_FIRSTHDR(&msg); cmsg; cmsg = CMSG_NXTHDR(&msg, cmsg)) {
...
#endif /*SCTP_EXT */
			{
				...
				DBG("sctp recv: message from %s:%d stream %d  ppid %x"
					" flags %x%s tsn %u"
					" cumtsn %u assoc_id %d\n",
						ip_addr2a(&ri.src_ip), ri.src_port, sinfo->sinfo_stream,
		...
		if(likely(cfg_get(sctp, sctp_cfg, assoc_tracking) && sinfo)) {
			ri.proto_reserved1 = sctp_con_track(sinfo->sinfo_assoc_id,
					ri.bind_address, &ri.src_su, SCTP_CON_RCV_SEEN);
			/* debugging */
...
/* on linux msg->name has priority over assoc_id. To try first assoc_id
		...
		if(likely(cfg_get(sctp, sctp_cfg, assoc_tracking))) {
			tmp_id = sctp_con_addr_get_id_assoc(
					&dst->to, dst->send_sock, &tmp_assoc_id, 0);
