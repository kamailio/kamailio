		...
		int _n, int _nc, db_key_t _o, db1_res_t **_r, int rw)
{
	...
	do {
		...
		if(handle->db[order[i]].status == DB_ON) {
			if((ret = db_exec_query(&handle->db[order[i]].dbf,
						handle->db[order[i]].dbh, table, _k, _op, _v, _c, _n,
						_nc, _o, _r))
					< 0) {
				...
				if(rw) {
					if(err[i] == 0 && handle->db[order[i]].status == DB_ON) {
						if(db_handle_error(handle, handle->db[order[i]].no)
								< 0) {
	...
	for(i = 0; i < DB_NUM; i++) {
		for(j = i + 1; j < DB_NUM; j++) {
			if((handle->db[i].status == DB_OFF
					   || handle->db[i].status == DB_INACTIVE)
					&& (handle->db[j].status == DB_ON)) {
				tmp = order[i];
				order[i] = order[j];
				order[j] = tmp;
			} else if(handle->db[i].failover_time
					  > handle->db[j].failover_time) {
				tmp = order[i];
				order[i] = order[j];
				order[j] = tmp;
			}
