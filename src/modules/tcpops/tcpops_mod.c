...
static int w_tcp_keepalive_enable4(
		sip_msg_t *msg, char *con, char *idle, char *cnt, char *intvl);
static int w_tcp_keepalive_enable3(
		sip_msg_t *msg, char *idle, char *cnt, char *intvl);
...
static int w_tcpops_set_connection_lifetime2(
		sip_msg_t *msg, char *con, char *time);
...
static int w_tcp_con_alive(sip_msg_t *msg, char *paddr, char *p2);
...
static cmd_export_t cmds[] = {
	...
	{"tcp_conid_alive", (cmd_function)w_tcp_conid_alive, 1, fixup_igp_all,
			fixup_free_igp_all, ANY_ROUTE},
	{"tcp_con_alive", (cmd_function)w_tcp_con_alive, 1, fixup_spve_null,
...
static int mod_init(void)
{
	...
	if(tcp_closed_event) {
		...
		if(sr_event_register_cb(SREV_TCP_CLOSED, tcpops_handle_tcp_closed)
				!= 0) {
 ...
 *
 */
static int ki_tcp_keepalive_enable_cid(
		sip_msg_t *msg, int i_con, int i_idle, int i_cnt, int i_intvl)
{
	int fd;
	int closefd = 0;
...
static int ki_tcp_keepalive_enable(
		sip_msg_t *msg, int i_idle, int i_cnt, int i_intvl)
{
	int fd;
...
static int ki_tcp_conid_state(sip_msg_t *msg, int i_conid)
{
	...
	if(s_con->state
			== S_CONN_EOF) { /* Socket closed or about to close under our feet */
		ret = -2;
		goto done;
	}
	if(s_con->state
			== S_CONN_ERROR) { /* Error on read/write - will close soon */
		ret = -3;
		goto done;
	}
	...
	if(s_con->state
			== S_CONN_CONNECT) { /* Outbound connection moving to S_CONN_OK */
		ret = 3;
		goto done;
	}
...
static int ki_tcpops_set_connection_lifetime_cid(
		sip_msg_t *msg, int i_conid, int i_time)
{
	struct tcp_connection *s_con;
	int ret = -1;
...
static int w_tcpops_set_connection_lifetime2(
		sip_msg_t *msg, char *conid, char *time)
{
	_IVALUE(conid)
	_IVALUE(time)
 ...
 *
 */
static int ki_tcp_con_alive(sip_msg_t *msg, str *saddr)
{
	return ki_tcp_get_conid_helper(msg, saddr, NULL);
}
...
static sr_kemi_t sr_kemi_tcpops_exports[] = {
	...
	{ str_init("tcpops"), str_init("tcp_con_alive"),
		SR_KEMIP_INT, ki_tcp_con_alive,
