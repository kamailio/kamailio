...
static cmd_export_t cmds[] = {
	...
	{"change_reply_status", change_reply_status_f, 2,
			change_reply_status_fixup, fixup_free_fparam_all, ONREPLY_ROUTE},
	{"change_reply_status_code", change_reply_status_code_f, 1,
			fixup_igp_null, fixup_free_igp_null, ONREPLY_ROUTE},
	{"remove_body", (cmd_function)w_remove_body_f, 0, 0, 0, ANY_ROUTE},
	{"keep_hf", (cmd_function)w_keep_hf_f, 0, fixup_regexp_null, fixup_free_regexp_null, ANY_ROUTE},
	{"keep_hf", (cmd_function)w_keep_hf_f, 1, fixup_regexp_null, fixup_free_regexp_null, ANY_ROUTE},
...
static int ki_bl_iterator_append(sip_msg_t *msg, str *iname, str *text)
{
	...
	if(sval.s == NULL) {
		PKG_MEM_ERROR_FMT("failed append text after %.*s\n",
				_bl_iterators[k].it.len, _bl_iterators[k].it.s);
		return -1;
	}
