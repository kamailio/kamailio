...
unsigned int net_address(unsigned int ip, unsigned int mask)
{
	return (ip & mask);
}
...
int parse_ip_address(char *c, unsigned int *address) //inet_aton
{
...
int parse_ip_netmask(char *c, char **ip, unsigned int *mask)
{
	...
	if((q = strchr(p, '/')) == NULL) {
		...
		*ip = (char *)malloc(q - p + 1);
		...
		} else /* we may have a 255.255.192.0 mask */
