...
PyObject *sr_apy_kemi_exec_func_ex(
		sr_kemi_t *ket, PyObject *self, PyObject *args, int idx)
{
	str fname;
	int i;
	int ret;
	...
	if(ket->ptypes[1] == SR_KEMIP_NONE) {
		...
		if(ket->ptypes[0] == SR_KEMIP_INT && ket->ptypes[1] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(
					   args, "ii:kemi-param-nn", &vps[0].n, &vps[1].n)) {
			...
			if(!PyArg_ParseTuple(
					   args, "is:kemi-param-ns", &vps[0].n, &vps[1].s.s)) {
			...
			if(!PyArg_ParseTuple(
					   args, "si:kemi-param-sn", &vps[0].s.s, &vps[1].n)) {
			...
			LM_DBG("params[%d] for: %.*s are str-int: [%.*s] [%d]\n", i,
				  ...
				  && ket->ptypes[1] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(
					   args, "ss:kemi-param-ss", &vps[0].s.s, &vps[1].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are str: [%.*s] [%.*s]\n", i,
					fname.len, fname.s, vps[0].s.len, vps[0].s.s, vps[1].s.len,
					vps[1].s.s);
		} else {
			LM_ERR("not implemented yet\n");
			return sr_kemi_apy_return_false();
		...
		if(ket->ptypes[0] == SR_KEMIP_INT && ket->ptypes[1] == SR_KEMIP_INT
			...
			LM_DBG("params[%d] for: %.*s are int-int-int: [%d] [%d] [%d]\n", i,
					fname.len, fname.s, vps[0].n, vps[1].n, vps[2].n);
		} else if(ket->ptypes[0] == SR_KEMIP_INT
				  && ket->ptypes[1] == SR_KEMIP_INT
			...
			LM_DBG("params[%d] for: %.*s are int-int-str: [%d] [%d] [%.*s]\n",
					i, fname.len, fname.s, vps[0].n, vps[1].n, vps[2].s.len,
					vps[2].s.s);
		} else if(ket->ptypes[0] == SR_KEMIP_INT
				  && ket->ptypes[1] == SR_KEMIP_STR
				  && ket->ptypes[2] == SR_KEMIP_INT) {
			...
			LM_DBG("params[%d] for: %.*s are int-str-int: [%d] [%.*s] [%d]\n",
					i, fname.len, fname.s, vps[0].n, vps[1].s.len, vps[1].s.s,
					vps[2].n);
		} else if(ket->ptypes[0] == SR_KEMIP_INT
				  && ket->ptypes[1] == SR_KEMIP_STR
				  && ket->ptypes[2] == SR_KEMIP_STR) {
			...
			LM_DBG("params[%d] for: %.*s are str-str-int: [%.*s] [%d] [%.*s]\n",
					i, fname.len, fname.s, vps[0].s.len, vps[0].s.s, vps[1].n,
					vps[2].s.len, vps[2].s.s);
		} else if(ket->ptypes[0] == SR_KEMIP_STR
				  && ket->ptypes[1] == SR_KEMIP_STR
				  && ket->ptypes[2] == SR_KEMIP_INT) {
			...
			LM_DBG("params[%d] for: %.*s are str-str-int: [%.*s] [%.*s]"
				   " [%d]\n",
					i, fname.len, fname.s, vps[0].s.len, vps[0].s.s,
		...
		i = 4;
		if(ket->ptypes[0] == SR_KEMIP_STR && ket->ptypes[1] == SR_KEMIP_STR
				&& ket->ptypes[2] == SR_KEMIP_STR
				&& ket->ptypes[3] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "ssss:kemi-param-ssss", &vps[0].s.s,
					   &vps[1].s.s, &vps[2].s.s, &vps[3].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%s] [%s] [%s]\n", i,
					fname.len, fname.s, vps[0].s.s, vps[1].s.s, vps[2].s.s,
					vps[3].s.s);
		} else if(ket->ptypes[0] == SR_KEMIP_STR
				  && ket->ptypes[1] == SR_KEMIP_STR
				  && ket->ptypes[2] == SR_KEMIP_STR
				  && ket->ptypes[3] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "sssn:kemi-param-sssn", &vps[0].s.s,
					   &vps[1].s.s, &vps[2].s.s, &vps[3].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%s] [%s] [%d]\n", i,
				  ...
				  && ket->ptypes[2] == SR_KEMIP_INT
				  && ket->ptypes[3] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "ssns:kemi-param-ssns", &vps[0].s.s,
					   &vps[1].s.s, &vps[2].n, &vps[3].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%s] [%d] [%s]\n", i,
					fname.len, fname.s, vps[0].s.s, vps[1].s.s, vps[2].n,
					vps[3].s.s);
		} else if(ket->ptypes[0] == SR_KEMIP_STR
				  && ket->ptypes[1] == SR_KEMIP_STR
				  && ket->ptypes[2] == SR_KEMIP_INT
				  && ket->ptypes[3] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "ssnn:kemi-param-ssnn", &vps[0].s.s,
					   &vps[1].s.s, &vps[2].n, &vps[3].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%s] [%d] [%d]\n", i,
				  ...
				  && ket->ptypes[2] == SR_KEMIP_STR
				  && ket->ptypes[3] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "snss:kemi-param-snss", &vps[0].s.s,
					   &vps[1].n, &vps[2].s.s, &vps[3].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%d] [%s] [%s]\n", i,
					fname.len, fname.s, vps[0].s.s, vps[1].n, vps[2].s.s,
					vps[3].s.s);
		} else if(ket->ptypes[0] == SR_KEMIP_STR
				  && ket->ptypes[1] == SR_KEMIP_INT
				  && ket->ptypes[2] == SR_KEMIP_STR
				  && ket->ptypes[3] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "snsn:kemi-param-snsn", &vps[0].s.s,
					   &vps[1].n, &vps[2].s.s, &vps[3].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%d] [%s] [%d]\n", i,
				  ...
				  && ket->ptypes[2] == SR_KEMIP_INT
				  && ket->ptypes[3] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "snns:kemi-param-snns", &vps[0].s.s,
					   &vps[1].n, &vps[2].n, &vps[3].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%d] [%d] [%s]\n", i,
					fname.len, fname.s, vps[0].s.s, vps[1].n, vps[2].n,
					vps[3].s.s);
		} else if(ket->ptypes[0] == SR_KEMIP_STR
				  && ket->ptypes[1] == SR_KEMIP_INT
				  && ket->ptypes[2] == SR_KEMIP_INT
				  && ket->ptypes[3] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "snnn:kemi-param-snnn", &vps[0].s.s,
					   &vps[1].n, &vps[2].n, &vps[3].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%d] [%d] [%d]\n", i,
				  ...
				  && ket->ptypes[2] == SR_KEMIP_STR
				  && ket->ptypes[3] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "nsss:kemi-param-nsss", &vps[0].n,
					   &vps[1].s.s, &vps[2].s.s, &vps[3].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%s] [%s] [%s]\n", i,
					fname.len, fname.s, vps[0].n, vps[1].s.s, vps[2].s.s,
					vps[3].s.s);
		} else if(ket->ptypes[0] == SR_KEMIP_INT
				  && ket->ptypes[1] == SR_KEMIP_STR
				  && ket->ptypes[2] == SR_KEMIP_STR
				  && ket->ptypes[3] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "nssn:kemi-param-nssn", &vps[0].n,
					   &vps[1].s.s, &vps[2].s.s, &vps[3].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%s] [%s] [%d]\n", i,
				  ...
				  && ket->ptypes[2] == SR_KEMIP_INT
				  && ket->ptypes[3] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "nsns:kemi-param-nsns", &vps[0].n,
					   &vps[1].s.s, &vps[2].n, &vps[3].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%s] [%d] [%s]\n", i,
					fname.len, fname.s, vps[0].n, vps[1].s.s, vps[2].n,
					vps[3].s.s);
		} else if(ket->ptypes[0] == SR_KEMIP_INT
				  && ket->ptypes[1] == SR_KEMIP_STR
				  && ket->ptypes[2] == SR_KEMIP_INT
				  && ket->ptypes[3] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "nsnn:kemi-param-nsnn", &vps[0].n,
					   &vps[1].s.s, &vps[2].n, &vps[3].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%s] [%d] [%d]\n", i,
				  ...
				  && ket->ptypes[2] == SR_KEMIP_STR
				  && ket->ptypes[3] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "nnss:kemi-param-nnss", &vps[0].n,
					   &vps[1].n, &vps[2].s.s, &vps[3].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%d] [%s] [%s]\n", i,
					fname.len, fname.s, vps[0].n, vps[1].n, vps[2].s.s,
					vps[3].s.s);
		} else if(ket->ptypes[0] == SR_KEMIP_INT
				  && ket->ptypes[1] == SR_KEMIP_INT
				  && ket->ptypes[2] == SR_KEMIP_STR
				  && ket->ptypes[3] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "nnsn:kemi-param-nnsn", &vps[0].n,
					   &vps[1].n, &vps[2].s.s, &vps[3].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%d] [%s] [%d]\n", i,
				  ...
				  && ket->ptypes[2] == SR_KEMIP_INT
				  && ket->ptypes[3] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "nnns:kemi-param-nnns", &vps[0].n,
					   &vps[1].n, &vps[2].n, &vps[3].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%d] [%d] [%s]\n", i,
				  ...
				  && ket->ptypes[2] == SR_KEMIP_INT
				  && ket->ptypes[3] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "nnnn:kemi-param-nnnn", &vps[0].n,
					   &vps[1].n, &vps[2].n, &vps[3].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%d] [%d] [%d]\n", i,
					fname.len, fname.s, vps[0].n, vps[1].n, vps[2].n, vps[3].n);
		} else {
		...
		if(ket->ptypes[0] == SR_KEMIP_STR && ket->ptypes[1] == SR_KEMIP_STR
			...
			if(!PyArg_ParseTuple(args, "sssss:kemi-param-sssss", &vps[0].s.s,
					   &vps[1].s.s, &vps[2].s.s, &vps[3].s.s, &vps[4].s.s)) {
			...
			if(!PyArg_ParseTuple(args, "ssssn:kemi-param-ssssn", &vps[0].s.s,
					   &vps[1].s.s, &vps[2].s.s, &vps[3].s.s, &vps[4].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%s] [%s] [%s] [%d]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_INT
				  && ket->ptypes[4] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "sssns:kemi-param-sssns", &vps[0].s.s,
					   &vps[1].s.s, &vps[2].s.s, &vps[3].n, &vps[4].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%s] [%s] [%d] [%s]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_INT
				  && ket->ptypes[4] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "sssnn:kemi-param-sssnn", &vps[0].s.s,
					   &vps[1].s.s, &vps[2].s.s, &vps[3].n, &vps[4].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%s] [%s] [%d] [%d]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_STR
				  && ket->ptypes[4] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "ssnss:kemi-param-ssnss", &vps[0].s.s,
					   &vps[1].s.s, &vps[2].n, &vps[3].s.s, &vps[4].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%s] [%d] [%s] [%s]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_STR
				  && ket->ptypes[4] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "ssnsn:kemi-param-ssnsn", &vps[0].s.s,
					   &vps[1].s.s, &vps[2].n, &vps[3].s.s, &vps[4].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%s] [%d] [%s] [%d]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_INT
				  && ket->ptypes[4] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "ssnns:kemi-param-ssnns", &vps[0].s.s,
					   &vps[1].s.s, &vps[2].n, &vps[3].n, &vps[4].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%s] [%d] [%d] [%s]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_INT
				  && ket->ptypes[4] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "ssnnn:kemi-param-ssnnn", &vps[0].s.s,
					   &vps[1].s.s, &vps[2].n, &vps[3].n, &vps[4].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%s] [%d] [%d] [%d]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_STR
				  && ket->ptypes[4] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "snsss:kemi-param-snsss", &vps[0].s.s,
					   &vps[1].n, &vps[2].s.s, &vps[3].s.s, &vps[4].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%d] [%s] [%s] [%s]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_STR
				  && ket->ptypes[4] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "snssn:kemi-param-snssn", &vps[0].s.s,
					   &vps[1].n, &vps[2].s.s, &vps[3].s.s, &vps[4].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%d] [%s] [%s] [%d]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_INT
				  && ket->ptypes[4] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "snsns:kemi-param-snsns", &vps[0].s.s,
					   &vps[1].n, &vps[2].s.s, &vps[3].n, &vps[4].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%d] [%s] [%d] [%s]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_INT
				  && ket->ptypes[4] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "snsnn:kemi-param-snsnn", &vps[0].s.s,
					   &vps[1].n, &vps[2].s.s, &vps[3].n, &vps[4].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%d] [%s] [%d] [%d]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_STR
				  && ket->ptypes[4] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "snnss:kemi-param-snnss", &vps[0].s.s,
					   &vps[1].n, &vps[2].n, &vps[3].s.s, &vps[4].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%d] [%d] [%s] [%s]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_STR
				  && ket->ptypes[4] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "snnsn:kemi-param-snnsn", &vps[0].s.s,
					   &vps[1].n, &vps[2].n, &vps[3].s.s, &vps[4].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%d] [%d] [%s] [%d]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_INT
				  && ket->ptypes[4] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "snnns:kemi-param-snnns", &vps[0].s.s,
					   &vps[1].n, &vps[2].n, &vps[3].n, &vps[4].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%d] [%d] [%d] [%s]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_INT
				  && ket->ptypes[4] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "snnnn:kemi-param-snnnn", &vps[0].s.s,
					   &vps[1].n, &vps[2].n, &vps[3].n, &vps[4].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%s] [%d] [%d] [%d] [%d]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_STR
				  && ket->ptypes[4] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "nssss:kemi-param-nssss", &vps[0].n,
					   &vps[1].s.s, &vps[2].s.s, &vps[3].s.s, &vps[4].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%s] [%s] [%s] [%s]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_STR
				  && ket->ptypes[4] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "nsssn:kemi-param-nsssn", &vps[0].n,
					   &vps[1].s.s, &vps[2].s.s, &vps[3].s.s, &vps[4].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%s] [%s] [%s] [%d]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_INT
				  && ket->ptypes[4] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "nssns:kemi-param-nssns", &vps[0].n,
					   &vps[1].s.s, &vps[2].s.s, &vps[3].n, &vps[4].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%s] [%s] [%d] [%s]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_INT
				  && ket->ptypes[4] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "nssnn:kemi-param-nssnn", &vps[0].n,
					   &vps[1].s.s, &vps[2].s.s, &vps[3].n, &vps[4].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%s] [%s] [%d] [%d]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_STR
				  && ket->ptypes[4] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "nsnss:kemi-param-nsnss", &vps[0].n,
					   &vps[1].s.s, &vps[2].n, &vps[3].s.s, &vps[4].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%s] [%d] [%s] [%s]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_STR
				  && ket->ptypes[4] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "nsnsn:kemi-param-nsnsn", &vps[0].n,
					   &vps[1].s.s, &vps[2].n, &vps[3].s.s, &vps[4].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%s] [%d] [%s] [%d]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_INT
				  && ket->ptypes[4] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "nsnns:kemi-param-nsnns", &vps[0].n,
					   &vps[1].s.s, &vps[2].n, &vps[3].n, &vps[4].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%s] [%d] [%d] [%s]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_INT
				  && ket->ptypes[4] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "nsnnn:kemi-param-nsnnn", &vps[0].n,
					   &vps[1].s.s, &vps[2].n, &vps[3].n, &vps[4].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%s] [%d] [%d] [%d]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_STR
				  && ket->ptypes[4] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "nnsss:kemi-param-nnsss", &vps[0].n,
					   &vps[1].n, &vps[2].s.s, &vps[3].s.s, &vps[4].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%d] [%s] [%s] [%s]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_STR
				  && ket->ptypes[4] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "nnssn:kemi-param-nnssn", &vps[0].n,
					   &vps[1].n, &vps[2].s.s, &vps[3].s.s, &vps[4].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%d] [%s] [%s] [%d]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_INT
				  && ket->ptypes[4] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "nnsns:kemi-param-nnsns", &vps[0].n,
					   &vps[1].n, &vps[2].s.s, &vps[3].n, &vps[4].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%d] [%s] [%d] [%s]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_INT
				  && ket->ptypes[4] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "nnsnn:kemi-param-nnsnn", &vps[0].n,
					   &vps[1].n, &vps[2].s.s, &vps[3].n, &vps[4].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%d] [%s] [%d] [%d]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_STR
				  && ket->ptypes[4] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "nnnss:kemi-param-nnnss", &vps[0].n,
					   &vps[1].n, &vps[2].n, &vps[3].s.s, &vps[4].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%d] [%d] [%s] [%s]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_STR
				  && ket->ptypes[4] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "nnnsn:kemi-param-nnnsn", &vps[0].n,
					   &vps[1].n, &vps[2].n, &vps[3].s.s, &vps[4].n)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%d] [%d] [%s] [%d]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_INT
				  && ket->ptypes[4] == SR_KEMIP_STR) {
			if(!PyArg_ParseTuple(args, "nnnns:kemi-param-nnnns", &vps[0].n,
					   &vps[1].n, &vps[2].n, &vps[3].n, &vps[4].s.s)) {
			...
			LM_DBG("params[%d] for: %.*s are: [%d] [%d] [%d] [%d] [%s]\n", i,
				  ...
				  && ket->ptypes[3] == SR_KEMIP_INT
				  && ket->ptypes[4] == SR_KEMIP_INT) {
			if(!PyArg_ParseTuple(args, "nnnnn:kemi-param-nnnnn", &vps[0].n,
					   &vps[1].n, &vps[2].n, &vps[3].n, &vps[4].n)) {
		...
		if(ket->ptypes[0] == SR_KEMIP_STR && ket->ptypes[1] == SR_KEMIP_STR
			...
			if(!PyArg_ParseTuple(args, "ssssss:kemi-param-ssssss", &vps[0].s.s,
					   &vps[1].s.s, &vps[2].s.s, &vps[3].s.s, &vps[4].s.s,
					   &vps[5].s.s)) {
	...
	switch(i) {
		...
		case 2:
			if(ket->ptypes[0] == SR_KEMIP_INT) {
				if(ket->ptypes[1] == SR_KEMIP_INT) {
					if(ket->rtype == SR_KEMIP_XVAL) {
						xret = ((sr_kemi_xfmnn_f)(ket->func))(
								lmsg, vps[0].n, vps[1].n);
						return sr_kemi_apy_return_xval(ket, xret);
					} else {
					...
					if(ket->rtype == SR_KEMIP_XVAL) {
						xret = ((sr_kemi_xfmns_f)(ket->func))(
								lmsg, vps[0].n, &vps[1].s);
						return sr_kemi_apy_return_xval(ket, xret);
					} else {
				...
				if(ket->ptypes[1] == SR_KEMIP_INT) {
					if(ket->rtype == SR_KEMIP_XVAL) {
						xret = ((sr_kemi_xfmsn_f)(ket->func))(
								lmsg, &vps[0].s, vps[1].n);
						return sr_kemi_apy_return_xval(ket, xret);
					} else {
					...
					if(ket->rtype == SR_KEMIP_XVAL) {
						xret = ((sr_kemi_xfmss_f)(ket->func))(
								lmsg, &vps[0].s, &vps[1].s);
						return sr_kemi_apy_return_xval(ket, xret);
					} else {
		...
		case 3:
			if(ket->ptypes[0] == SR_KEMIP_STR && ket->ptypes[1] == SR_KEMIP_STR
					&& ket->ptypes[2] == SR_KEMIP_STR) {
				if(ket->rtype == SR_KEMIP_XVAL) {
					xret = ((sr_kemi_xfmsss_f)(ket->func))(
							lmsg, &vps[0].s, &vps[1].s, &vps[2].s);
					return sr_kemi_apy_return_xval(ket, xret);
				} else {
					...
					return sr_kemi_apy_return_int(ket, ret);
					  ...
					  && ket->ptypes[2] == SR_KEMIP_INT) {
				if(ket->rtype == SR_KEMIP_XVAL) {
					xret = ((sr_kemi_xfmssn_f)(ket->func))(
							lmsg, &vps[0].s, &vps[1].s, vps[2].n);
					return sr_kemi_apy_return_xval(ket, xret);
				} else {
					...
					return sr_kemi_apy_return_int(ket, ret);
					  ...
					  && ket->ptypes[2] == SR_KEMIP_STR) {
				if(ket->rtype == SR_KEMIP_XVAL) {
					xret = ((sr_kemi_xfmsns_f)(ket->func))(
							lmsg, &vps[0].s, vps[1].n, &vps[2].s);
					return sr_kemi_apy_return_xval(ket, xret);
				} else {
					...
					return sr_kemi_apy_return_int(ket, ret);
					  ...
					  && ket->ptypes[2] == SR_KEMIP_INT) {
				if(ket->rtype == SR_KEMIP_XVAL) {
					xret = ((sr_kemi_xfmsnn_f)(ket->func))(
							lmsg, &vps[0].s, vps[1].n, vps[2].n);
					return sr_kemi_apy_return_xval(ket, xret);
				} else {
					...
					return sr_kemi_apy_return_int(ket, ret);
					  ...
					  && ket->ptypes[2] == SR_KEMIP_STR) {
				if(ket->rtype == SR_KEMIP_XVAL) {
					xret = ((sr_kemi_xfmnss_f)(ket->func))(
							lmsg, vps[0].n, &vps[1].s, &vps[2].s);
					return sr_kemi_apy_return_xval(ket, xret);
				} else {
					...
					return sr_kemi_apy_return_int(ket, ret);
					  ...
					  && ket->ptypes[2] == SR_KEMIP_INT) {
				if(ket->rtype == SR_KEMIP_XVAL) {
					xret = ((sr_kemi_xfmnsn_f)(ket->func))(
							lmsg, vps[0].n, &vps[1].s, vps[2].n);
					return sr_kemi_apy_return_xval(ket, xret);
				} else {
					...
					return sr_kemi_apy_return_int(ket, ret);
					  ...
					  && ket->ptypes[2] == SR_KEMIP_STR) {
				if(ket->rtype == SR_KEMIP_XVAL) {
					xret = ((sr_kemi_xfmnns_f)(ket->func))(
							lmsg, vps[0].n, vps[1].n, &vps[2].s);
					return sr_kemi_apy_return_xval(ket, xret);
				} else {
					...
					return sr_kemi_apy_return_int(ket, ret);
					  ...
					  && ket->ptypes[2] == SR_KEMIP_INT) {
				if(ket->rtype == SR_KEMIP_XVAL) {
					xret = ((sr_kemi_xfmnnn_f)(ket->func))(
							lmsg, vps[0].n, vps[1].n, vps[2].n);
					return sr_kemi_apy_return_xval(ket, xret);
				} else {
		...
		case 4:
			if(ket->ptypes[0] == SR_KEMIP_STR && ket->ptypes[1] == SR_KEMIP_STR
					&& ket->ptypes[2] == SR_KEMIP_STR
					&& ket->ptypes[3] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmssss_f)(ket->func))(
						lmsg, &vps[0].s, &vps[1].s, &vps[2].s, &vps[3].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_STR
					  && ket->ptypes[2] == SR_KEMIP_STR
					  && ket->ptypes[3] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmsssn_f)(ket->func))(
						lmsg, &vps[0].s, &vps[1].s, &vps[2].s, vps[3].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_STR
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmssns_f)(ket->func))(
						lmsg, &vps[0].s, &vps[1].s, vps[2].n, &vps[3].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_STR
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmssnn_f)(ket->func))(
						lmsg, &vps[0].s, &vps[1].s, vps[2].n, vps[3].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_STR
					  && ket->ptypes[3] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmsnss_f)(ket->func))(
						lmsg, &vps[0].s, vps[1].n, &vps[2].s, &vps[3].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_STR
					  && ket->ptypes[3] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmsnsn_f)(ket->func))(
						lmsg, &vps[0].s, vps[1].n, &vps[2].s, vps[3].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmsnns_f)(ket->func))(
						lmsg, &vps[0].s, vps[1].n, vps[2].n, &vps[3].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmsnnn_f)(ket->func))(
						lmsg, &vps[0].s, vps[1].n, vps[2].n, vps[3].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_STR
					  && ket->ptypes[2] == SR_KEMIP_STR
					  && ket->ptypes[3] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmnsss_f)(ket->func))(
						lmsg, vps[0].n, &vps[1].s, &vps[2].s, &vps[3].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_STR
					  && ket->ptypes[2] == SR_KEMIP_STR
					  && ket->ptypes[3] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmnssn_f)(ket->func))(
						lmsg, vps[0].n, &vps[1].s, &vps[2].s, vps[3].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_STR
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmnsns_f)(ket->func))(
						lmsg, vps[0].n, &vps[1].s, vps[2].n, &vps[3].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_STR
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmnsnn_f)(ket->func))(
						lmsg, vps[0].n, &vps[1].s, vps[2].n, vps[3].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_STR
					  && ket->ptypes[3] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmnnss_f)(ket->func))(
						lmsg, vps[0].n, vps[1].n, &vps[2].s, &vps[3].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_STR
					  && ket->ptypes[3] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmnnsn_f)(ket->func))(
						lmsg, vps[0].n, vps[1].n, &vps[2].s, vps[3].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmnnns_f)(ket->func))(
						lmsg, vps[0].n, vps[1].n, vps[2].n, &vps[3].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmnnnn_f)(ket->func))(
						lmsg, vps[0].n, vps[1].n, vps[2].n, vps[3].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else {
				LM_ERR("invalid parameters for: %.*s\n", fname.len, fname.s);
				return sr_kemi_apy_return_false();
		...
		case 5:
			if(ket->ptypes[0] == SR_KEMIP_STR && ket->ptypes[1] == SR_KEMIP_STR
					...
					&& ket->ptypes[3] == SR_KEMIP_STR
					&& ket->ptypes[4] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmsssss_f)(ket->func))(lmsg, &vps[0].s,
						&vps[1].s, &vps[2].s, &vps[3].s, &vps[4].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_STR
					  && ket->ptypes[2] == SR_KEMIP_STR
					  && ket->ptypes[3] == SR_KEMIP_STR
					  && ket->ptypes[4] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmssssn_f)(ket->func))(lmsg, &vps[0].s,
						&vps[1].s, &vps[2].s, &vps[3].s, vps[4].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_STR
					  && ket->ptypes[2] == SR_KEMIP_STR
					  && ket->ptypes[3] == SR_KEMIP_INT
					  && ket->ptypes[4] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmsssns_f)(ket->func))(lmsg, &vps[0].s,
						&vps[1].s, &vps[2].s, vps[3].n, &vps[4].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_STR
					  && ket->ptypes[2] == SR_KEMIP_STR
					  && ket->ptypes[3] == SR_KEMIP_INT
					  && ket->ptypes[4] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmsssnn_f)(ket->func))(lmsg, &vps[0].s,
						&vps[1].s, &vps[2].s, vps[3].n, vps[4].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_STR
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_STR
					  && ket->ptypes[4] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmssnss_f)(ket->func))(lmsg, &vps[0].s,
						&vps[1].s, vps[2].n, &vps[3].s, &vps[4].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_STR
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_STR
					  && ket->ptypes[4] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmssnsn_f)(ket->func))(lmsg, &vps[0].s,
						&vps[1].s, vps[2].n, &vps[3].s, vps[4].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_STR
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_INT
					  && ket->ptypes[4] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmssnns_f)(ket->func))(lmsg, &vps[0].s,
						&vps[1].s, vps[2].n, vps[3].n, &vps[4].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_STR
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_INT
					  && ket->ptypes[4] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmssnnn_f)(ket->func))(lmsg, &vps[0].s,
						&vps[1].s, vps[2].n, vps[3].n, vps[4].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_STR
					  && ket->ptypes[3] == SR_KEMIP_STR
					  && ket->ptypes[4] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmsnsss_f)(ket->func))(lmsg, &vps[0].s,
						vps[1].n, &vps[2].s, &vps[3].s, &vps[4].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_STR
					  && ket->ptypes[3] == SR_KEMIP_STR
					  && ket->ptypes[4] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmsnssn_f)(ket->func))(lmsg, &vps[0].s,
						vps[1].n, &vps[2].s, &vps[3].s, vps[4].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_STR
					  && ket->ptypes[3] == SR_KEMIP_INT
					  && ket->ptypes[4] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmsnsns_f)(ket->func))(lmsg, &vps[0].s,
						vps[1].n, &vps[2].s, vps[3].n, &vps[4].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_STR
					  && ket->ptypes[3] == SR_KEMIP_INT
					  && ket->ptypes[4] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmsnsnn_f)(ket->func))(lmsg, &vps[0].s,
						vps[1].n, &vps[2].s, vps[3].n, vps[4].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_STR
					  && ket->ptypes[4] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmsnnss_f)(ket->func))(lmsg, &vps[0].s,
						vps[1].n, vps[2].n, &vps[3].s, &vps[4].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_STR
					  && ket->ptypes[4] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmsnnsn_f)(ket->func))(lmsg, &vps[0].s,
						vps[1].n, vps[2].n, &vps[3].s, vps[4].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_INT
					  && ket->ptypes[4] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmsnnns_f)(ket->func))(lmsg, &vps[0].s,
						vps[1].n, vps[2].n, vps[3].n, &vps[4].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_STR
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_INT
					  && ket->ptypes[4] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmsnnnn_f)(ket->func))(lmsg, &vps[0].s,
						vps[1].n, vps[2].n, vps[3].n, vps[4].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_STR
					  ...
					  && ket->ptypes[3] == SR_KEMIP_STR
					  && ket->ptypes[4] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmnssss_f)(ket->func))(lmsg, vps[0].n,
						&vps[1].s, &vps[2].s, &vps[3].s, &vps[4].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_STR
					  ...
					  && ket->ptypes[3] == SR_KEMIP_STR
					  && ket->ptypes[4] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmnsssn_f)(ket->func))(lmsg, vps[0].n,
						&vps[1].s, &vps[2].s, &vps[3].s, vps[4].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_STR
					  ...
					  && ket->ptypes[3] == SR_KEMIP_INT
					  && ket->ptypes[4] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmnssns_f)(ket->func))(lmsg, vps[0].n,
						&vps[1].s, &vps[2].s, vps[3].n, &vps[4].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_STR
					  ...
					  && ket->ptypes[3] == SR_KEMIP_INT
					  && ket->ptypes[4] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmnssnn_f)(ket->func))(lmsg, vps[0].n,
						&vps[1].s, &vps[2].s, vps[3].n, vps[4].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_STR
					  ...
					  && ket->ptypes[3] == SR_KEMIP_STR
					  && ket->ptypes[4] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmnsnss_f)(ket->func))(lmsg, vps[0].n,
						&vps[1].s, vps[2].n, &vps[3].s, &vps[4].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_STR
					  ...
					  && ket->ptypes[3] == SR_KEMIP_STR
					  && ket->ptypes[4] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmnsnsn_f)(ket->func))(lmsg, vps[0].n,
						&vps[1].s, vps[2].n, &vps[3].s, vps[4].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_STR
					  ...
					  && ket->ptypes[3] == SR_KEMIP_INT
					  && ket->ptypes[4] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmnsnns_f)(ket->func))(lmsg, vps[0].n,
						&vps[1].s, vps[2].n, vps[3].n, &vps[4].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_STR
					  ...
					  && ket->ptypes[3] == SR_KEMIP_INT
					  && ket->ptypes[4] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmnsnnn_f)(ket->func))(lmsg, vps[0].n,
						&vps[1].s, vps[2].n, vps[3].n, vps[4].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_INT
					  ...
					  && ket->ptypes[3] == SR_KEMIP_STR
					  && ket->ptypes[4] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmnnsss_f)(ket->func))(lmsg, vps[0].n, vps[1].n,
						&vps[2].s, &vps[3].s, &vps[4].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_STR
					  && ket->ptypes[3] == SR_KEMIP_STR
					  && ket->ptypes[4] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmnnssn_f)(ket->func))(lmsg, vps[0].n, vps[1].n,
						&vps[2].s, &vps[3].s, vps[4].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_STR
					  && ket->ptypes[3] == SR_KEMIP_INT
					  && ket->ptypes[4] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmnnsns_f)(ket->func))(lmsg, vps[0].n, vps[1].n,
						&vps[2].s, vps[3].n, &vps[4].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_STR
					  && ket->ptypes[3] == SR_KEMIP_INT
					  && ket->ptypes[4] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmnnsnn_f)(ket->func))(lmsg, vps[0].n, vps[1].n,
						&vps[2].s, vps[3].n, vps[4].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_STR
					  && ket->ptypes[4] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmnnnss_f)(ket->func))(lmsg, vps[0].n, vps[1].n,
						vps[2].n, &vps[3].s, &vps[4].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_STR
					  && ket->ptypes[4] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmnnnsn_f)(ket->func))(lmsg, vps[0].n, vps[1].n,
						vps[2].n, &vps[3].s, vps[4].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_INT
					  && ket->ptypes[4] == SR_KEMIP_STR) {
				ret = ((sr_kemi_fmnnnns_f)(ket->func))(lmsg, vps[0].n, vps[1].n,
						vps[2].n, vps[3].n, &vps[4].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else if(ket->ptypes[0] == SR_KEMIP_INT
					  && ket->ptypes[1] == SR_KEMIP_INT
					  && ket->ptypes[2] == SR_KEMIP_INT
					  && ket->ptypes[3] == SR_KEMIP_INT
					  && ket->ptypes[4] == SR_KEMIP_INT) {
				ret = ((sr_kemi_fmnnnnn_f)(ket->func))(
						lmsg, vps[0].n, vps[1].n, vps[2].n, vps[3].n, vps[4].n);
				return sr_kemi_apy_return_int(ket, ret);
			} else {
				LM_ERR("invalid parameters for: %.*s\n", fname.len, fname.s);
				return sr_kemi_apy_return_false();
		...
		case 6:
			if(ket->ptypes[0] == SR_KEMIP_STR && ket->ptypes[1] == SR_KEMIP_STR
					...
					&& ket->ptypes[5] == SR_KEMIP_STR) {
				/* ssssss */
				ret = ((sr_kemi_fmssssss_f)(ket->func))(lmsg, &vps[0].s,
						&vps[1].s, &vps[2].s, &vps[3].s, &vps[4].s, &vps[5].s);
				return sr_kemi_apy_return_int(ket, ret);
			} else {
				LM_ERR("invalid parameters for: %.*s\n", fname.len, fname.s);
				return sr_kemi_apy_return_false();
	...
	if(unlikely(cfg_get(core, core_cfg, latency_limit_action) > 0)
		...
		if(tdiff >= cfg_get(core, core_cfg, latency_limit_action)) {
			...
			LOG(cfg_get(core, core_cfg, latency_log),
					...
					(ket->mname.len > 0) ? ket->mname.s : "",
					(ket->mname.len > 0) ? "." : "", ket->fname.s, tdiff,
					(pframe && pframe->f_code)
							? PyBytes_AsString(pframe->f_code->co_filename)
							: "",
					(pframe && pframe->f_code)
							? PyBytes_AsString(pframe->f_code->co_name)
							: "",
					(pframe && pframe->f_code)
...
#define SR_APY_KSR_METHODS_SIZE \
	(SR_APY_KEMI_EXPORT_SIZE + SR_APY_KSR_MODULES_SIZE)
...
static struct PyModuleDef KSR_moduledef = {
		PyModuleDef_HEAD_INIT, "KSR", NULL, -1, NULL, NULL, NULL, NULL, NULL};
...
static struct PyModuleDef KSR_x_moduledef = {PyModuleDef_HEAD_INIT, "KSR.x",
		NULL, -1, _sr_apy_kemi_x_Methods, NULL, NULL, NULL, NULL};
...
static PyObject *init_KSR(void)
{
	...
	_sr_apy_ksr_modules_list =
			malloc(SR_APY_KSR_MODULES_SIZE * sizeof(PyObject *));
	if(_sr_apy_ksr_modules_list == NULL) {
		LM_ERR("no more pkg memory\n");
		return NULL;
	}
	memset(_sr_KSRMethods, 0, SR_APY_KSR_METHODS_SIZE * sizeof(PyMethodDef));
	memset(_sr_apy_ksr_modules_list, 0,
			SR_APY_KSR_MODULES_SIZE * sizeof(PyObject *));
	...
	if(emods_size == 0 || emods[0].kexp == NULL) {
		...
		_sr_crt_KSRMethods[0].ml_meth =
				sr_apy_kemi_export_associate(&_sr_apy_kemi_test[0]);
		...
		for(i = 0; emods[0].kexp[i].func != NULL; i++) {
			...
			if(_sr_crt_KSRMethods[i].ml_meth == NULL) {
				LM_ERR("failed to associate kemi function with python "
					   "export\n");
				free(_sr_KSRMethods);
				_sr_KSRMethods = NULL;
				return NULL;
...
static const char *app_python_rpc_reload_doc[2] = {"Reload python file", 0};
...
static void app_python_rpc_reload(rpc_t *rpc, void *ctx)
{
	...
	*_sr_python_reload_version += 1;
	LM_INFO("marking for reload Python script file: %.*s (%d => %d)\n",
			_sr_python_load_file.len, _sr_python_load_file.s, v,
	...
	rpc->struct_add(vh, "dd", "old", v, "new", *_sr_python_reload_version);
...
rpc_export_t app_python_rpc_cmds[] = {
		{"app_python.reload", app_python_rpc_reload, app_python_rpc_reload_doc,
				0},
