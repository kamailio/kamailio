...
tls_tracker_ops_api_t hc_tls_tracker_ops;
...
int prepare_keylog_file(str session_keylog_filename)
{
	...
	keylog_file = (char *)pkg_mallocxz(
			session_keylog_filename.len * sizeof(char) + 1);
	if(keylog_file == NULL) {
		PKG_MEM_ERROR;
		return -1;
	}
    ...
    */
	...
	if(bio_keylog == NULL) {
		LM_DBG("Error opening keylog file: %.*s\n", session_keylog_filename.len,
				session_keylog_filename.s);
		return -1;
	}
...
static int mod_init(void)
{
	...
	if(load_tls_tracker_ops_api(&hc_tls_tracker_ops) < 0) {
