...
int auth_data_hash_size =
		1024; /**< the size of the hash table 							*/
int auth_vector_timeout =
		60; /**< timeout for a sent auth vector to expire in sec 		*/
int auth_used_vector_timeout =
		3600; /**< timeout for a used auth vector to expire in sec 		*/
int max_nonce_reuse =
		0; /**< how many times a nonce can be reused (provided nc is incremented)	*/
int auth_data_timeout =
		60; /**< timeout for a hash entry to expire when empty in sec 	*/
int add_authinfo_hdr =
		1; /**< should an Authentication-Info header be added on 200 OK responses? 	*/
int av_request_at_once =
		1; /**< how many auth vectors to request in a MAR 				*/
int av_request_at_sync =
		1; /**< how many auth vectors to request in a sync MAR 		*/
static str registration_qop = str_init(
		"auth,auth-int"); /**< the qop options to put in the authorization challenges */
static str invite_qop = str_init(
		"auth"); /**< the qop options to put in the authorization challenges for INVITE*/
str registration_qop_str =
		STR_NULL; /**< the qop options to put in the authorization challenges */
str invite_qop_str = STR_NULL;
int av_check_only_impu =
		0; /**< Should we check IMPU (0) or IMPU and IMPI (1), when searching for authentication vectors? */
static str s_qop_s = str_init(", qop=\"");
static str s_qop_e = str_init("\"");

static str registration_default_algorithm = str_init(
		"AKAv1-MD5"); /**< default algorithm for registration (if none present)*/
unsigned char registration_default_algorithm_type =
		1; /**< fixed default algorithm for registration (if none present)	 */
...
str scscf_name_str = str_init(
		"sip:scscf.ims.smilecoms.com:6060"); /**< fixed name of the S-CSCF 							*/
...
static int mod_init(void)
{
	registration_default_algorithm_type =
			get_algorithm_type(registration_default_algorithm);
	...
	if(auth_used_vector_timeout < 0) {
		LM_WARN("bad value for auth_used_vector_timeout parameter (=%d), must "
				"be positive. Fixed to 3600\n",
	...
	if(max_nonce_reuse < 0) {
		LM_WARN("bad value for max_nonce_reuse parameter (=%d), must be "
				"positive. Fixed to 0\n",
				max_nonce_reuse);
		max_nonce_reuse = 0;
	}
	...
	if(registration_qop.s && registration_qop.len > 0) {
		registration_qop_str.len =
				s_qop_s.len + registration_qop.len + s_qop_e.len;
	...
	if(invite_qop.s && invite_qop.len > 0) {
		invite_qop_str.len = s_qop_s.len + invite_qop.len + s_qop_e.len;
		...
		memcpy(invite_qop_str.s + invite_qop_str.len, invite_qop.s,
				invite_qop.len);
