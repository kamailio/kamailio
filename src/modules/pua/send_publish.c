		...
		str *extra_headers, int is_body)
{
	...
	if(is_body) {
		if(content_type == NULL || content_type->s == NULL
				|| content_type->len == 0) {
...
static int find_and_update_record(
		ua_pres_t *dialog, int hash_code, int lexpire, str *etag)
{
	ua_pres_t *presentity;
	...
	if(dbmode == PUA_DB_ONLY) {
		...
		update_htable(presentity, dialog->desired_expires, lexpire, etag,
				hash_code, NULL);
		lock_release(&HashT->p_records[hash_code].lock);
		return 1;
	}
...
void publ_cback_func(struct cell *t, int type, struct tmcb_params *ps)
{
	...
	LM_DBG("completed with status %d [contact:%.*s]\n", ps->code,
			hentity->pres_uri->len, hentity->pres_uri->s);
	...
	size = sizeof(ua_pres_t) + sizeof(str)
		   + (hentity->pres_uri->len + hentity->tuple_id.len + hentity->id.len)
					 * sizeof(char);
	...
	memcpy(presentity->tuple_id.s, hentity->tuple_id.s, hentity->tuple_id.len);
...
int send_publish(publ_info_t *publ)
{
	...
	insert:
		...
		if(presentity->tuple_id.s && presentity->tuple_id.len) {
			...
			tuple_id->s =
					(char *)pkg_malloc(presentity->tuple_id.len * sizeof(char));
			if(tuple_id->s == NULL) {
				PKG_MEM_ERROR;
				if(dbmode != PUA_DB_ONLY)
					lock_release(&HashT->p_records[hash_code].lock);
			...
			memcpy(tuple_id->s, presentity->tuple_id.s,
					presentity->tuple_id.len);
...
send_publish:
	...
	if(publ->flag & UPDATE_TYPE)
		LM_DBG("etag:%.*s\n", etag.len, etag.s);
	str_hdr = publ_build_hdr(
			(publ->expires < 0) ? pua_default_expires : publ->expires, ev,
			&publ->content_type, (publ->flag & UPDATE_TYPE) ? &etag : NULL,
	...
	result = tmb.t_request(&uac_r, publ->pres_uri, /*! Request-URI */
			publ->pres_uri,						   /*! To */
			publ->pres_uri,						   /*! From */
			publ->outbound_proxy ? publ->outbound_proxy
		...
		publ_info_t *publ, str *body, str *tuple_id, int ua_flag)
{
	...
	size = sizeof(ua_pres_t) + sizeof(str)
		   + (publ->pres_uri->len + +publ->content_type.len + publ->id.len + 1)
					 * sizeof(char);
	...
	memcpy(cb_param->pres_uri->s, publ->pres_uri->s, publ->pres_uri->len);
	...
	if(body && body->s && body->len) {
		...
		memcpy(cb_param->body->s, body->s, body->len);
		cb_param->body->len = body->len;
		size += body->len;
	}
	if(publ->etag) {
		cb_param->etag.s = (char *)cb_param + size;
		memcpy(cb_param->etag.s, publ->etag->s, publ->etag->len);
	...
	if(publ->content_type.s && publ->content_type.len) {
		...
		memcpy(cb_param->content_type.s, publ->content_type.s,
