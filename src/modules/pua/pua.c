...
static cmd_export_t cmds[] = {
		{"pua_update_contact", (cmd_function)w_pua_update_contact, 0, 0, 0,
				REQUEST_ROUTE},
...
static int mod_init(void)
{
	...
	if(db_check_table_version(&pua_dbf, pua_db, &db_table, PUA_TABLE_VERSION)
			< 0) {
...
static int db_restore(void)
{
	...
	result_cols[desired_expires_col = n_result_cols++] =
			&str_desired_expires_col;
	...
	if(db_fetch_query(&pua_dbf, pua_fetch_rows, pua_db, 0, 0, 0, result_cols, 0,
			   n_result_cols, 0, &res)
			< 0) {
	...
	do {
		...
		for(i = 0; i < res->n; i++) {
			...
			extra_headers.s =
					(char *)row_vals[extra_headers_col].val.string_val;
			if(extra_headers.s)
			...
			size = sizeof(ua_pres_t) + sizeof(str)
				   + (pres_uri.len + pres_id.len + tuple_id.len) * sizeof(char);
			if(extra_headers.s)
				size += sizeof(str) + extra_headers.len * sizeof(char);

			if(watcher_uri.s)
				size += sizeof(str)
						+ (watcher_uri.len + call_id.len + to_tag.len
			...
			if(watcher_uri.s && watcher_uri.len) {
				...
				p->remote_contact.s =
						(char *)shm_malloc(remote_contact.len * sizeof(char));
				if(p->remote_contact.s == NULL) {
					SHM_MEM_ERROR;
					goto error;
				}
				memcpy(p->remote_contact.s, remote_contact.s,
						remote_contact.len);
...
int update_pua(ua_pres_t *p)
{
	...
	if(p->watcher_uri == NULL || p->watcher_uri->len == 0) {
		...
		str_hdr = publ_build_hdr(expires, get_event(p->event), NULL, &p->etag,
				p->extra_headers, 0);
		if(str_hdr == NULL) {
		...
		result = tmb.t_request(&uac_r, p->pres_uri, /* Request-URI */
				p->pres_uri,						/* To */
				p->pres_uri,						/* From */
		...
		str_hdr = subs_build_hdr(
				&p->contact, expires, p->event, p->extra_headers);
...
static void db_update(unsigned int ticks, void *param)
{
	...
	int puri_col, pid_col, expires_col, flag_col, etag_col, tuple_col,
			event_col;
	int watcher_col, callid_col, totag_col, fromtag_col, record_route_col,
			cseq_col;
		...
		while(p) {
			...
			switch(p->db_flag) {
				...
				case UPDATEDB_FLAG: {
					...
					if(pua_dbf.query(pua_db, q_cols, 0, q_vals, result_cols,
							   n_query_update, 1, 0, &res)
							< 0) {
					...
					if(res && res->n > 0) {
						if(pua_dbf.update(pua_db, q_cols, 0, q_vals, db_cols,
								   db_vals, n_query_update, n_update_cols)
								< 0) {
				...
				case INSERTDB_FLAG: {
					...
					q_vals[desired_expires_col].val.int_val =
							p->desired_expires;
					q_vals[event_col].val.int_val = p->event;
					...
					if(p->remote_contact.s) {
						q_vals[remote_contact_col].val.str_val =
								p->remote_contact;
					...
					if(p->extra_headers)
						q_vals[extra_headers_col].val.str_val =
								*(p->extra_headers);
					else {
					...
					if(pua_dbf.insert(pua_db, q_cols, q_vals, n_query_cols)
							< 0) {
...
static const char *pua_rpc_cleanup_doc[2] = {"Execute pua cleanup.", 0};

rpc_export_t pua_rpc[] = {
		{"pua.cleanup", pua_rpc_cleanup, pua_rpc_cleanup_doc, 0}, {0, 0, 0, 0}};
