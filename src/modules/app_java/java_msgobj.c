...
jobject *fill_sipmsg_object(JNIEnv *env, struct sip_msg *msg)
{
	...
	fid = (*env)->GetFieldID(
			env, SipMsgClass, "unparsed", "Ljava/lang/String;");
	if(!fid) {
		(*env)->ExceptionClear(env);
	...
	jStrParam = (*env)->NewStringUTF(
			env, msg->new_uri.len <= 0 ? "" : msg->new_uri.s);
	(*env)->SetObjectField(env, SipMsgInstance, fid, jStrParam);
	if((*env)->ExceptionCheck(env)) {
		handle_exception();
		return NULL;
	...
	jStrParam = (*env)->NewStringUTF(
			env, msg->dst_uri.len <= 0 ? "" : msg->dst_uri.s);
	(*env)->SetObjectField(env, SipMsgInstance, fid, jStrParam);
	if((*env)->ExceptionCheck(env)) {
		handle_exception();
		return NULL;
	...
	if(!fid) {
		(*env)->ExceptionClear(env);
		LM_ERR("%s: Can't find symbol "
			   "org.siprouter.SipMsg.parsed_orig_ruri_ok\n",
				APP_NAME);
		return NULL;
	}
	...
	fid = (*env)->GetFieldID(
			env, SipMsgClass, "add_to_branch_s", "Ljava/lang/String;");
	if(!fid) {
		(*env)->ExceptionClear(env);
	...
	jStrParam = (*env)->NewStringUTF(env,
			(msg->add_to_branch_len <= 0) ? "" : strdup(msg->add_to_branch_s));
	(*env)->SetObjectField(env, SipMsgInstance, fid, jStrParam);
	if((*env)->ExceptionCheck(env)) {
		handle_exception();
		return NULL;
	...
	fid = (*env)->GetFieldID(
			env, SipMsgClass, "set_global_address", "Ljava/lang/String;");
	if(!fid) {
		(*env)->ExceptionClear(env);
		LM_ERR("%s: Can't find symbol "
			   "org.siprouter.SipMsg.set_global_address\n",
				APP_NAME);
		return NULL;
	}
	jStrParam = (*env)->NewStringUTF(
	...
	fid = (*env)->GetFieldID(
			env, SipMsgClass, "set_global_port", "Ljava/lang/String;");
	if(!fid) {
		(*env)->ExceptionClear(env);
	...
	jStrParam = (*env)->NewStringUTF(env,
			(msg->set_global_port.len <= 0 || msg->set_global_port.s == NULL)
					? ""
					: msg->set_global_port.s);
	(*env)->SetObjectField(env, SipMsgInstance, fid, jStrParam);
	if((*env)->ExceptionCheck(env)) {
		handle_exception();
		return NULL;
