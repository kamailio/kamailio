...
void bin_print(bin_data *x)
{
	...
	for(i = 0; i < x->len; i += w) {
		...
		for(j = 0; j < w; j++) {
			if(i + j < x->len)
				fprintf(stderr, "%02X ", (unsigned char)x->s[i + j]);
			else
				fprintf(stderr, "   ");
		}
		printf("\t");
...
int bin_decode_short(bin_data *x, short *v)
{
	...
	if(x->max + 2 > x->len)
		return 0;
#endif
	*v = (unsigned char)x->s[x->max] | (unsigned char)x->s[x->max + 1] << 8;
	x->max += 2;
#if BIN_DEBUG
...
int bin_encode_int(bin_data *x, int k)
{
	...
	switch(len) {
		case 4:
			LM_ERR(":bin_encode_int: [%d]:[%.02x %.02x %.02x %.02x] new len "
				   "%04x\n",
					k, x->s[x->len - 4], x->s[x->len - 3], x->s[x->len - 2],
					x->s[x->len - 1], x->len);
			break;
		case 8:
			LM_ERR("bin_encode_int: [%d]:[%.02x %.02x %.02x %.02x%.02x %.02x "
				   "%.02x %.02x] new len %04x\n",
					k, x->s[x->len - 8], x->s[x->len - 7], x->s[x->len - 6],
					x->s[x->len - 5], x->s[x->len - 4], x->s[x->len - 3],
					x->s[x->len - 2], x->s[x->len - 1], x->len);
			break;
	}
#endif
...
int bin_encode_uint(bin_data *x, unsigned int k)
{
	...
	switch(len) {
		case 4:
			LM_ERR("bin_encode_uint: [%u]:[%.02x %.02x %.02x %.02x] new len "
				   "%04x\n",
					k, x->s[x->len - 4], x->s[x->len - 3], x->s[x->len - 2],
					x->s[x->len - 1], x->len);
			break;
		case 8:
			LM_ERR("bin_encode_uint: [%u]:[%.02x %.02x %.02x %.02x%.02x %.02x "
				   "%.02x %.02x] new len %04x\n",
					k, x->s[x->len - 8], x->s[x->len - 7], x->s[x->len - 6],
					x->s[x->len - 5], x->s[x->len - 4], x->s[x->len - 3],
					x->s[x->len - 2], x->s[x->len - 1], x->len);
...
int bin_encode_time_t(bin_data *x, time_t k)
{
	...
	switch(len) {
		case 4:
			LM_ERR("bin_encode_time_t: [%u]:[%.02x %.02x %.02x %.02x] new len "
				   "%04x\n",
					(unsigned int)k, x->s[x->len - 4], x->s[x->len - 3],
					x->s[x->len - 2], x->s[x->len - 1], x->len);
			break;
		case 8:
			LM_ERR("bin_encode_time_t: [%u]:[%.02x %.02x %.02x %.02x%.02x "
				   "%.02x %.02x %.02x] new len %04x\n",
					(unsigned int)k, x->s[x->len - 8], x->s[x->len - 7],
					x->s[x->len - 6], x->s[x->len - 5], x->s[x->len - 4],
					x->s[x->len - 3], x->s[x->len - 2], x->s[x->len - 1],
					x->len);
			break;
	}
#endif
...
{
	if(!bin_expand(x, 2 + s->len))
		return 0;
	if(s->len > 65535)
		LM_ERR("bin_encode_str: Possible loss of characters in encoding "
			   "(string > 65535bytes) %d bytes \n",
				s->len);
	...
	LM_ERR(":bin_encode_str : [%d]:[%.02x %.02x]:[%.*s] new len %04x\n", s->len,
			x->s[x->len - s->len - 2], x->s[x->len - s->len - 1], s->len, s->s,
			x->len);
#endif
	return 1;
}
...
int bin_decode_str(bin_data *x, str *s)
{
	...
	if(x->max + 2 > x->len)
		return 0;
#endif
	s->len = (unsigned char)x->s[x->max] | (unsigned char)x->s[x->max + 1] << 8;
	x->max += 2;
	if(x->max + s->len > x->len)
		return 0;
	s->s = x->s + x->max;
	x->max += s->len;
#if BIN_DEBUG
...
{
	if(!bin_encode_char(x, pi->barring))
		goto error;
	if(!bin_encode_str(x, &(pi->public_identity)))
		goto error;
	return 1;
error:
...
{
	str s;
	if(!bin_decode_char(x, &(pi->barring)))
		goto error;
	if(!bin_decode_str(x, &s) || !str_shm_dup(&(pi->public_identity), &s))
		goto error;
...
error:
	LM_ERR("bin_decode_public_identity: Error while decoding (at %d (%04x)).\n",
			x->max, x->max);
	if(pi) {
		if(pi->public_identity.s)
			shm_free(pi->public_identity.s);
	}
	return 0;
}
...
{
	unsigned char c = spt->condition_negated << 7 | spt->registration_type << 4
					  | spt->type;
	// cond negated, reg type, spt type
	if(!bin_encode_uchar(x, c))
		goto error;

	//group
	if(!bin_encode_int(x, spt->group))
		goto error;

	//spt
	switch(spt->type) {
		case 1:
			if(!bin_encode_str(x, &(spt->request_uri)))
				goto error;
			break;
		case 2:
			if(!bin_encode_str(x, &(spt->method)))
				goto error;
			break;
		case 3:
			if(!bin_encode_short(x, spt->sip_header.type))
				goto error;
			if(!bin_encode_str(x, &(spt->sip_header.header)))
				goto error;
			if(!bin_encode_str(x, &(spt->sip_header.content)))
				goto error;
			break;
		case 4:
			if(!bin_encode_char(x, spt->session_case))
				goto error;
			break;
		case 5:
			if(!bin_encode_str(x, &(spt->session_desc.line)))
				goto error;
			if(!bin_encode_str(x, &(spt->session_desc.content)))
				goto error;
			break;
	}
	return 1;
...
static int bin_decode_spt(bin_data *x, ims_spt *spt)
{
	...
	if(!bin_decode_int(x, &(spt->group)))
		goto error;

	switch(spt->type) {
		case 1:
			if(!bin_decode_str(x, &s) || !str_shm_dup(&(spt->request_uri), &s))
				goto error;
			break;
		case 2:
			if(!bin_decode_str(x, &s) || !str_shm_dup(&(spt->method), &s))
				goto error;
			break;
		case 3:
			if(!bin_decode_short(x, &(spt->sip_header.type)))
				goto error;
			if(!bin_decode_str(x, &s)
					|| !str_shm_dup(&(spt->sip_header.header), &s))
				goto error;
			if(!bin_decode_str(x, &s)
					|| !str_shm_dup(&(spt->sip_header.content), &s))
				goto error;
			break;
		case 4:
			if(!bin_decode_char(x, &(spt->session_case)))
				goto error;
			break;
		case 5:
			if(!bin_decode_str(x, &s)
					|| !str_shm_dup(&(spt->session_desc.line), &s))
				goto error;
			if(!bin_decode_str(x, &s)
					|| !str_shm_dup(&(spt->session_desc.content), &s))
				goto error;
			break;
	}
	return 1;
...
static int bin_encode_filter_criteria(bin_data *x, ims_filter_criteria *fc)
{
	...
	//priority
	if(!bin_encode_int(x, fc->priority))
		goto error;

	//profile part indicator
	if(fc->profile_part_indicator)
		ppindicator = (*fc->profile_part_indicator) + 1;
	else
		ppindicator = 0;
	if(!bin_encode_char(x, ppindicator))
		goto error;

	// trigger point
	if(fc->trigger_point) {
		if(!bin_encode_char(x, fc->trigger_point->condition_type_cnf))
			goto error;

		if(!bin_encode_ushort(x, fc->trigger_point->spt_cnt))
			goto error;

		for(i = 0; i < fc->trigger_point->spt_cnt; i++)
			if(!bin_encode_spt(x, fc->trigger_point->spt + i))
				goto error;
	} else {
		if(!bin_encode_char(x, 100))
			goto error;
	}

	//app server
	if(!bin_encode_str(x, &(fc->application_server.server_name)))
		goto error;
	if(!bin_encode_char(x, fc->application_server.default_handling))
		goto error;
	if(!bin_encode_str(x, &(fc->application_server.service_info)))
		goto error;
...
static int bin_decode_filter_criteria(bin_data *x, ims_filter_criteria *fc)
{
	...
	//priority
	if(!bin_decode_int(x, &(fc->priority)))
		goto error;

	// profile part indicator
	if(!bin_decode_char(x, &ppindicator))
		goto error;
	if(!ppindicator) {
	...
	if(cnf == 100)
		...
		if(!bin_decode_ushort(x, &tp->spt_cnt))
			goto error;
		...
		for(i = 0; i < tp->spt_cnt; i++)
			if(!bin_decode_spt(x, tp->spt + i))
				goto error;
	}
	//app server uri
	if(!bin_decode_str(x, &s)
			|| !str_shm_dup(&(fc->application_server.server_name), &s))
		goto error;
	// app server default handling
	if(!bin_decode_char(x, &(fc->application_server.default_handling)))
		goto error;
	// app server service info
	if(!bin_decode_str(x, &s)
			|| !str_shm_dup(&(fc->application_server.service_info), &s))
		goto error;
...
error:
	...
	if(fc) {
		if(fc->trigger_point) {
			if(fc->trigger_point) {
				if(fc->trigger_point->spt)
					shm_free(fc->trigger_point->spt);
			}
			shm_free(fc->trigger_point);
		}
		if(fc->application_server.server_name.s)
			shm_free(fc->application_server.server_name.s);
		if(fc->application_server.service_info.s)
			shm_free(fc->application_server.service_info.s);
	}
	return 0;
}
...
static int bin_encode_service_profile(bin_data *x, ims_service_profile *sp)
{
	...
	//public identity
	if(!bin_encode_ushort(x, sp->public_identities_cnt))
		return 0;
	for(i = 0; i < sp->public_identities_cnt; i++)
		if(!bin_encode_public_identity(x, sp->public_identities + i))
			goto error;

	//filter criteria
	if(!bin_encode_ushort(x, sp->filter_criteria_cnt))
		return 0;
	for(i = 0; i < sp->filter_criteria_cnt; i++)
		if(!bin_encode_filter_criteria(x, sp->filter_criteria + i))
			goto error;
	...
	if(sp->cn_service_auth)
		...
		i = 0xFFFFFFFF;
	if(!bin_encode_int(x, i))
		goto error;

	//shared_ifc
	if(!bin_encode_ushort(x, sp->shared_ifc_set_cnt))
		return 0;
	for(i = 0; i < sp->shared_ifc_set_cnt; i++)
		if(!bin_encode_int(x, sp->shared_ifc_set[i]))
			goto error;
...
static int bin_decode_service_profile(bin_data *x, ims_service_profile *sp)
{
	...
	//public identities
	if(!bin_decode_ushort(x, &(sp->public_identities_cnt)))
		goto error;
	...
	for(i = 0; i < sp->public_identities_cnt; i++)
		if(!bin_decode_public_identity(x, sp->public_identities + i))
			goto error;

	// filter criteria
	if(!bin_decode_ushort(x, &(sp->filter_criteria_cnt)))
		goto error;
	...
	for(i = 0; i < sp->filter_criteria_cnt; i++)
		if(!bin_decode_filter_criteria(x, sp->filter_criteria + i))
			goto error;

	// cn service auth
	if(!bin_decode_int(x, &i))
		goto error;
	if(i == 0xFFFFFFFF)
	...
	//shared ifc
	if(!bin_decode_ushort(x, &(sp->shared_ifc_set_cnt)))
		goto error;
	...
	for(i = 0; i < sp->shared_ifc_set_cnt; i++)
		if(!bin_decode_int(x, sp->shared_ifc_set + i))
			goto error;
...
error:
	...
	if(sp) {
		if(sp->public_identities)
			shm_free(sp->public_identities);
		if(sp->filter_criteria)
			shm_free(sp->filter_criteria);
		if(sp->cn_service_auth)
			shm_free(sp->cn_service_auth);
		if(sp->shared_ifc_set)
			shm_free(sp->shared_ifc_set);
	}
	return 0;
}
...
{
	int i;
	if(!bin_encode_str(x, &(s->private_identity)))
		goto error;
	if(!bin_encode_ushort(x, s->service_profiles_cnt))
		goto error;

	for(i = 0; i < s->service_profiles_cnt; i++)
		if(!bin_encode_service_profile(x, s->service_profiles + i))
			goto error;
...
ims_subscription *bin_decode_ims_subscription(bin_data *x)
{
	...
	imss->sl =
			-1; /* we set this to -1 as it's not currently in a slot at creation stage */

	if(!bin_decode_str(x, &s) || !str_shm_dup(&(imss->private_identity), &s))
		goto error;
	if(!bin_decode_ushort(x, &(imss->service_profiles_cnt)))
		goto error;
	...
	for(i = 0; i < imss->service_profiles_cnt; i++)
		if(!bin_decode_service_profile(x, imss->service_profiles + i))
			goto error;
	...
	return imss;
error:
	LM_ERR("bin_decode_ims_subscription: Error while decoding (at %d "
		   "(%04x)).\n",
			x->max, x->max);
	if(imss) {
		if(imss->private_identity.s)
			shm_free(imss->private_identity.s);
		if(imss->service_profiles)
			shm_free(imss->service_profiles);
		shm_free(imss);
	}
	return 0;
}
