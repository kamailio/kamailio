...
#define is_acc_flag_set(_rq, _flag) \
	(((_flag) != -1) && (isflagset((_rq), (_flag)) == 1))
...
static inline struct hdr_field *get_rpl_to(
		struct cell *t, struct sip_msg *reply)
{
...
static inline void env_set_code_status(int code, struct sip_msg *reply)
{
	...
	if(reply == FAKED_REPLY || reply == NULL) {
		...
		if(acc_env.reason.len == 0
				|| reply == NULL) { /* already extracted in case of locally generated replies */
			acc_env.reason.s = error_text(code);
			acc_env.reason.len = strlen(acc_env.reason.s);
		}
	} else {
...
{
	if((parse_headers(req, HDR_CALLID_F | HDR_CSEQ_F | HDR_FROM_F | HDR_TO_F, 0)
			   < 0)
...
void acc_onreq(struct cell *t, int type, struct tmcb_params *ps)
{
	...
	if(ps->req && !skip_cancel(ps->req)
		...
		tmcb_types =
				...
				((is_invite
						 && (is_mc_on(ps->req) || is_acc_prepare_on(ps->req)))
								? TMCB_ON_FAILURE
								: 0);
...
static inline int should_acc_reply(
		struct sip_msg *req, struct sip_msg *rpl, int code)
{
	...
	if(code < 200
			&& !(early_media && parse_headers(rpl, HDR_CONTENTLENGTH_F, 0) == 0
...
static inline void acc_onreply_in(
		struct cell *t, struct sip_msg *req, struct sip_msg *reply, int code)
{
	/* don't parse replies in which we are not interested */
	/* missed calls enabled ? */
	if((reply && reply != FAKED_REPLY)
			&& (should_acc_reply(req, reply, code)
...
static void acc_onreply(
		tm_cell_t *t, sip_msg_t *req, sip_msg_t *reply, int code)
{
	str new_uri_bk;
	int br = -1;
	hdr_field_t *hdr;
...
static inline void acc_onack(
		struct cell *t, struct sip_msg *req, struct sip_msg *ack, int code)
{
