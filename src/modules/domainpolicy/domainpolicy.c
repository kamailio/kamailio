...
int domainpolicy_db_ver(const str *db_url)
{
	...
	if(db_check_table_version(&domainpolicy_dbf, dbh, &domainpolicy_table,
			   DOMAINPOLICY_TABLE_VERSION)
			< 0) {
...
static int check_rule(
		str *rule, char *service, int service_len, struct avp_stack *stack)
{
	...
	if((service_len != 11)
			|| (strncasecmp("d2p+sip:fed", service, 11)
	...
	if(domainpolicy_dbf.query(db_handle, keys, 0, vals, cols, 2, 4, 0, &res)
			< 0) {
...
int dp_can_connect_str(str *domain, int rec_level)
{
	 ...
	 */
	     ...
	     */
			if(next_naptr && (next_naptr->order == naptr->order)
					&& IS_D2PNAPTR(next_naptr)) {
	 ...
	 */
		...
		if(reg_replace(pattern.s, replacement.s, domain->s, &result) < 0) {
		...
		if(ret == 1) {
			...
			if(rec_level > 0) {
				stack_push(
						&stack, domain_replacement_name.s.s, (char *)domain->s);
				stack.succeeded = 0;
			}
			failed = 1;
		} else {
...
int dp_apply_policy(struct sip_msg *_msg, char *_s1, char *_s2)
{
	...
	didsomething =
			0; /* if no AVP is set, there is no need to set the DURI in the end */
	...
	avp = search_first_avp(
			domain_prefix_avp_name_str, domain_prefix_name, &val, 0);
	if(avp) {
		if(!(avp->flags & AVP_VAL_STR) || !val.s.s || !val.s.len) {
		...
		memcpy(at, val.s.s, val.s.len);
		at = at + val.s.len;
		*at = '.';
		at = at + 1; /* add . as delimiter between prefix and domain */
		didsomething = 1;
	} else {
	...
	avp = search_first_avp(
			domain_replacement_avp_name_str, domain_replacement_name, &val, 0);
	if(avp) {
		if(!(avp->flags & AVP_VAL_STR) || !val.s.s || !val.s.len) {
		...
		memcpy(at, domain->s, domain->len);
		at = at + domain->len;
	}

	/* search for suffix and add it to duri buffer */
	avp = search_first_avp(
			domain_suffix_avp_name_str, domain_suffix_name, &val, 0);
	if(avp) {
		if(!(avp->flags & AVP_VAL_STR) || !val.s.s || !val.s.len) {
		...
		*at = '.';
		at = at + 1; /* add . as delimiter between domain and suffix */
		memcpy(at, val.s.s, val.s.len);
		at = at + val.s.len;
		didsomething = 1;
	} else {
	...
	avp = search_first_avp(
			port_override_avp_name_str, port_override_name, &val, 0);
	if(avp) {
		if(!(avp->flags & AVP_VAL_STR) || !val.s.s || !val.s.len) {
		...
		*at = ':';
		at = at + 1; /* add : as delimiter between domain and port */
		memcpy(at, val.s.s, val.s.len);
		at = at + val.s.len;
		didsomething = 1;
	} else {
	...
	avp = search_first_avp(
			transport_override_avp_name_str, transport_override_name, &val, 0);
	if(avp) {
		if(!(avp->flags & AVP_VAL_STR) || !val.s.s || !val.s.len) {
		...
		if(_msg->parsed_uri.transport.len) {
			...
			*at = ';';
			at = at + 1; /* add : as delimiter between domain and port */
			memcpy(at, _msg->parsed_uri.transport.s,
					_msg->parsed_uri.transport.len);
			at = at + _msg->parsed_uri.transport.len;
		} else {
