...
static param_export_t params[] = {{"workers", INT_PARAM, &_evapi_workers},
		...
		{"max_clients", PARAM_INT, &_evapi_max_clients}, {0, 0, 0}};
...
static int evapi_get_data(sip_msg_t *msg, pv_elem_t *pdata, str *sdata)
{
	...
	if(ret < 0) {
		if(ret == -2) {
			...
			_evapi_data.s =
					(char *)evapi_malloc(_evapi_data_size * sizeof(char));
			if(_evapi_data.s == NULL) {
			...
			_evapi_data.len = _evapi_data_size;
			ret = pv_printf_mode(
					msg, pdata, 0, _evapi_data.s, &_evapi_data.len);
			if(ret < 0) {
