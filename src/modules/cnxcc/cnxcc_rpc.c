...
void rpc_check_client_stats(rpc_t *rpc, void *ctx)
{
	...
	clist_foreach_safe(credit_data->call_list, call, tmp, next)
	{
		...
		if(credit_data->type == CREDIT_MONEY)
			...
			snprintf(row_buffer, sizeof(row_buffer),
					"id:%d,confirmed:%s,local_consumed_amount:%d,global_"
					"consumed_amount:%d,local_max_amount:%d,global_max_amount:%"
					"d,call_id:%.*s,start_timestamp:%d;",
					index, call->confirmed ? "yes" : "no",
					(int)call->consumed_amount,
					(int)credit_data->consumed_amount, (int)call->max_amount,
					(int)credit_data->max_amount, call->sip_data.callid.len,
					call->sip_data.callid.s, call->start_timestamp);
		...
		hash_tables_t *hts, str *result, credit_type_t type)
{
	...
	if(hts->credit_data_by_client->table)
		for(index = 0; index < hts->credit_data_by_client->size; index++)
			clist_foreach_safe(&hts->credit_data_by_client->table[index],
					h_entry, tmp, next)
			{
				...
				if(type == CREDIT_TIME) {
					snprintf(row_buffer, sizeof(row_buffer),
							...
							credit_data->concurrent_calls, type,
