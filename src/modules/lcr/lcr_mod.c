...
static int mod_init(void)
{
	...
	if((defunct_capability_param > 0) || (ping_interval_param > 0)) {
		if(defunct_gw_avp_param && *defunct_gw_avp_param) {
			...
			if(pv_get_avp_name(0, &(avp_spec->pvp), &defunct_gw_avp, &avp_flags)
					!= 0) {
				LM_ERR("invalid AVP definition <%s>\n", defunct_gw_avp_param);
				return -1;
			}
		...
		if(lcr_id_avp_param && *lcr_id_avp_param) {
			...
			if(pv_get_avp_name(0, &(avp_spec->pvp), &lcr_id_avp, &avp_flags)
					!= 0) {
				LM_ERR("invalid AVP definition <%s>\n", lcr_id_avp_param);
				return -1;
			}
		...
		if(ping_interval_param > 0) {
			...
			while(at < past) {
				...
				if((str2int(&s, &(ping_valid_reply_codes[ping_rc_count])) != 0)
	...
	if(mt_pv_values_param) {
		...
		if(pv_get_avp_name(0, &(avp_spec->pvp), &mt_pv_values_avp, &avp_flags)
				!= 0) {
			LM_ERR("invalid AVP definition <%s>\n", mt_pv_values_param);
			return -1;
		}
		...
		unsigned int tag_len, unsigned int flags, unsigned int defunct_until)
{
	...
	gws[i].gw_id = gw_id;
	if(gw_name_len)
		memcpy(&(gws[i].gw_name[0]), gw_name, gw_name_len);
	...
	if(prefix_len)
		memcpy(&(gws[i].prefix[0]), prefix, prefix_len);
	gws[i].tag_len = tag_len;
	if(tag_len)
		memcpy(&(gws[i].tag[0]), tag, tag_len);
	gws[i].flags = flags;
...
static int prefix_len_insert(
		struct rule_info **table, unsigned short prefix_len)
{
		...
		unsigned int *null_gw_ip_addr, unsigned int *gw_cnt)
{
	...
	for(i = 0; i < RES_ROW_N(res); i++) {
		...
		if(!VAL_NULL(ROW_VALUES(row)) && (VAL_TYPE(ROW_VALUES(row)) != DB1_STR)
		...
		if(VAL_NULL(ROW_VALUES(row) + 3)) {
			...
			switch(VAL_INT(ROW_VALUES(row) + 3)) {
				...
				case SIPS_URI_T:
					...
					break;
				default:
					LM_ERR("lcr_gw has unknown or unsupported URI scheme <%u> "
						   "at "
						   "row <%u>\n",
		...
		if(VAL_NULL(ROW_VALUES(row) + 4)) {
			...
			switch(transport_code) {
				...
				case PROTO_SCTP:
					...
					break;
				default:
					LM_ERR("lcr_gw has unknown or unsupported transport <%u> "
						   "at "
						   " row <%u>\n",
							transport_code, i);
					return 0;
...
int reload_tables()
{
	unsigned int i, n, lcr_id, rule_id, gw_id, from_uri_len, mt_tvalue_len,
			request_uri_len, stopper, prefix_len, enabled, gw_cnt,
			null_gw_ip_addr, priority, weight, tmp;
	...
	for(lcr_id = 1; lcr_id <= lcr_count_param; lcr_id++) {
			...
			for(i = 0; i < RES_ROW_N(res); i++) {
				...
				if(VAL_NULL(ROW_VALUES(row) + 5) == 1) {
					...
					switch(VAL_TYPE(ROW_VALUES(row) + 5)) {
						...
						case DB1_STRING:
							request_uri =
									(char *)VAL_STRING(ROW_VALUES(row) + 5);
				...
				if(!rule_hash_table_insert(rules, lcr_id, rule_id, prefix_len,
						   prefix, from_uri_len, from_uri, from_uri_re,
						   mt_tvalue_len, mt_tvalue, request_uri_len,
		...
		if(!insert_gws(res, gws, &null_gw_ip_addr, &gw_cnt))
			goto err;
		...
		if(DB_CAPABILITY(lcr_dbf, DB_CAP_FETCH)) {
			...
			if(lcr_dbf.query(
					   dbh, key_cols, op, vals, target_cols, 1, 4, 0, &res)
					< 0) {
				LM_ERR("db query on lcr_rule_target table failed\n");
				goto err;
			}
		...
		do {
			...
			for(i = 0; i < RES_ROW_N(res); i++) {
				...
				tmp = rule_hash_table_insert_target(
						rules, gws, rule_id, gw_id, priority, weight);
				if(tmp == 2) {
		...
		unsigned int gw_cnt, str *ruri_user)
{
	...
	for(i = 0; i < gw_cnt; i++) {
		...
		value.len = encode_avp_value(encoded_value, index, gws[index].scheme,
		...
		unsigned int *gw_indexes)
{
	...
	LM_DBG("load_gws_dummy(%u, %.*s, %.*s, %.*s)\n", lcr_id, ruri_user->len,
			ruri_user->s, from_uri->len, from_uri->s, request_uri->len,
			request_uri->s);
		...
		while(rule) {
			...
			if((from_uri->len > 0) && (rule->mt_tvalue_len > 0)) {
				if(mtree_api.mt_match(&msg, &mtree_param, &(furi.user), 2)
					...
					LM_DBG("from uri user <%.*s> was not found in mtree "
						   "<%.*s>\n",
							furi.user.len, furi.user.s, mtree_param.len,
							mtree_param.s);
					goto next;
				}
				for(avp = search_first_avp(
							mt_pv_values_avp_type, mt_pv_values_avp, &val, &st);
						avp; avp = search_next_avp(&st, &val)) {
					if((val.s.len == rule->mt_tvalue_len)
							&& strncmp(val.s.s, rule->mt_tvalue, val.s.len)
									   == 0)
						break;
				}
				if(avp == NULL) {
					LM_DBG("from uri user <%.*s> did not match mt_tvalue "
						   "<%.*s>\n",
							furi.user.len, furi.user.s, rule->mt_tvalue_len,
							rule->mt_tvalue);
					goto next;
				}
			}
			...
			if(rule->request_uri_len != 0) {
				if(request_uri->len == 0) {
					LM_ERR("lcr_rule has non-null request_uri and request_uri "
...
static int ki_defunct_gw(sip_msg_t *_m, int defunct_period)
{
	...
	/* Get AVP values */
	if(search_first_avp(lcr_id_avp_type, lcr_id_avp, &lcr_id_val, 0) == NULL) {
		LM_ERR("lcr_id_avp was not found\n");
		return -1;
	}
	gws = gw_pt[lcr_id_val.n];
	if(search_first_avp(defunct_gw_avp_type, defunct_gw_avp, &index_val, 0)
			== NULL) {
...
static int check_extra_codes(unsigned int code)
{
	...
	for(i = 0; i < ping_rc_count; i++) {
		if(ping_valid_reply_codes[i] == code)
			return 0;
	}
	return -1;
}
