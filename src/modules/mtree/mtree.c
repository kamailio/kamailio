...
int mt_add_to_tree(m_tree_t *pt, str *sp, str *svalue)
{
	...
	if(pt == NULL || sp == NULL || sp->s == NULL || svalue == NULL
			|| svalue->s == NULL) {
	...
	if(pt->type == MT_TREE_IVAL) {
		...
		tvalues->tvalue.s.s =
				(char *)shm_malloc((svalue->len + 1) * sizeof(char));
...
int mt_add_tvalues(struct sip_msg *msg, m_tree_t *pt, str *tomatch)
{
	...
	if(pv_get_avp_name(msg, &pv_values.pvp, &values_avp_name, &values_name_type)
			< 0) {
...
int mt_match_prefix(struct sip_msg *msg, m_tree_t *it, str *tomatch, int mode)
{
	int l, len, n;
	int i, j, k = 0;
	mt_node_t *itn;
...
#define MT_MAX_DST_LIST 64
	...
	if(it == NULL || tomatch == NULL || tomatch->s == NULL) {
		LM_ERR("bad parameters\n");
		return -1;
	}
	...
	if(pv_get_avp_name(msg, &pv_dstid.pvp, &dstid_avp_name, &dstid_name_type)
			< 0) {
		LM_ERR("cannot get dstid avp name\n");
		return -1;
	}
	if(pv_get_avp_name(msg, &pv_weight.pvp, &weight_avp_name, &weight_name_type)
			< 0) {
	...
	for(i = 0; i < MT_NODE_SIZE; i++) {
		...
		while(tvalues != NULL) {
			if(type == MT_TREE_IVAL) {
				...
				LM_INFO("[%.*s] [s:%.*s]\n", len + 1, code,
						tvalues->tvalue.s.len, tvalues->tvalue.s.s);
			}
			tvalues = tvalues->next;
		}
...
int mt_table_spec(char *val)
{
	...
	for(pit = params_list; pit; pit = pit->next) {
		LM_DBG("parm: %.*s=%.*s\n", pit->name.len, pit->name.s, pit->body.len,
				pit->body.s);
...
int mt_rpc_add_tvalues(rpc_t *rpc, void *ctx, m_tree_t *pt, str *tomatch)
{
	...
	while(itn != NULL && l < tomatch->len && l < MT_MAX_DEPTH) {
		...
		while(tvalues != NULL) {
			...
			if(pt->type == MT_TREE_IVAL) {
				if(rpc->struct_add(vstruct, "d", "TVALUE", tvalues->tvalue.n)
						< 0) {
					rpc->fault(ctx, 500, "Internal error adding tvalue");
					return -1;
				}
			} else { /* pt->type == MT_TREE_SVAL */
				if(rpc->struct_add(vstruct, "S", "TVALUE", &tvalues->tvalue.s)
						< 0) {
					rpc->fault(ctx, 500, "Internal error adding tvalue");
					return -1;
				}
...
int mt_rpc_match_prefix(
		rpc_t *rpc, void *ctx, m_tree_t *it, str *tomatch, int mode)
{
	int l, len, n;
	int i, j;
	mt_node_t *itn;
...
#define MT_MAX_DST_LIST 64
	...
	for(i = 0; i < n; i++) {
		if(tmp_list[2 * i] != 0) {
			...
			if(rpc->add(vstruct, "dd", "WEIGHT", tmp_list[2 * i + 1], "DSTID",
					   tmp_list[2 * i])
					< 0) {
