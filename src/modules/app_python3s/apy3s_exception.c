...
static char *apy3s_make_message(const char *fmt, va_list args);

void apy3s_handle_exception(const char *fmt, ...)
{
	PyObject *pResult;
	const char *msg;
	char *buf;
	...
	if(fmt == NULL)
		...
		va_start(ap, fmt);
		srcbuf = apy3s_make_message(fmt, ap);
		va_end(ap);
	}
...
static char *apy3s_make_message(const char *fmt, va_list ap)
{
	int n;
	size_t size;
	char *p, *np;
