...
void tps_shuffle(char *in, int size)
{
	char tmp;
	int last;
	unsigned int r;
	...
	MD5Init(&ctx);
	MD5Update(&ctx, _tps_key.s, _tps_key.len);
	MD5Update(&ctx, _tps_key.s, _tps_key.len);
	U_MD5Final(md5, &ctx);

	md5i = (unsigned int*)md5;

	crc = (unsigned int)crcitt_string(_tps_key.s, _tps_key.len);
	for (last = size; last > 1; last--)
	{
		r = (md5i[(crc+last+_tps_key.len)%4]
				+ _tps_key.s[(crc+last+_tps_key.len)%_tps_key.len]) % last;
		tmp = in[r];
		in[r] = in[last - 1];
		in[last - 1] = tmp;
	}
...
void tps_mask_init(void)
{
	...
	memcpy(_tps_EB64, TPS_EB64I, sizeof(TPS_EB64I));
	tps_shuffle(_tps_EB64, 64);
	LM_DBG("original table: %s\n", TPS_EB64I);
	LM_DBG("updated table: %s\n", _tps_EB64);
	for(i=0; i<256; i++)
		_tps_DB64[i] = -1;
	for(i=0; i<64; i++)
		_tps_DB64[(int)_tps_EB64[i]] = i;
...
char* tps_mask_encode(char *in, int ilen, str *prefix, int *olen)
{
	...
	*olen = (((ilen+2)/3)<<2) + ((prefix!=NULL&&prefix->len>0)?prefix->len:0);
	//LM_DBG("Length of Callid  : %d Olen : %d\n", ilen, *olen);
	...
	for(idx=0; idx<ilen; idx+=3)
	{
		...
		*(p++) = _tps_EB64[(block >> 18) & 0x3f];
		*(p++) = _tps_EB64[(block >> 12) & 0x3f];
		*(p++) = (left>0)?_tps_EB64[(block >> 6) & 0x3f]:_tps_PD64[0];
		*(p++) = (left>1)?_tps_EB64[block & 0x3f]:_tps_PD64[0];
		
		//LM_DBG("OUTpart  : %d  idx = %d ilen= %d p= %d \n", *out,idx,ilen,*p);
	}
	//LM_DBG("Final OUT  : %d ", *out);
	return out;
}
...
char* tps_mask_decode(char *in, int ilen, str *prefix, int extra, int *olen)
{
	...
	for(n=0,i=ilen-1; in[i]==_tps_PD64[0]; i--)
		n++;
