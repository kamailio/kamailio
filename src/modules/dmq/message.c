 ...
 */
int ki_dmq_do_custom(sip_msg_t *msg, str *peer, str *from, str *body,
		str *content_type, int handle)
{
...
#define DMQ_MSG_BUF_SIZE 65507
	...
	wlen = snprintf(buf, sizeof(buf),
			...
			/* Via */
			sproto.len, sproto.s, host.len, host.s, port.len, port.s,
...
int dmq_do_custom(struct sip_msg *msg, char *peer, char *from, char *body,
		char *content_type, int handle)
{
	str peer_str;
	str from_str;
	str body_str;
	...
	return ki_dmq_do_custom(
			msg, &peer_str, &from_str, &body_str, &ct_str, handle);
}

int dmq_process_custom(struct sip_msg *msg, char *peer, char *from, char *body,
		char *content_type)
{
	return dmq_do_custom(msg, peer, from, body, content_type, 0);
}
int ki_dmq_process_custom(
		sip_msg_t *msg, str *peer, str *from, str *body, str *content_type)
{
	return ki_dmq_do_custom(msg, peer, from, body, content_type, 0);
}

int dmq_handle_custom(struct sip_msg *msg, char *peer, char *from, char *body,
		char *content_type)
{
	return dmq_do_custom(msg, peer, from, body, content_type, 1);
}
int ki_dmq_handle_custom(
		sip_msg_t *msg, str *peer, str *from, str *body, str *content_type)
{
