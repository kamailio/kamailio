			...
			if((_len) == -1) {                                              \
				if(_attrs[_attr].t != PW_TYPE_INTEGER) {                    \
					if(_attrs[_attr].t != PW_TYPE_IPADDR) {                 \
						LM_ERR("attribute %d is not of type integer or "    \
							   "ipaddr\n",                                  \
								_attrs[_attr].v);                           \
						goto error;                                         \
					}                                                       \
...
int ki_radius_load_caller_avps(struct sip_msg *_m, str *user)
{
	...
	if(!rc_avpair_add(
			   rh, &send, caller_attrs[SA_SERVICE_TYPE].v, &service, -1, 0)) {
	...
	for(i = 0; i < extra_cnt; i++) {
		if(val_arr[i].len == -1) {
			...
			ADD_EXTRA_AVPAIR(
					caller_attrs, offset + i, &(val_arr[i].s), val_arr[i].len);
		} else {
...
int ki_radius_load_callee_avps(struct sip_msg *_m, str *user)
{
	...
	if(!rc_avpair_add(
			   rh, &send, callee_attrs[SA_SERVICE_TYPE].v, &service, -1, 0)) {
	...
	for(i = 0; i < extra_cnt; i++) {
		if(val_arr[i].len == -1) {
			/* Add integer attribute */
			ADD_EXTRA_AVPAIR(
					callee_attrs, offset + i, &(val_arr[i].s), val_arr[i].len);
		} else {
			/* Add string attribute */
			ADD_EXTRA_AVPAIR(
					callee_attrs, offset + i, val_arr[i].s, val_arr[i].len);
...
int ki_radius_is_user_in(struct sip_msg *_m, str *user, str *group)
{
	...
	if(!rc_avpair_add(rh, &send, group_attrs[SA_SIP_GROUP].v, group->s,
			   group->len, 0)) {
	...
	if(!rc_avpair_add(
			   rh, &send, group_attrs[SA_SERVICE_TYPE].v, &service, -1, 0)) {
		LM_ERR("in adding SA_SERVICE_TYPE <%u>\n", service);
		goto error;
	}
	...
	for(i = 0; i < extra_cnt; i++) {
		if(val_arr[i].len == -1) {
			/* Add integer attribute */
			ADD_EXTRA_AVPAIR(
					group_attrs, offset + i, &(val_arr[i].s), val_arr[i].len);
		} else {
			/* Add string attribute */
			ADD_EXTRA_AVPAIR(
					group_attrs, offset + i, val_arr[i].s, val_arr[i].len);
...
int radius_does_uri_user_host_exist(struct sip_msg *_m, str *user, str *host)
{
	...
	for(i = 0; i < extra_cnt; i++) {
		if(val_arr[i].len == -1) {
			/* Add integer attribute */
			ADD_EXTRA_AVPAIR(
					uri_attrs, offset + i, &(val_arr[i].s), val_arr[i].len);
		} else {
			/* Add string attribute */
			ADD_EXTRA_AVPAIR(
					uri_attrs, offset + i, val_arr[i].s, val_arr[i].len);
