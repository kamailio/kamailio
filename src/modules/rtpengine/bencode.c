...
static bencode_item_t *__bencode_decode(
		bencode_buffer_t *buf, const char *s, const char *end);
...
static void *__bencode_alloc(bencode_buffer_t *buf, unsigned int size)
{
	...
	unsigned int align_size =
			((size + BENCODE_ALLOC_ALIGN - 1) / BENCODE_ALLOC_ALIGN)
			* BENCODE_ALLOC_ALIGN;
...
static bencode_item_t *__bencode_item_alloc(
		bencode_buffer_t *buf, unsigned int payload)
{
	bencode_item_t *ret;
...
static void __bencode_container_add(
		bencode_item_t *parent, bencode_item_t *child)
{
	if(!parent)
		return;
	if(!child)
...
static bencode_item_t *__bencode_string_alloc(bencode_buffer_t *buf,
		const void *base, int str_len, int iov_len, int iov_cnt,
		bencode_type_t type)
{
	bencode_item_t *ret;
	int len_len;
		...
		const struct iovec *iov, int iov_cnt, int str_len)
{
	...
	return __bencode_string_alloc(
			buf, iov, str_len, iov_cnt, iov_cnt, BENCODE_IOVEC);
}
...
static int __bencode_iovec_cpy(
		struct iovec *out, const struct iovec *in, int num)
{
...
static int __bencode_iovec_dump(struct iovec *out, bencode_item_t *item)
{
	...
	if(item->type == BENCODE_IOVEC)
		out += __bencode_iovec_cpy(
				out, item->iov[1].iov_base, item->iov[1].iov_len);
		...
		bencode_item_t *root, int *cnt, unsigned int head, unsigned int tail)
{
	...
	ret = __bencode_alloc(
			root->buffer, sizeof(*ret) * (root->iov_cnt + head + tail));
	if(!ret)
		return NULL;
...
static void __bencode_hash_insert(
		bencode_item_t *key, struct __bencode_hash *hash)
{
...
static bencode_item_t *__bencode_decode_dictionary(
		bencode_buffer_t *buf, const char *s, const char *end)
{
...
static bencode_item_t *__bencode_decode_list(
		bencode_buffer_t *buf, const char *s, const char *end)
{
...
static bencode_item_t *__bencode_decode_integer(
		bencode_buffer_t *buf, const char *s, const char *end)
{
	long long int i;
	const char *orig = s;
...
static bencode_item_t *__bencode_decode_string(
		bencode_buffer_t *buf, const char *s, const char *end)
{
...
static int __bencode_dictionary_key_match(
		bencode_item_t *key, const char *keystr, int keylen)
{
...
bencode_item_t *bencode_dictionary_get_len(
		bencode_item_t *dict, const char *keystr, int keylen)
{
	bencode_item_t *key;
	unsigned int bucket, i;
	struct __bencode_hash *hash;
	...
	if(dict->value == 1) {
		...
		i = bucket =
				__bencode_hash_str_len((const unsigned char *)keystr, keylen);
		while(1) {
			key = hash->buckets[i];
			if(!key)
...
void bencode_buffer_destroy_add(
		bencode_buffer_t *buf, free_func_t func, void *p)
{
