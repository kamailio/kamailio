		...
		struct sip_msg *msg, peer_reponse_t *resp, dmq_node_t *node)
{
	...
	switch(action) {
		case RTPENGINE_DMQ_INSERT:
			...
			if(!rtpp_node) {
				LM_ERR("rtpengine dmq fail to get rtpp node for rtpengine_url=%.*s\n",
		...
		case RTPENGINE_DMQ_REMOVE:
			enum rtpe_operation op = OP_DELETE;
...
int rtpengine_dmq_send(str *body, dmq_node_t *node)
{
	...
	if(node) {
		...
		rtpengine_dmqb.send_message(
				rtpengine_dmq_peer, body, node, NULL, 1, &rtpengine_dmq_content_type);
	} else {
		LM_DBG("sending dmq broadcast...\n");
		rtpengine_dmqb.bcast_message(
				rtpengine_dmq_peer, body, 0, NULL, 1, &rtpengine_dmq_content_type);
	}
	return 0;
}

int rtpengine_dmq_replicate_action(
		rtpengine_dmq_action_t action, str callid, str viabranch,
		struct rtpengine_hash_entry *entry, dmq_node_t *node)
{
	srjson_doc_t jdoc;
	...
	if(viabranch.s != NULL && viabranch.len > 0) {
		srjson_AddStrToObject(&jdoc, jdoc.root, "viabranch", viabranch.s, viabranch.len);
	}
	if(entry && entry->node) {
		srjson_AddStrToObject(&jdoc, jdoc.root, "rtpengine_url", entry->node->rn_url.s, entry->node->rn_url.len);
...
{
	return rtpengine_dmq_replicate_action(RTPENGINE_DMQ_INSERT, callid, viabranch,
			entry, NULL);
}

int rtpengine_dmq_replicate_remove(
		str callid, str viabranch)
{
	return rtpengine_dmq_replicate_action(RTPENGINE_DMQ_REMOVE, callid, viabranch,
			NULL, NULL);
}
