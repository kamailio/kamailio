...
void secf_rpc_del_dst(rpc_t *rpc, void *ctx)
{
	...
	if(rpc->scan(ctx, "d", &number) < 1) {
		rpc->fault(ctx, 500,
				"Invalid Parameters. Usage: secfilter.del_dst "
				"number\n     Example: secfilter.del_dst "
				"555123123");
		return;
	}
	text = int2str(number, &data.len);
	...
	if(!data.s) {
		PKG_MEM_ERROR;
		rpc->fault(ctx, 500, "Error remove values in the blacklist");
		return;
	}
	memcpy(data.s, text, data.len);
    lock_get(&(*secf_data)->lock);
...
void secf_rpc_stats(rpc_t *rpc, void *ctx)
{
	...
	if(rpc->struct_add(bh, "dddddddddddd", "User-Agent", secf_stats[BL_UA],
			   "Country", secf_stats[BL_COUNTRY], "From-Domain",
			   secf_stats[BL_FDOMAIN], "To-Domain", secf_stats[BL_TDOMAIN],
			   "Contact-Domain", secf_stats[BL_CDOMAIN], "IP-Address",
			   secf_stats[BL_IP], "From-Name", secf_stats[BL_FNAME], "To-Name",
			   secf_stats[BL_TNAME], "Contact-Name", secf_stats[BL_CNAME],
			   "From-User", secf_stats[BL_FUSER], "To-User",
			   secf_stats[BL_TUSER], "Contact-User", secf_stats[BL_CUSER])
			< 0) {
		rpc->fault(ctx, 500, "Internal error creating inner struct");
		return;
	}
	...
	if(rpc->struct_add(wh, "dddddddddddd", "User-Agent", secf_stats[WL_UA],
			   "Country", secf_stats[WL_COUNTRY], "From-Domain",
			   secf_stats[WL_FDOMAIN], "To-Domain", secf_stats[WL_TDOMAIN],
			   "Contact-Domain", secf_stats[WL_CDOMAIN], "IP-Address",
			   secf_stats[WL_IP], "From-Name", secf_stats[WL_FNAME], "To-Name",
			   secf_stats[WL_TNAME], "Contact-Name", secf_stats[WL_CNAME],
			   "From-User", secf_stats[WL_FUSER], "To-User",
			   secf_stats[WL_TUSER], "Contact-User", secf_stats[WL_CUSER])
			< 0) {
		rpc->fault(ctx, 500, "Internal error creating inner struct");
		return;
	}
	...
	if(rpc->struct_add(oh, "dd", "Destination", secf_stats[BL_DST],
			   "SQL-Injection", secf_stats[BL_SQL])
			< 0) {
		rpc->fault(ctx, 500, "Internal error creating inner struct");
		return;
	}
