...
#define TELS_SCH 0x736c6574
	...
	payload[3] = flags2;
	j = i;
	i += encode_parameters(&payload[i], uri_parsed->params.s, uri_str.s,
			&uri_parsed->params.len, 'u');
	if(i < j)
		goto error;
	return i;
error:
...
int print_encoded_uri(FILE *fd, unsigned char *payload, int paylen,
		char *hdrstart, int hdrlen, char *prefix)
{
	...
	fprintf(fd, "%s", prefix);
	for(j = 0; j < paylen; j++)
		fprintf(fd, "%s%d%s", j == 0 ? "ENCODED-URI:[" : ":", payload[j],
				j == paylen - 1 ? "]\n" : "");
	if(uriidx > hdrlen) {
		...
		int paylen, FILE *fd, char also_hdr, char *prefix)
{
	...
	if(flags1 & PARAMETERS_F) {
		...
		fprintf(fd, "%sgetParameter=(SAVP)",
				prefix); /*SVP = Attribute Value Pair*/
		for(k = 0; k <= m; k++) {
			if((aux3[k] == ';' || (k == m))
					&& aux2 == NULL) { /*no parameterValue was found*/
			...
			} else if((aux3[k] == ';' || (k == m)) && aux2 != NULL) {
				fprintf(fd, "%.*s=%.*s;", (int)(aux2 - aux), aux,
						(int)(aux3 - aux2 - 1 + k), aux2 + 1);
				aux2 = NULL;
				aux = aux3 + 1 + k;
			} else if(aux3[k] == '=') {
				aux2 = aux3 + k;
