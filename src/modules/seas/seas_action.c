...
static inline struct sip_msg *parse_ac_msg(
		hdr_flags_t flags, char *start, int len);
...
static inline int ac_jain_pong(as_p the_as, unsigned char processor_id,
		unsigned int flags, char *action, int len);
...
int dispatch_actions(void)
{
	...
	* because it confuses LM_*() */
	...
	fds[0].revents = 0;
	my_parent = getppid();
	snprintf(whoami, MAX_WHOAMI_LEN, "[%.*s] Action dispatcher",
			my_as->name.len, my_as->name.s);
	if(jain_ping_timeout && servlet_ping_timeout)
		ktimeout = jain_ping_timeout < servlet_ping_timeout
						   ? jain_ping_timeout
						   : servlet_ping_timeout;
...
again:
	if(0 > (j = read(fd, my_as->u.as.ac_buffer.s + my_as->u.as.ac_buffer.len,
					k))) {
		if(errno == EINTR)
			goto again;
		LM_ERR("reading data for as %.*s (%s)\n", my_as->name.len,
				my_as->name.s, strerror(errno));
		return -1;
	} else if(j == 0) {
...
static inline int process_pings(struct ha *the_table)
{
	...
	lock_get(the_table->mutex);
	{
		...
		for(i = 0; i < the_table->count; i++) {
			...
			elapsed = (now.tv_sec - tmp->sent.tv_sec) * 1000
					  + (now.tv_usec - tmp->sent.tv_usec) / 1000;
...
int process_action(as_p the_as)
{
	...
	ac_len = (the_as->ac_buffer.s[0] << 24) | (the_as->ac_buffer.s[1] << 16)
			 | (the_as->ac_buffer.s[2] << 8)
			 | ((the_as->ac_buffer.s[3]) & 0xFF);
	type = the_as->ac_buffer.s[4];
	processor_id = the_as->ac_buffer.s[5];
	flags = (the_as->ac_buffer.s[6] << 24) | (the_as->ac_buffer.s[7] << 16)
			| (the_as->ac_buffer.s[8] << 8) | ((the_as->ac_buffer.s[9]) & 0xFF);
	...
	while(the_as->ac_buffer.len >= ac_len) {
		...
		switch(type) {
			...
			case REPLY_FIN:
				...
				ac_reply(the_as, processor_id, flags, the_as->ac_buffer.s + 10,
						ac_len - 10);
				break;
			case UAC_REQ:
				LM_DBG("Processing an UAC REQUEST action from AS (length=%d): "
					   "%.*s\n",
						ac_len, the_as->name.len, the_as->name.s);
				ac_uac_req(the_as, processor_id, flags,
						the_as->ac_buffer.s + 10, ac_len - 10);
				break;
			case AC_CANCEL:
				LM_DBG("Processing a CANCEL REQUEST action from AS "
					   "(length=%d): %.*s\n",
						ac_len, the_as->name.len, the_as->name.s);
				ac_cancel(the_as, processor_id, flags, the_as->ac_buffer.s + 10,
						ac_len - 10);
				break;
			case SL_MSG:
				LM_DBG("Processing a STATELESS MESSAGE action from AS "
					   "(length=%d): %.*s\n",
						ac_len, the_as->name.len, the_as->name.s);
				ac_sl_msg(the_as, processor_id, flags, the_as->ac_buffer.s + 10,
						ac_len - 10);
				break;
			case JAIN_PONG:
				LM_DBG("Processing a PONG\n");
				ac_jain_pong(the_as, processor_id, flags,
						the_as->ac_buffer.s + 10, ac_len - 10);
				break;
			default:
				LM_DBG("Processing an UNKNOWN TYPE action from AS (length=%d): "
					   "%.*s\n",
						ac_len, the_as->name.len, the_as->name.s);
				break;
		}
		memmove(the_as->ac_buffer.s, the_as->ac_buffer.s + ac_len,
				(the_as->ac_buffer.len) - ac_len);
		(the_as->ac_buffer.len) -= ac_len;
		if(the_as->ac_buffer.len > 10) {
			ac_len = (the_as->ac_buffer.s[0] << 24)
					 | (the_as->ac_buffer.s[1] << 16)
					 | (the_as->ac_buffer.s[2] << 8)
					 | ((the_as->ac_buffer.s[3]) & 0xFF);
			type = the_as->ac_buffer.s[4];
			processor_id = the_as->ac_buffer.s[5];
			flags = (the_as->ac_buffer.s[6] << 24)
					| (the_as->ac_buffer.s[7] << 16)
					| (the_as->ac_buffer.s[8] << 8)
					| ((the_as->ac_buffer.s[9]) & 0xFF);
		} else {
			return 0;
		}
	}
		...
		char *action, int len)
{
	...
	if(0 > (headers.len = extract_allowed_headers(my_msg, 1, -1,
					HDR_CONTENTLENGTH_F | HDR_ROUTE_F | HDR_TO_F | HDR_FROM_F
							| HDR_CALLID_F | HDR_CSEQ_F,
					headers.s, MAX_HEADER))) {
		LM_ERR("Unable to extract allowed headers!!\n");
		goto error;
	}
	if(flags & SPIRAL_FLAG) {
		memcpy(headers.s + headers.len, SPIRAL_HDR CRLF,
				SPIRAL_HDR_LEN + CRLF_LEN);
	...
	if(seas_f.tmb.t_lookup_ident(&t_invite, cancelled_hashIdx, cancelled_label)
			< 0) {
	...
	ret = seas_f.tmb.t_cancel_uac(&headers, &body, cancelled_hashIdx,
			cancelled_label, uac_cb, (void *)the_param);
	if(ret == 0) {
		LM_ERR("t_cancel_uac failed\n");
		as_action_fail_resp(
				uac_id, SE_CANCEL, SE_CANCEL_MSG, SE_CANCEL_MSG_LEN);
		goto error;
	} else {
		the_param->label = ret;
	}
 ...
 * Returns: what
 */
int ac_reply(as_p the_as, unsigned char processor_id, unsigned int flags,
		char *action, int len)
{
	...
	if(c->uas.status >= 200) {
		LM_ERR("ac_reply: trying to reply to a \"%.*s\" transaction"
			   "that is already in completed state\n",
				REQ_LINE(c->uas.request).method.len,
				REQ_LINE(c->uas.request).method.s);
		goto error;
	}
	...
	if(0 > (i = extract_allowed_headers(my_msg, 0, i,
					HDR_VIA_F | HDR_TO_F | HDR_FROM_F | HDR_CSEQ_F
							| HDR_CALLID_F | HDR_CONTENTLENGTH_F,
					headers, MAX_HEADER))) {
    ...
    * responses...*/
	if(is_invite(c) && my_msg->first_line.u.reply.statuscode >= 200
			&& my_msg->first_line.u.reply.statuscode < 300) {
		...
		the_param->who = my_as;
		the_param->destroy_cb_set = 0;
		if(seas_f.tmb.register_tmcb(
				   0, c, TMCB_E2EACK_IN, uas_e2e_ack_cb, the_param, &param_free)
				<= 0) {
	...
	LM_DBG("Trying to construct a SipReply with: ReasonPhrase:[%.*s] "
		   "body:[%.*s] headers:[%.*s] totag:[%.*s]\n",
			my_msg->first_line.u.reply.reason.len,
			my_msg->first_line.u.reply.reason.s, body.len, body.s,
			new_header.len, new_header.s, totag.len, totag.s);
	...
	if(seas_f.tmb.t_reply_with_body(c, my_msg->first_line.u.reply.statuscode,
			   &(my_msg->first_line.u.reply.reason), &body, &new_header, &totag)
			< 0) {
 ...
 *
 */
int ac_sl_msg(as_p the_as, unsigned char processor_id, unsigned int flags,
		char *action, int len)
{
	struct sip_msg *my_msg;
	str *uri;
	...
	if(my_msg->first_line.type == SIP_REQUEST)
		LM_DBG("forwarding request:\"%.*s\" statelessly \n",
				my_msg->first_line.u.request.method.len + 1
						+ my_msg->first_line.u.request.uri.len,
				my_msg->first_line.u.request.method.s);
	else
		LM_DBG("forwarding reply:\"%.*s\" statelessly \n",
				my_msg->first_line.u.reply.status.len + 1
						+ my_msg->first_line.u.reply.reason.len,
				my_msg->first_line.u.reply.status.s);
	...
	set_force_socket(my_msg, grep_sock_info(&my_msg->via1->host,
									 my_msg->via1->port, my_msg->via1->proto));
	/* or also could be:
...
static inline void free_sip_msg_lite(struct sip_msg *my_msg)
{
	if(my_msg) {
		...
		if(my_msg->new_uri.s) {
			pkg_free(my_msg->new_uri.s);
			my_msg->new_uri.len = 0;
		}
		if(my_msg->dst_uri.s) {
			pkg_free(my_msg->dst_uri.s);
			my_msg->dst_uri.len = 0;
		}
		if(my_msg->path_vec.s) {
			pkg_free(my_msg->path_vec.s);
			my_msg->path_vec.len = 0;
		}
		if(my_msg->headers)
			free_hdr_field_lst(my_msg->headers);
		if(my_msg->add_rm)
 ...
 *
 */
int ac_uac_req(as_p the_as, unsigned char processor_id, unsigned int flags,
		char *action, int len)
{
	unsigned int cseq;
	...
	if(check_transaction_quadruple(my_msg) == 0) {
		as_action_fail_resp(
				uac_id, SE_UAC, "Headers missing (to,from,call-id,cseq)?", 0);
    ...
    */
	...
	if(clen != 0) {
		...
		LM_DBG("Trying to construct a Sip Request with: body:%d[%.*s] "
			   "headers:%d[%.*s]\n",
				body.len, body.len, body.s, headers.len, headers.len,
				headers.s);
		/*t_reply_with_body un-ref-counts the transaction, so don't use it anymore*/
	} else {
		body.s = NULL;
   ...
   */
	...
	set_uac_req(&uac_r, &(my_msg->first_line.u.request.method), &headers, &body,
			my_dlg, TMCB_DONT_ACK | TMCB_LOCAL_RESPONSE_OUT, uac_cb,
			(void *)the_param);
	...
	if(ret < 0) {
		err_ret = err2reason_phrase(
				ret, &sip_error, err_buf, sizeof(err_buf), "SEAS/UAC");
...
int print_local_uri(as_p as, char processor_id, char *where, int len)
{
	...
	switch(si->address.af) {
		case AF_INET:
			i = snprintf(where, len, "sip:%d.%d.%d.%d:%u%.*s",
					si->address.u.addr[0], si->address.u.addr[1],
					si->address.u.addr[2], si->address.u.addr[3], si->port_no,
					proto.len, proto.s);
			break;
		case AF_INET6:
			i = snprintf(where, len, "sip:[%x:%x:%x:%x:%x:%x:%x:%x]:%u%.*s",
					htons(si->address.u.addr16[0]),
					htons(si->address.u.addr16[1]),
					htons(si->address.u.addr16[2]),
					htons(si->address.u.addr16[3]),
					htons(si->address.u.addr16[4]),
					htons(si->address.u.addr16[5]),
					htons(si->address.u.addr16[6]),
					htons(si->address.u.addr16[7]), si->port_no, proto.len,
					proto.s);
			break;
		default:
...
static inline int calculate_hooks(dlg_t *_d)
{
	...
	if(_d->route_set) {
		...
		if(puri.lr.s) {
			if(_d->rem_target.s)
				_d->hooks.request_uri = &_d->rem_target;
			else
				_d->hooks.request_uri = &_d->rem_uri;
	...
	if((_d->hooks.request_uri) && (_d->hooks.request_uri->s)
	...
	if((_d->hooks.next_hop) && (_d->hooks.next_hop->s)
 ...
 *
 */
int extract_allowed_headers(struct sip_msg *my_msg, int strip_top_vias,
		int allow_top_Rroutes, hdr_flags_t forbidden_hdrs, char *headers,
		int headers_len)
{
	struct hdr_field *hf;
	rr_t *rb;
	...
	for(hf = my_msg->headers; hf; hf = hf->next) {
		if((hf->type > 0) && (forbidden_hdrs & HDR_T2F(hf->type))) {
			...
			if(i <= strip_top_vias) {
				...
				LM_DBG("Stripping vias [%.*s]\n",
						(int)(vb->name.s - hf->name.s), hf->name.s);
			...
			if(i <= rtcnt) {
				if((len + hf->len) < headers_len) {
					...
					LM_ERR("Unable to keep recordroute (not enough space left "
						   "in headers) Discarding \"%.*s\" \n",
							hf->name.len, hf->name.s);
					goto error;
				}
...
void uas_e2e_ack_cb(struct cell *t, int type, struct tmcb_params *rcvd_params)
{
	...
	if(!(buffer = create_as_event_t(t, rcvd_params->req, ev_info->processor_id,
				 &mylen, E2E_ACK))) {
...
void uac_cb(struct cell *t, int type, struct tmcb_params *rcvd_params)
{
	...
	if(type == TMCB_LOCAL_COMPLETED && !ev_info->destroy_cb_set) {
		if(seas_f.tmb.register_tmcb(
				   NULL, t, TMCB_DESTROY, uac_cleanup_cb, (void *)ev_info, 0)
				<= 0) {
	...
	LM_DBG("transaction %p Nr_of_outgoings:%d is_Local:%c\n", t,
			t->nr_of_outgoings, is_local(t) ? 'y' : 'n');
	...
	if(!(buffer = create_as_action_reply(t, rcvd_params, ev_info->uac_id,
				 ev_info->processor_id, &mylen))) {
...
char *create_as_action_reply(struct cell *c, struct tmcb_params *params,
		int uac_id, char processor_id, int *evt_len)
{
	int i;
