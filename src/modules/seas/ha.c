...
static inline int parse_ping(
		char *string, int *ping_period, int *pings_lost, int *ping_timeout);
...
int prepare_ha(void)
{
	...
	if(parse_ping(jain_ping_config, &jain_ping_period, &jain_pings_lost,
			...
			< 0)
		goto error;
	if(parse_ping(servlet_ping_config, &servlet_ping_period,
			   &servlet_pings_lost, &servlet_ping_timeout)
			< 0)
		goto error;
...
int print_pingtable(struct ha *ta, int idx, int lock)
{
	...
	for(i = 0; i < ta->size; i++) {
		if((ta->begin + ta->count) > ta->size) {
			if((i < ta->begin && i < ((ta->begin + ta->count) % ta->size))
					|| (i >= ta->begin && i < (ta->begin + ta->count)))
				fprintf(stderr, "*");
			else
				fprintf(stderr, "=");
		} else {
...
int spawn_pinger(void)
{
	...
	while(1) {
		...
		if(next_jain != INT_MAX) {
			next_jain = jain_ping_period
						- ((now.tv_sec - last_jain.tv_sec) * 1000
								+ (now.tv_usec - last_jain.tv_usec) / 1000);
		}
		if(next_servlet != INT_MAX) {
			next_servlet =
					servlet_ping_period
					- ((now.tv_sec - last_servlet.tv_sec) * 1000
							+ (now.tv_usec - last_servlet.tv_usec) / 1000);
		}
...
static inline int send_ping(struct as_entry *the_as, struct timeval *now)
{
	...
	lock_get(the_as->u.as.jain_pings.mutex);
	{
		if(the_as->u.as.jain_pings.count == the_as->u.as.jain_pings.size) {
			...
			the_as->u.as.jain_pings.end = (the_as->u.as.jain_pings.end + 1)
										  % the_as->u.as.jain_pings.size;
			the_as->u.as.jain_pings.count++;
		}
