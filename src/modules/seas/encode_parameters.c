		...
		unsigned char *where, void *pars, char *hdrstart, void *_body, char to)
{
	...
	if(to == 't') {
		...
		for(parametro = params; parametro && parametro->next;
				parametro = parametro->next)
			;
       ...
       each parameter is codified with its {param_name_start_idx,[param_value_start_idx|next_param_start_idx]} */
		...
		while(i < paramlen) {
			...
			if(paramstart[i]
					== '=') { /* '=' found, look for the next ';' and let 'i' pointing to it*/
				where[j++] = (unsigned char)(&paramstart[i + 1] - hdrstart);
		...
		where[j++] = (unsigned char)(&paramstart[i + 1] - hdrstart);
		if(j % 2
				== 0) /*this is because maybe the LAST parameter doesn't have an '='*/
			where[j++] = (unsigned char)(&paramstart[i + 1] - hdrstart);
		return j;
	}
	return 0;
