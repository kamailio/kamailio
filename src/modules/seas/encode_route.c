		...
		char *hdr, int hdrlen, rr_t *route_parsed, unsigned char *where)
{
	...
	for(route_offset = 0, i = 0, myroute = route_parsed; myroute;
			myroute = myroute->next, i++) {
...
int encode_route(char *hdrstart, int hdrlen, rr_t *body, unsigned char *where)
{
	...
	if(parse_uri(body->nameaddr.uri.s, body->nameaddr.uri.len, &puri) < 0) {
		...
		if((j = encode_uri2(
					hdrstart, hdrlen, body->nameaddr.uri, &puri, &where[i]))
				< 0) {
	...
	i += encode_parameters(
			&where[i], (void *)body->params, hdrstart, body, 'n');
	return i;
}
		...
		unsigned char *payload, int paylen, char *prefix)
{
	...
	for(i = 0, offset = 2 + numroutes; i < numroutes; i++) {
		print_encoded_route(fd, hdr, hdrlen, &payload[offset], payload[2 + i],
				strcat(prefix, "  "));
		offset += payload[2 + i];
		...
		int paylen, char *prefix)
{
	...
	fprintf(fd, "%s", prefix);
	for(i = 0; i < paylen; i++)
		fprintf(fd, "%s%d%s", i == 0 ? "ENCODED ROUTE=[" : ":", payload[i],
				i == paylen - 1 ? "]\n" : "");
	i = 2;
	if(flags & HAS_NAME_F) {
		fprintf(fd, "%sNAME=[%.*s]\n", prefix, payload[i + 1],
		...
		int paylen, FILE *fd, char segregationLevel, char *prefix)
{
	...
	if(!segregationLevel) {
		return dump_standard_hdr_test(
				hdr, hdrlen, (unsigned char *)payload, paylen, fd);
	}
	...
	if(segregationLevel & (JUNIT | SEGREGATE | ONLY_URIS)) {
		for(i = 0, offset = 2 + numroutes; i < numroutes; i++) {
			dump_route_test(hdr, hdrlen, &payload[offset], payload[2 + i], fd,
		...
		FILE *fd, char segregationLevel, char *prefix)
{
	...
	if((!(segregationLevel & JUNIT)) && (segregationLevel & ONLY_URIS)) {
		return dump_standard_hdr_test(
				hdr, hdrlen, (unsigned char *)(payload + i), payload[1], fd);
	}
	if((segregationLevel & JUNIT) && (segregationLevel & ONLY_URIS)) {
	...
	if(segregationLevel & JUNIT) {
		...
		if(flags & HAS_NAME_F) {
			...
			fprintf(fd, "(null)\n");
		return print_uri_junit_tests(hdr, hdrlen, &payload[i], payload[1], fd,
				0, "getAddress.getURI.");
	}
	return 0;
}
