 ...
 *]
 */
int encode_contact_body(char *hdr, int hdrlen, contact_body_t *contact_parsed,
		unsigned char *where)
{
	...
	for(contact_offset = 0, i = 0, mycontact = contact_parsed->contacts;
			mycontact; mycontact = mycontact->next, i++) {
		if((k = encode_contact(hdr, hdrlen, mycontact, &tmp[contact_offset]))
				< 0) {
 ...
 */
int encode_contact(
		char *hdrstart, int hdrlen, contact_t *body, unsigned char *where)
{
...
int print_encoded_contact_body(FILE *fd, char *hdr, int hdrlen,
		unsigned char *payload, int paylen, char *prefix)
{
	...
	fprintf(fd, "%s", prefix);
	for(i = 0; i < paylen; i++)
		fprintf(fd, "%s%d%s", i == 0 ? "ENCODED CONTACT BODY:[" : ":",
	...
	for(i = 0, offset = 2 + numcontacts; i < numcontacts; i++) {
		print_encoded_contact(fd, hdr, hdrlen, &payload[offset], payload[2 + i],
				strcat(prefix, "  "));
		offset += payload[2 + i];
...
int print_encoded_contact(FILE *fd, char *hdr, int hdrlen,
		unsigned char *payload, int paylen, char *prefix)
{
	...
	fprintf(fd, "%s", prefix);
	for(i = 0; i < paylen; i++)
		fprintf(fd, "%s%d%s", i == 0 ? "ENCODED CONTACT=[" : ":", payload[i],
				i == paylen - 1 ? "]\n" : "");
	i = 2;
	if(flags & HAS_NAME_F) {
	...
	print_encoded_parameters(fd, (unsigned char *)&payload[i + payload[1]], hdr,
			paylen - i - payload[1], prefix);
	return 0;
}
 ...
 *
 */
int dump_contact_body_test(char *hdr, int hdrlen, unsigned char *payload,
		int paylen, FILE *fd, char segregationLevel, char *prefix)
{
	...
	if(segregationLevel & (JUNIT | SEGREGATE | ONLY_URIS)) {
		for(i = 0, offset = 2 + numcontacts; i < numcontacts; i++) {
			dump_contact_test(hdr, hdrlen, &payload[offset], payload[2 + i], fd,
					segregationLevel, prefix);
			offset += payload[2 + i];
		}
	}
	return 1;
...
int dump_contact_test(char *hdr, int hdrlen, unsigned char *payload, int paylen,
		FILE *fd, char segregationLevel, char *prefix)
{
	...
	if(!(segregationLevel & JUNIT) && (segregationLevel & ONLY_URIS))
		return dump_standard_hdr_test(
				hdr, hdrlen, (&payload[i]), (int)payload[1], fd);
	if((segregationLevel & JUNIT) && (segregationLevel & ONLY_URIS))
	...
	if((segregationLevel & JUNIT) && !(segregationLevel & ONLY_URIS)) {
		...
		for(i += payload[1]; i < paylen - 1; i += 2) {
			...
			printf("%.*s;",
					(payload[i + 2] - payload[i + 1]) == 0
							? 0
							: (payload[i + 2] - payload[i + 1] - 1),
					&hdr[payload[i + 1]]);
		}
		fprintf(fd, "\n");
	}
