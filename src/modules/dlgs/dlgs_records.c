...
int dlgs_add_item(sip_msg_t *msg, str *src, str *dst, str *data)
{
	...
	while(it != NULL && it->hashid == hid) {
		if(sf.callid.len == it->callid.len
			...
			if(sf.branch.len == it->branch.len
				...
				/* Same Call-ID but different branch-id, check state */
				if(it->state == DLGS_STATE_TERMINATED
						|| it->state == DLGS_STATE_NOTANSWERED) {
					LM_DBG("allowing serial fork attempt - old state: %d, new "
						   "branch-id [%.*s]\n",
							it->state, sf.branch.len, sf.branch.s);
					break;
				} else {
					lock_release(&dsht->slots[idx].lock);
					LM_DBG("possible spiraling or parallel forking attempt for "
						   "call-id [%.*s], branch-id [%.*s], "
						   "not supported\n",
							sf.callid.len, sf.callid.s, sf.branch.len,
							sf.branch.s);
					return 1;
				}
			}
	...
	/* add */
	LM_DBG("adding new attempt to hash table with call-id [%.*s], branch-id "
		   "[%.*s]\n",
			sf.callid.len, sf.callid.s, sf.branch.len, sf.branch.s);
...
dlgs_item_t *dlgs_get_item(sip_msg_t *msg)
{
	...
	while(it != NULL && it->hashid == hid) {
		if(cid->len == it->callid.len
			...
			/* Don't check branch-id, spiraling or parallel forking not supported */
			/* found */
			return it;
		}
		it = it->next;
