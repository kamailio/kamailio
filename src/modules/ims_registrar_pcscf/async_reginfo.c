...
int init_reginfo_event_list()
{
	...
	sem_new(reginfo_event_list->empty,
		...
		int source_flag, int reg_info_event, str *extra_headers, str *pres_uri)
{
	...
	if(subs_remote_target) {
		LM_DBG("subs_remote_target [%.*s]\n", subs_remote_target->len,
				subs_remote_target->s);
	...
	if(subs_outbound_proxy) {
		LM_DBG("subs_outbound_proxy [%.*s]\n", subs_outbound_proxy->len,
				subs_outbound_proxy->s);
...
void push_reginfo_event(reginfo_event_t *event)
{
	...
	if(reginfo_queue_size_threshold > 0
			&& reginfo_event_list->size > reginfo_queue_size_threshold) {
		LM_WARN("Reginfo queue is size [%d] and has exceed "
				"reginfo_queue_size_threshold of [%d]",
				reginfo_event_list->size, reginfo_queue_size_threshold);
	}
	...
	for(;;) {
		...
		switch(ev->event) {
			case REG_EVENT_PUBLISH:
				LM_DBG("Sending out-of-band publish with pres_uri [%.*s] "
					   "publ_id [%.*s] publ_content_type [%.*s] extra_headers "
					   "[%.*s]"
					   "expires [%d] event [%d]\n",
						ev->pres_uri.len, ev->pres_uri.s, ev->publ_id.len,
						ev->publ_id.s, ev->publ_content_type.len,
						ev->publ_content_type.s, ev->extra_headers.len,
						ev->extra_headers.s, ev->expires, ev->reg_info_event);
				LM_DBG("publ_body [%.*s] \n", ev->publ_body.len,
						ev->publ_body.s);
			...
			case REG_EVENT_SUBSCRIBE:
				...
				LM_DBG("Sending out-of-band subscribe with remote_target "
					   "[%.*s] pres_uri [%.*s] subs_watcher_uri [%.*s] "
					   "subs_contact [%.*s] extra_headers [%.*s] "
					   "expires [%d] event [%d] flag [%d] source_flag [%d]\n",
						subs.remote_target->len, subs.remote_target->s,
						subs.pres_uri->len, subs.pres_uri->s,
						subs.watcher_uri->len, subs.watcher_uri->s,
						subs.contact->len, subs.contact->s,
						subs.extra_headers->len, subs.extra_headers->s,
						subs.expires, subs.event, subs.flag, subs.source_flag);
