...
#define SET_STR_VAL(_str, _res, _r, _c)                                       \
	if(RES_ROWS(_res)[_r].values[_c].nul == 0) {                              \
		switch(RES_ROWS(_res)[_r].values[_c].type) {                          \
			case DB1_STRING:                                                  \
				(_str).s =                                                    \
						(char *)RES_ROWS(_res)[_r].values[_c].val.string_val; \
				(_str).len = strlen((_str).s);                                \
				break;                                                        \
			case DB1_STR:                                                     \
				(_str).len = RES_ROWS(_res)[_r].values[_c].val.str_val.len;   \
				(_str).s =                                                    \
						(char *)RES_ROWS(_res)[_r].values[_c].val.str_val.s;  \
				break;                                                        \
			case DB1_BLOB:                                                    \
				(_str).len = RES_ROWS(_res)[_r].values[_c].val.blob_val.len;  \
				(_str).s =                                                    \
						(char *)RES_ROWS(_res)[_r].values[_c].val.blob_val.s; \
				break;                                                        \
			default:                                                          \
				(_str).len = 0;                                               \
				(_str).s = NULL;                                              \
...
static int mod_init(void)
{
	...
	if(db_check_table_version(&msilo_dbf, db_con, &ms_db_table, S_TABLE_VERSION)
			< 0) {
 ...
 */
static int get_non_mandatory_headers(
		struct sip_msg *msg, char *buf, int buf_len)
{
	...
	if(ms_extra_hdrs_avp_name.n != 0) {
		...
		avp = search_first_avp(
				ms_extra_hdrs_avp_type, ms_extra_hdrs_avp_name, &avp_value, 0);
...
#define MS_BUF1_SIZE 1024
	...
	if(ms_max_messages > 0) {
		...
		if(msilo_dbf.query(db_con, db_keys, 0, db_vals, db_cols, 2, 1, 0, &res)
				< 0) {
		...
		if(RES_ROW_N(res) >= ms_max_messages) {
			LM_ERR("too many messages for AoR '%.*s@%.*s'\n", puri.user.len,
					puri.user.s, puri.host.len, puri.host.s);
	...
	if(ms_snd_time_avp_name.n != 0) {
		avp = NULL;
		avp = search_first_avp(
				ms_snd_time_avp_type, ms_snd_time_avp_name, &avp_value, 0);
		if(avp != NULL && is_avp_str_val(avp)) {
			if(ms_extract_time(&avp_value.s, &db_vals[nr_keys].val.int_val)
					!= 0)
				db_vals[nr_keys].val.int_val = 0;
		}
	}
	...
	extra_hdrs.len =
			get_non_mandatory_headers(msg, extra_hdrs_buf, EXTRA_HDRS_BUF_LEN);
	if(extra_hdrs.len < 0) {
		goto error;
	}
	...
	if(fixup_get_svalue(msg, (gparam_p)*ms_offline_message_sp, &notify_body)
					!= 0
			|| notify_body.len <= 0) {
	...
	if(ms_contact != NULL
			&& fixup_get_svalue(msg, (gparam_p)*ms_contact_sp, &notify_contact)
					   == 0
			&& notify_contact.len > 0) {
	...
	if(ms_use_contact && msg->contact != NULL && msg->contact->body.s != NULL
		...
		if((msg->contact->parsed != NULL
			...
			ctaddr.len = ((contact_body_t *)(msg->contact->parsed))
								 ->contacts->uri.len;
...
static int m_dump(sip_msg_t *msg, str *owner_s)
{
	...
	if(puri.user.len <= 0 || puri.user.s == NULL || puri.host.len <= 0
			|| puri.host.s == NULL) {
	...
	for(i = 0; i < RES_ROW_N(db_res); i++) {
		...
		if(ms_extra_hdrs != NULL) {
			if(fixup_get_svalue(
					   msg, (gparam_p)*ms_extra_hdrs_sp, &extra_hdrs_str)
					!= 0) {
		...
		if(tmp_extra_hdrs.len > 0) {
			...
			memcpy(tmp_extra_hdrs.s + extra_hdrs_str.len, str_vals[4].s,
					str_vals[4].len);
		} else {
...
void m_clean_silo(unsigned int ticks, void *param)
{
	...
	while(p) {
		...
		if((p->flag & MS_MSG_ERRO)
				&& (p->flag & MS_MSG_TSND)) { /* set snd time to 0 */
...
void m_send_ontimer(unsigned int ticks, void *param)
{
	...
	for(i = 0; i < RES_ROW_N(db_res); i++) {
		...
		SET_STR_VAL(str_vals[2], db_res, i, 3); /* body */
		if(str_vals[0].s == NULL || str_vals[1].s == NULL
				|| str_vals[2].s == NULL) {
		...
		hdr_str.len = 1024;
		if(m_build_headers(&hdr_str, str_vals[3] /*ctype*/,
				   ms_reminder /*from*/, 0 /*Date*/, extra_hdrs_str /*extra*/)
				< 0) {
			LM_ERR("headers building failed [%d]\n", mid);
			if(msilo_dbf.free_result(db_con, db_res) < 0)
		...
		memcpy(puri.s + 4 + str_vals[0].len + 1, str_vals[1].s,
				str_vals[1].len);
		...
		/** sending using TM function: t_uac */
		body_str.len = 1024;
		stime = (time_t)RES_ROWS(db_res)[i].values[5 /*snd time*/].val.int_val;
...
int ms_reset_stime(int mid)
{
	...
	if(msilo_dbf.update(
			   db_con, db_keys, db_ops, db_vals, db_cols, db_cvals, 1, 1)
			!= 0) {
