...
static char *tr_parse_rdn(str *in, trans_t *t);
...
static int tr_init_buffers(void)
{
	int i;

	_tr_buffer_list = (char **)pkg_mallocxz(TR_BUFFER_SLOTS * sizeof(char *));
	if(_tr_buffer_list == NULL) {
		PKG_MEM_ERROR;
		return -1;
	}
	for(i = 0; i < TR_BUFFER_SLOTS; i++) {
		_tr_buffer_list[i] = (char *)pkg_malloc(TR_BUFFER_SIZE);
...
void rdn_tr_destroy_buffers(void)
{
	...
	for(i = 0; i < TR_BUFFER_SLOTS; i++) {
		if(_tr_buffer_list[i] != NULL)
			pkg_free(_tr_buffer_list[i]);
	}
	pkg_free(_tr_buffer_list);
	_tr_buffer_list = NULL;
	_tr_buffer = NULL;
	_tr_buffer_idx = 0;
}
 ...
 *
 */
static char *tr_set_crt_buffer(void)
{
...
static int base16_encode_update(char *dst, size_t length, char *src)
{
	size_t i;
...
static int parse_uui(const str *uui_header, str *fn, int *pd)
{
	char *a;
	int n;
	int i = 0;
	...
	if(uui_header->len > 2) {
		if((uui_header->s[0] == '0') && (uui_header->s[1] == '0')) {
			...
			if(a != NULL) {
				...
				i = 2;
				while(i < (n - 3)) {
					length = (RDN_DIGIT_2_SHORT(text[i + 2])) * 16
							 + RDN_DIGIT_2_SHORT(text[i + 3]);
...
static int base16_decode_update(char *dst, str *src)
{
	char fn_text[64];
	str fn_str = {0, 0};
	fn_str.s = fn_text;
...
static int cast_int_to_hex_ts(char *dst, str *src)
{
	src->s[src->len] = '\0';
...
static int get_sds_disposition_type(size_t length, char *src)
{
	int SDSindex = 0;
...
static int get_sds_message_type(size_t length, char *src)
{
	int SDSindex = 0;
	char *points_to_SDS = NULL;
...
static int get_sds_notificatn_type(size_t length, char *src)
{
	int SDSindex = 0;
	char *points_to_SDS = NULL;
...
static int get_sds_message_id(char *dst, size_t length, char *src)
{
	int SDSindex = 0;
	int i = 0;
	char *points_to_SDS = NULL;
...
static int get_sds_signalling(char *dst, size_t length, char *src)
{
	int SDSindex = 0;
	int i = 0;
	int len = 0;
...
static int get_sds_date_time(char *dst, size_t length, char *src)
{
	int SDSindex = 0;
	int i = 0;
...
static int put_sds_date_time(char *dst, size_t length, char *src)
{
	int i = 0;
...
static int extract_text_from_SDS(char **dst, str *src)
{
	int len = 0;
	char *points_to_SDS = NULL;
...
static int build_sds_header_for_txt_body(char *dst, size_t length, char *src)
{
	int len = length;
