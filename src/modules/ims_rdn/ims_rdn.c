...
#include "../../modules/ims_registrar_scscf/api.h"
#include "../../modules/ims_usrloc_scscf/udomain.h"
...
static char *rdnlistfile = CFG_DIR "ims_rdn.list";
static char *rdnlistfile_dpsi = CFG_DIR "dpsi.list";
static char *cx_userdata_templates = CFG_DIR "CxUserData_tmpl.xml";
static int rdn_node_type = RDN_NODE_TYPE_DEFAULT;


// module APIs (ims_usrloc_scscf, tm, ims_registrar_scscf)
static usrloc_api_t ul; /*!< Structure containing pointers to usrloc functions*/
static struct tm_binds tmb;
static registrar_api_t ra;

// pointer to the first levels of the digit trees in shared memory
// currently, only the RDN_USER_PARAM_E164 and the RDN_USER_PARAM_EIRENE are used
static rdn_digit_node_t *rdn_first_levels[] = {0, 0, 0, 0};

// the user parameters for the types of digit trees
// currently, only the RDN_USER_PARAM_E164 and the RDN_USER_PARAM_EIRENE are used
static char *rdn_user[] = {RDN_USER_PARAM_INET, RDN_USER_PARAM_E164,
		RDN_USER_PARAM_DIAL, RDN_USER_PARAM_EIRENE};

// pointer to the DPSI table in shared memory
static struct rdn_hash_entry **dpsi_table = 0;

// pointer to the XML template for unreg/term delivery in shared memory
static str unreg_term_template = {0, 0};
...
static int prepare_term_unreg(struct sip_msg *_m, char *_t, char *_s);
static int analyse_eirene_ruri(struct sip_msg *_m, char *_t, char *_s);
static int analyse_e164_ruri(struct sip_msg *_m, char *_t, char *_s);
static int analyse_dpsi_ruri(struct sip_msg *_m, char *_t, char *_s);
static int analyse_domain_ruri(struct sip_msg *_m, char *_t, char *_s);
static int analyse_e164_pvar(
		struct sip_msg *msg, char *pvar_input, char *unused);
static int analyse_eirene_pvar(
		struct sip_msg *msg, char *pvar_input, char *unused);
static int analyse_dpsi_pvar(
		struct sip_msg *msg, char *pvar_input, char *unused);
static int analyse_domain_pvar(
		struct sip_msg *msg, char *pvar_input, char *unused);

/* global variable for special handling */
static rdn_special_handling_t special_handling_global = {
		{0, 0}, // hardcoded_ruri
		0,		// del
		{0, 0}, // insrt
		{0, 0}, // mcx_domain
...
static unsigned int calc_hash(str *key)
{
	...
	return h % RDN_HASH_SIZE;
}

// Create new rdn_hash_entry structure from given key and service
static struct rdn_hash_entry *new_rdn_hash_entry(
		str *key, rdn_service_t *service)
{
	struct rdn_hash_entry *e;
...
static int key_exists(str *search_key)
{
	struct rdn_hash_entry *np;
...
{
	dpsi_table = (struct rdn_hash_entry **)shm_malloc(
	...
	memset(dpsi_table, 0, sizeof(struct rdn_hash_entry *) * RDN_HASH_SIZE);
	return 0;
}
...
static void deleteSpecialHandling(
		rdn_special_handling_t **temp_special_handling)
{
...
static void free_digit_node(rdn_digit_node_t *dignode)
{
	int idx;
	for(idx = 0; idx < RDN_NUM_DIGITS; idx++) {
...
static void mod_destroy(void)
{
	int flix = sizeof(rdn_first_levels) / sizeof(rdn_digit_node_t *);
...
static param_export_t params[] = {{"list_file", PARAM_STRING, &rdnlistfile},
		...
		{"rdn_node_type", PARAM_INT, &rdn_node_type}, {0, 0, 0}};
...
struct module_exports exports = {
		"ims_rdn", DEFAULT_DLFLAGS, /* dlopen flags */
		...
		0, mod_destroy,				/* destroy function */
};
...
static rdn_special_handling_t *createSpecialHandling(
		rdn_special_handling_t *temp)
{
	int err = 0;
	rdn_special_handling_t *new_sh = 0;
	if(temp) {
		new_sh = (rdn_special_handling_t *)shm_malloc(
				sizeof(rdn_special_handling_t));
		if(new_sh) {
			str nilstr = {0, 0};
...
static int lookup_userix(char *user_parm)
{
	...
	int res = sizeof(rdn_first_levels) / sizeof(rdn_digit_node_t *);
	if(res > max_users)
		res = max_users;
...
static rdn_service_t *createService(int scscf_flag, str as_name,
		str session_type, rdn_special_handling_t *special_handling)
{
...
static rdn_digit_node_t *createLevel(void)
{
	rdn_digit_node_t *new_dignode = (rdn_digit_node_t *)shm_malloc(
			RDN_NUM_DIGITS * sizeof(rdn_digit_node_t));
	if(new_dignode) {
		int idx;
		for(idx = 0; idx < RDN_NUM_DIGITS; idx++) {
...
static rdn_distinct_psi_t *createDp(str user_part, int scscf_flag, str as_name,
		str session_type, rdn_special_handling_t *special_handling)
{
	rdn_distinct_psi_t *new_dp =
			(rdn_distinct_psi_t *)shm_malloc(sizeof(rdn_distinct_psi_t));
...
static rdn_service_t *find_service_from_key(str *search_key)
{
	if(dpsi_table) {
		struct rdn_hash_entry *np;
...
static void add_dp_2_config(str add_key, int scscf_flag, str as_name,
		str session_type, rdn_special_handling_t *special_handling)
{
	// only add service, if not already added
	if(dpsi_table) {
		if(!key_exists(&add_key)) {
			rdn_distinct_psi_t *new_dp = 0;
			...
			if(new_dp) {
				...
				e = new_rdn_hash_entry(&new_dp->key, new_dp->service);
				if(!e)
...
static void add_service_2_config(str digits, int scscf_flag, str as_name,
		str session_type, rdn_special_handling_t *special_handling, int flix)
{

	int counter = 0;
	rdn_digit_node_t *level;
	...
	while(*digits.s && level && counter != digits.len) {
		...
		short digval = RDN_DIGIT_2_SHORT(*digits.s);
		digits.s++;
		counter++;
...
static int load_ims_rdn_config(char *lfile, rdn_config_type config)
{
	char line[256], *p;
	FILE *f = NULL;
	str as_name;
	...
	while(p) {
		...
		if(config == rdn_dpsi_config) {
			...
			while(*p && *p != ' ' && *p != '\t' && *p != '\r' && *p != '\n'
					&& *p != '#') {
				if((digval = RDN_DIGIT_2_SHORT(*act_digit.s)) < 0) {
					if(is_first_digit && *p == '+') {
						e164_present = 1;
						p++;
						digits.s = p;
		...
		if(config == rdn_config || config == rdn_dpsi_config) {
			/* eat all white spaces */
			while(*p && (*p == ' ' || *p == '\t' || *p == '\r' || *p == '\n'))
				p++;
			...
			if((scscf_flag == 0
				...
				if(config == rdn_dpsi_config) {
					...
					int flix =
							lookup_userix(e164_present ? RDN_USER_PARAM_E164
													   : RDN_USER_PARAM_EIRENE);
					if(flix < 0) {
						LM_ERR("Cannot add RDN service. Digit Tree not "
							   "prepared for user=%s",
								e164_present ? RDN_USER_PARAM_E164
											 : RDN_USER_PARAM_EIRENE);
						goto error;
					}
...
static int load_template(char *tfile, rdn_template_type config)
{
...
static int mod_init(void)
{
	...
	if(load_ims_rdn_config(rdnlistfile_dpsi, rdn_dpsi_config) != 0) {
	...
	if((rdn_node_type & RDN_NODE_TYPE_MASK) == RDN_NODE_TYPE_SCSCF) {
		...
		if(load_template(cx_userdata_templates, rdn_cx_userdata_template)
				!= 0) {
...
static void extract_digit_block(
		char *pChar, int iChar, int remaining, short **pDigblk, int *pLen)
{
	...
	//                        of the digit block
	//assert(pChar);  // can be used in kamailio assert?
	//assert(iChar >= 0);
	...
	if(iChar >= remaining || (digval = RDN_DIGIT_2_SHORT(pChar[iChar])) < 0) {
		// no characters left or not a digit -> stop recursion and allocate digit block
		if(*pLen)
			(*pDigblk) = (short *)pkg_malloc(sizeof(short) * (*pLen));
		if((*pDigblk) == NULL)
			PKG_MEM_ERROR;
	} else {
...
// Parameters
//    rdn_digit_node_t* rdn_level......points to the level that shall be processes
//                                 MUST NOT be NULL
...
static rdn_service_t *find_rdn_service(
		rdn_digit_node_t *rdn_level, short *pDigit, int remaining)
{
	rdn_digit_node_t *dignode = rdn_level + *pDigit; // pointer to DIGIT node
	rdn_digit_node_t *next_level = dignode->next_level;
	if(next_level && --remaining) {
		rdn_service_t *service =
				find_rdn_service(next_level, pDigit + 1, remaining);
		if(service) {
			LM_DBG("Found service for child digit node <%i>", *pDigit);
			return service;
		}
	}
...
static rdn_service_t *find_service_from_digits(
		char *pChar, int remaining, char *user_param)
{
	...
	while(RDN_DIGIT_2_SHORT(*pChar) < 0 && count-- > 0)
		pChar++, remaining--;
	...
	if(digblk)
		pkg_free(digblk); // free heap memory
	return service;
}

static rdn_service_t *find_service_from_ruri(
		struct sip_uri *uri, char *user_param)
{
...
*/
/*
static void update_sip_uri_domain(struct sip_msg *msg, str *new_domain)
{
    // 1) Basic checks
...
static void modify_routing_logic(struct sip_msg *msg)
{
    ...
    //    e.g., from a global or from rdn_special_handling_t
		...
		struct sip_msg *_m, char *_t, char *_s, char *user_param, int mode)
{
	...
	if(mode) {
		if(user_param) {
			...
			service = find_service_from_ruri(&req->parsed_uri, user_param);
		} else {
...
static int common_rdn_pvar_analyse(
		str *uri, char *user_param, int mode, rdn_analysis_result_t *result)
{
	...
	if(mode) {
		if(user_param) {
			...
			service = find_service_from_ruri(&parsed_uri, user_param);
		} else {
		...
		struct sip_msg *msg, char *pvar_input, char *unused)
{
	...
	if(common_rdn_pvar_analyse(&uri, RDN_USER_PARAM_E164, 1, &result) < 0) {
	...
	// TEMP LOG
	LM_DBG("set as_name='%.*s', scscf_name='%.*s', "
		...
		struct sip_msg *msg, char *pvar_input, char *unused)
{
	...
	if(common_rdn_pvar_analyse(&uri, RDN_USER_PARAM_EIRENE, 1, &result) < 0) {
...
static int analyse_eirene_ruri(struct sip_msg *_m, char *_t, char *_s)
{
	...
	return analyse_ruri(_m, _t, _s, RDN_USER_PARAM_EIRENE, 1);
}
...
static int analyse_e164_ruri(struct sip_msg *_m, char *_t, char *_s)
{
	...
	return analyse_ruri(_m, _t, _s, RDN_USER_PARAM_E164, 1);
}
