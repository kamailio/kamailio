...
str isc_my_uri = str_init(
		"scscf.ims.smilecoms.com:6060"); /**< Uri of myself to loop the message in str	*/
str isc_my_uri_sip = {0,
		0}; /**< Uri of myself to loop the message in str with leading "sip:" */
int isc_expires_grace =
		120; /**< expires value to add to the expires in the 3rd party register*/
int isc_fr_timeout = 5000; /**< default ISC response timeout in ms */
int isc_fr_inv_timeout =
		20000;			   /**< default ISC invite response timeout in ms */
	...
	if(is_route_type(FAILURE_ROUTE) && !firstflag) {
		...
		for(k = 0; k < t->nr_of_outgoings; k++) {
			...
			if(m) {
				...
				LM_ERR("On failure, previously matched trigger no longer "
					   "matches?!\n");
	...
	if(dir == DLG_MOBILE_ORIGINATING) {
		...
		if(k) {
			...
			LM_DBG("Checking if RURI has changed...comparing: <%.*s> and "
				   "<%.*s>\n",
					old_mark.aor.len, old_mark.aor.s, s.len, s.s);
	...
	if(dir == DLG_MOBILE_ORIGINATING) {
		...
		if(k) {
			if(str1 == 0 || strlen(str1) != 1) {
				LM_ERR("wrong parameter - must be \"0\" (initial registration) "
					   "or \"1\"(previously registered) \n");
				return ret;
			} else if(str1[0] == '0')
				k = 0;
			else
			...
			m = isc_checker_find(
					s, old_mark.direction, old_mark.skip, msg, k, d);
			while(m) {
