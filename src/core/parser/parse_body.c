...
#define boundary_param_len (sizeof("boundary") - 1)
	...
	for(p = list; p; p = p->next)
		if((p->name.len == boundary_param_len) && is_boundary(p->name.s)) {
			boundary->s = p->body.s;
...
inline static char *get_multipart_body(
		char *buf, char *buf_end, str *boundary, int *len)
{
	char *beg, *end;
...
#define content_type_len (sizeof("Content-Type") - 1)
		...
		while((c = search_boundary(c, buf_end, &boundary))) {
			...
			while((*c != '\r') && (*c != '\n')) {
				...
				if((LOWER_DWORD(READ(c)) == _cont_)
					...
					return get_multipart_body(c, buf_end, &boundary, len);
				}
			next_hf:
				/* go to the next line */
				while((c < buf_end) && (*c != '\n'))
 ...
 */
int part_multipart_headers_cmp(char *buffer, char *end_buffer,
		unsigned short content_type, unsigned short content_subtype,
		char *content_id, char *content_length)
{
	int error = 0;
	char *error_msg = NULL;
	...
	if((buffer == NULL) || (end_buffer == NULL)) {
		...
		while((!found) && (!error) && (cpy_c < cpy_d)) {
				...
				if((LOWER_DWORD(READ(cpy_c)) == _cont_)
					...
					if((!found_content_type) && ((cpy_c + 5) < cpy_d)
						...
						if((value_ini != NULL) && (value_fin != NULL)) {
							...
							error = -4;
							error_msg = "Failed to perform trim_leading_hts || "
										"trim_leading_e_r";
						}
						...
						if((value_ini != NULL) && (value_fin != NULL)) {
							cpy_c = value_fin;
							if(strncmp(content_id, value_ini,
									   value_fin - value_ini)
									== 0) {
								found_content_id = 1;
							} else {
						...
						if((value_ini != NULL) && (value_fin != NULL)) {
							cpy_c = value_fin;
							if(strncmp(content_length, value_ini,
									   value_fin - value_ini)
									== 0) {
								found_content_length = 1;
							} else {
			...
			found = found_content_type * found_content_id
