...
struct socket_info *get_send_socket2(struct socket_info *force_send_socket,
		union sockaddr_union *to, int proto, enum ss_mismatch *mismatch)
{
	...
	if(unlikely(force_send_socket)) {
		...
		if(unlikely(orig->proto != proto
					&& !(orig->proto == PROTO_TLS && proto == PROTO_WS))) {
			force_send_socket = find_si(&(force_send_socket->address),
					force_send_socket->port_no, proto);
		...
		if(likely((force_send_socket->socket != -1
			...
			if(!(force_send_socket->flags & SI_IS_MCAST))
				LM_WARN("not listening on the requested socket (%s:%s:%d),"
						...
						ip_addr2a(&force_send_socket->address),
						force_send_socket->port_no);
			else if(likely(mismatch))
				*mismatch = SS_MISMATCH_MCAST;
		}
	};
not_forced:
	...
	switch(proto) {
		...
		case PROTO_TCP:
			...
			switch(to->s.sa_family) {
				case AF_INET:
					...
					break;
				case AF_INET6:
					send_sock = sendipv6_tcp;
					break;
				default:
					LM_ERR("don't know how to forward to af %d\n",
							to->s.sa_family);
			}
			break;
#endif
		...
		case PROTO_SCTP:
			if((bind_address == 0)
				...
				switch(to->s.sa_family) {
					case AF_INET:
						send_sock = sendipv4_sctp;
						break;
					case AF_INET6:
						send_sock = sendipv6_sctp;
						break;
					default:
						LM_ERR("don't know how to forward to af %d\n",
								to->s.sa_family);
...
int check_self(str *host, unsigned short port, unsigned short proto)
{
	...
	if(grep_aliases(host->s, host->len, port, proto) == 0) {
		ret = (_check_self_func_list == NULL)
					  ? 0
					  : run_check_self_func(host, port, proto);
	}
		...
		struct dest_info *send_info, unsigned int mbmode)
{
	...
	if(dst) {
		...
		if(cfg_get(core, core_cfg, use_dns_failover)) {
			...
			err = dns_sip_resolve2su(
					&dns_srv_h, &send_info->to, dst, port, &proto, dns_flags);
			if(err != 0) {
				LM_ERR("resolving \"%.*s\" failed: %s [%d]\n", dst->len,
						ZSW(dst->s), dns_strerror(err), err);
				ret = E_BAD_ADDRESS;
				goto error;
			}
		} else
	...
	if((err != 0) && (err != -E_DNS_EOR)) {
		LM_ERR("resolving %.*s host name in uri failed: %s [%d] (dropping "
			   "packet)\n",
...
static int do_forward_reply(struct sip_msg *msg, int mode)
{
	...
	if(update_sock_struct_from_via(&dst.to, msg, msg->via2) == -1)
		goto error;
#ifdef USE_COMP
	...
	if(
#ifdef USE_TCP
			dst.proto == PROTO_TCP || dst.proto == PROTO_WS
#ifdef USE_TLS
			|| dst.proto == PROTO_TLS || dst.proto == PROTO_WSS
#endif
#ifdef USE_SCTP
			||
#endif /* USE_SCTP */
	...
	LM_DBG("reply forwarded (2nd via address: %.*s port %d)\n",
			msg->via2->host.len, msg->via2->host.s, (int)msg->via2->port);
