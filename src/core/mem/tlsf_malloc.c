...
static block_header_t *block_next(const block_header_t *block)
{
	block_header_t *next = offset_to_block(
			block_to_ptr(block), block_size(block) - sizeof(block_header_t *));
...
static block_header_t *search_suitable_block(
		control_t *control, int *fli, int *sli)
{
	int fl = *fli;
	int sl = *sli;
...
static void remove_free_block(
		control_t *control, block_header_t *block, int fl, int sl)
{
...
static block_header_t *block_split(block_header_t *block, size_t size)
{
	...
	block_header_t *remaining = offset_to_block(
			block_to_ptr(block), size - sizeof(block_header_t *));
	...
	tlsf_assert(
			block_size(block) == remain_size + size + block_header_overhead);
...
static block_header_t *block_merge_prev(
		control_t *control, block_header_t *block)
{
...
static block_header_t *block_merge_next(
		control_t *control, block_header_t *block)
{
	block_header_t *next = block_next(block);
...
static void block_trim_free(
		control_t *control, block_header_t *block, size_t size)
{
...
static void *block_prepare_used(control_t *control, block_header_t *block,
		size_t size, const char *file, const char *function, unsigned long line,
		const char *mname)
#else
static void *block_prepare_used(
		control_t *control, block_header_t *block, size_t size)
#endif
{
	void *p = 0;
...
#ifdef DBG_TLSF_MALLOC
void *tlsf_malloc(tlsf_t tlsf, size_t size, const char *file,
...
#endif
{
	...
	MDBG("tlsf_malloc(%p, %zu) called from %s: %s(%u)\n", tlsf, size, file,
			function, line);
	ptr = block_prepare_used(
			control, block, adjust, file, function, line, mname);
...
void *tlsf_mallocxn(tlsf_t tlsf, size_t size, const char *file,
		const char *function, unsigned int line, const char *mname)
#else
void *tlsf_mallocxn(tlsf_t tlsf, size_t size)
#endif
{
	void *p;
...
#ifdef DBG_TLSF_MALLOC
void tlsf_free(tlsf_t tlsf, void *ptr, const char *file, const char *function,
...
void tlsf_status(tlsf_t pool)
{
	...
	LOG_FP(DEFAULT_FACILITY, memlog,
			"tlsf_status: ", "max used (+overhead)=%zu, max fragments=%zu\n",
			control->max_used, control->max_fragments);

	/* print a summary of the 2 levels bucket list */
	LOG_FP(DEFAULT_FACILITY, memlog, "tlsf_status: ",
			"Free blocks matrix ('.': none, 'X': between 2^X and (2^(X+1)-1) "
			"free blocks, X=A..Z, A=0, B=1, ...)\n");
	LOG_FP(DEFAULT_FACILITY, memlog, "tlsf_status: ",
			"> first-level: %d block list arrays between 2^fl and 2^(fl+1) "
			"bytes (fl=%d..%d)\n",
			FL_INDEX_COUNT, FL_INDEX_SHIFT, FL_INDEX_MAX);
	LOG_FP(DEFAULT_FACILITY, memlog, "tlsf_status: ",
			"v second-level: %d block lists between 2^fl+sl*2^(fl-%d) and "
			"2^fl+(sl+1)*2^(fl-%d)-1 bytes (sl=0..%d)\n",
			SL_INDEX_COUNT, SL_INDEX_COUNT_LOG2, SL_INDEX_COUNT_LOG2,
			SL_INDEX_COUNT - 1);
	for(sl = 0; sl < SL_INDEX_COUNT; sl++) {
		...
		LOG_FP(DEFAULT_FACILITY, memlog, "tlsf_status: ", "%2d|%.*s|\n", sl,
				FL_INDEX_COUNT, summary);
	}
}
...
static mem_counter *get_mem_counter(mem_counter **root, block_header_t *f)
{
	...
	for(x = *root; x; x = x->next)
		if(x->file == f->alloc_info.file && x->func == f->alloc_info.func
...
void tlsf_sums(tlsf_t pool)
{
	...
	LOG_FP(DEFAULT_FACILITY, memlog,
			"tlsf_sums: ", "pool (%p) summarizing all alloc'ed. fragments:\n",
			pool);
	...
	while(x) {
		LOG_FP(DEFAULT_FACILITY, memlog,
				"tlsf_sums: ", " count=%6d size=%10lu bytes from %s: %s(%ld)\n",
				x->count, x->size, x->file, x->func, x->line);
		root = x->next;
		free(x);
		x = root;
	}
...
int tlsf_malloc_init_pkg_manager(void)
{
	...
	if(_tlsf_pkg_pool) {
		...
		if(_tlsf_pkg_block == NULL) {
			...
			fprintf(stderr,
					"could not create tlsf pkg memory pool: %ld bytes\n",
					pkg_mem_size);
			return -1;
		}
	} else {
...
#ifdef DBG_TLSF_MALLOC
	_tlsf_shm_lock = tlsf_malloc(_tlsf_shm_block, sizeof(gen_lock_t), _SRC_LOC_,
...
#ifdef DBG_TLSF_MALLOC
void *tlsf_shm_malloc(void *tlsfmp, size_t size, const char *file,
...
void *tlsf_shm_mallocxz(void *tlsfmp, size_t size, const char *file,
		const char *func, unsigned int line, const char *mname)
{
	void *r;
	tlsf_shm_lock();
...
void *tlsf_shm_mallocxn(void *tlsfmp, size_t size, const char *file,
		const char *func, unsigned int line, const char *mname)
{
	void *r;
	tlsf_shm_lock();
	r = tlsf_mallocxn(tlsfmp, size, file, func, line, mname);
	tlsf_shm_unlock();
	return r;
}
void *tlsf_shm_realloc(void *tlsfmp, void *p, size_t size, const char *file,
		const char *func, unsigned int line, const char *mname)
{
	void *r;
	tlsf_shm_lock();
...
void *tlsf_shm_reallocxf(void *tlsfmp, void *p, size_t size, const char *file,
		const char *func, unsigned int line, const char *mname)
{
	void *r;
	tlsf_shm_lock();
...
void *tlsf_shm_resize(void *tlsfmp, void *p, size_t size, const char *file,
		const char *func, unsigned int line, const char *mname)
{
	void *r;
	tlsf_shm_lock();
...
void *tlsf_shm_mallocxn(void *tlsfmp, size_t size)
{
	void *r;
	tlsf_shm_lock();
	r = tlsf_mallocxn(tlsfmp, size);
	tlsf_shm_unlock();
	return r;
}
...
int tlsf_malloc_init_shm_manager(void)
{
	...
	if(_tlsf_shm_pool) {
		...
		if(!_tlsf_shm_block) {
			...
			fprintf(stderr,
					"could not create tlsf shm memory pool: %ld bytes\n",
					shm_mem_size);
			return -1;
		}
	} else {
	...
	ma.xmallocxn = tlsf_shm_mallocxn;
