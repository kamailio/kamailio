...
#define shm_malloc(s)                                                      \
	_shm_root.xmalloc(_shm_root.mem_block, (s), _SRC_LOC_, _SRC_FUNCTION_, \
			_SRC_LINE_, _SRC_MODULE_)
#define shm_mallocxz(s)                                                      \
	_shm_root.xmallocxz(_shm_root.mem_block, (s), _SRC_LOC_, _SRC_FUNCTION_, \
			_SRC_LINE_, _SRC_MODULE_)
#define shm_mallocxn(s)                                                      \
	_shm_root.xmallocxn(_shm_root.mem_block, (s), _SRC_LOC_, _SRC_FUNCTION_, \
			_SRC_LINE_, _SRC_MODULE_)
...
#define shm_realloc(p, s)                                        \
	_shm_root.xrealloc(_shm_root.mem_block, (p), (s), _SRC_LOC_, \
			_SRC_FUNCTION_, _SRC_LINE_, _SRC_MODULE_)
...
#define shm_resize(p, s)                                        \
	_shm_root.xresize(_shm_root.mem_block, (p), (s), _SRC_LOC_, \
			_SRC_FUNCTION_, _SRC_LINE_, _SRC_MODULE_)
#define shm_free(p)                                                      \
	_shm_root.xfree(_shm_root.mem_block, (p), _SRC_LOC_, _SRC_FUNCTION_, \
			_SRC_LINE_, _SRC_MODULE_)
#define shm_free_unsafe(p)                                      \
	_shm_root.xfree_unsafe(_shm_root.mem_block, (p), _SRC_LOC_, \
			_SRC_FUNCTION_, _SRC_LINE_, _SRC_MODULE_)
#else
...
#define shm_mallocxn(s) _shm_root.xmallocxn(_shm_root.mem_block, (s))
...
#define SHM_MEM_CRITICAL \
	LM_CRIT("could not allocate shared memory from shm pool\n")

#ifdef __SUNPRO_C
#define SHM_MEM_ERROR_FMT(...) \
	LM_ERR("could not allocate shared memory from shm pool" __VA_ARGS__)
#define SHM_MEM_CRITICAL_FMT(...) \
	LM_CRIT("could not allocate shared memory from shm pool" __VA_ARGS__)
#else
#define SHM_MEM_ERROR_FMT(fmt, args...) \
	LM_ERR("could not allocate shared memory from shm pool - " fmt, ##args)
#define SHM_MEM_CRITICAL_FMT(fmt, args...) \
	LM_CRIT("could not allocate shared memory from shm pool - " fmt, ##args)
#endif
