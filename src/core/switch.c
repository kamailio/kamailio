...
static struct switch_cond_table *mk_switch_cond_table(int n)
{
	...
	sct->jump = (struct action **)((char *)sct->cond
								   + ROUND_POINTER(n * sizeof(sct->cond[0])));
	sct->def = 0;
	return sct;
}
...
static struct switch_jmp_table *mk_switch_jmp_table(int jmp_size, int rest)
{
	...
	size = ROUND_POINTER(sizeof(*jt)) + ROUND_INT(jmp_size * sizeof(jt->tbl[0]))
	...
	jt->rest.cond =
			(int *)((char *)jt->tbl + ROUND_INT(jmp_size * sizeof(jt->tbl[0])));
	jt->rest.jump =
			(struct action **)((char *)jt->rest.cond
							   + ROUND_POINTER(
									   rest * sizeof(jt->rest.cond[0])));
	jt->rest.n = rest;
	return jt;
}
...
static struct match_cond_table *mk_match_cond_table(int n)
{
	...
	mct->n = n;
	mct->match =
			(struct match_str *)((char *)mct + ROUND_POINTER(sizeof(*mct)));
	mct->jump = (struct action **)((char *)mct->match
								   + ROUND_POINTER(n * sizeof(mct->match[0])));
	mct->def = 0;
	return mct;
}
...
int fix_switch(struct action *t)
{
	...
	for(c = (struct case_stms *)t->val[1].u.data; c; c = c->next) {
		if(c->ct_rve) {
			...
			if(rval_expr_eval_int(0, 0, &c->label.match_int, c->ct_rve) < 0) {
	...
	if(n == 0) {
		if(default_found) {
			...
			LM_CRIT("default only switch with side-effect not expected at this "
					"point\n");
			ret = E_BUG;
			goto error;
		} else {
	...
	for(c = (struct case_stms *)t->val[1].u.data; c; c = c->next) {
		if(c->ct_rve) {
			...
			if(c->type == MATCH_RE) {
				...
				if(regcomp(regex, s.s, REG_EXTENDED | REG_NOSUB | c->re_flags)
						!= 0) {
					pkg_free(regex);
					regex = 0;
