...
static const char *dns_cache_view_doc[] = {
		"dns cache dump in a human-readable format", 0};

static const char *dns_cache_rpc_lookup_doc[] = {"perform a dns lookup", 0};

static const char *dns_cache_delete_all_doc[] = {
		"deletes all the non-permanent entries from the DNS cache", 0};

static const char *dns_cache_delete_all_force_doc[] = {
		"deletes all the entries from the DNS cache including the permanent "
		"ones",
		0};

static const char *dns_cache_add_a_doc[] = {
		"adds an A record to the DNS cache", 0};

static const char *dns_cache_add_aaaa_doc[] = {
		"adds an AAAA record to the DNS cache", 0};
static const char *dns_cache_add_srv_doc[] = {
		"adds an SRV record to the DNS cache", 0};

static const char *dns_cache_delete_a_doc[] = {
		"deletes an A record from the DNS cache", 0};

static const char *dns_cache_delete_aaaa_doc[] = {
		"deletes an AAAA record from the DNS cache", 0};

static const char *dns_cache_delete_srv_doc[] = {
		"deletes an SRV record from the DNS cache", 0};

static const char *dns_cache_delete_naptr_doc[] = {
		"deletes a NAPTR record from the DNS cache", 0};

static const char *dns_cache_delete_cname_doc[] = {
		"deletes a CNAME record from the DNS cache", 0};

static const char *dns_cache_delete_txt_doc[] = {
		"deletes a TXT record from the DNS cache", 0};

static const char *dns_cache_delete_ebl_doc[] = {
		"deletes an EBL record from the DNS cache", 0};


static const char *dns_cache_delete_ptr_doc[] = {
		"deletes an PTR record from the DNS cache", 0};
...
static const char *dns_cache_stats_get_doc[] = {
		"returns the dns measurement counters.", 0};
...
static const char *dst_blst_stats_get_doc[] = {
		"returns the dst blocklist measurement counters.", 0};
...
static void system_listMethods(rpc_t *rpc, void *c)
{
	...
	for(i = 0; i < rpc_sarray_crt_size; i++) {
		if(rpc->add(c, "s", rpc_sarray[i]->name) < 0)
			return;
	}
}
...
static void core_runinfo(rpc_t *rpc, void *c)
{
	...
	rpc->struct_printf(s, "utime_days", "%dd %dh %dm %ds", uptime / 86400,
			(uptime % 86400) / 3600, ((uptime % 86400) % 3600) / 60,
...
static void core_psx(rpc_t *rpc, void *c)
{
	...
	for(p = 0; p < *process_count; p++) {
		rpc->add(c, "{", &handle);
		rpc->struct_add(
				handle, "dds", "IDX", p, "PID", pt[p].pid, "DSC", pt[p].desc);
	}
}
...
static void core_psa(rpc_t *rpc, void *c)
{
	...
	for(p = 0; p < *process_count; p++) {
		rpc->add(c, "{", &handle);
		rpc->struct_add(handle, "dddds", "index", p, "pid", pt[p].pid, "status",
				pt[p].status, "rank", pt[p].rank, "description", pt[p].desc);
	}
}
...
static void core_shmmem(rpc_t *rpc, void *c)
{
	...
	shm_info(&mi);
	rpc->add(c, "{", &handle);
	rpc->struct_add(handle, "dddddd", "total",
			(unsigned int)(mi.total_size >> rs), "free",
			(unsigned int)(mi.free_size >> rs), "used",
			(unsigned int)(mi.used_size >> rs), "real_used",
			(unsigned int)(mi.real_used >> rs), "max_used",
			(unsigned int)(mi.max_used >> rs), "fragments",
			(unsigned int)mi.total_frags);
}
...
static void core_sfmalloc(rpc_t *rpc, void *c)
{
	...
	if(rpc->scan(c, "d", &r) >= 1) {
		if(r >= (int)SF_HASH_POOL_SIZE) {
			rpc->fault(c, 500, "invalid hash number %d (max %d)", r,
					(unsigned int)SF_HASH_POOL_SIZE - 1);
			return;
		} else if(r < 0)
			goto all;
		rpc->add(c, "{", &handle);
		rpc->struct_add(handle, "dd", "hash  ", r, "size  ", r * SF_ROUNDTO);
		for(i = 0; i < SFM_POOLS_NO; i++) {
			rpc->struct_add(handle, "dddd", "pool  ", i, "frags ",
					(unsigned int)shm_block->pool[i].pool_hash[r].no, "misses",
					(unsigned int)shm_block->pool[i].pool_hash[r].misses,
					"mem   ",
					(unsigned int)shm_block->pool[i].pool_hash[r].no * r
							* SF_ROUNDTO);
		}
	}
	return;
all:
	...
	for(i = 0; i < SFM_POOLS_NO; i++) {
		...
		rpc->add(c, "{", &handle);
		rpc->struct_add(handle, "dddddd", "pool  ", i, "frags ",
				(unsigned int)frags, "t. misses", (unsigned int)misses,
				"mem   ", (unsigned int)mem, "missed",
				(unsigned int)shm_block->pool[i].missed, "hits",
				(unsigned int)shm_block->pool[i].hits);
		pool_frags += frags;
	}
	main_s_frags = 0;
	for(r = 0; r < SF_HASH_POOL_SIZE; r++) {
	...
	rpc->add(c, "{", &handle);
	rpc->struct_add(handle, "ddddddddddddd", "max_frags      ",
			(unsigned int)max_frags, "max_frags_pool ", max_frags_pool,
			"max_frags_hash", max_frags_hash, "max_misses     ",
			(unsigned int)max_misses, "max_misses_pool", max_misses_pool,
			"max_misses_hash", max_misses_hash, "max_mem        ",
			(unsigned int)max_mem, "max_mem_pool   ", max_mem_pool,
			"max_mem_hash   ", max_mem_hash, "in_pools_frags ",
			(unsigned int)pool_frags, "main_s_frags   ",
			(unsigned int)main_s_frags, "main_b_frags   ",
			(unsigned int)main_b_frags, "main_frags     ",
			(unsigned int)(main_b_frags + main_s_frags));
}
...
static void core_tcpinfo(rpc_t *rpc, void *c)
{
	...
	if(!tcp_disable) {
		tcp_get_info(&ti);
		rpc->add(c, "{", &handle);
		rpc->struct_add(handle, "dddddd", "readers", ti.tcp_readers,
...
static void core_tcp_options(rpc_t *rpc, void *c)
{
	...
	if(!tcp_disable) {
		tcp_options_get(&t);
		rpc->add(c, "{", &handle);
		rpc->struct_add(handle, "ddddddddddddddddddddddd", "connect_timeout",
				t.connect_timeout_s, "send_timeout", TICKS_TO_S(t.send_timeout),
				...
				"no_connect", t.no_connect, "fd_cache", t.fd_cache, "async",
				t.async, "connect_wait", t.tcp_connect_wait, "conn_wq_max",
				t.tcpconn_wq_max, "wq_max", t.tcp_wq_max, "defer_accept",
				t.defer_accept, "delayed_ack", t.delayed_ack, "syncnt",
				t.syncnt, "linger2", t.linger2, "keepalive", t.keepalive,
				"keepidle", t.keepidle, "keepintvl", t.keepintvl, "keepcnt",
				t.keepcnt, "crlf_ping", t.crlf_ping, "accept_aliases",
				t.accept_aliases, "alias_flags", t.alias_flags,
...
static void core_tcp_list(rpc_t *rpc, void *c)
{
	...
	for(i = 0; i < TCP_ID_HASH_SIZE; i++) {
		for(con = tcpconn_id_hash[i]; con; con = con->id_next) {
			...
			rpc->struct_add(handle, "dssdddsdsd", "id", con->id, "type", type,
					"state", state, "timeout", timeout, "lifetime", lifetime,
					"ref_count", con->refcnt, "src_ip", src_ip, "src_port",
					con->rcv.src_port, "dst_ip", dst_ip, "dst_port",
					con->rcv.dst_port);
		}
	}
	TCPCONN_UNLOCK;
...
static void core_udp4rawinfo(rpc_t *rpc, void *c)
{
	...
	rpc->add(c, "{", &handle);
	rpc->struct_add(handle, "ddd", "udp4_raw",
			cfg_get(core, core_cfg, udp4_raw), "udp4_raw_mtu",
			cfg_get(core, core_cfg, udp4_raw_mtu), "udp4_raw_ttl",
			cfg_get(core, core_cfg, udp4_raw_ttl));
#else  /* USE_RAW_SOCKS */
...
static void core_aliases_list(rpc_t *rpc, void *c)
{
	...
	rpc->add(c, "{", &hr);
	rpc->struct_add(hr, "s", "myself_callbacks",
			is_check_self_func_list_set() ? "yes" : "no");
	rpc->struct_add(hr, "[", "aliases", &hs);
	for(a = aliases; a; a = a->next) {
		rpc->struct_add(hs, "{", "alias", &ha);
		rpc->struct_add(
				ha, "sS", "proto", proto2a(a->proto), "address", &a->alias);
		if(a->port)
			rpc->struct_add(ha, "d", "port", a->port);
		else
			rpc->struct_add(ha, "s", "port", "*");
	}
}
...
static void core_sockets_list(rpc_t *rpc, void *c)
{
	...
	do {
		...
		for(si = list ? *list : 0; si; si = si->next) {
			...
			if(si->addr_info_lst) {
				rpc->struct_add(ha, "ss", "proto", get_proto_name(proto),
...
static void core_modules(rpc_t *rpc, void *c)
{
	...
	for(t = get_loaded_modules(); t; t = t->next) {
		if(rpc->add(c, "s", t->exports.name) < 0)
			return;
	}
}
...
static void core_ppdefines_full(rpc_t *rpc, void *c)
{
	...
	while((ppdef = pp_get_define(i)) != NULL && ppdef->name.s != NULL) {
		...
		if(ppdef->value.s != NULL) {
			rpc->struct_add(vp, "SdS", "name", &ppdef->name, "type",
					ppdef->dtype, "value", &ppdef->value);
		} else {
			rpc->struct_add(vp, "Sds", "name", &ppdef->name, "type",
					ppdef->dtype, "value", "none");
		}
...
static rpc_export_t core_rpc_methods[] = {
		...
		{"core.prints", core_prints, core_prints_doc, RET_ARRAY},
		{"core.printi", core_printi, core_printi_doc, RET_ARRAY},
		{"core.echo", core_echo, core_echo_doc, RET_ARRAY},
		...
		{"dns.view", dns_cache_view, dns_cache_view_doc, RET_ARRAY},
