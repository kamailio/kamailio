...
static void system_listMethods(rpc_t *rpc, void *c)
{
	...
	for(i = 0; i < rpc_sarray_crt_size; i++) {
		if(rpc->add(c, "s", rpc_sarray[i]->r.name) < 0)
			return;
	}
}
...
static void core_shmmem(rpc_t *rpc, void *c)
{
	...
	shm_info(&mi);
	rpc->add(c, "{", &handle);
	rpc->struct_add(handle, "jjjjjj", "total", (mi.total_size >> rs), "free",
			(mi.free_size >> rs), "used", (mi.used_size >> rs), "real_used",
			(mi.real_used >> rs), "max_used", (mi.max_used >> rs), "fragments",
			mi.total_frags);
}
...
static void core_sockets_list(rpc_t *rpc, void *c)
{
	...
	do {
		...
		for(si = list ? *list : 0; si; si = si->next) {
			...
			if(si->addr_info_lst) {
				rpc->struct_add(ha, "sss", "af", get_af_name(si->address.af),
						"proto", get_proto_name(proto), "address",
						si->address_str.s);
