...
inline static int handle_ser_child(struct process_table *p, int fd_i)
{
	...
	switch(cmd) {
		...
		case CONN_WRITE_REQ: {
			 ...
			 * WS/WSS keep the inline encode+watch path below. */
		...
		case CONN_CONNECT_REQ: {
			...
			if(cc != NULL) {
				...
				if(unlikely((cc->state == S_CONN_BAD)
				...
				if(likely(cc->type == PROTO_TCP || cc->type == PROTO_TLS)) {
					if(unlikely(tcp_reactor_wsq_add(cc, creq->buf, creq->len,
										creq->send_flags)
								< 0)) {
						cc->state = S_CONN_BAD;
					...
					if(!(cc->flags & F_CONN_POOL_BUSY)) {
						...
						if(unlikely(tcp_reactor_enqueue_job(cc, TCP_R_WRITE)
									< 0)) {
 ...
 * contract (plain TCP and TLS run through the pool; WS/WSS still inline).
 *
