...
inline static int bool_rvstrop2(
		enum rval_expr_op op, int *res, struct rvalue *rv1, struct rvalue *rv2)
{
	str *s1;
	str *s2;
	regex_t tmp_re;
...
inline static int rval_str_lop2(struct run_act_ctx *h, struct sip_msg *msg,
		int *res, enum rval_expr_op op, struct rvalue *l, struct rval_cache *c1,
		struct rvalue *r, struct rval_cache *c2)
{
	struct rvalue *rv1;
	struct rvalue *rv2;
 ...
 * @return 0 success, -1 on error
 */
inline static int rval_int_strop1(struct run_act_ctx *h, struct sip_msg *msg,
		int *res, enum rval_expr_op op, struct rvalue *l, struct rval_cache *c1)
{
	struct rvalue *rv1;
	int ret;
...
inline static int rv_defined(struct run_act_ctx *h, struct sip_msg *msg,
		int *res, struct rvalue *rv, struct rval_cache *cache)
{
	avp_t *r_avp;
	int_str avp_val;
	pv_value_t pval;
 ...
 * @return 0 on success, -1 on error
 */
inline static int int_rve_defined(struct run_act_ctx *h, struct sip_msg *msg,
 ...
 */
int rval_expr_eval_int(struct run_act_ctx *h, struct sip_msg *msg, int *res,
		struct rval_expr *rve)
{
	...
	switch(rve->op) {
		...
		case RVE_LNOT_OP:
		case RVE_BNOT_OP:
			if(unlikely((ret = rval_expr_eval_int(h, msg, &i1, rve->left.rve))
						< 0))
				break;
		...
		case RVE_IDIFF_OP:
			if(unlikely((ret = rval_expr_eval_int(h, msg, &i1, rve->left.rve))
						< 0))
				break;
			if(unlikely((ret = rval_expr_eval_int(h, msg, &i2, rve->right.rve))
						< 0))
				break;
		...
		case RVE_LAND_OP:
			...
			if(i1 == 0) {
				*res = 0;
				...
				if(unlikely((ret = rval_expr_eval_int(
		...
		case RVE_DIFF_OP:
			 ...
			 */
			rval_cache_init(&c1);
			if(unlikely((ret = rval_expr_eval_rvint(
...
int rval_expr_eval_rvint(struct run_act_ctx *h, struct sip_msg *msg,
		struct rvalue **res_rv, int *res_i, struct rval_expr *rve,
		struct rval_cache *cache)
{
	...
	switch(rve->op) {
		...
		case RVE_PLUS_OP:
			...
			if(rv1 == 0) {
				if(unlikely((r = rval_expr_eval_int(h, msg, &j, rve->right.rve))
							< 0)) {
					LM_ERR("rval expression evaluation failed (%d,%d-%d,%d)"
						   "\n",
		...
		case RVE_PLUS_OP:
			...
			switch(type) {
				case RV_INT:
					...
					if(unlikely((r = rval_expr_eval_int(
