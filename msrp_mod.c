...
msrp_options_t  msrp_opt = MSRP_OPTIONS_DEFAULT;
...
static param_export_t params[]={
	...
	{"use_path_addr",     PARAM_STR,   &msrp_use_path_addr},
        {"msrp_hdr_size",     PARAM_INT,   &msrp_opt.msrp_frame_hdrs_max_size},
        {"msrp_body_size",    PARAM_INT,   &msrp_opt.msrp_frame_body_max_size},
        {"msrp_use_sys_mem",  PARAM_INT,   &msrp_opt.msrp_frame_use_sys_mem},
	{0, 0, 0}
};
...
static int mod_init(void)
{
	...
	if(!module_loaded("tls")) {
        ...
        if (msrp_opt.msrp_frame_use_sys_mem > 0)
        {
            msrp_opt.msrp_frame_use_sys_mem=1;
        }  
        init_msrp_options(&msrp_opt); 
        msrp_print_options();
	return 0;
}
