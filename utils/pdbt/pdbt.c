...
void print_usage(char *program)
{
	...
	LINFO("  %s -s <csv file> -m <mmap file> [-k <ids>] [-o] [-u <tree file>] "
		  "[-l <log level>] build\n",
			program);
	LINFO("  %s (-m <mmap file>|-r <host>:<port>) [-q <timeout>] [-f <query "
		  "file>] [-t <carrier text file>] [-l <log level>] query "
		  "<number>...\n",
			program);
	LINFO("\n");
	LINFO("  Commands:\n");
	...
	LINFO("               Numbers on the command line will not be "
		  "processed.\n");
	LINFO("    -t <file>: Specifies the file containing carrier names.\n");
	LINFO("               In addition to the carrier code these names will be "
		  "shown\n");
	LINFO("               when querying numbers.\n");
	LINFO("               Each carrier id and name must be in a separate "
		  "line.\n");
	...
	LINFO("              Format: <id>[,<id>...]\n");
	LINFO("    -r <host>:<port>: Host and port to be used for remote server "
		  "queries.\n");
	LINFO("    -q <timeout>: Timeout for remote server queries in "
		  "milliseconds.\n");
	LINFO("                  Default is 500 ms.\n");
	LINFO("    -o: Try to optimize the data structure when building a mmap "
		  "image.\n");
	LINFO("    -u: Write (possibly optimized) tree structure in human-readable "
...
void print_stats(struct dt_node_t *root)
{
	...
	LINFO("| %ld nodes in tree (%ld bytes, %ld KB, %ld MB)\n", (long int)s,
			(long int)s * sizeof(struct dt_node_t),
			(long int)s * sizeof(struct dt_node_t) / 1024,
			(long int)s * sizeof(struct dt_node_t) / 1024 / 1024);
	l = dt_leaves(root);
	LINFO("| %ld nodes are leaves (%ld bytes, %ld KB, %ld MB)\n", (long int)l,
			(long int)l * sizeof(struct dt_node_t),
			(long int)l * sizeof(struct dt_node_t) / 1024,
			(long int)l * sizeof(struct dt_node_t) / 1024 / 1024);
	c = dt_loaded_nodes(root);
	LINFO("| %ld carrier nodes in tree\n", (long int)c);
	LINFO("| \n");
	LINFO("| After saving with leaf node compression:\n");
	LINFO("| %ld nodes in tree (%ld bytes, %ld KB, %ld MB)\n", (long int)s - l,
			(long int)(s - l) * sizeof(struct dtm_node_t),
			(long int)(s - l) * sizeof(struct dtm_node_t) / 1024,
			(long int)(s - l) * sizeof(struct dtm_node_t) / 1024 / 1024);
	LINFO("+----------------------------------------\n");
}
...
int file_query(char *filename, query_func_t query_func, void *data)
{
	...
	while((read = getline(&line, &len, fp)) != -1) {
		p = line;
		while((*p >= '0') && (*p <= '9') && (p < line + len))
			p++;
		comment = p;
		while((*p >= 32) && (p < line + len))
			p++;
		*p = '\0';
 ...
 */
int dt_write_tree_recursor(
		const struct dt_node_t *node, const int fd, char *number)
{
	int i;
	int ret;
	int slen;
	...
	if(slen > 0) {
		...
		bufsize =
				slen + 1 + 1 + 3 + 1
				+ 1; // line buffer (telephone number + colon + white space + carrier ID + newline + \0)
	...
	for(i = 0; i < 10; i++) {
		...
		number[slen] = i + '0';
		number[slen + 1] =
				'\0'; /* must always be done because other recursive invocations operate on `number' too */
 ...
 */
dtm_node_index_t save_recursor(
		struct dt_node_t *root, int fd, dtm_node_index_t n)
{
	dtm_node_index_t i;
	...
	if(write(fd, &node, sizeof(struct dtm_node_t))
			!= sizeof(struct dtm_node_t)) {
		LERR("could not write %d bytes of node data at file address %d: "
			 "errno=%d (%s)\n",
...
*/
int keep_carrier_func(
		carrier_t carrier, int keep_carriers_num, carrier_t keep_carriers[])
{
	int i;

	for(i = 0; i < keep_carriers_num; i++) {
		if(keep_carriers[i] == carrier)
			return 1;
	}
...
int merge_carrier_recursor(struct dt_node_t *node, int keep_carriers_num,
		carrier_t keep_carriers[], carrier_t lastcarrier)
{
	...
	if(node->carrier > 0) {
		if(!keep_carrier_func(
				   node->carrier, keep_carriers_num, keep_carriers)) {
			sum++;
			if(lastcarrier == 0)
				node->carrier =
						0; /* first carrier we encountered. we can remove it since we are not interested in it. */
			else {
				node->carrier =
						OTHER_CARRIERID; /* we already have a carrier we are interested in. this is an exception, set it to a special carrier id. */
			}
		}
	}

	if(node->carrier > 0)
		currentcarrier = node->carrier;
	else
		currentcarrier = lastcarrier;

	/* merge children carriers */
	for(i = 0; i < 10; i++) {
		sum += merge_carrier_recursor(node->child[i], keep_carriers_num,
				keep_carriers, currentcarrier);
	}
...
*/
int merge_carrier(struct dt_node_t *root, int keep_carriers_num,
		carrier_t keep_carriers[])
{
 ...
 */
int query_udp(char *number, int timeout, struct pollfd *pfds,
		struct sockaddr_in *dstaddr, socklen_t dstaddrlen)
{
	struct pdb_msg msg;
	...
	switch(PDB_VERSION) {
		case PDB_VERSION_1:
			pdb_msg_format_send(&msg, PDB_VERSION, PDB_TYPE_REQUEST_ID,
					PDB_CODE_DEFAULT, htons(id), buf, reqlen);
			//            pdb_msg_dbg(msg);
			...
			ret = sendto(pfds->fd, (struct pdb_msg *)&msg, msg.hdr.length,
					MSG_DONTWAIT, (struct sockaddr *)dstaddr, dstaddrlen);
			if(ret < 0) {
		...
		default:
			/* send request to all servers */
			ret = sendto(pfds->fd, buf, reqlen, MSG_DONTWAIT,
...
int main(int argc, char *argv[])
{
	...
	while((opt = getopt(argc, argv, "s:m:f:u:t:r:q:k:ol:vh")) != -1) {
		switch(opt) {
			...
			case 'r':
				...
				tmp = strchr(host_str, ':');
				if(tmp == NULL) {
					LERR("syntax error in remote host:port specification "
						 "'%s'.\n",
							host_str);
					return -1;
				}
	...
	if(strcmp(argv[optind], "build") == 0) {
		...
		LINFO("Node size is %ld bytes (%ld for dtm)\n",
				(long int)sizeof(struct dt_node_t),
				(long int)sizeof(struct dtm_node_t));
		print_stats(&root);
		...
		if((mmap_file != NULL) && (host_str != NULL)) {
			LERR("you cannot query a pdb_server and a mmap file at the same "
				 "time.\n");
			return 1;
		}
