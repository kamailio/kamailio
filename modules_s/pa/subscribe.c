...
static struct {
	...
	{ EVENT_PRESENCE, { MIMETYPE(APPLICATION,PIDFXML), MIMETYPE(APPLICATION,XPIDFXML), MIMETYPE(APPLICATION,LPIDFXML), 0 } },
...
int check_message(struct sip_msg* _m)
{
	...
	if (_m->event) {
		...
		LOG(L_ERR, "check_message -1- accepts_mimes=%p\n", accepts_mimes);
