...
int t_add_transaction( struct s_table* hash_table , struct sip_msg* p_msg )
{
   ...
   /* inbound request */
   new_cell->transaction.inbound_request =  sip_msg_cloner(p_msg) ;
   ...
   while( p_cell )
   {
            ...
            if ( /*to length*/ p_cell->transaction.inbound_request->to->body.len == p_msg->to->body.len )
                //if ( /*tag length*/ (!p_cell->transaction.inbound_request->tag && !p_msg->tag) || (p_cell->transaction.inbound_request->tag && p_msg->tag && p_cell->transaction.inbound_request->tag->body.len == p_msg->tag->body.len) )
                  if ( /*callid length*/ p_cell->transaction.inbound_request->callid->body.len == p_msg->callid->body.len )
                     if ( /*cseq length*/ p_cell->transaction.inbound_request->cseq->body.len == p_msg->cseq->body.len )
                               ...
                               if ( /*to*/ !memcmp( p_cell->transaction.inbound_request->to->body.s , p_msg->to->body.s , p_msg->to->body.len)  )
                                  //if ( /*tag*/ (!p_cell->transaction.inbound_request->tag && !p_msg->tag) || (p_cell->transaction.inbound_request->tag && p_msg->tag && !memcmp( p_cell->transaction.inbound_request->tag->body.s , p_msg->tag->body.s , p_msg->tag->body.len )) )
                                     if ( /*callid*/ !memcmp( p_cell->transaction.inbound_request->callid->body.s , p_msg->callid->body.s , p_msg->callid->body.len ) )
                                        if ( /*cseq*/ !memcmp( p_cell->transaction.inbound_request->cseq->body.s , p_msg->cseq->body.s , p_msg->cseq->body.len ) )
      ...
      { /* it's a ACK request*/
         ...
         if ( /*from length*/ p_cell->transaction.inbound_request->from->body.len == p_msg->from->body.len )
            if ( /*to length*/ p_cell->transaction.inbound_request->to->body.len == p_msg->to->body.len )
               if ( /*callid length*/ p_cell->transaction.inbound_request->callid->body.len == p_msg->callid->body.len )
                  if ( /*cseq_nr length*/ get_cseq(p_cell->transaction.inbound_request)->number.len == get_cseq(p_msg)->number.len )
                      if ( /*cseq_method length*/ get_cseq(p_cell->transaction.inbound_request)->method.len == 6 /*INVITE*/ )
                            ...
                            if ( /*from*/ !memcmp( p_cell->transaction.inbound_request->from->body.s , p_msg->from->body.s , p_msg->from->body.len ) )
                               if ( /*to*/ !memcmp( p_cell->transaction.inbound_request->to->body.s , p_msg->to->body.s , p_msg->to->body.len)  )
                                  //if ( /*tag*/ !memcmp( p_cell->transaction.tag->s , p_msg->tag->body.s , p_msg->tag->body.len ) )
                                     if ( /*callid*/ !memcmp( p_cell->transaction.inbound_request->callid->body.s , p_msg->callid->body.s , p_msg->callid->body.len ) )
                                        if ( /*cseq_nr*/ !memcmp( get_cseq(p_cell->transaction.inbound_request)->number.s , get_cseq(p_msg)->number.s , get_cseq(p_msg)->number.len ) )
                                           if ( /*cseq_method*/ !memcmp( get_cseq(p_cell->transaction.inbound_request)->method.s , "INVITE" , 6 ) )
                                              { /* WE FOUND THE GOLDEN EGG !!!! */
                                                 T = p_cell;
                                                 unref_transaction ( p_cell );
                                                 return 1;
                             ...
                             if ( /*from*/ !memcmp( p_cell->transaction.inbound_request->from->body.s , p_msg->from->body.s , p_msg->from->body.len ) )
                                if ( /*to*/ !memcmp( p_cell->transaction.inbound_request->to->body.s , p_msg->to->body.s , p_msg->to->body.len)  )
                                   //if ( /*tag*/ (!p_cell->transaction.inbound_request->tag && !p_msg->tag) || (p_cell->transaction.inbound_request->tag && p_msg->tag && !memcmp( p_cell->transaction.inbound_request->tag->body.s , p_msg->tag->body.s , p_msg->tag->body.len )) )
                                      if ( /*callid*/ !memcmp( p_cell->transaction.inbound_request->callid->body.s , p_msg->callid->body.s , p_msg->callid->body.len ) )
                                          if ( /*cseq_nr*/ !memcmp( get_cseq(p_cell->transaction.inbound_request)->number.s , get_cseq(p_msg)->number.s , get_cseq(p_msg)->number.len ) )
                                             if ( /*cseq_method*/ strcmp( get_cseq(p_cell->transaction.inbound_request)->method.s , "CANCEL" ) )
...
int t_forward( struct s_table* hash_table , struct sip_msg* p_msg , unsigned int dest_ip_param , unsigned int dest_port_param )
{
   ...
   if ( T->transaction.outbound_request[0]==NULL )
   {
      ...
      T->transaction.outbound_request[0] = (struct retrans_buff*)sh_malloc( sizeof(struct retrans_buff) );
      T->transaction.outbound_request[0]->type      = SIP_REQUEST;
      T->transaction.nr_of_outgoings = 1;
...
int t_on_reply_received( struct s_table  *hash_table , struct sip_msg  *p_msg )
{
   ...
   if ( T->transaction.inbound_request->first_line.u.request.method_value==METHOD_INVITE && p_msg->first_line.u.reply.statusclass>2 )
   {
      // sendACK   TO DO !!!!!!!!!!
      // remove_from_retransmission_list( T->transaction.outbound_request[branch] );     TO DO !!!!!
   ...
   if ( T->transaction.inbound_response && (T->transaction.status/100)>1 )
   ...
   {  /* no reply sent yet or sent but not final*/
      ...
      if ( p_msg->first_line.u.reply.statusclass==1 && T->transaction.inbound_request->first_line.u.request.method_value!=METHOD_INVITE )
      ...
      {
           if ( p_msg->first_line.u.reply.statuscode > T->transaction.status )
              t_relay_reply( T , branch , p_msg );
         return 1;
      }
...
int t_retransmit_reply( struct s_table *hash_table , struct sip_msg* p_msg )
{
   ...
   if ( T  && T->transaction.inbound_response )
   {
       //sendto(  );   TO DO !!!!!!
      return 0;
   }
            ...
            while( p_cell )
             {
                ...
                if ( p_cell->transaction.label = entry_label )
                   /* has the transaction the wanted branch? */
                   if ( p_cell->transaction.nr_of_outgoings>branch_id && p_cell->transaction.outbound_request[branch_id] )
                    {/* WE FOUND THE GOLDEN EGG !!!! */
                       p_Trans = &p_cell;
                       *p_branch = branch_id;
                       unref_Transaction( p_cell );
...
int t_store_incoming_reply( struct cell* Trans, unsigned int branch, struct sip_msg* p_msg )
{
   ...
   if ( Trans->transaction.outbound_response[branch] )
      free_sip_msg( Trans->transaction.outbound_response[branch] );
   Trans->transaction.outbound_response[branch] = sip_msg_cloner( p_msg );
}
...
int t_all_final( struct cell *Trans )
{
   ...
   for( i=0 ; i<Trans->transaction.nr_of_outgoings ; i++  )
      if (  !Trans->transaction.outbound_response[i] || (Trans->transaction.outbound_response[i]) && Trans->transaction.outbound_response[i]->first_line.u.reply.statuscode<200 )
         return 0;
...
int relay_lowest_reply_upstream( struct cell *Trans , struct sip_msg *p_msg )
{
   ...
   for(  ; i<T->transaction.nr_of_outgoings ; i++ )
      if ( T->transaction.outbound_response[i] && T->transaction.outbound_response[i]->first_line.u.reply.statuscode>=200 && T->transaction.outbound_response[i]->first_line.u.reply.statuscode<lowest_v )
      {
         lowest_i =i;
         lowest_v = T->transaction.outbound_response[i]->first_line.u.reply.statuscode;
      }
...
int push_reply_from_uac_to_uas( struct sip_msg *p_msg , unsigned int branch )
{
   ...
   if ( T->transaction.inbound_response )
   {
      sh_free( T->transaction.inbound_response->buffer );
