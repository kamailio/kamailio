...
#define DIGEST_SHA256	  ", algorithm=SHA-256"
#define DIGEST_SHA256_LEN (sizeof(DIGEST_SHA256)-1)
 ...
 * If algorithm is null, SHA-256 is used.
 *
		...
		str* nonce, str* algorithm, struct qp* qop, int hftype, str *ahf)
{
	...
	*p = '"'; p++;
	...
	if (algorithm) {
		...
		memcpy(p, DIGEST_SHA256, DIGEST_SHA256_LEN ); p += DIGEST_SHA256_LEN;
	}
