...
static int sip_capture_store(struct _sipcapture_object *sco, str *dtable, _capture_mode_data_t * cm_data)
{
    ...
    db_keys[44] = &status_column;
    db_vals[44].type = DB1_STR;
    db_vals[44].nul = 0;
    db_vals[44].val.str_val = sco->status;
...
static int sip_capture(struct sip_msg *msg, str *_table, _capture_mode_data_t * cm_data)
{
	...
	if(msg->first_line.type == SIP_REQUEST) {
        ...
        EMPTY_STR(sco.status);
	}
	else if(msg->first_line.type == SIP_REPLY) {
        sco.method = ((struct cseq_body*)msg->cseq->parsed)->method;
        sco.status = msg->first_line.u.reply.status;
