...
peer* get_first_connected_route(cdp_session_t* cdp_session, routing_entry *r, int app_id, int vendor_id) {
        ...
        peer *p;
        int j;
        unsigned char isCdpSession = 0; /* V1.2 */
        ...
        if (cdp_session)
        {
           ...
           if ((cdp_session->sticky_peer_fqdn.len > 0) && cdp_session->sticky_peer_fqdn.s)
           {
              ...
              p = get_peer_by_fqdn(&cdp_session->sticky_peer_fqdn); /*V1.1 NB!!! get_peer_by_fqdn does lock_get(peer_list_lock) AND lock_release;*/
        ...
        for (i = r; i; i = i->next)
        {
                if (peer_count >= LB_MAX_PEERS)
                        break;
                if (isCdpSession)   /*V1.2--we  dont want to hold two locks again in below code that calls get_peer_by_fqdn*/
                {
                ...
                if (isCdpSession)   /*V1.2--we  dont want to hold two locks again in below code that calls get_peer_by_fqdn*/
                {
                   AAASessionsLock(cdp_session->hash);
                }
        ...
        if (isCdpSession) { /*V1.2 just to conform to test mechanism*/
