...
src_configure() {
	...
	use redis && KAMODULES="${KAMODULES} ndb_redis topos_redis"

	use json && KAMODULES="${KAMODULES} json jsonrpcc jsonrpcs"

	use pua && KAMODULES="${KAMODULES} pua pua_bla pua_dialoginfo pua_reginfo pua_rpc pua_usrloc"

	use ims && KAMODULES="${KAMODULES} ims_auth ims_charging ims_dialog ims_diameter_server ims_icscf ims_isc ims_ocs ims_qos ims_registrar_pcscf ims_registrar_scscf ims_usrloc_pcscf ims_usrloc_scscf cdp cdp_avp"
...
src_install () {
	...
	cd "${S}"
	dodoc ChangeLog COPYING INSTALL ISSUES README README.md
}
