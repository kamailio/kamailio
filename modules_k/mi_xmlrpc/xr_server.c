...
xmlrpc_value*  default_method	(xmlrpc_env* 	env, 
	...
	if ( f == 0 ) {
		LOG(L_ERR, "ERROR: mi_xmlrpc: default_method: Command %s is not available!\n", methodName);
		xmlrpc_env_set_fault_formatted(env, XMLRPC_NO_SUCH_METHOD_ERROR, "Requested command (%s) is not available!", methodName);
		goto error;
	}
	...
	if ( mi_cmd == NULL ){
		LOG(L_ERR, "ERROR: mi_xmlrpc: default_method: error parsing mi tree\n");
		if ( !env->fault_occurred ) xmlrpc_env_set_fault(env, XMLRPC_INTERNAL_ERROR, "The xmlrpc request could not be parsed into a MI tree!");
		goto error;
	}
	...
	if ( ( mi_rpl = run_mi_cmd(f, mi_cmd) ) == 0 ){
		LOG(L_ERR, "ERROR: mi_xmlrpc: default_method: Command (%s) processing failed.\n", methodName);
		xmlrpc_env_set_fault_formatted(env, XMLRPC_INTERNAL_ERROR, "Command (%s) processing failed.\n", methodName);
 		goto error;
	}
	...
	if ( rpl_opt == 1 ) {
		...
		if ( xr_build_response_array( env, mi_rpl ) != 0 ){
			LOG(L_ERR, "ERROR: mi_xmlrpc: default_method: Failed to parse the xmlrpc response from the mi tree.\n");
			if ( !env->fault_occurred ) xmlrpc_env_set_fault(env, XMLRPC_INTERNAL_ERROR, "Failed to parse the xmlrpc response from the mi tree.");
 			goto error;
		}
		...
		if ( (response = xr_build_response( env, mi_rpl )) == 0 ){
			LOG(L_ERR, "ERROR: mi_xmlrpc: default_method: Failed to parse the xmlrpc response from the mi tree.\n");
			if ( !env->fault_occurred ) xmlrpc_env_set_fault_formatted(env, XMLRPC_INTERNAL_ERROR, "Failed to parse the xmlrpc response from the mi tree.");
 			goto error;
		}
...
int set_default_method ( xmlrpc_env * env )
{
	...
	if ( env->fault_occurred ) {
		LOG(L_ERR, "ERROR: mi_xmlrpc: set_default_method: Failed to add default method: %s\n", env->fault_string);
		return -1;
	}
