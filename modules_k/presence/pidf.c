...
#if 0
/* function for updating a message body */
int update_xml(str* body, str* new_body)
{
	...
	xmlNodePtr p_root, new_p_root ;
	xmlNodePtr node = NULL;
	xmlNodePtr new_node = NULL, aux_node, aux2_node;
	xmlNodePtr tuple, new_tuple;
	char* atrib = NULL, *new_atrib = NULL ;
	int ok= 0, i;
	int renew_old_tag[50];
	...
	if((doc== NULL) || (new_doc == NULL) )
	{
		LOG(L_ERR,"PRESENCE:update_xml: ERROR while parsing the xml body"
				" message\n");
		return -1;
	}

	p_root = xmlDocGetNodeByName(doc,"presence", NULL);
	new_p_root = xmlDocGetNodeByName(new_doc,"presence", NULL);

/*

	for (new_node = new_p_root->children; new_node!=NULL; new_node = new_node->next)
	{
		new_atrib = xmlNodeGetAttrContentByName(new_node, "id");
		LOG(L_ERR,"%s - %s\n", new_node->name, new_atrib);
	}
		LOG(L_ERR,"\n----------------\n");
*/
	...
	for (new_node = new_p_root->children; new_node!=NULL; new_node = new_node->next) 
	{
		...
		{	
			...
			for (node = p_root->children; node; node = node->next) 
			{
				...
				if(xmlStrcasecmp (node->name,(unsigned char*)"text")!=0)
				{	
					if( xmlStrcasecmp(node->name,new_node->name) == 0)
					{	
						tuple=node;
						new_tuple=new_node;

						atrib = xmlNodeGetAttrContentByName(node, "id");
						new_atrib = xmlNodeGetAttrContentByName(new_node, "id");
						
						if( (atrib!=NULL) && (new_atrib!=NULL))
						{	
							if(xmlStrcasecmp((unsigned char*)atrib,(unsigned char*)new_atrib)==0)
							{	
								ok=1;
								aux2_node= node->children;
								for(aux_node = node->children->next; aux_node; aux_node = aux_node->next)
								{
									xmlUnlinkNode(aux2_node);	
									xmlFreeNode(aux2_node);
									aux2_node= aux_node;
								}	

								node->children = new_node->children;
								for(aux_node=node->children; aux_node; aux_node=aux_node->next)
								{
								...
								renew_old_tag[i] = 1; /* mark that it has been renewed and should not be deleted */
							}
						}
						else	/* if the tags are equal but there is no attribute */
							if(atrib == NULL && new_atrib == NULL)	
							{
								ok = 1;
								aux2_node= node->children;
			...
			if(!ok) /* if the tag does not exist in the old body, append it */
			{
				xmlAddChild (p_root, new_node);	
				renew_old_tag[i] =1;
			}
		}
	}
	i= 0;
	aux_node = p_root->children;
	for (node = p_root->children->next; node; node = node->next) 
	{
		if(renew_old_tag[i]==0)
		{
	...
	xmlDocDumpFormatMemory(doc, &(body->s), &(body->len), 1);	
	return 0;
