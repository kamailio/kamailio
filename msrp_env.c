...
static int init_fake_sipmsg(); /* once-off method to initialize internal buffer */
/**
 *
 */
...
static msrp_options_t opts        = MSRP_OPTIONS_DEFAULT;

static char * _msrp_faked_sipmsg_buf = NULL;  /* changed to  ptr, from global []*/ 
...
sip_msg_t *msrp_fake_sipmsg(msrp_frame_t *mf)
{
	int len;
        ...
        if (unlikely(!_msrp_faked_sipmsg_buf)) 
        {
             if (unlikely(init_fake_sipmsg() != 0)) 
             {
                LM_CRIT("Failed initializing faked sip buffer!!");
                return NULL;
             }
        }
	...
	if(msrp_param_sipmsg==0)
        {
                LM_WARN("msrp_param_sipmsg is NULL...msg will be dropped!!");
		return NULL;
        }
  
	if (mf->buf.len >= GET_MSRP_MAX_FRAME_SIZE(&opts)  - sizeof(MSRP_FAKED_SIPMSG_START)) 
        {
                LM_WARN("msg size [%lu] > than internal max buffer size[%u] ...msg will be dropped!!", mf->buf.len + sizeof(MSRP_FAKED_SIPMSG_START), GET_MSRP_MAX_FRAME_SIZE(&opts) );
		return NULL;
        }
 ...
 *
 */
static int init_fake_sipmsg() 
{
    
        if (_msrp_faked_sipmsg_buf)
        {
        	LM_WARN("Cannot initialize faked sip buffer more than once!!");
        	return -1;
        }
        if (set_msrp_options_on_buffer(&_msrp_faked_sipmsg_buf)!=0)
        {
        	LM_CRIT(" memory alloc failure!! ");
        	return -1;
        }           
        get_current_msrp_options(&opts);
        return 0;
}
