...
#define CHECK(txt, v1, val, f, pd)                                          \
	do {                                                                    \
		...
		if((unsigned long long)v1 != ret) {                                 \
			fprintf(stderr,                                                 \
					"ERROR:" #f ": %s, expected %llx (%llx), got"           \
					" %llx\n",                                              \
					(txt), (unsigned long long)v1, (unsigned long long)val, \
					ret);                                                   \
			exit(-1);                                                       \
		}                                                                   \
	} while(0)
...
#define PROFILE_PRINT(pd)                                                   \
	do {                                                                    \
		printf("profile: %s (%ld/%ld) total %llu max %llu average %llu\n",  \
				(pd)->name, (pd)->entries, (pd)->exits, (pd)->total_cycles, \
				(pd)->max_cycles,                                           \
				(pd)->entries ? (pd)->total_cycles                          \
										/ (unsigned long long)(pd)->entries \
							  : 0ULL);                                      \
	} while(0)
#endif
...
int main(int argc, char **argv)
{
	...
	for(i = 0; i < 100; i++) {
		for(r = 0; r < 32; r++) {
			v = (1U << r);
			CHECK("first debruijn 32bit", r, v, bit_scan_forward_debruijn32,
					&pdf1);
			...
			v += (v - 1);
			CHECK("last debruijn 32bit", r, v, bit_scan_reverse_debruijn32,
					&pdl1);
		...
		for(r = 0; r < 64; r++) {
			ll = (1ULL << r);
			CHECK("first debruijn 64bit", r, ll, bit_scan_forward_debruijn64,
					&pdf5);
			...
			ll += ll - 1;
			CHECK("last debruijn 64bit", r, ll, bit_scan_reverse_debruijn64,
					&pdl5);
