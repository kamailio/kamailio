...
static char *version =
		"basex test 0.1 "
		"BASE" QUOTE_MACRO(BASEX) ": " QUOTE_MACRO(B_ENC) ", " QUOTE_MACRO(
				B_DEC) ""
...
int seed_prng()
{
	...
	try_again:
		if(read(rfd, (void *)&seed, sizeof(seed)) == -1) {
			...
			fprintf(stderr,
					"WARNING: could not read from /dev/urandom: "
					" %s (%d)\n",
	...
	for(r = 0; r < loops; r++) {
		...
		if(verbose > 2)
			printf("loop %d, current len %d, offset %d, start %p\n", r,
					c_len - offs, offs, &ibuf[offs]);
		else if((verbose > 1) && (r % 10 == 0))
			putchar('.');
		...
		if(l != e_len) {
			fprintf(stderr,
					"ERROR: invalid length for encoding: %d "
		...
		if(l != c_len) {
			fprintf(stderr,
					"ERROR: invalid length for decoding: %d "
					"instead of %d (loops=%d)\n",
					l, c_len, r);
			exit(-1);
		}
